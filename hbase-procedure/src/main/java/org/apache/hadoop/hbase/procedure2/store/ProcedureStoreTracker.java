...
name|partial
decl_stmt|;
comment|/* ----------------------      * |  updated | deleted |  meaning      * |     0    |   0     |  proc exists, but hasn't been updated since last resetUpdates().      * |     1    |   0     |  proc was updated (but not deleted).      * |     1    |   1     |  proc was deleted.      * |     0    |   1     |  proc doesn't exist (maybe never created, maybe deleted in past).     /* ----------------------      */
...
block|}
block|}
comment|/**      * If an active (non-deleted) procedure in current BitSetNode has been updated in {@code other}      * BitSetNode, then delete it from current node.      * @return true if node changed, i.e. some procedure(s) from {@code other} was subtracted from      * current node.      */
specifier|public
name|boolean
name|subtract
parameter_list|(
...
parameter_list|)
block|{
comment|// Assert that other node intersects with this node.
assert|assert
operator|!
operator|(
name|other
...
block|}
block|}
comment|/**    * Iterates over    * {@link BitSetNode}s in this.map and subtracts with corresponding ones from {@code other}    * tracker.    * @return true if tracker changed, i.e. some procedure from {@code other} were subtracted from    * current tracker.    */
specifier|public
name|boolean
name|subtract
parameter_list|(
...
parameter_list|)
block|{
comment|// Can not intersect partial bitmap.
assert|assert
operator|!
name|partial
operator|&&
...
operator|==
literal|null
comment|// No node in other map with key<= currentEntry.getKey().
comment|// First entry in other map doesn't intersect with currentEntry.
operator|||
name|otherTrackerEntry
operator|.
name|getValue
...
condition|)
block|{
comment|// No node in other map intersects with currentBitSetNode's range.
continue|continue;
block|}
block|}
do|do
