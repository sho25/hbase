...
comment|// ==========================================================================
comment|/**    * Iterates over log files from latest (ignoring currently active one) to oldest, deleting the    * ones which don't contain anything useful for recovery.    * @throws IOException    */
specifier|private
name|void
name|removeInactiveLogs
parameter_list|()
...
name|IOException
block|{
comment|// TODO: can we somehow avoid first iteration (starting from newest log) and still figure out
comment|// efficient way to cleanup old logs.
comment|// Alternatively, a complex and maybe more efficient method would be using this iteration to
comment|// rewrite latest states of active procedures to a new log file and delete all old ones.
if|if
condition|(
name|logs
operator|.
...
argument_list|)
decl_stmt|;
comment|// If nothing was subtracted, delete the log file since it doesn't contain any useful proc
comment|// states.
if|if
condition|(
operator|!
...
block|}
block|}
comment|// Delete the logs from oldest to newest and stop at first log that can't be deleted to avoid
comment|// holes in the log file sequence (for better debug capability).
while|while
condition|(
literal|true
condition|)
