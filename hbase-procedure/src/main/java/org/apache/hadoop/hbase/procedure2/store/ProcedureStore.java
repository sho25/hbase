...
name|ProcedureStore
block|{
comment|/**    * Store listener interface.    *<p/>    * The main process should register a listener and respond to the store events.    */
specifier|public
interface|interface
...
parameter_list|()
block|{     }
comment|/**      * Suggest that the upper layer should update the state of some procedures. Ignore this call      * will not effect correctness but performance.      *<p/>      * For a WAL based ProcedureStore implementation, if all the procedures stored in a WAL file      * have been deleted, or updated later in another WAL file, then we can delete the WAL file. If      * there are old procedures in a WAL file which are never deleted or updated, then we can not      * delete the WAL file and this will cause we hold lots of WAL file and slow down the master      * restarts. So here we introduce this method to tell the upper layer that please update the      * states of these procedures so that we can delete the old WAL file.      * @param procIds the id for the procedures      */
specifier|default
name|void
name|forceUpdate
parameter_list|(
...
parameter_list|()
function_decl|;
comment|/**      * Calling this method does not need to convert the protobuf message to the Procedure class, so      * if it returns true we can call {@link #skipNext()} to skip the procedure without      * deserializing. This could increase the performance.      * @return true if the iterator next element is a completed procedure.      */
name|boolean
name|isNextFinished
parameter_list|()
function_decl|;
