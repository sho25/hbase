...
block|}
block|}
comment|/**    * Map the the procId returned by submitProcedure(), the Root-ProcID, to the ProcedureResult.    * Once a Root-Procedure completes (success or failure), the result will be added to this map.    * The user of ProcedureExecutor should call getResult(procId) to get the result.    */
specifier|private
specifier|final
name|ConcurrentHashMap
argument_list|<
...
name|debug
argument_list|(
literal|"Loading procedure state="
operator|+
name|proc
operator|.
name|getState
...
name|debug
argument_list|(
literal|"The procedure is completed state="
operator|+
name|proc
operator|.
name|getState
...
condition|)
block|{
comment|// TODO: See ProcedureWALFormatReader.readInitEntry() some procedure
comment|// may be started way before this stuff.
for|for
control|(
name|Procedure
name|proc
...
comment|// TODO: Handle corrupted procedure returned (probably just a WARN)
