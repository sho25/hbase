...
return|return;
block|}
comment|// For a sub procedure which root parent has not been finished, we still need to retain the
comment|// wal even if the procedure itself is finished.
if|if
condition|(
name|proc
operator|.
...
block|}
block|}
comment|// 4. Check the waiting procedures to see if some of them can be added to runnable.
name|waitingList
operator|.
name|forEach
argument_list|(
...
argument_list|)
expr_stmt|;
comment|// 6. restore locks
name|restoreLocks
argument_list|()
expr_stmt|;
comment|// 7. Push the procedure to the scheduler
name|failedList
operator|.
name|forEach
argument_list|(
...
comment|// should be handled by parent procedures when rolling back. For example, when rolling back a
comment|// MergeTableProcedure, we will schedule new procedures to bring the offline regions online,
comment|// instead of rolling back the original procedures which offlined the regions(in fact these
comment|// procedures can not be rolled back...).
if|if
condition|(
name|proc
operator|.
