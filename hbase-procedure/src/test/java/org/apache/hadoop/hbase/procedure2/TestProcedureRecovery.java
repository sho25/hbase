...
argument_list|()
decl_stmt|;
comment|// Submit a procedure with the same nonce and expect the same procedure would return.
name|long
name|procId2
init|=
name|ProcedureTestingUtility
...
argument_list|)
decl_stmt|;
comment|// Restart (use a latch to prevent the step execution until we submitted proc2)
name|CountDownLatch
name|latch
init|=
operator|new
...
argument_list|)
expr_stmt|;
comment|// The original proc is not completed and the new submission should have the same proc Id.
name|assertTrue
argument_list|(
name|procId
operator|==
