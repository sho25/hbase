...
argument_list|)
expr_stmt|;
comment|//added simple for testing
name|this
operator|.
name|useSecure
operator|=
...
comment|//Since delegation token doesn't work in mini cluster
if|if
condition|(
name|User
operator|.
...
return|;
block|}
comment|/**    * Attempt to assign the given load queue item into its target region group.    * If the hfile boundary no longer fits into a region, physically splits    * the hfile such that the new bottom half will fit and returns the list of    * LQI's corresponding to the resultant hfiles.    *    * protected for testing    */
specifier|protected
name|List
argument_list|<
name|LoadQueueItem
