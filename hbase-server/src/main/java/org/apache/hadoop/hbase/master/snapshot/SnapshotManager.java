...
comment|// The region quota may not be updated correctly if there are concurrent restore snapshot
comment|// requests for the same table
name|tableRegionCount
operator|=
...
argument_list|()
decl_stmt|;
comment|// Update region quota when snapshotRegionCount is larger. If we updated the region count
comment|// to a smaller value before retoreSnapshot and the retoreSnapshot fails, we may fail to
comment|// reset the region count to its original value if the region quota is consumed by other
comment|// tables in the namespace
if|if
condition|(
name|tableRegionCount
operator|>
...
argument_list|)
expr_stmt|;
comment|// Update the region quota if snapshotRegionCount is smaller. This step should not fail
comment|// because we have reserved enough region quota before hand
if|if
condition|(
name|tableRegionCount
operator|>
...
name|error
argument_list|(
literal|"Region quota exceeded while restoring the snapshot "
operator|+
name|snapshot
operator|.
name|getName
...
argument_list|)
expr_stmt|;
comment|// If QEE is thrown before restoreSnapshot, quota information is not updated, so we
comment|// should throw the exception directly. If QEE is thrown after restoreSnapshot, there
comment|// must be unexpected reasons, we also throw the exception directly
throw|throw
name|e
throw|;
block|}
...
condition|)
block|{
comment|// reset the region count for table
name|checkAndUpdateNamespaceRegionQuota
argument_list|(
name|tableRegionCount
argument_list|,
...
try|try
block|{
name|checkAndUpdateNamespaceQuota
argument_list|(
name|manifest
argument_list|,
name|tableName
