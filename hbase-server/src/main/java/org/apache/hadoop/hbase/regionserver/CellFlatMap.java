...
literal|1
decl_stmt|;
comment|// if the key is found and to be included, for all possibilities, the answer is the found index
if|if
condition|(
name|index
operator|>=
...
name|index
expr_stmt|;
comment|// The compliment Operator (~) converts the returned insertion point to the real one
if|if
condition|(
name|index
operator|<
...
name|result
operator|+
literal|" (initial index "
operator|+
name|index
operator|+
literal|") "
operator|+
literal|" out of boundary, when looking for key "
operator|+
name|key
operator|+
literal|". The minCellIdx is "
operator|+
name|minCellIdx
operator|+
literal|" and the maxCellIdx is "
operator|+
name|maxCellIdx
operator|+
literal|". Finally, descending? "
operator|+
name|descending
operator|+
literal|" and was the key requested inclusively? "
operator|+
name|inclusive
argument_list|)
throw|;
...
name|IllegalArgumentException
argument_list|(
literal|"Inconsistent range, when looking from "
operator|+
name|fromKey
operator|+
literal|" to "
...
argument_list|)
decl_stmt|;
comment|// "+1" because the max index is one after the true index
return|return
name|createSubCellFlatMap
argument_list|(
name|minCellIdx
...
comment|// all interfaces returning Entries are unsupported because we are dealing only with the keys
annotation|@
name|Override
specifier|public
name|Entry
