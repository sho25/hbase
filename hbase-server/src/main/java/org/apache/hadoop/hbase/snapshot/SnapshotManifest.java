...
expr_stmt|;
block|}
comment|// 2.2. iterate through all the mob files and create "references".
for|for
control|(
name|int
name|i
...
comment|// (batch size of the files in a directory) on the namenode. Otherwise, we get back the
comment|// files in batches and may miss files being added/deleted. This could be more robust
comment|// (iteratively
...
parameter_list|)
block|{
comment|//the mob directory might not be created yet, so do nothing here
block|}
block|}
else|else
block|{
