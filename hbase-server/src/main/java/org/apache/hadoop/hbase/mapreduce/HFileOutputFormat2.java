...
begin_comment
comment|/**  * Writes HFiles. Passed Cells must arrive in order.  * Writes current time as the sequence id for the file. Sets the major compacted  * attribute on created @{link {@link HFile}s. Calling write(null,null) will forcibly roll  * all HFiles being written.  *<p>  * Using this class as part of a MapReduce job is best done  * using {@link #configureIncrementalLoad(Job, HTableDescriptor, RegionLocator, Class)}.  */
end_comment
...
argument_list|)
expr_stmt|;
comment|// Config for data block encoding
name|String
...
name|trace
argument_list|(
literal|"failed to get region location, so use default writer: "
operator|+
name|Bytes
operator|.
name|toString
...
name|warn
argument_list|(
literal|"failed to set block storage policy of ["
operator|+
name|cfPath
operator|+
literal|"] to ["
...
condition|)
block|{
comment|// record this table name for creating writer by favored nodes
name|LOG
operator|.
name|info
argument_list|(
