...
return|return;
block|}
comment|// get the status of the snapshots temporary directory and check if it has changes
comment|// The top-level directory timestamp is not updated, so we have to check the inner-level.
try|try
block|{
name|Path
name|snapshotTmpDir
...
parameter_list|)
block|{
comment|// Nothing todo, if the tmp dir is empty
block|}
