...
name|error
argument_list|(
literal|"Can't restore cache from persisted file "
operator|+
name|persistencePath
operator|+
literal|"; file removed; cache is cold!"
argument_list|,
name|ioex
argument_list|)
expr_stmt|;
...
name|error
argument_list|(
literal|"Can't restore cache from persisted file in rebuild "
operator|+
literal|"because can't deserialise; file removed; cache is cold!"
argument_list|,
name|cnfe
argument_list|)
expr_stmt|;
...
return|;
block|}
comment|/**    * The current version of the persisted cache file.    */
specifier|private
specifier|static
specifier|final
name|int
name|PERSISTED_CACHE_VERSION
init|=
literal|0
decl_stmt|;
specifier|private
...
argument_list|)
expr_stmt|;
comment|// Write out a metdata protobuf block in case we change format at later date, etc.
comment|// Add our magic as preamble.
name|fos
operator|.
name|write
argument_list|(
...
argument_list|)
expr_stmt|;
comment|// Read protobuf magic and then metadata. See persistToFile for where we wrote
comment|// out metadata for format.
name|byte
index|[]
name|pbmagic
init|=
...
condition|)
block|{
comment|// Throw exception. In finally we remove the file ALWAYS.
throw|throw
operator|new
name|HBaseIOException
argument_list|(
literal|"Failed read of protobuf magic ("
operator|+
name|Bytes
operator|.
name|toString
...
argument_list|)
operator|+
literal|"); old format (HBASE-16993)? "
operator|+
literal|"Failed read of persisted cache file="
operator|+
name|persistencePath
argument_list|)
throw|;
...
parameter_list|)
block|{
comment|// Throw exception if failed parse. In finally we remove the
throw|throw
operator|new
name|HBaseIOException
argument_list|(
literal|"Failed read of persisted cache metadata file="
operator|+
name|persistencePath
argument_list|,
name|e
...
name|HBaseIOException
argument_list|(
literal|"Unexpected version of persisted cache metadata file="
operator|+
name|persistencePath
operator|+
literal|"; expected="
...
throw|;
block|}
comment|// Ok. Read metadata. All seems good. Go ahead and pull in the persisted cache.
name|ois
operator|=
operator|new
name|ObjectInputStream
...
name|info
argument_list|(
literal|"Deleted persisted cache file "
operator|+
name|persistencePath
argument_list|)
expr_stmt|;
...
return|;
block|}
comment|/**    * Item in cache. We expect this to be where most memory goes. Java uses 8    * bytes just for object headers; after this, we want to use as little as    * possible.    */
specifier|static
class|class
name|BucketEntry
implements|implements
