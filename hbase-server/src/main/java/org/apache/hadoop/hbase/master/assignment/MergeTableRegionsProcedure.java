...
block|{
comment|// Need to reopen parent regions to pickup missed recovered.edits. Do it by creating
comment|// child assigns and then stepping back to MERGE_TABLE_REGIONS_CLOSE_REGIONS.
comment|// Just assign the primary regions recovering the missed recovered.edits -- no replicas.
comment|// May need to cycle here a few times if heavy writes.
comment|// TODO: Add an assign read-only.
for|for
control|(
name|RegionInfo
name|ri
...
name|info
argument_list|(
literal|"Found recovered.edits under {}, reopen to pickup missed edits!"
argument_list|,
name|ri
argument_list|)
expr_stmt|;
...
return|;
block|}
comment|/**    * Return list of regions that have recovered.edits... usually its an empty list.    * @param env the master env    * @throws IOException IOException    */
specifier|private
name|List
argument_list|<
name|RegionInfo
...
block|}
block|}
comment|/**    * Set the region states to MERGING state    * @param env MasterProcedureEnv    */
specifier|public
name|void
name|setRegionStateToMerging
parameter_list|(
