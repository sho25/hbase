...
name|State
block|{
comment|/**      * The end keys of each stripe. The last stripe end is always open-ended, so it's not stored      * here. It is invariant that the start key of the stripe is the end key of the previous one      * (and is an open boundary for the first one).      */
specifier|public
name|byte
index|[]
index|[]
...
index|[]
decl_stmt|;
comment|/**      * Files by stripe. Each element of the list corresponds to stripeEndKey with the corresponding      * index, except the last one. Inside each list, the files are in reverse order by seqNum.      * Note that the length of this is one higher than that of stripeEndKeys.      */
specifier|public
name|ArrayList
argument_list|<
name|ImmutableList
...
name|kvComparator
expr_stmt|;
comment|// TODO: create this in a shared manner in StoreEngine when there's one
name|this
operator|.
name|config
operator|=
...
name|CompactionResultsMergeCopy
name|cmc
init|=
operator|new
name|CompactionResultsMergeCopy
...
argument_list|()
decl_stmt|;
comment|// Validate the file start keys, and remove the bad ones to level 0.
for|for
control|(
name|int
name|i
...
return|;
block|}
comment|/**    * Finds the stripe index by end key.    */
specifier|private
specifier|final
name|int
...
return|;
block|}
comment|/**    * Finds the stripe index for the stripe containing a key provided externally for get/scan.    */
specifier|private
specifier|final
name|int
name|findStripeForRow
...
return|;
block|}
comment|/**    * Gets the start key for a given stripe.    * @param stripeIndex Stripe index.    * @return Start key. May be an open key.    */
specifier|public
specifier|final
name|byte
index|[]
...
block|}
block|}
comment|/**    * Non-static helper class for merging compaction results.    * Since we want to merge them atomically (more or less), it operates on lazy copies, and    * then applies copies to real lists as necessary.    */
specifier|private
class|class
name|CompactionResultsMergeCopy
block|{
...
name|warn
argument_list|(
literal|"The newly compacted files doesn't have stripe rows set: "
operator|+
name|sf
operator|.
name|getPath
...
block|}
block|}
comment|/**      * See {@link #addCompactionResults(Collection, Collection)} - updates the stripe list with      * new candidate stripes/removes old stripes; produces new set of stripe end keys.      * @param newStripes  New stripes - files by end key.      */
specifier|private
name|void
name|processNewCandidateStripes
parameter_list|(
...
name|IOException
argument_list|(
literal|"Compaction intends to create a new stripe that replaces an"
operator|+
literal|" existing one, but the latter contains some files."
argument_list|)
throw|;
block|}
if|if
...
name|IOException
argument_list|(
literal|"The new stripes produced by compaction are not contiguous"
argument_list|)
throw|;
block|}
block|}
