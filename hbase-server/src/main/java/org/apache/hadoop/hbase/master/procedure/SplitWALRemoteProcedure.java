...
parameter_list|)
block|{
comment|// When send to a wrong target server, it need construct a new SplitWALRemoteProcedure.
comment|// Thus return null for this procedure and let SplitWALProcedure to handle this.
name|LOG
operator|.
name|warn
argument_list|(
literal|"dispatch WAL {} to {} failed, will retry on another server"
argument_list|,
name|walPath
argument_list|,
name|worker
