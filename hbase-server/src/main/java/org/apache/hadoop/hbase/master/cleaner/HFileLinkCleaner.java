...
try|try
block|{
comment|// Also check if the HFile is in the HBASE_TEMP_DIRECTORY; this is where the referenced
comment|// file gets created when cloning a snapshot.
name|hfilePath
operator|=
name|HFileLink
operator|.
...
return|;
block|}
comment|// check whether the HFileLink still exists in mob dir.
name|hfilePath
operator|=
name|HFileLink
operator|.
...
name|debug
argument_list|(
literal|"Couldn't get the references, not deleting file, just in case. filePath="
operator|+
name|filePath
operator|+
literal|", backRefDir="
