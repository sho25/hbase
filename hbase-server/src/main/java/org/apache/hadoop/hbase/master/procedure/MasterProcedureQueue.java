...
begin_comment
comment|/**  * ProcedureRunnableSet for the Master Procedures.  * This RunnableSet tries to provide to the ProcedureExecutor procedures  * that can be executed without having to wait on a lock.  * Most of the master operations can be executed concurrently, if they  * are operating on different tables (e.g. two create table can be performed  * at the same, time assuming table A and table B) or against two different servers; say  * two servers that crashed at about the same time.  *  *<p>Each procedure should implement an interface providing information for this queue.  * for example table related procedures should implement TableProcedureInterface.  * each procedure will be pushed in its own queue, and based on the operation type  * we may take smarter decision. e.g. we can abort all the operations preceding  * a delete table, or similar.  */
end_comment
...
argument_list|)
decl_stmt|;
comment|// Two queues to ensure that server procedures run ahead of table precedures always.
specifier|private
specifier|final
name|ProcedureFairRunQueues
argument_list|<
...
literal|1
decl_stmt|;
comment|/**    * Keeps count across server and table queues.    */
specifier|private
name|int
name|queueSize
decl_stmt|;
...
expr_stmt|;
block|}
comment|/**    * Try to acquire the write lock on the specified server.    * @see #releaseServerExclusiveLock(ServerProcedureInterface)    * @param spi Server to lock    * @return true if we were able to acquire the lock on the server, otherwise false.    */
specifier|public
name|boolean
...
return|;
block|}
comment|/**    * Release the write lock    * @see #tryAcquireServerExclusiveLock(ServerProcedureInterface)    * @param spi the server that has the write lock    */
specifier|public
name|void
...
expr_stmt|;
block|}
comment|/**    * Try to acquire the read lock on the specified server.    * @see #releaseServerSharedLock(ServerProcedureInterface)    * @param spi Server to lock    * @return true if we were able to acquire the lock on the server, otherwise false.    */
specifier|public
name|boolean
...
return|;
block|}
comment|/**    * Release the read lock taken    * @see #tryAcquireServerSharedLock(ServerProcedureInterface)    * @param spi the server that has the read lock    */
specifier|public
name|void
...
expr_stmt|;
block|}
comment|/**      * @return True if only one instance of a shared lock outstanding.      */
specifier|synchronized
name|boolean
...
block|}
block|}
comment|/**    * Run Queue for Server procedures.    */
specifier|private
specifier|static
class|class
name|ServerRunQueue
