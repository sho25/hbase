...
expr_stmt|;
block|}
comment|/**    * Should be private    * @deprecated Since 2.3.0. Should be for internal use only. Used by testing.    */
annotation|@
name|Deprecated
annotation|@
name|VisibleForTesting
...
operator|++
expr_stmt|;
comment|// If some one tries to get the descriptor for
...
name|debug
argument_list|(
literal|"Exception during readTableDecriptor; tableName={}"
argument_list|,
name|tablename
argument_list|,
name|e
...
condition|)
block|{
comment|// If we tried to access hbase:meta and it not there, create it.
name|td
operator|=
name|createMetaTableDescriptor
argument_list|(
...
name|info
argument_list|(
literal|"Creating new hbase:meta table default descriptor/schema {}"
argument_list|,
name|td
argument_list|)
expr_stmt|;
...
condition|)
block|{
comment|// This get will create hbase:meta if it does not exist. Will also populate cache.
name|get
argument_list|(
name|TableName
operator|.
...
literal|true
decl_stmt|;
comment|// Add hbase:meta descriptor. The get will create hbase:meta in fs if doesn't
comment|// exist. FSUtils listing table names in fs skip meta dirs. TODO: Fill out
comment|// FSUtils with methods to get userspace tables and system tables.
name|tds
operator|.
name|put
argument_list|(
...
name|NotImplementedException
argument_list|(
literal|"Cannot add Descriptor for reserved subdirectory name: "
operator|+
name|htd
operator|.
name|getTableName
...
return|;
block|}
comment|/**    * Create a new TableDescriptor in the specified table directory and filesystem. Happens when we    * create a new table or snapshot a table. This method doesn't require creationg of an    * {@link FSTableDescriptors} instance so it takes a bunch of arguments. Users of the method    * above used to create an FSTableDescriptors instance just to run the method. That was fine    * until construction started expecting to be able to read the hbase:meta schema. Snapshotting    * to some random dir would fail construction if no hbase:meta schema available.    * @param fs Filesystem to write to. Snapshot can set it to other than that of running system.    * @param tableDir table directory under which we should write the file    * @param htd description of the table to write    * @param forceCreation if<tt>true</tt>,then even if previous table descriptor is present it will    *          be overwritten    * @return<tt>true</tt> if the we successfully created the file,<tt>false</tt> if the file    *         already exists and we weren't forcing the descriptor creation.    * @throws IOException if a filesystem error occurs    */
specifier|public
specifier|static
name|boolean
name|createTableDescriptorForTableDirectory
