...
expr_stmt|;
block|}
comment|/**    * @param fsreadonly True if we are read-only when it comes to filesystem    * operations; i.e. on remove, we do not do delete in fs.    */
specifier|public
name|FSTableDescriptors
parameter_list|(
specifier|final
...
name|IOException
argument_list|(
literal|"No descriptor found for non table = "
operator|+
name|tablename
argument_list|)
throw|;
...
name|NotImplementedException
argument_list|(
literal|"Cannot add a table descriptor for a reserved subdirectory name: "
operator|+
name|htd
operator|.
name|getHTableDescriptor
...
return|;
block|}
comment|/**    * Checks if a current table info file exists for the given table    *     * @param tableName name of table    * @return true if exists    * @throws IOException    */
specifier|public
name|boolean
...
return|;
block|}
comment|/**    * Find the most current table info file for the given table in the hbase root directory.    * @return The file status of the current table info file or null if it does not exist    */
specifier|private
name|FileStatus
name|getTableInfoPath
parameter_list|(
...
return|;
block|}
comment|/**    * @param fs    * @param rootdir    * @param tableName    * @return Modification time for the table {@link #TABLEINFO_FILE_PREFIX} file    * or<code>0</code> if no tableinfo file found.    * @throws IOException    */
specifier|private
name|long
name|getTableInfoModtime
parameter_list|(
...
parameter_list|)
block|{
comment|// we have old HTableDescriptor here
try|try
block|{
name|HTableDescriptor
name|htd
...
name|warn
argument_list|(
literal|"Found old table descriptor, converting to new format for table "
operator|+
name|htd
operator|.
name|getTableName
argument_list|()
operator|+
literal|"; NOTE table will be in ENABLED state!"
argument_list|)
expr_stmt|;
name|td
operator|=
...
return|;
block|}
comment|/**    * Deletes all the table descriptor files from the file system.    * Used in unit tests only.    * @throws NotImplementedException if in read only mode    */
specifier|public
name|void
...
name|NotImplementedException
argument_list|(
literal|"Cannot delete a table descriptor - in read only mode"
argument_list|)
throw|;
block|}
name|Path
...
block|}
block|}
comment|/**    * Create new HTableDescriptor in HDFS. Happens when we are creating table.    * Used by tests.    * @return True if we successfully created file.    */
specifier|public
name|boolean
...
return|;
block|}
comment|/**    * Create new HTableDescriptor in HDFS. Happens when we are creating table. If    * forceCreation is true then even if previous table descriptor is present it    * will be overwritten    *     * @return True if we successfully created file.    */
specifier|public
name|boolean
name|createTableDescriptor
parameter_list|(
...
return|;
block|}
comment|/**    * Create tables descriptor for given HTableDescriptor. Default TableDescriptor state    * will be used (typically ENABLED).    */
specifier|public
name|boolean
name|createTableDescriptor
parameter_list|(
