...
argument_list|)
decl_stmt|;
comment|/**    * @deprecated since 2.2.0, will be removed in 3.0.0, with no replacement. End user should not    *             depend on this value.    */
annotation|@
name|Deprecated
specifier|public
specifier|static
...
return|;
block|}
comment|/**    * Attempts to do an atomic load of many hfiles into a region. If it fails, it returns a list of    * hfiles that need to be retried. If it is successful it will return an empty list.    *<p>    * NOTE: To maintain row atomicity guarantees, region server callable should succeed atomically    * and fails atomically.    *<p>    * Protected for testing.    * @return empty list if success, list of items to retry on recoverable failure    */
annotation|@
name|VisibleForTesting
specifier|protected
name|List
...
name|RETRY_ON_IO_EXCEPTION
operator|+
literal|" is disabled. Unable to recover"
argument_list|)
expr_stmt|;
throw|throw
name|e
...
block|}
block|}
comment|/**    * Perform bulk load on the given table.    * @param hfofDir the directory that was provided as the output path of a job using    *          HFileOutputFormat    * @param tableName the table to load into    */
specifier|public
name|Map
argument_list|<
name|LoadQueueItem
...
return|;
block|}
comment|/**    * Perform bulk load on the given table.    * @param family2Files map of family to List of hfiles    * @param tableName the table to load into    */
specifier|public
name|Map
argument_list|<
name|LoadQueueItem
