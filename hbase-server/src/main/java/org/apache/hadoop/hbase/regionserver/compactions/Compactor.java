...
block|}
block|}
comment|/**    * Used to prevent compaction name conflict when multiple compactions running parallel on the    * same store.    */
specifier|private
specifier|static
specifier|final
name|AtomicInteger
...
return|;
block|}
comment|/**    * Performs the compaction.    * @param fd FileDetails of cell sink writer    * @param scanner Where to read from.    * @param writer Where to write to.    * @param smallestReadPoint Smallest read point.    * @param cleanSeqId When true, remove seqId(used to be mvcc) value which is&lt;= smallestReadPoint    * @param major Is a major compaction.    * @return Whether compaction ended; false if it was interrupted for some reason.    */
specifier|protected
name|boolean
