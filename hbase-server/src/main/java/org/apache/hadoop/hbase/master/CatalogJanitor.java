...
return|;
block|}
comment|/**    * Report made by {@link ReportMakingVisitor}.    */
specifier|static
class|class
name|Report
block|{
...
block|}
block|}
comment|/**    * Simple datastructure to hold a MetaRow content.    */
specifier|static
class|class
name|MetaRow
block|{
comment|/**      * A marker for use in case where there is a hole at the very      * first row in hbase:meta. Should never happen.      */
specifier|private
specifier|static
specifier|final
name|MetaRow
...
argument_list|)
decl_stmt|;
comment|/**      * Row from hbase:meta table.      */
specifier|final
name|byte
index|[]
name|metaRow
decl_stmt|;
comment|/**      * The decoded RegionInfo gotten from hbase:meta.      */
specifier|final
name|RegionInfo
name|regionInfo
decl_stmt|;
...
init|)
block|{
comment|/* Used to generate an overlap.       Get g = new Get(Bytes.toBytes("t2,40,1563939166317.5a8be963741d27e9649e5c67a34259d9."));       g.addColumn(HConstants.CATALOG_FAMILY, HConstants.REGIONINFO_QUALIFIER);       try (Table t = connection.getTable(TableName.META_TABLE_NAME)) {         Result r = t.get(g);         byte [] row = g.getRow();         row[row.length - 3]<<= ((byte)row[row.length -3]);         Put p = new Put(g.getRow());         p.addColumn(HConstants.CATALOG_FAMILY, HConstants.REGIONINFO_QUALIFIER,             r.getValue(HConstants.CATALOG_FAMILY, HConstants.REGIONINFO_QUALIFIER));         t.put(p);       }       */
name|MetaTableAccessor
operator|.
name|scanMetaForTableRegions
argument_list|(
