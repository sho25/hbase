...
name|status
decl_stmt|;
comment|// currentRunningState is updated when ServerCrashProcedure get scheduled, child procedures update
comment|// progress will not update the state because the actual state is overwritten by its next state
specifier|private
name|ServerCrashState
...
name|msg
init|=
literal|"Processing ServerCrashProcedure of "
operator|+
name|serverName
decl_stmt|;
if|if
...
literal|0
condition|?
literal|"; remaining num of running child procedures = "
operator|+
name|childrenLatch
else|:
literal|""
