...
name|IOException
block|{
comment|// If passed a TimeRangeTracker, use it. Set timeRangeTrackerSet so we don't destroy it.
comment|// TODO: put the state of the TRT on the TRT; i.e. make a read-only version (TimeRange) when
comment|// it no longer writable.
name|this
operator|.
name|timeRangeTrackerSet
operator|=
...
literal|null
decl_stmt|;
comment|// Used with ROW_COL bloom
name|KeyValue
name|bloomKeyKV
init|=
literal|null
...
block|{
comment|// TODO : Caching of kv components becomes important in these cases
name|res
operator|=
name|CellComparator
operator|.
...
block|{
comment|// hbase:meta does not have blooms. So we need not have special interpretation
comment|// of the hbase:meta cells
name|newKey
operator|=
operator|!
name|CellUtil
...
expr_stmt|;
block|}
comment|/**      * @param trt A premade TimeRangeTracker to use rather than build one per append (building one      * of these is expensive so good to pass one in if you have one).      * @return this (for chained invocation)      */
specifier|public
name|Builder
