...
control|)
block|{
comment|// Sort the procedures so meta and system regions are first in the returned array.
name|procedures
index|[
name|index
operator|++
...
return|;
block|}
comment|// Make this static for the method below where we use it typing the AssignProcedure array we
comment|// return as result.
specifier|private
specifier|static
specifier|final
name|AssignProcedure
index|[]
name|ASSIGN_PROCEDURE_ARRAY_TYPE
init|=
operator|new
name|AssignProcedure
index|[]
block|{}
decl_stmt|;
comment|/**    * @param assignments Map of assignments from which we produce an array of AssignProcedures.    * @param size Count of assignments to make (the caller may know the total count)    * @return Assignments made from the passed in<code>assignments</code>    */
specifier|private
name|AssignProcedure
index|[]
name|createAssignProcedures
...
name|UnassignProcedure
index|[]
name|UNASSIGN_PROCEDURE_ARRAY_TYPE
init|=
operator|new
name|UnassignProcedure
index|[
...
control|)
block|{
comment|// Region State can be OPEN even if we did controlled cluster shutdown; Master does not close
comment|// the regions in this case. The RegionServer does the close so hbase:meta is state in
comment|// hbase:meta is not updated -- Master does all updates -- and is left with OPEN as region
comment|// state in meta. How to tell difference between ordered shutdown and crashed-down cluster
comment|// then? We can't. Not currently. Perhaps if we updated hbase:meta with CLOSED on ordered
comment|// shutdown. This would slow shutdown though and not all edits would make it in anyways.
comment|// TODO: Examine.
comment|// Because we can't be sure it an ordered shutdown, we run ServerCrashProcedure always.
comment|// ServerCrashProcedure will try to retain old deploy when it goes to assign.
if|if
condition|(
name|regionNode
operator|.
...
block|{
comment|// Server is online. This a failover. Master is starting into already-running cluster.
name|failover
operator|=
literal|true
expr_stmt|;
...
block|}
block|}
comment|// Kill servers with online regions just-in-case. Runs ServerCrashProcedure.
for|for
control|(
name|ServerName
name|serverName
...
name|info
argument_list|(
literal|"KILL RegionServer="
operator|+
name|serverName
operator|+
literal|" hosting regions but not online."
argument_list|)
expr_stmt|;
name|killRegionServer
argument_list|(
...
argument_list|)
expr_stmt|;
comment|// Assign offline regions. Uses round-robin.
if|if
condition|(
name|offlineRegionsToAssign
operator|.
...
expr_stmt|;
block|}
comment|/**    * This is a very particular check. The {@link org.apache.hadoop.hbase.master.ServerManager} is    * where you go to check on state of 'Servers', what Servers are online, etc. Here we are    * checking the state of a server that is post expiration, a ServerManager function that moves a    * server from online to dead. Here we are seeing if the server has moved beyond a particular    * point in the recovery process such that it is safe to move on with assigns; etc.    * @return True if this Server does not exist or if does and it is marked as OFFLINE (which    *   happens after all WALs have been split on this server making it so assigns, etc. can    *   proceed). If null, presumes the ServerStateNode was cleaned up by SCP.    */
name|boolean
