...
block|}
block|}
comment|// Track the regionserver reported online regions in memory.
synchronized|synchronized
init|(
name|rsReports
init|)
...
block|}
comment|// Do not need to acquire region state lock as this is only for showing metrics.
name|ServerName
name|sn
init|=
name|rsn
...
block|}
empty_stmt|;
comment|/**    * Query META if the given<code>RegionInfo</code> exists, adding to    *<code>AssignmentManager.regionStateStore</code> cache if the region is found in META.    * @param regionEncodedName encoded name for the region to be loaded from META into    *<code>AssignmentManager.regionStateStore</code> cache    * @return<code>RegionInfo</code> instance for the given region if it is present in META    *          and got successfully loaded into<code>AssignmentManager.regionStateStore</code>    *          cache,<b>null</b> otherwise.    * @throws UnknownRegionException if any errors occur while querying meta.    */
specifier|public
name|RegionInfo
...
name|error
argument_list|(
literal|"Error trying to load region {} from META"
argument_list|,
name|regionEncodedName
argument_list|,
name|e
...
name|UnknownRegionException
argument_list|(
literal|"Error while trying load region from meta"
argument_list|)
throw|;
block|}
block|}
...
return|;
block|}
comment|/**    * @return a snapshot of rsReports    */
specifier|public
name|Map
argument_list|<
name|ServerName
...
return|;
block|}
comment|/**    * Provide regions state count for given table.    * e.g howmany regions of give table are opened/closed/rit etc    *    * @param tableName TableName    * @return region states count    */
specifier|public
name|RegionStatesCount
...
block|{
name|int
name|openRegionsCount
init|=
literal|0
decl_stmt|;
name|int
name|closedRegionCount
init|=
literal|0
decl_stmt|;
name|int
...
decl_stmt|;
name|int
name|splitRegionCount
init|=
literal|0
decl_stmt|;
name|int
name|totalRegionCount
init|=
literal|0
decl_stmt|;
if|if
