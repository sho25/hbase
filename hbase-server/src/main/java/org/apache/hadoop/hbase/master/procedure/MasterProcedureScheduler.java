...
condition|)
block|{
comment|// the queue is not suspended or removed from the fairq (run-queue)
comment|// because someone has an xlock on it.
comment|// so, if the queue is not-linked we should add it
if|if
condition|(
name|queue
operator|.
...
return|;
block|}
comment|/**    * Suspend the procedure if the event is not ready yet.    * @param event the event to wait on    * @param procedure the procedure waiting on the event    * @param suspendQueue true if the entire queue of the procedure should be suspended    * @return true if the procedure has to wait for the event to be ready, false otherwise.    */
specifier|public
name|boolean
name|waitEvent
parameter_list|(
...
try|try
block|{
comment|// TODO: This will change once we have the new AM
name|ServerQueue
name|queue
init|=
name|getServerQueue
...
block|}
block|}
comment|/**    * Wake every procedure waiting for the specified events.    * (By design each event has only one "wake" caller)    * @param events the list of events to wake    * @param count the number of events in the array to wake    */
specifier|public
name|void
name|wakeEvents
parameter_list|(
...
condition|)
block|{
comment|// you should override this method if you are using the default constructor
throw|throw
operator|new
name|UnsupportedOperationException
argument_list|()
