...
argument_list|)
expr_stmt|;
comment|// For the following conditions, we will put the queue back into execution
comment|// 1. The procedure has already held the lock, or the lock has been restored when restarting,
...
argument_list|()
decl_stmt|;
comment|// if we have the lock access, we are ready
if|if
condition|(
name|s
operator|.
...
argument_list|)
decl_stmt|;
comment|// if we need to hold the xlock, then we need to make sure that no one holds any lock, including
comment|// the shared lock, otherwise, we just need to make sure that no one holds the xlock
return|return
name|xlockReq
condition|?
operator|!
...
return|;
block|}
comment|// loop until we find out a procedure which is ready to run, or if we have checked all the
comment|// procedures, then we give up and remove the queue from run queue.
for|for
control|(
name|int
name|i
...
condition|)
block|{
comment|// the queue is empty, remove from run queue
if|if
condition|(
name|rq
operator|.
...
return|;
block|}
comment|// we are not ready to run, add back and try the next procedure
name|rq
operator|.
name|add
argument_list|(
...
expr_stmt|;
block|}
comment|// no procedure is ready for execution, remove from run queue
name|removeFromRunQueue
argument_list|(
name|fairq
argument_list|,
...
block|{
comment|// No cleanup for other procedure types, yet.
return|return;
block|}
block|}
specifier|private
...
argument_list|)
decl_stmt|;
comment|// Only SCP will acquire/release server lock so do not need to check the return value here.
name|lock
operator|.
name|releaseExclusiveLock
argument_list|(
