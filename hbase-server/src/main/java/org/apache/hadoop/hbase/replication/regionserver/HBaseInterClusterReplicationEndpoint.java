...
argument_list|)
expr_stmt|;
comment|// Now, we have marked the batch as done replicating, record its size
name|numReplicated
operator|+=
name|batchSize
expr_stmt|;
...
condition|)
block|{
comment|// Something went wrong here and we don't know what, let's just fail and retry.
name|LOG
operator|.
name|warn
argument_list|(
literal|"The number of edits replicated is different from the number received,"
operator|+
literal|" failing for now."
argument_list|)
expr_stmt|;
return|return
literal|false
