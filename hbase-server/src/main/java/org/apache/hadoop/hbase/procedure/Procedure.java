...
expr_stmt|;
block|}
comment|/**    * Create a procedure.    *    * Users should generally not directly create instances of this.  They are created them    * implicitly via {@link ProcedureCoordinator#createProcedure(String, byte[], List)}}    *    * @param coord coordinator to call back to for general errors (e.g.    *          {@link ProcedureCoordinator#rpcConnectionFailure(String, IOException)}).    * @param wakeFreq frequency to check for errors while waiting    * @param timeout amount of time to allow the procedure to run before cancelling    * @param procName name of the procedure instance    * @param args argument data associated with the procedure instance    * @param expectedMembers names of the expected members    */
specifier|public
name|Procedure
parameter_list|(
name|ProcedureCoordinator
...
block|}
block|}
comment|/**    * Sends a message to members that all {@link Subprocedure#inBarrier} calls have completed.    * After this executes, the coordinator can assume that any state resources about this barrier    * procedure state has been released.    */
specifier|public
name|void
name|sendGlobalBarrierComplete
parameter_list|()
