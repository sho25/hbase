...
return|return;
block|}
comment|// If we haven't already requested a roll, check if we have exceeded logrollsize
if|if
condition|(
operator|!
name|isLogRollRequested
argument_list|()
operator|&&
name|writer
operator|.
...
name|debug
argument_list|(
literal|"Requesting log roll because of file size threshold; length="
operator|+
name|writer
operator|.
name|getLength
...
block|}
block|}
comment|// find all the sync futures between these two txids to see if we need to issue a hsync, if no
comment|// sync futures then just use the default one.
specifier|private
name|boolean
name|isHsync
parameter_list|(
...
specifier|private
name|void
name|drainNonMarkerEditsAndFailSyncs
parameter_list|()
block|{
if|if
condition|(
...
argument_list|()
expr_stmt|;
comment|// fail the sync futures which are under the txid of the first remaining edit, if none, fail
comment|// all the sync futures.
name|long
name|txid
init|=
name|toWriteAppends
...
name|IOException
argument_list|(
literal|"WAL is closing, only marker edit is allowed"
argument_list|)
decl_stmt|;
for|for
control|(
...
return|;
block|}
comment|// This is used by sync replication, where we are going to close the wal soon after we reopen all
comment|// the regions. Will be overridden by sub classes.
specifier|protected
name|boolean
name|markerEditOnly
parameter_list|()
