...
block|{
comment|//TODO handle htrace API change, see HBASE-18895
comment|//TraceScope scope = Trace.continueSpan(span);
try|try
block|{
name|appended
operator|=
...
block|{
comment|//TODO handle htrace API change, see HBASE-18895
comment|//assert scope == NullScope.INSTANCE || !scope.isDetached();
comment|//scope.close(); // append scope is complete
block|}
block|}
else|else
block|{
...
block|}
comment|//TODO handle htrace API change, see HBASE-18895
comment|//scope = Trace.continueSpan(blockOnSync(future));
if|if
condition|(
name|future
operator|!=
