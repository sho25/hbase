...
expr_stmt|;
block|}
comment|// See the comments in syncFailed why we need to do this.
specifier|private
name|void
name|cleanup
parameter_list|()
...
condition|)
block|{
comment|// all outstanding appends have been acked, just finish all syncs.
name|long
name|maxSyncTxid
init|=
name|doneTxid
...
return|return;
block|}
comment|// usually waitingRoll is false so we check it without lock first.
if|if
condition|(
name|waitingRoll
condition|)
...
condition|)
block|{
comment|// there is a roll request
name|sync
argument_list|(
name|writer
argument_list|,
