...
argument_list|)
decl_stmt|;
comment|// regionLock takes shared lock on table too.
name|lock
operator|=
name|region
operator|.
...
name|info
argument_list|(
literal|"Acquiring MOB major compaction lock "
operator|+
name|lock
argument_list|)
expr_stmt|;
...
name|error
argument_list|(
literal|"Interrupted exception when waiting for lock: "
operator|+
name|lock
argument_list|,
name|e
...
condition|)
block|{
comment|// Remove lock from queue on the master so that if it's granted in future, we don't
comment|// keep holding it until compaction finishes
name|lock
operator|.
name|unlock
argument_list|()
...
name|warn
argument_list|(
literal|"Cannot obtain table lock, maybe a sweep tool is running on this "
operator|+
literal|"table["
operator|+
name|getTableName
...
name|warn
argument_list|(
literal|"Cannot obtain lock because RegionServices not available. Are we running as "
operator|+
literal|"compaction tool?"
argument_list|)
expr_stmt|;
block|}
comment|// If no lock, retain delete markers to be safe.
if|if
condition|(
name|lock
operator|==
