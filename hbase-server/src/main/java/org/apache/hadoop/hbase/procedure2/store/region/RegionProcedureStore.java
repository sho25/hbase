...
specifier|final
name|String
name|HFILECLEANER_PLUGINS
init|=
literal|"hbase.procedure.store.region.hfilecleaner.plugins"
decl_stmt|;
specifier|private
specifier|static
specifier|final
...
name|leaseRecovery
decl_stmt|;
comment|// Used to delete the compacted hfiles. Since we put all data on WAL filesystem, it is not
comment|// possible to move the compacted hfiles to the global hfile archive directory, we have to do it
comment|// by ourselves.
specifier|private
name|HFileCleaner
name|cleaner
decl_stmt|;
...
operator|+
literal|" version to finish them, and then try to upgrade again."
operator|+
literal|" See https://hbase.apache.org/book.html#upgrade2.2 for more details."
operator|+
literal|" The full procedure list: {}"
argument_list|,
name|clazz
argument_list|,
name|procs
...
argument_list|)
expr_stmt|;
comment|// check whether there are unsupported procedures, this could happen when we are migrating from
comment|// 2.1-. We used to do this in HMaster, after loading all the procedures from procedure store,
comment|// but here we have to do it before migrating, otherwise, if we find some unsupported
comment|// procedures, the users can not go back to 2.1 to finish them any more, as all the data are now
comment|// in the new region based procedure store, which is not supported in 2.1-.
name|checkUnsupportedProcedure
argument_list|(
name|activeProcsByType
argument_list|)
...
name|info
argument_list|(
literal|"Migrated {} existing procedures from the old storage format."
argument_list|,
name|procs
operator|.
name|size
...
name|warn
argument_list|(
literal|"Failed to create archive directory {}. Usually this should not happen but it will"
operator|+
literal|" be created again when we actually archive the hfiles later, so continue"
argument_list|,
name|archiveDir
argument_list|)
expr_stmt|;
...
name|HFileCleaner
argument_list|(
literal|"RegionProcedureStoreHFileCleaner"
argument_list|,
name|cleanerInterval
argument_list|,
name|server
...
begin_comment
comment|/**    * Insert procedure may be called by master's rpc call. There are some check about the rpc call    * when mutate region. Here unset the current rpc call and set it back in finally block. See    * HBASE-23895 for more details.    */
end_comment

begin_function
specifier|private
name|void
name|runWithoutRpcCall
parameter_list|(
name|Runnable
name|runnable
parameter_list|)
