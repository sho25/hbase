...
name|IOException
argument_list|(
literal|"Failed to create remote wal dir "
operator|+
name|remoteWALDirForPeer
argument_list|)
throw|;
...
argument_list|)
assert|;
comment|// for serial peer, we need to reopen all the regions and then update the last pushed sequence
comment|// id, before replaying any remote wals, so that the serial replication will not be stuck, and
comment|// also guarantee the order when replicating the remote wal back.
name|setNextState
argument_list|(
name|serial
condition|?
...
comment|// and crash.
comment|// Disable the peer if we are going to transit from STANDBY to DOWNGRADE_ACTIVE, and the
comment|// replication is serial, as we need to update the lastPushedSequence id after we reopen all
comment|// the regions, and for performance reason here we will update in batch, without using CAS, if
comment|// we are still replicating at RS side, we may accidentally update the last pushed sequence id
comment|// to a less value and cause the replication to be stuck.
name|env
operator|.
name|getReplicationPeerManager
argument_list|()
...
operator|+
literal|"replication peer state from {} to {}, sleep {} secs and retry"
argument_list|,
name|peerId
argument_list|,
name|fromState
...
name|PeerSyncReplicationStateTransitionState
operator|.
name|SYNC_REPLICATION_UPDATE_LAST_PUSHED_SEQ_ID_FOR_SERIAL_PEER
argument_list|)
expr_stmt|;
block|}
else|else
...
name|warn
argument_list|(
literal|"Failed to update last pushed sequence id for peer {} when transiting sync "
operator|+
literal|"replication peer state from {} to {}, sleep {} secs and retry"
argument_list|,
name|peerId
...
operator|+
literal|" sync replication peer state from {} to {}, sleep {} secs and retry"
argument_list|,
name|peerId
argument_list|,
name|fromState
...
operator|+
literal|"state from {} to {}, sleep {} secs and retry"
argument_list|,
name|peerId
argument_list|,
name|fromState
...
operator|+
literal|"peer state from {} to {}, sleep {} secs and retry"
argument_list|,
name|peerId
argument_list|,
name|fromState
