...
name|disruptor
decl_stmt|;
comment|/**    * An executorservice that runs the disruptor AppendEventHandler append executor.    */
specifier|private
specifier|final
name|ExecutorService
name|appendExecutor
...
block|}
block|}
comment|/**    * If the number of un-archived WAL files is greater than maximum allowed, check the first    * (oldest) WAL file, and returns those regions which should be flushed so that it can    * be archived.    * @return regions (encodedRegionNames) to flush in order to archive oldest WAL file.    * @throws IOException    */
name|byte
index|[]
index|[]
name|findRegionsToForceFlush
