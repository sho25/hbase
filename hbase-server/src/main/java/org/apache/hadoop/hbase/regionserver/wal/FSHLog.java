...
literal|false
decl_stmt|;
comment|/**    * Set when this WAL is for meta only (we run a WAL for all regions except meta -- it has its    * own dedicated WAL).    */
specifier|private
specifier|final
name|boolean
name|forMeta
...
name|Long
argument_list|>
name|oldestUnflushedRegionSequenceIds
init|=
operator|new
name|ConcurrentSkipListMap
argument_list|<
...
argument_list|)
decl_stmt|;
comment|/**    * Map of encoded region names to their lowest or OLDEST sequence/edit id in memstore currently    * being flushed out to hfiles. Entries are moved here from    * {@link #oldestUnflushedRegionSequenceIds} while the lock {@link #regionSequenceIdLock} is held    * (so movement between the Maps is atomic). This is not related to the id we use above for    * {@link #highestSyncedSequence} and {@link #highestUnsyncedSequence} which is the sequence from    * the disruptor ring buffer, an internal detail.    */
specifier|private
specifier|final
name|Map
argument_list|<
...
name|Long
argument_list|>
name|lowestFlushingRegionSequenceIds
init|=
operator|new
name|TreeMap
argument_list|<
...
expr_stmt|;
block|}
comment|// Shutdown the disruptor.  Will stop after all entries have been processed.  Make sure we have
comment|// stopped incoming appends before calling this else it will not shutdown.  We are
...
argument_list|)
throw|;
comment|// TODO: trace model here does not work any more.  It does not match how we append.
name|TraceScope
name|traceScope
init|=
name|Trace
...
name|error
argument_list|(
literal|"Error syncing, request close of hlog "
argument_list|,
name|e
argument_list|)
expr_stmt|;
...
parameter_list|)
block|{
comment|// TODO: Add metric for handler syncs done at a time.
if|if
condition|(
name|this
operator|.
...
comment|// TODO: Trace only working for appends, not for syncs.
name|TraceScope
name|scope
init|=
name|truck
...
block|{
comment|// This scope only makes sense for the append. Syncs will be pulled-up short so tracing
comment|// will not give a good representation. TODO: Fix.
if|if
condition|(
name|scope
operator|!=
...
name|println
argument_list|(
literal|" --perf  Write the same key<N> times to a WAL: e.g. FSHLog --perf 10"
argument_list|)
expr_stmt|;
block|}
end_function
...
argument_list|)
decl_stmt|;
comment|// Put up a WAL and just keep adding same edit to it.  Simple perf test.
name|Configuration
name|conf
init|=
name|HBaseConfiguration
