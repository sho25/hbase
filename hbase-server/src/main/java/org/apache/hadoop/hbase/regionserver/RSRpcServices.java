...
decl_stmt|;
comment|/** RPC scheduler to use for the master. */
specifier|public
specifier|static
specifier|final
name|String
...
init|=
literal|"hbase.master.rpc.scheduler.factory.class"
decl_stmt|;
...
return|;
block|}
comment|/**    * Execute an increment mutation.    */
specifier|private
name|Result
name|increment
parameter_list|(
...
name|error
argument_list|(
literal|"ZooKeeper permission watcher initialization failed"
argument_list|,
name|e
argument_list|)
expr_stmt|;
...
condition|)
block|{
comment|// passed in requested state is same as current state. No action required
return|return
name|response
return|;
block|}
...
name|warn
argument_list|(
literal|"OpenRegionRequest for {} does not have a start code"
argument_list|,
name|request
operator|.
...
name|warn
argument_list|(
literal|"CloseRegionRequest for {} does not have a start code"
argument_list|,
name|request
operator|.
name|getRegion
...
specifier|private
name|void
name|checkShouldRejectReplicationRequest
parameter_list|(
name|List
argument_list|<
name|WALEntry
...
return|return;
block|}
comment|// We can ensure that all entries are for one peer, so only need to check one entry's
comment|// table name. if the table hit sync replication at peer side and the peer cluster
comment|// is (or is transiting to) state ACTIVE or DOWNGRADE_ACTIVE, we should reject to apply
comment|// those entries according to the design doc.
name|TableName
name|table
init|=
name|TableName
...
name|DoNotRetryIOException
argument_list|(
literal|"Reject to apply to sink cluster because sync replication state of sink cluster "
operator|+
literal|"is ACTIVE or DOWNGRADE_ACTIVE, table: "
operator|+
name|table
argument_list|)
throw|;
block|}
block|}
comment|/**    * Replicate WAL entries on the region server.    * @param controller the RPC controller    * @param request the request    */
annotation|@
name|Override
annotation|@
name|QosPriority
...
specifier|private
name|boolean
name|shouldRejectRequestsFromClient
parameter_list|(
name|HRegion
name|region
parameter_list|)
...
specifier|private
name|boolean
name|isReplicationRequest
parameter_list|(
name|Action
name|action
parameter_list|)
block|{
comment|// replication request can only be put or delete.
if|if
condition|(
operator|!
name|action
...
return|;
block|}
comment|// replication will set a special attribute so we can make use of it to decide whether a request
comment|// is for replication.
return|return
name|mutation
operator|.
...
condition|)
block|{
comment|// We only allow replication in standby state and it will not set the atomic flag.
if|if
condition|(
name|rejectIfFromClient
condition|)
...
condition|)
block|{
comment|// fail if it is not a replication request
name|failRegionAction
argument_list|(
name|responseBuilder
argument_list|,
...
condition|)
block|{
comment|// No more cells for the scan here, we need to ensure that the mayHaveMoreCellsInRow of
comment|// last result is false. Otherwise it's possible that: the first nextRaw returned
comment|// because BATCH_LIMIT_REACHED (BTW it happen to exhaust all cells of the scan),so the
comment|// last result's mayHaveMoreCellsInRow will be true. while the following nextRaw will
comment|// return with moreRows=false, which means moreResultsInRegion would be false, it will
comment|// be a contradictory state (HBASE-21206).
name|int
name|lastIdx
init|=
name|results
...
specifier|private
name|void
name|executeOpenRegionProcedures
parameter_list|(
name|OpenRegionRequest
name|request
parameter_list|,
...
parameter_list|)
block|{
comment|// Here we do not fail the whole method since we also need deal with other
comment|// procedures, and we can not ignore this one, so we still schedule a
comment|// AssignRegionHandler and it will report back to master if we still can not get the
comment|// TableDescriptor.
name|LOG
operator|.
name|warn
argument_list|(
literal|"Failed to get TableDescriptor of {}, will try again in the handler"
argument_list|,
name|regionInfo
operator|.
name|getTable
...
specifier|private
name|void
name|executeCloseRegionProcedures
parameter_list|(
name|CloseRegionRequest
name|request
parameter_list|)
...
name|warn
argument_list|(
literal|"Failed to instantiating remote procedure {}, pid={}"
argument_list|,
name|request
operator|.
name|getProcClass
...
name|debug
argument_list|(
literal|"Executing remote procedure {}, pid={}"
argument_list|,
name|callable
operator|.
name|getClass
...
condition|)
block|{
comment|// Avoid reading from the TableDescritor every time(usually it will read from the file
comment|// system)
name|Map
argument_list|<
name|TableName
