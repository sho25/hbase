...
decl_stmt|;
name|boolean
name|enforceMaxResultSizeAtCellLevel
init|=
name|clientHandlesPartials
operator|&&
name|serverGuaranteesOrderOfPartials
...
name|IOException
argument_list|(
literal|"NextState returned from call to nextRaw was invalid"
argument_list|)
throw|;
block|}
if|if
...
condition|)
block|{
comment|// The state should always contain an estimate of the result size because that
comment|// estimate must be used to decide when partial results are formed.
name|boolean
name|skipResultSizeCalculation
init|=
name|state
operator|.
...
argument_list|)
expr_stmt|;
comment|// If the calculation can't be skipped, then do it now.
if|if
condition|(
operator|!
name|skipResultSizeCalculation
...
block|}
block|}
comment|// The size limit was reached. This means there are more cells remaining in
comment|// the row but we had to stop because we exceeded our max result size. This
comment|// indicates that we are returning a partial result
specifier|final
name|boolean
name|partial
init|=
