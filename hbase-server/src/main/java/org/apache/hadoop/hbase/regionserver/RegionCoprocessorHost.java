...
name|sharedData
expr_stmt|;
comment|// Pick which version of the WAL related events we'll call.
comment|// This way we avoid calling the new version on older RegionObservers so
comment|// we can maintain binary compatibility.
comment|// See notes in javadoc for RegionObserver
name|useLegacyPre
operator|=
...
name|IOException
block|{
comment|// Once we don't need to support the legacy call, replace RegionOperation with a version
comment|// that's ObserverContext<RegionEnvironment> and avoid this cast.
specifier|final
name|RegionEnvironment
name|env
init|=
