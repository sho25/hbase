...
decl_stmt|;
comment|// flags if we should check EXEC permissions
name|boolean
name|shouldCheckExecPermissions
decl_stmt|;
specifier|private
specifier|volatile
name|boolean
...
specifier|private
name|void
name|requireCoveringPermission
parameter_list|(
name|OpType
name|request
parameter_list|,
...
argument_list|()
decl_stmt|;
comment|// First check table or CF level permissions, if they grant access we can
comment|// early out before needing to enumerate over per KV perms.
name|List
argument_list|<
name|Action
argument_list|>
name|cellCheckActions
init|=
name|Lists
operator|.
name|newArrayList
argument_list|()
decl_stmt|;
comment|// TODO: permissionGranted should support checking multiple actions or
comment|// we should convert actions into a bitmap and pass that around. See
comment|// HBASE-7123.
name|AuthResult
name|results
index|[]
...
index|]
operator|+
literal|", added to cellCheckActions"
argument_list|)
expr_stmt|;
block|}
name|cellCheckActions
...
block|}
block|}
comment|// If all permissions checks passed, we can early out
if|if
condition|(
name|cellCheckActions
operator|.
...
name|trace
argument_list|(
literal|"All permissions checks passed, we can early out"
argument_list|)
expr_stmt|;
block|}
for|for
...
name|trace
argument_list|(
literal|"No cells found with scan"
argument_list|)
expr_stmt|;
block|}
name|AuthResult
...
throw|;
block|}
comment|// Log that authentication succeeded. We need to trade off logging maybe
comment|// thousands of fine grained decisions with providing detail.
for|for
control|(
name|byte
index|[]
