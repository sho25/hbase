...
begin_comment
comment|/**  * Provides basic authorization checks for data access and administrative  * operations.  *  *<p>  * {@code AccessController} performs authorization checks for HBase operations  * based on:  *<ul>  *<li>the identity of the user performing the operation</li>  *<li>the scope over which the operation is performed, in increasing  *   specificity: global, table, column family, or qualifier</li>  *<li>the type of action being performed (as mapped to  *   {@link Permission.Action} values)</li>  *</ul>  * If the authorization check fails, an {@link AccessDeniedException}  * will be thrown for the operation.  *</p>  *  *<p>  * To perform authorization checks, {@code AccessController} relies on the  * RpcServerEngine being loaded to provide  * the user identities for remote requests.  *</p>  *  *<p>  * The access control lists used for authorization can be manipulated via the  * exposed {@link AccessControlService} Interface implementation, and the associated  * {@code grant}, {@code revoke}, and {@code user_permission} HBase shell  * commands.  *</p>  */
end_comment
...
argument_list|)
expr_stmt|;
comment|// set up the list of users with superuser privilege
name|User
name|user
init|=
name|userProvider
...
argument_list|()
decl_stmt|;
comment|// hasSomeAccess is called from bulkload pre hooks
name|List
argument_list|<
name|UserPermission
argument_list|>
...
comment|// any concrete set of table names when regex is present.
if|if
condition|(
name|regex
operator|==
...
block|}
block|}
comment|// Throws exception if none of the matched tables are authorized for the user.
if|if
condition|(
name|numMatchedTables
operator|>
...
name|AccessDeniedException
argument_list|(
literal|"Insufficient permissions for accessing tables"
argument_list|)
throw|;
block|}
block|}
