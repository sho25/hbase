...
argument_list|()
decl_stmt|;
comment|// A flag to indicate if we are using ZK for region assignment
specifier|private
specifier|final
name|boolean
...
argument_list|()
decl_stmt|;
comment|/**    * Constructs a new assignment manager.    *    * @param server instance of HMaster this AM running inside    * @param serverManager serverManager for associated HMaster    * @param balancer implementation of {@link LoadBalancer}    * @param service Executor service    * @param metricsMaster metrics manager    * @param tableLockManager TableLock manager    * @throws KeeperException    * @throws IOException    */
specifier|public
name|AssignmentManager
parameter_list|(
name|Server
...
block|}
block|}
comment|// TODO: This code is tied to ZK anyway, so for now leaving it as is,
comment|// will refactor when whole region assignment will be abstracted from ZK
name|BaseCoordinatedStateManager
name|cp
init|=
operator|(
...
block|}
block|}
comment|/**    * Handles various states an unassigned node can be in.    *<p>    * Method is called when a state change is suspected for an unassigned node.    *<p>    * This deals with skipped transitions (we got a CLOSED but didn't see CLOSING    * yet).    * @param rt region transition    * @param coordination coordination for opening region    * @param ord details about opening region    */
name|void
name|handleRegion
parameter_list|(
specifier|final
...
comment|// In case not using ZK, we don't need this check because
comment|// we have the latest info in memory, and the caller
comment|// will do another round checking any way.
if|if
condition|(
name|useZKForAssignment
operator|&&
...
name|error
argument_list|(
literal|"Failed to transtion region from "
operator|+
name|current
operator|+
literal|" to "
