...
argument_list|)
decl_stmt|;
comment|// A temp ZK watcher for bulk assigner to avoid deadlock,
comment|// will be removed in HBASE-6977
comment|//
comment|// A separate ZK watcher used for async ZK node offline.
comment|// We can't use that exiting one because it could lead to
comment|// deadlocks if its event thread asks for a locker held by a bulk
comment|// assigner thread. This watcher is just for async ZK node offline.
comment|// In HBASE-6977, we are going to process assignment ZK events
comment|// outside of ZK event thread, so there won't be deadlock
comment|// threat anymore.  That's when this watcher to be removed.
specifier|private
specifier|final
name|ZooKeeperWatcher
name|asyncOfflineZKWatcher
decl_stmt|;
...
name|conf
argument_list|,
literal|"async offline ZK watcher"
argument_list|,
name|server
argument_list|)
expr_stmt|;
...
try|try
block|{
comment|// Set the ZK watcher explicitly
name|ZKAssign
operator|.
name|getData
argument_list|(
...
name|abort
argument_list|(
literal|"Unexpected exception watching ZKAssign node"
argument_list|,
name|e
argument_list|)
expr_stmt|;
