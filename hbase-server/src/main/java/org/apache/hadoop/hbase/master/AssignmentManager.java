...
condition|)
block|{
comment|// Get the region from region states map/meta. However, we
comment|// may still can't get it, for example, for online region merge,
comment|// the znode uses the new region to be created, which may not in meta
comment|// yet if the merging is still going on during the master recovery.
name|hri
operator|=
name|regionStates
operator|.
