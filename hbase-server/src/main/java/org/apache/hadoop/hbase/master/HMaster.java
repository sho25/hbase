...
block|}
block|}
comment|/**    * Check<code>hbase:meta</code> is assigned. If not, assign it.    * @param status MonitoredTask    * @param previouslyFailedMetaRSs    * @param replicaId    * @throws InterruptedException    * @throws IOException    * @throws KeeperException    */
name|void
name|assignMeta
parameter_list|(
name|MonitoredTask
...
name|info
argument_list|(
literal|"Meta with replicaId "
operator|+
name|replicaId
operator|+
literal|" was in transition on "
operator|+
name|currentMetaServer
argument_list|)
expr_stmt|;
...
name|info
argument_list|(
literal|"Re-assigning hbase:meta with replicaId, "
operator|+
name|replicaId
operator|+
literal|" it was on "
...
return|;
block|}
comment|/**    * Perform normalization of cluster (invoked by {@link RegionNormalizerChore}).    *    * @return true if normalization step was performed successfully, false otherwise    *   (specifically, if HMaster hasn't been initialized properly or normalization    *   is globally disabled)    * @throws IOException    */
specifier|public
name|boolean
...
name|debug
argument_list|(
literal|"Skipping normalizing "
operator|+
name|table
operator|+
literal|" since its namespace has quota"
argument_list|)
expr_stmt|;
continue|continue;
block|}
