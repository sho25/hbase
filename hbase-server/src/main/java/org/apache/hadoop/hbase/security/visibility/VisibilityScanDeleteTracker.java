...
argument_list|)
decl_stmt|;
comment|/**    * This tag is used for the DELETE cell which has no visibility label.    */
specifier|private
specifier|static
specifier|final
name|List
...
control|)
block|{
comment|// visibilityTagsDeleteFamily is ArrayList
name|Triple
argument_list|<
name|List
argument_list|<
...
condition|)
block|{
comment|// A return type of FAMILY_DELETED will cause skip for all remaining cells from
comment|// this
comment|// family. We would like to match visibility expression on every put cells after
comment|// this and only remove those matching with the family delete visibility. So we
comment|// are
comment|// returning FAMILY_VERSION_DELETED from here.
return|return
name|DeleteResult
operator|.
...
control|)
block|{
comment|// visibilityTagsDeleteFamilyVersion is ArrayList
name|Triple
argument_list|<
name|List
argument_list|<
...
literal|null
expr_stmt|;
comment|// Can nullify this because we are moving to the next column
name|visibilityTagsDeleteColumns
operator|=
literal|null
expr_stmt|;
