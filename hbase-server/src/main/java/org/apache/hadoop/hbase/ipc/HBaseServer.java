...
begin_comment
comment|/** A client for an IPC service.  IPC calls take a single Protobuf message as a  * parameter, and return a single Protobuf message as their value.  A service runs on  * a port and is defined by a parameter class and a value class.  *  *  *<p>Copied local so can fix HBASE-900.  *  * @see HBaseClient  */
end_comment
...
return|;
block|}
comment|/** Returns the server instance called under or null.  May be called under    * {@link #call(Class, RpcRequestBody, long, MonitoredRPCHandler)} implementations,    * and under protobuf methods of paramters and return values.    * Permits applications to access the server context.    * @return HBaseServer    */
specifier|public
specifier|static
name|RpcServer
name|get
...
comment|// responseQueuesSizeThrottler is shared among all responseQueues,
comment|// it bounds memory occupied by responses in all responseQueues
specifier|final
name|SizeBasedThrottler
name|responseQueuesSizeThrottler
decl_stmt|;
comment|// RESPONSE_QUEUE_MAX_SIZE limits total size of responses in every response queue
specifier|private
specifier|static
specifier|final
name|long
name|DEFAULT_RESPONSE_QUEUES_MAX_SIZE
init|=
literal|1024
operator|*
literal|1024
...
specifier|final
name|String
name|RESPONSE_QUEUES_MAX_SIZE
init|=
literal|"ipc.server.response.queue.maxsize"
decl_stmt|;
specifier|volatile
specifier|protected
name|boolean
...
condition|)
block|{
comment|// Error flag is set, so returning here closes connection and
comment|// clears responseQueue.
return|return
literal|true
return|;
block|}
...
condition|)
block|{
comment|// Connection was closed when we tried to submit response, but we
comment|// increased responseQueues size already. It shoud be
comment|// decreased here.
name|responseQueuesSizeThrottler
operator|.
name|decrease
argument_list|(
...
literal|false
decl_stmt|;
comment|// indicates if connection was closed
specifier|protected
name|SocketChannel
name|channel
decl_stmt|;
