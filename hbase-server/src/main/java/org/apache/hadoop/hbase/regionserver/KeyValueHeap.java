...
return|;
block|}
comment|/**    * Gets the next row of keys from the top-most scanner.    *<p>    * This method takes care of updating the heap.    *<p>    * This can ONLY be called when you are using Scanners that implement InternalScanner as well as    * KeyValueScanner (a {@link StoreScanner}).    * @param result    * @param limit    * @return state where NextState#hasMoreValues() is true if more keys exist after this    *         one, false if scanner is done    */
specifier|public
name|NextState
name|next
parameter_list|(
...
name|IOException
argument_list|(
literal|"Invalid state returned from InternalScanner#next"
argument_list|)
throw|;
block|}
name|boolean
...
return|;
block|}
comment|/**    * Gets the next row of keys from the top-most scanner.    *<p>    * This method takes care of updating the heap.    *<p>    * This can ONLY be called when you are using Scanners that implement InternalScanner as well as    * KeyValueScanner (a {@link StoreScanner}).    * @param result    * @return state where NextState#hasMoreValues() is true if more keys exist after this    *         one, false if scanner is done    */
specifier|public
name|NextState
name|next
parameter_list|(
