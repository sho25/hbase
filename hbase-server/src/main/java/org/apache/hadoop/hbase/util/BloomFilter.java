...
block|{
comment|/**    * Check if the specified key is contained in the bloom filter.    * Used in ROW_COL blooms where the blooms are serialized as KeyValues    * @param keyCell the key to check for the existence of    * @param bloom bloom filter data to search. This can be null if auto-loading    *        is supported.    * @return true if matched by bloom, false if not    */
name|boolean
name|contains
parameter_list|(
name|Cell
