...
comment|// rack at a time. For example, if 2 racks (r1 and r2) with 8 servers (s1..s8) each, it will
comment|// choose s1 from r1, s1 from r2, s2 from r1, s2 from r2, ...
name|void
...
literal|0
decl_stmt|;
comment|// Restore the current process index from the currentProcessIndexMap
name|Integer
name|currentProcessIndex
init|=
name|currentProcessIndexMap
operator|.
name|get
...
condition|)
block|{
comment|// Reset the server index for the current rack
name|serverIndex
operator|=
literal|0
expr_stmt|;
block|}
comment|// Keep track of the next processing index
name|currentProcessIndexMap
operator|.
name|put
argument_list|(
