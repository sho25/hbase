...
name|MasterProtos
operator|.
name|GetFailedSplitMergeLegacyRegionsResponse
...
argument_list|()
decl_stmt|;
comment|// Case 1. find orphan region on fs
comment|// orphan regions may due to a failed split region procedure, which daughter regions are created
comment|// then the procedure is aborted. Or merged region is created then the procedure is aborted.
name|List
argument_list|<
name|String
argument_list|>
...
name|RegionErrorType
operator|.
name|ORPHAN_REGION_ON_FS
argument_list|)
argument_list|)
expr_stmt|;
comment|// Case 2. find unassigned daughter regions or merged regions
name|List
argument_list|<
name|String
argument_list|>
name|unassignedDaughterOrMergedRegions
init|=
name|findUnassignedDaughterOrMergedRegions
argument_list|(
name|tableName
argument_list|)
decl_stmt|;
...
name|RegionErrorType
operator|.
name|DAUGHTER_MERGED_REGION_NOT_ONLINE
argument_list|)
argument_list|)
expr_stmt|;
comment|// if these regions in problemRegions are currently handled by SplitTableRegionProcedure or
comment|// MergeTableRegionsProcedure, we should remove them from this map
name|master
operator|.
name|getProcedures
argument_list|()
...
lambda|->
block|{
lambda|if (p instanceof SplitTableRegionProcedure
argument_list|)
block|{
name|problemRegions
operator|.
...
begin_comment
comment|// check if regions are still problematic now
end_comment

begin_expr_stmt
name|checkRegionStillProblematic
argument_list|(
name|problemRegions
argument_list|,
name|tableName
...
name|ORPHAN_REGION_ON_FS
case|:
comment|// region is build for this directory, it is not a problematic region any more
if|if
condition|(
name|r
operator|!=
...
name|IOException
argument_list|(
literal|"there should be no problematic region of this type"
argument_list|)
throw|;
block|}
block|}
...
argument_list|)
expr_stmt|;
comment|// find unassigned merged or split daughter region
return|return
name|checkRegions
operator|.
name|stream
...
name|IOException
block|{
comment|// get available regions from meta, merged region should be consider available
name|HashSet
argument_list|<
name|String
argument_list|>
...
argument_list|)
expr_stmt|;
comment|// get regionInfo from fs
name|Path
name|tableDir
init|=
name|FSUtils
