...
name|MasterProtos
operator|.
name|GetRegionStateInMetaResponse
import|;
end_import
...
argument_list|)
decl_stmt|;
comment|// TODO: support CompactType.MOB
...
name|warn
argument_list|(
literal|"CompactType.MOB is not supported yet, will run regular compaction."
operator|+
literal|" Refer to HBASE-23571."
argument_list|)
expr_stmt|;
return|return
name|super
...
block|{
comment|// Is it a MOB name? These work differently.
name|byte
index|[]
name|regionName
init|=
...
block|{
comment|// If unknown RegionInfo and not a MOB region, it is unknown.
throw|throw
operator|new
name|ServiceException
argument_list|(
...
name|debug
argument_list|(
literal|"Received region space usage report but HMaster is not ready to process it, "
operator|+
literal|"skipping"
argument_list|)
expr_stmt|;
...
name|debug
argument_list|(
literal|"Received space quota region size report but HMaster is not ready to process it,"
operator|+
literal|"skipping"
argument_list|)
expr_stmt|;
...
block|}
block|}
comment|/**    * Update state of the region in meta only. This is required by hbck in some situations to cleanup    * stuck assign/ unassign regions procedures for the table.    *    * @return previous states of the regions    */
annotation|@
name|Override
specifier|public
name|GetRegionStateInMetaResponse
...
name|trace
argument_list|(
literal|"region info loaded from meta table: {}"
argument_list|,
name|info
argument_list|)
expr_stmt|;
...
name|info
argument_list|(
literal|"{} set region={} state from {} to {}"
argument_list|,
name|master
operator|.
name|getClientIdAuditPrefix
...
argument_list|)
expr_stmt|;
comment|//Loads from meta again to refresh AM cache with the new region state
name|this
operator|.
name|master
operator|.
...
argument_list|()
operator|+
literal|" list tables in rsgroup "
operator|+
name|groupName
argument_list|)
expr_stmt|;
...
argument_list|()
operator|+
literal|" get configured namespaces and tables in rsgroup "
operator|+
name|groupName
argument_list|)
expr_stmt|;
