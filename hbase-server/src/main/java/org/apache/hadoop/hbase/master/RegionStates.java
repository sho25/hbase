...
return|;
block|}
comment|/**    * @return True if hbase:meta table region is in transition.    */
specifier|public
specifier|synchronized
name|boolean
...
argument_list|)
decl_stmt|;
comment|// When region is online and remove from regionsInTransition,
comment|// update the RIT duration to assignment manager metrics
if|if
condition|(
name|regionState
operator|!=
...
name|HRegionInfo
argument_list|>
name|regionsToCleanIfNoMetaEntry
init|=
operator|new
name|HashSet
argument_list|<
...
return|;
block|}
comment|/**    * This method does an RPC to hbase:meta. Do not call this method with a lock/synchronize held.    * @param hris The hris to check if empty in hbase:meta and if so, clean them up.    */
specifier|private
name|void
name|cleanIfNoMetaEntry
parameter_list|(
...
try|try
block|{
comment|// This is RPC to meta table. It is done while we have a synchronize on
comment|// regionstates. No progress will be made if meta is not available at this time.
comment|// This is a cleanup task. Not critical.
if|if
condition|(
name|MetaTableAccessor
operator|.
...
name|warn
argument_list|(
literal|"Got exception while deleting "
operator|+
name|hri
operator|+
literal|" directories from file system."
argument_list|,
name|e
argument_list|)
expr_stmt|;
...
return|;
block|}
comment|/**    * Gets current state of all regions of the table.    * This method looks at the in-memory state.  It does not go to<code>hbase:meta</code>.    * Method guaranteed to return keys for all states    * in {@link org.apache.hadoop.hbase.master.RegionState.State}    *    * @param tableName    * @return Online regions from<code>tableName</code>    */
specifier|public
specifier|synchronized
name|Map
argument_list|<
...
return|;
block|}
comment|/**    * Get the HRegionInfo from cache, if not there, from the hbase:meta table.    * Be careful. Does RPC. Do not hold a lock or synchronize when you call this method.    * @param  regionName    * @return HRegionInfo for the region    */
annotation|@
...
argument_list|)
decl_stmt|;
comment|// When region transform old region state to new region state,
comment|// accumulate the RIT duration to new region state.
if|if
condition|(
name|oldRegionState
operator|!=
