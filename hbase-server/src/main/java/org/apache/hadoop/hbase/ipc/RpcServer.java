...
specifier|final
name|String
name|MULTI_MUTATIONS
init|=
literal|"multi.mutations"
decl_stmt|;
specifier|private
specifier|static
specifier|final
name|String
name|MULTI_SERVICE_CALLS
init|=
literal|"multi.service_calls"
decl_stmt|;
specifier|private
specifier|static
specifier|final
name|String
name|GET_SLOW_LOG_RESPONSES
init|=
literal|"GetSlowLogResponses"
decl_stmt|;
specifier|private
specifier|static
specifier|final
name|String
name|CLEAR_SLOW_LOGS_RESPONSES
init|=
literal|"ClearSlowLogsResponses"
decl_stmt|;
specifier|private
specifier|final
name|boolean
...
specifier|final
name|int
name|DEFAULT_TRACE_LOG_MAX_LENGTH
init|=
literal|1000
decl_stmt|;
specifier|protected
...
name|rsRpcServices
decl_stmt|;
comment|/**    * Use to add online slowlog responses    */
specifier|private
name|SlowLogRecorder
name|slowLogRecorder
decl_stmt|;
...
condition|)
block|{
comment|// send logs to ring buffer owned by slowLogRecorder
specifier|final
name|String
name|className
init|=
...
block|}
block|}
comment|/**    * Logs an RPC response to the LOG file, producing valid JSON objects for    * client Operations.    * @param param The parameters received in the call.    * @param methodName The name of the method invoked    * @param call The string representation of the call    * @param tooLarge To indicate if the event is tooLarge    * @param tooSlow To indicate if the event is tooSlow    * @param clientAddress The address of the client who made this call.    * @param startTime The time that the call was initiated, in ms.    * @param processingTime The duration that the call took to run, in ms.    * @param qTime The duration that the call spent on the queue    *   prior to being initiated, in ms.    * @param responseSize The size in bytes of the response buffer.    * @param userName UserName of the current RPC Call    */
name|void
name|logResponse
parameter_list|(
name|Message
...
expr_stmt|;
block|}
comment|/**    * Truncate to number of chars decided by conf hbase.ipc.trace.log.max.length    * if TRACE is on else to 150 chars Refer to Jira HBASE-20826 and HBASE-20942    * @param strParam stringifiedParam to be truncated    * @return truncated trace log string    */
annotation|@
name|VisibleForTesting
name|String
name|truncateTraceLog
...
return|;
block|}
comment|/**    * Used by {@link org.apache.hadoop.hbase.procedure2.store.region.RegionProcedureStore}. For    * master's rpc call, it may generate new procedure and mutate the region which store procedure.    * There are some check about rpc when mutate region, such as rpc timeout check. So unset the rpc    * call to avoid the rpc check.    * @return the currently ongoing rpc call    */
specifier|public
specifier|static
name|Optional
argument_list|<
...
return|;
block|}
comment|/**    * Used by {@link org.apache.hadoop.hbase.procedure2.store.region.RegionProcedureStore}. Set the    * rpc call back after mutate region.    */
specifier|public
specifier|static
name|void
name|setCurrentCall
