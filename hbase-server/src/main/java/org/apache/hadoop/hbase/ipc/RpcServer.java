...
comment|/**    * This flag is used to indicate to sub threads when they should go down.  When we call    * {@link #startThreads()}, all threads started will consult this flag on whether they should    * keep going.  It is set to false when {@link #stop()} is called.    */
specifier|volatile
name|boolean
name|running
init|=
literal|true
decl_stmt|;
comment|/**    * This flag is set to true after all threads are up and 'running' and the server is then opened    * for business by the calle to {@link #openServer()}.    */
specifier|volatile
name|boolean
name|started
init|=
