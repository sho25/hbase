...
argument_list|()
expr_stmt|;
comment|// This will return back the BBs which we
comment|// got from pool.
name|this
operator|.
name|cellBlockStream
operator|=
...
name|cellBlockSize
decl_stmt|;
comment|// The byte[] should also hold the totalSize of the header, message and the cellblock
name|byte
index|[]
name|b
init|=
...
index|]
decl_stmt|;
comment|// The RpcClient expects the int to be in a format that code be decoded by
comment|// the DataInputStream#readInt(). Hence going with the Bytes.toBytes(int)
comment|// form of writing int.
name|Bytes
operator|.
name|putInt
argument_list|(
