...
argument_list|(
literal|"Unknown TransitionType: "
operator|+
name|type
argument_list|)
throw|;
...
condition|)
block|{
comment|// Throw exception to backoff and retry when failed open too many times
throw|throw
operator|new
name|HBaseIOException
argument_list|(
literal|"Failed confirm OPEN of "
operator|+
name|regionNode
operator|+
literal|" (remote log may yield more detail on why)."
argument_list|)
throw|;
block|}
else|else
...
name|warn
argument_list|(
literal|"There is no outstanding remote region procedure for {}, serverName={}, code={},"
operator|+
literal|" seqId={}, proc={}, should be a retry, ignore"
argument_list|,
name|regionNode
argument_list|,
name|serverName
...
return|return;
block|}
comment|// The procId could be -1 if it is from an old region server, we need to deal with it so that we
comment|// can do rolling upgraing.
if|if
condition|(
name|procId
operator|>=
...
name|warn
argument_list|(
literal|"The pid of remote region procedure for {} is {}, the reported pid={}, serverName={},"
operator|+
literal|" code={}, seqId={}, proc={}, should be a retry, ignore"
argument_list|,
name|regionNode
argument_list|,
name|remoteProc
...
condition|)
block|{
comment|// this means we are waiting for the sub procedure, so wake it up
name|remoteProc
operator|.
name|serverCrashed
argument_list|(
...
block|{
comment|// we are in RUNNING state, just update the region state, and we will process it later.
name|env
operator|.
name|getAssignmentManager
argument_list|()
...
name|IllegalArgumentException
argument_list|(
literal|"Unknown RegionTransitionType: "
operator|+
name|type
argument_list|)
throw|;
...
name|IllegalArgumentException
argument_list|(
literal|"Unknown transition type: "
operator|+
name|type
argument_list|)
throw|;
