...
name|warn
argument_list|(
literal|"Could not pick lowest locality region server"
argument_list|)
expr_stmt|;
return|return
name|Cluster
...
block|{
comment|// Pick lowest locality region on this server
name|thisRegion
operator|=
name|pickLowestLocalityRegionOnServer
argument_list|(
name|cluster
argument_list|,
name|thisServer
...
return|;
block|}
comment|// Pick the least loaded server with good locality for the region
name|int
name|otherServer
init|=
name|cluster
...
return|;
block|}
comment|// Let the candidate region be moved to its highest locality server.
name|int
name|otherRegion
init|=
operator|-
...
name|cluster
operator|.
name|getLowestLocalityRegionServer
argument_list|()
return|;
block|}
specifier|private
...
name|cluster
operator|.
name|hasUnevenRegionDistribution
argument_list|()
condition|)
block|{
name|Pair
...
name|Integer
argument_list|>
name|leastAndMostLoadedServers
init|=
name|cluster
operator|.
name|findLeastAndMostLoadedServers
argument_list|()
decl_stmt|;
return|return
name|moveFromTableWithEnoughRegions
argument_list|(
name|cluster
argument_list|,
name|leastAndMostLoadedServers
...
block|}
return|return
name|findBestActionForTableServer
argument_list|(
name|cluster
argument_list|,
name|tableServer
...
block|}
block|}
comment|/**      * Returns a move fromServer -> toServer such that after the move fromServer will still have at least      * the min # regions in terms of table skew calculation      */
specifier|private
name|Action
name|moveFromTableWithEnoughRegions
parameter_list|(
...
name|cluster
operator|.
name|minRegionsIfEvenlyDistributed
argument_list|(
name|table
argument_list|)
decl_stmt|;
...
name|fromServer
argument_list|,
name|pickRandomRegionFromTableOnServer
argument_list|(
name|cluster
argument_list|,
name|fromServer
...
return|;
block|}
comment|/**      * Picks a random subset of tables, then for each table T checks across cluster and returns first      * server (if any) which holds too many regions from T. Returns Optional.absent() if no servers      * are found that hold too many regions.      */
specifier|private
name|Optional
argument_list|<
name|TableAndServer
...
name|cluster
operator|.
name|maxRegionsIfEvenlyDistributed
argument_list|(
name|table
argument_list|)
decl_stmt|;
name|int
name|numShouldHaveMaxRegions
init|=
name|cluster
operator|.
name|numServersWithMaxRegionsIfEvenlyDistributed
argument_list|(
name|table
argument_list|)
decl_stmt|;
name|int
name|numWithMaxRegions
init|=
literal|0
decl_stmt|;
for|for
...
index|]
decl_stmt|;
comment|// if more than max, server clearly has too many regions
if|if
condition|(
name|numRegions
operator|>
...
break|break;
block|}
comment|// if equal to max, check to see if we are within acceptable limit
if|if
condition|(
name|numRegions
operator|==
...
name|numShouldHaveMaxRegions
expr_stmt|;
comment|// If we have found a table with more than max, we are done
if|if
condition|(
name|tableServer
operator|.
...
break|break;
block|}
comment|// Otherwise, check to see if there are too many servers with maxRegions
if|if
condition|(
name|numWithMaxRegions
operator|>
...
return|;
block|}
comment|/**      * Returns an list of integers that stores [upper - lower] unique integers in random order      * s.t. for each integer i lower<= i< upper      */
specifier|private
name|List
argument_list|<
name|Integer
...
return|;
block|}
comment|/**      * Pick a random region from the specified server and table. Returns -1 if no regions from      * the given table lie on the given server      */
specifier|protected
name|int
name|pickRandomRegionFromTableOnServer
parameter_list|(
...
return|;
block|}
comment|/**      * Returns servers in the cluster that store fewer than k regions for the given table (sorted by      * servers with the fewest regions from givenTable first)      */
specifier|public
name|List
argument_list|<
name|Integer
argument_list|>
name|getServersWithFewerThanKRegionsFromTable
parameter_list|(
specifier|final
name|Cluster
name|cluster
...
return|;
block|}
comment|/**      * Given a table T for which server S stores too many regions, attempts to find a      * SWAP operation that will better balance the cluster      */
specifier|public
name|Action
name|findBestActionForTableServer
parameter_list|(
...
return|;
block|}
comment|// If we cannot perform a swap, we should do nothing
return|return
name|Cluster
operator|.
name|NullAction
...
name|Integer
argument_list|>
name|candidateServers
parameter_list|)
block|{
comment|// Because conditions (1) and (4) are true apriori, we only need to meet one of conditions (2) or (3)
name|List
argument_list|<
name|Integer
argument_list|>
...
literal|35
decl_stmt|;
comment|/**      * Ranges from 0.0 to 1.0 and is the proportion of how much the most skewed table      * (as opposed to the average skew across all tables) should affect TableSkew cost      */
specifier|private
specifier|static
specifier|final
name|String
name|MAX_TABLE_SKEW_WEIGHT_KEY
init|=
literal|"hbase.master.balancer.stochastic.maxTableSkewWeight"
decl_stmt|;
specifier|private
name|float
name|DEFAULT_MAX_TABLE_SKEW_WEIGHT
init|=
literal|0.0f
decl_stmt|;
specifier|private
specifier|final
name|float
name|maxTableSkewWeight
decl_stmt|;
specifier|private
specifier|final
name|float
name|avgTableSkewWeight
decl_stmt|;
comment|// Number of moves for each table required to bring the cluster to a perfectly balanced
comment|// state (i.e. as if you had round-robin-ed regions across cluster)
specifier|private
name|int
index|[]
name|numMovesPerTable
decl_stmt|;
name|TableSkewCostFunction
parameter_list|(
name|Configuration
...
name|numMovesPerTable
operator|=
name|computeNumMovesPerTable
argument_list|()
expr_stmt|;
block|}
comment|/**      * Adjusts computed number of moves after two regions have been swapped      */
annotation|@
name|Override
specifier|protected
name|void
...
parameter_list|)
block|{
comment|// If different tables, simply perform two moves
if|if
condition|(
name|cluster
operator|.
...
return|return;
block|}
comment|// If same table, do nothing
block|}
comment|/**      * Adjusts computed number of moves per table after a region has been moved      */
annotation|@
name|Override
specifier|protected
name|void
...
index|]
operator|=
name|computeNumMovesForTable
argument_list|(
name|table
argument_list|)
expr_stmt|;
block|}
comment|/**      * Returns a mapping of table -> numMoves, where numMoves is the number of regions required to bring      * each table to a fully balanced state (i.e. as if its regions had been round-robin-ed across the cluster).      */
specifier|private
name|int
index|[]
name|computeNumMovesPerTable
parameter_list|()
block|{
comment|// Determine # region moves required for each table to have regions perfectly distributed across cluster
name|int
index|[]
name|numMovesPerTable
init|=
...
return|;
block|}
comment|/**      * Computes the number of moves required across all servers to bring the given table to a balanced state      * (i.e. as if its regions had been round-robin-ed across the cluster). We only consider moves as # of regions      * that need to be sent, not received, so that we do not double count region moves.      */
specifier|private
name|int
name|computeNumMovesForTable
parameter_list|(
...
return|;
block|}
comment|/**      * Returns mapping of tableIndex -> tableSkewScore, where tableSkewScore is a double between 0 to 1 with      * 0 indicating no table skew (i.e. perfect distribution of regions among servers), and 1 representing      * pathological table skew (i.e. all of a servers regions belonging to one table).      */
specifier|private
name|double
index|[]
name|computeSkewPerTable
...
block|{
name|int
name|numTotalRegions
init|=
name|cluster
operator|.
name|numRegionsPerTable
index|[
name|table
index|]
decl_stmt|;
...
return|;
block|}
comment|/**      * Returns the max of the values in the passed array      */
specifier|private
name|double
name|max
parameter_list|(
...
return|;
block|}
comment|/**      * Returns the average of the values in the passed array      */
specifier|private
name|double
name|average
parameter_list|(
...
expr_stmt|;
block|}
comment|// Still return high cost for single region replicas being cohosted even as cluster scales
return|return
name|Math
operator|.
name|sqrt
...
block|}
block|}
comment|/**    * Data structure that holds table and server indexes    */
specifier|static
class|class
name|TableAndServer
block|{
