...
return|;
block|}
comment|// randomly select one primaryIndex out of all region replicas in the same server
...
literal|1
decl_stmt|;
comment|// regionsByPrimaryPerServer is a sorted array. Since it contains the primary region
...
control|)
block|{
comment|// assume all regions are hosted by only one server
name|int
name|primaryIndex
init|=
name|cluster
