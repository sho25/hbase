...
literal|5
decl_stmt|;
comment|/**    * Here is where hbase-1.x used to default the lock for hbck1.    * It puts in place a lock when it goes to write/make changes.    */
annotation|@
name|VisibleForTesting
specifier|public
specifier|static
...
literal|false
decl_stmt|;
comment|// clean replication barriers of a table
specifier|private
name|boolean
name|fixAny
init|=
...
specifier|private
name|TableName
name|cleanReplicationBarrierTable
decl_stmt|;
specifier|private
name|int
name|maxMerge
...
expr_stmt|;
block|}
comment|/**    * @return A retry counter factory configured for retrying lock file creation.    */
specifier|public
specifier|static
name|RetryCounterFactory
name|createLockRetryCounterFactory
...
return|;
block|}
comment|/**    * @return A retry counter factory configured for retrying znode creation.    */
specifier|private
specifier|static
name|RetryCounterFactory
name|createZnodeRetryCounterFactory
...
return|;
block|}
comment|/**    * @return Return the tmp dir this tool writes too.    */
annotation|@
name|VisibleForTesting
specifier|public
specifier|static
...
expr_stmt|;
block|}
comment|/**      * @return Will be<code>null</code> unless you call {@link #call()}      */
name|Path
name|getHbckLockPath
parameter_list|()
block|{
return|return
name|this
...
argument_list|)
expr_stmt|;
comment|// Add a note into the file we write on why hbase2 is writing out an hbck1 lock file.
name|out
operator|.
name|writeBytes
argument_list|(
literal|" Written by an hbase-2.x Master to block an "
operator|+
literal|"attempt by an hbase-1.x HBCK tool making modification to state. "
operator|+
literal|"See 'HBCK must match HBase server version' in the hbase refguide."
argument_list|)
expr_stmt|;
name|out
operator|.
...
block|}
block|}
comment|/**    * Merge hdfs data by moving from contained HbckRegionInfo into targetRegionDir.    * @return number of file move fixes done to merge regions.    */
specifier|public
name|int
name|mergeRegionDirs
parameter_list|(
...
condition|)
block|{
comment|// This region is already being merged
name|HbckRegionInfo
name|hbInfo
init|=
name|getOrCreateInfo
...
name|println
argument_list|(
literal|"   -cleanReplicationBrarier [tableName] clean the replication barriers "
operator|+
literal|"of a specified table, tableName is required"
argument_list|)
expr_stmt|;
name|out
operator|.
...
name|equals
argument_list|(
literal|"-cleanReplicationBarrier"
argument_list|)
condition|)
block|{
name|setCleanReplicationBarrier
...
name|info
argument_list|(
literal|"we may need to clean some erroneous data due to bugs"
argument_list|)
expr_stmt|;
block|}
if|if
...
name|INVALID_TABLE
argument_list|,
literal|"table's replication scope is global: "
operator|+
name|cleanReplicationBarrierTable
argument_list|)
expr_stmt|;
...
name|INVALID_TABLE
argument_list|,
literal|"there is no barriers of this table: "
operator|+
name|cleanReplicationBarrierTable
argument_list|)
expr_stmt|;
