...
specifier|static
name|boolean
name|disableSplitAndMerge
init|=
literal|false
decl_stmt|;
comment|// disable split and merge
specifier|private
name|boolean
name|fixAssignments
init|=
...
block|}
block|}
comment|/**    * This method maintains a lock using a file. If the creation fails we return null    *    * @return FSDataOutputStream object corresponding to the newly opened lock file    * @throws IOException    */
specifier|private
name|FSDataOutputStream
...
name|boolean
index|[]
name|oldSplitAndMerge
init|=
literal|null
decl_stmt|;
if|if
condition|(
name|shouldDisableSplitAndMerge
argument_list|()
condition|)
block|{
name|admin
...
expr_stmt|;
block|}
comment|/**    * This borrows code from MasterFileSystem.bootstrap(). Explicitly creates it's own WAL, so be    * sure to close it as well as the region when you're finished.    *    * @return an open hbase:meta HRegion    */
specifier|private
name|HRegion
name|createNewMeta
parameter_list|()
...
expr_stmt|;
block|}
comment|/**    * Disable the split and merge    */
specifier|public
specifier|static
name|void
...
return|;
block|}
comment|/**    * The split and merge should be disabled if we are modifying HBase.    * It can be disabled if you want to prevent region movement from causing    * false positives.    */
specifier|public
name|boolean
name|shouldDisableSplitAndMerge
parameter_list|()
