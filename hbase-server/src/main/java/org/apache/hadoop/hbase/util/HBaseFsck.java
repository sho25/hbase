...
comment|// ShutdownHook and the main code.
specifier|private
specifier|static
name|AtomicBoolean
...
argument_list|)
expr_stmt|;
comment|//Reset the hbckLockCleanup to false so that subsequent calls using the same
comment|// Hbck object succeed. This is added for tests, which keep re-using the same
comment|// objects
name|hbckLockCleanup
operator|.
name|set
...
name|debug
argument_list|(
literal|"Error while removing hbck lock "
operator|+
name|e
operator|.
name|getMessage
