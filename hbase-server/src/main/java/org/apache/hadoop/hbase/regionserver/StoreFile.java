...
name|metadataMap
decl_stmt|;
comment|/**    * Regex that will work for straight filenames (<hfile>) and for reference names    * (<hfile>.<parentEncRegion>).  If reference, then the regex has more than just one group.    * Group 1, '([0-9a-f]+(?:_SeqId_[0-9]+_0?)',  is this file's id.  Group 2 '(.+)' is the    * reference's parent region name.  The ?: paren expressions are non-capture markers so not    * included in the groups count. The _SeqId_ portion comes from bulk loaded files.    */
specifier|public
specifier|static
specifier|final
name|String
...
argument_list|)
decl_stmt|;
comment|/**    * Regex strictly for references to hfilelinks.  (<hfile>-<region>-<table>.<parentEncRegion>).    * Group 1 is this file's hfilelink name.  Group 2 the referenced parent region name.  The '.'    * char is valid in table names but group 2's regex is greedy and interprets the table names    * correctly.   The _SeqId_ portion comes from bulk loaded files.    */
specifier|public
specifier|static
specifier|final
name|String
...
init|=
literal|"^([0-9a-f]+(?:_SeqId_[0-9]+_)?-[0-9a-f]+-"
operator|+
name|HTableDescriptor
operator|.
...
return|;
block|}
comment|/**    * @return<tt>true</tt> if this StoreFile is an HFileLink    */
name|boolean
name|isLink
parameter_list|()
block|{
...
return|;
block|}
comment|/*    * Return path to an hfilelink referred to by a Reference.  Presumes a directory    * hierarchy of<code>${hbase.rootdir}/tablename/regionname/familyname</code>.    * @param p Path to a Reference to hfilelink file.    * @return Calculated path to parent region file.    * @throws IllegalArgumentException when path regex fails to match.    */
specifier|static
name|Path
name|getReferredToLink
parameter_list|(
specifier|final
name|Path
name|p
...
name|warn
argument_list|(
literal|"Failed match of ref to hfilelink name"
operator|+
name|p
operator|.
name|toString
...
parameter_list|)
block|{
comment|// This didn't actually link to another file!
comment|// Handles the case where a file with the hfilelink pattern is actually a daughter
comment|// reference to a hfile link.  This can occur when a cloned table's hfilelinks get split.
name|FileStatus
name|actualRef
init|=
name|fs
...
name|path
operator|+
literal|" is a 0-len file, and actually an hfilelink missing target file!"
argument_list|,
name|fnfe
argument_list|)
expr_stmt|;
...
name|debug
argument_list|(
literal|"Size of link file is "
operator|+
name|actualLen
operator|+
literal|"!= 0; treating as a reference to"
operator|+
literal|" HFileLink "
operator|+
name|path
...
name|referencePath
operator|+
literal|" as a half store file"
operator|+
literal|" reader to an HFileLink!"
argument_list|)
expr_stmt|;
block|}
block|}
...
name|Comparators
block|{
comment|/**      * Comparator that compares based on the Sequence Ids of the      * the StoreFiles. Bulk loads that did not request a seq ID      * are given a seq id of -1; thus, they are placed before all non-      * bulk loads, and bulk loads with sequence Id. Among these files,      * the bulkLoadTime is used to determine the ordering.      * If there are ties, the path name is used as a tie-breaker.      */
specifier|static
specifier|final
name|Comparator
argument_list|<
