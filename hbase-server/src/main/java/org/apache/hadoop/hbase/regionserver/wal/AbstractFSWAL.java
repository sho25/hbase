...
specifier|final
name|String
name|SURVIVED_TOO_LONG_SEC_KEY
init|=
literal|"hbase.regionserver.wal.too.old.sec"
decl_stmt|;
specifier|private
specifier|static
specifier|final
name|int
name|SURVIVED_TOO_LONG_SEC_DEFAULT
init|=
literal|900
decl_stmt|;
comment|/** Don't log blocking regions more frequently than this. */
specifier|private
specifier|static
specifier|final
name|long
name|SURVIVED_TOO_LONG_LOG_INTERVAL_NS
init|=
name|TimeUnit
operator|.
name|MINUTES
...
specifier|final
name|String
name|SLOW_SYNC_TIME_MS
init|=
literal|"hbase.regionserver.wal.slowsync.ms"
decl_stmt|;
specifier|protected
specifier|static
specifier|final
...
init|=
literal|"hbase.regionserver.wal.roll.on.sync.ms"
decl_stmt|;
specifier|protected
specifier|static
specifier|final
name|int
name|DEFAULT_ROLL_ON_SYNC_TIME_MS
init|=
literal|10000
decl_stmt|;
comment|// in ms
...
init|=
literal|"hbase.regionserver.wal.slowsync.roll.threshold"
decl_stmt|;
specifier|protected
specifier|static
specifier|final
name|int
name|DEFAULT_SLOW_SYNC_ROLL_THRESHOLD
init|=
literal|100
decl_stmt|;
comment|// 100 slow sync warnings
specifier|protected
specifier|static
specifier|final
name|String
...
init|=
literal|"hbase.regionserver.wal.slowsync.roll.interval.ms"
decl_stmt|;
specifier|protected
specifier|static
specifier|final
name|int
name|DEFAULT_SLOW_SYNC_ROLL_INTERVAL_MS
init|=
literal|60
operator|*
literal|1000
decl_stmt|;
comment|// in ms, 1 minute
specifier|protected
specifier|static
specifier|final
name|String
name|WAL_SYNC_TIMEOUT_MS
init|=
literal|"hbase.regionserver.wal.sync.timeout"
decl_stmt|;
specifier|protected
specifier|static
specifier|final
...
argument_list|()
decl_stmt|;
comment|/** The slow sync will be logged; the very slow sync will cause the WAL to be rolled. */
specifier|protected
specifier|final
name|long
name|slowSyncNs
...
specifier|final
name|int
name|slowSyncRollThreshold
decl_stmt|;
specifier|protected
specifier|final
name|int
name|slowSyncCheckInterval
decl_stmt|;
specifier|protected
specifier|final
name|AtomicInteger
...
argument_list|()
decl_stmt|;
comment|// Last time we asked to roll the log due to a slow sync
specifier|private
specifier|volatile
name|long
name|lastTimeCheckSlowSync
init|=
name|EnvironmentEdgeManager
operator|.
name|currentTime
...
name|logSize
decl_stmt|;
comment|/**      * The nanoTime of the log rolling, used to determine the time interval that has passed since.      */
specifier|public
specifier|final
name|long
name|rollTimeNs
...
argument_list|)
decl_stmt|;
comment|/**    * Map of {@link SyncFuture}s owned by Thread objects. Used so we reuse SyncFutures.    * Thread local is used so JVM can GC the terminated thread for us. See HBASE-21228    *<p>    */
specifier|private
specifier|final
name|ThreadLocal
argument_list|<
...
name|append
argument_list|(
literal|" has not been archived for "
argument_list|)
operator|.
name|append
argument_list|(
...
expr_stmt|;
block|}
comment|// We got a new writer, so reset the slow sync count
name|lastTimeCheckSlowSync
operator|=
name|EnvironmentEdgeManager
operator|.
...
condition|)
block|{
comment|// let's clean all the records of this region
name|sequenceIdAccounting
operator|.
name|onRegionClose
argument_list|(
...
condition|)
block|{
comment|// A single sync took too long.
comment|// Elsewhere in checkSlowSync, called from checkLogRoll, we will look at cumulative
comment|// effects. Here we have a single data point that indicates we should take immediate
comment|// action, so do so.
name|LOG
operator|.
name|warn
argument_list|(
literal|"Requesting log roll because we exceeded slow sync threshold; time="
operator|+
name|TimeUnit
operator|.
name|NANOSECONDS
...
argument_list|()
expr_stmt|;
comment|// it's fine to unconditionally increment this
block|}
if|if
condition|(
operator|!
...
parameter_list|()
function_decl|;
comment|/**    * @return true if we exceeded the slow sync roll threshold over the last check    *              interval    */
specifier|protected
name|boolean
...
condition|)
block|{
comment|// If two or more slowSyncCheckInterval have elapsed this is a corner case
comment|// where a train of slow syncs almost triggered us but then there was a long
comment|// interval from then until the one more that pushed us over. If so, we
comment|// should do nothing and let the count reset.
if|if
condition|(
name|LOG
operator|.
...
name|debug
argument_list|(
literal|"checkSlowSync triggered but we decided to ignore it; "
operator|+
literal|"count="
operator|+
name|slowSyncCount
...
name|elapsedTime
operator|+
literal|" ms, slowSyncCheckInterval="
operator|+
name|slowSyncCheckInterval
operator|+
literal|" ms"
...
expr_stmt|;
block|}
comment|// Fall through to count reset below
block|}
else|else
block|{
name|LOG
...
name|warn
argument_list|(
literal|"Requesting log roll because we exceeded slow sync threshold; count="
operator|+
name|slowSyncCount
operator|.
name|get
