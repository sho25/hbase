...
name|logPrefix
decl_stmt|;
comment|/**    * We use walCreateLock to prevent wal recreation in different threads, and also prevent getWALs    * missing the newly created WAL, see HBASE-21503 for more details.    */
specifier|private
specifier|final
name|ReadWriteLock
name|walCreateLock
...
argument_list|()
operator|+
literal|" is not a valid wal file name"
argument_list|)
throw|;
block|}
block|}
...
argument_list|)
decl_stmt|;
comment|/**    * A WAL file name is of the format:&lt;wal-name&gt;{@link #WAL_FILE_NAME_DELIMITER}    *&lt;file-creation-timestamp&gt;[.&lt;suffix&gt;]. provider-name is usually made up of a    * server-name and a provider-id    * @param filename name of the file to validate    * @return<tt>true</tt> if the filename matches an WAL,<tt>false</tt> otherwise    */
specifier|public
specifier|static
name|boolean
...
specifier|static
name|String
name|getWALNameGroupFromWALName
parameter_list|(
name|String
name|name
parameter_list|,
...
block|}
block|}
comment|/**    * Get prefix of the log from its name, assuming WAL name in format of    * log_prefix.filenumber.log_suffix    * @param name Name of the WAL to parse    * @return prefix of the log    * @throws IllegalArgumentException if the name passed in is not a valid wal file name    * @see AbstractFSWAL#getCurrentFileName()    */
specifier|public
specifier|static
name|String
