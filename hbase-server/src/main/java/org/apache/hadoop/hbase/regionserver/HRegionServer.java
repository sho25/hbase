...
name|tableLockManager
decl_stmt|;
comment|/**    * Threadpool for doing scanner prefetches    */
specifier|protected
name|ThreadPoolExecutor
...
argument_list|()
argument_list|,
comment|/*HBaseRPCErrorHandler.class, OnlineRegions.class},*/
name|initialIsa
argument_list|,
...
condition|)
block|{
comment|// shutdown the prefetch threads
name|scanPrefetchThreadPool
operator|.
name|shutdownNow
argument_list|()
...
expr_stmt|;
block|}
comment|// start the scanner prefetch threadpool
name|int
name|numHandlers
init|=
name|conf
...
name|getInt
argument_list|(
literal|"hbase.regionserver.prefetcher.threads.max"
argument_list|,
name|conf
operator|.
name|getInt
...
block|}
block|}
comment|/**    * Get multi data from a table.    *    * @param controller the RPC controller    * @param request multi-the get request    * @throws ServiceException    */
annotation|@
name|Override
specifier|public
name|MultiGetResponse
...
name|scanner
expr_stmt|;
comment|// Use the region found in the online region list,
comment|// not that one in the RegionScannerHolder. So that we can
comment|// make sure the region is still open in this region server.
name|region
operator|=
name|getRegion
argument_list|(
...
argument_list|)
expr_stmt|;
comment|// TODO is this okey to assume the type and cast
operator|(
operator|(
name|PayloadCarryingRpcController
operator|)
...
parameter_list|)
block|{
comment|// That's ok, since the lease may be gone with
comment|// the prefetcher when cancelled.
block|}
if|if
condition|(
name|region
...
argument_list|)
decl_stmt|;
comment|// Do a bunch of mutations atomically.  Mutations are Puts and Deletes.  NOT Gets.
if|if
condition|(
name|request
operator|.
...
condition|)
block|{
comment|// MultiAction is union type.  Has a Get or a Mutate.
for|for
control|(
name|ClientProtos
operator|.
...
block|}
block|}
comment|// TODO: We are not updating a metric here.  Should we up requestCount?
if|if
condition|(
operator|!
name|mutations
...
condition|)
block|{
comment|// Get a result with no data.  The data will be carried alongside pbs, not as pbs.
name|result
operator|=
name|ProtobufUtil
operator|.
...
argument_list|)
expr_stmt|;
comment|// Add the Result to controller so it gets serialized apart from pb.  Get
comment|// Results could be big so good if they are not serialized as pb.
name|cellsToReturn
operator|.
name|add
argument_list|(
...
condition|)
block|{
comment|// Put the data into the cellsToReturn and the metadata about the result is all that
comment|// we will pass back in the protobuf result.
name|result
operator|=
name|ProtobufUtil
operator|.
...
block|}
block|}
comment|/**    * Replay the given changes when distributedLogReplay WAL edits from a failed RS. The guarantee is    * that the given mutations will be durable on the receiving RS if this method returns without any    * exception.    * @param rpcc the RPC controller    * @param request the request    * @throws ServiceException    */
annotation|@
name|Override
annotation|@
name|QosPriority
...
return|;
block|}
comment|/**    * Execute a list of Put/Delete mutations.    */
specifier|protected
name|void
name|doBatchOp
parameter_list|(
...
block|}
block|}
comment|/**    * Mutate a list of rows atomically.    *    * @param region    * @param mutations  * @param cellScanner if non-null, the mutation data -- the Cell content.    * @throws IOException    */
specifier|protected
name|void
name|mutateRows
parameter_list|(
...
return|;
block|}
comment|/**    * check if /hbase/recovering-regions/<current region encoded name> exists. Returns true if exists    * and set watcher as well.    * @param regionEncodedName region encode name    * @return true when /hbase/recovering-regions/<current region encoded name> exists    * @throws KeeperException    */
specifier|private
name|boolean
name|isRegionMarkedRecoveringInZK
parameter_list|(
...
block|}
block|}
comment|/**    * Return the last failed RS name under /hbase/recovering-regions/encodedRegionName    * @param encodedRegionName    * @return    * @throws IOException    * @throws KeeperException    */
specifier|private
name|String
name|getLastFailedRSFromZK
parameter_list|(
