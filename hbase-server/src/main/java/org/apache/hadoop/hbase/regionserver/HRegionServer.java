...
argument_list|)
decl_stmt|;
comment|/**    * Used to cache the open/close region procedures which already submitted.    * See {@link #submitRegionProcedure(long)}.    */
specifier|private
specifier|final
name|ConcurrentMap
argument_list|<
...
name|Long
argument_list|>
name|submittedRegionProcedures
init|=
operator|new
name|ConcurrentHashMap
argument_list|<>
argument_list|()
decl_stmt|;
comment|/**    * Used to cache the open/close region procedures which already executed.    * See {@link #submitRegionProcedure(long)}.    */
specifier|private
specifier|final
name|Cache
argument_list|<
...
name|ABORT_TIMEOUT
init|=
literal|"hbase.regionserver.abort.timeout"
decl_stmt|;
comment|// Default abort timeout is 1200 seconds for safe
specifier|private
specifier|static
specifier|final
name|long
name|DEFAULT_ABORT_TIMEOUT
init|=
literal|1200000
decl_stmt|;
...
specifier|final
name|String
name|PERIOD_COMPACTION
init|=
literal|"hbase.regionserver.compaction.check.period"
decl_stmt|;
specifier|private
specifier|final
name|int
name|compactionCheckFrequency
decl_stmt|;
specifier|private
specifier|static
specifier|final
...
name|PERIOD_FLUSH
init|=
literal|"hbase.regionserver.flush.check.period"
decl_stmt|;
specifier|private
specifier|final
name|int
...
name|blockCache
decl_stmt|;
comment|// The cache for mob files
specifier|private
name|MobFileCache
name|mobFileCache
decl_stmt|;
...
name|healthCheckChore
decl_stmt|;
comment|/** The Executor status collect chore. */
specifier|private
name|ExecutorStatusChore
name|executorStatusChore
decl_stmt|;
...
decl_stmt|;
comment|// A timer to shutdown the process if abort takes too long
specifier|private
name|Timer
name|abortMonitor
decl_stmt|;
...
argument_list|)
decl_stmt|;
comment|// no need to instantiate block cache and mob file cache when master not carry table
if|if
condition|(
operator|!
name|isMasterNotCarryTable
...
return|;
block|}
comment|/**    * Run test on configured codecs to make sure supporting libs are in place.    */
specifier|private
specifier|static
name|void
name|checkCodecs
...
name|warn
argument_list|(
literal|"Attempt to close server's AsyncClusterConnection failed."
argument_list|,
name|e
argument_list|)
expr_stmt|;
...
expr_stmt|;
name|int
name|currentMaxCompactedStoreFileRefCount
init|=
name|store
operator|.
...
expr_stmt|;
block|}
comment|// Executor status collect thread.
if|if
condition|(
name|this
operator|.
...
name|getInt
argument_list|(
literal|"hbase.regionserver.executor.switch.rpc.throttle.threads"
argument_list|,
literal|1
argument_list|)
argument_list|)
...
name|Timer
argument_list|(
literal|"Abort regionserver monitor"
argument_list|,
literal|true
argument_list|)
expr_stmt|;
name|TimerTask
name|abortTimeoutTask
init|=
literal|null
decl_stmt|;
try|try
...
name|ABORT_TIMEOUT_TASK
argument_list|,
name|SystemExitWhenAbortTimeout
operator|.
name|class
operator|.
name|getName
...
name|warn
argument_list|(
literal|"Initialize abort timeout task failed"
argument_list|,
name|e
argument_list|)
expr_stmt|;
...
block|}
block|}
comment|/**    * May be null if this is a master which not carry table.    *    * @return The block cache instance used by the regionserver.    */
annotation|@
name|Override
specifier|public
name|Optional
...
return|;
block|}
comment|/**    * May be null if this is a master which not carry table.    *    * @return The cache for mob files used by the regionserver.    */
annotation|@
name|Override
specifier|public
name|Optional
...
name|warn
argument_list|(
literal|"Failed to initialize SuperUsers on reloading of the configuration"
argument_list|)
expr_stmt|;
block|}
block|}
...
return|;
block|}
comment|// Ignore the region procedures which already submitted.
name|Long
name|previous
init|=
name|submittedRegionProcedures
...
name|warn
argument_list|(
literal|"Received procedure pid={}, which already submitted, just ignore it"
argument_list|,
name|procId
argument_list|)
expr_stmt|;
...
return|;
block|}
comment|// Ignore the region procedures which already executed.
if|if
condition|(
name|executedRegionProcedures
operator|.
...
name|warn
argument_list|(
literal|"Received procedure pid={}, which already executed, just ignore it"
argument_list|,
name|procId
argument_list|)
expr_stmt|;
...
return|;
block|}
comment|/**    * See {@link #submitRegionProcedure(long)}.    * @param procId the id of the open/close region procedure    */
specifier|public
name|void
...
return|;
block|}
comment|/**    * Force to terminate region server when abort timeout.    */
specifier|private
specifier|static
class|class
name|SystemExitWhenAbortTimeout
