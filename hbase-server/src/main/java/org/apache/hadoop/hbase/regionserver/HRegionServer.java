...
block|}
block|}
comment|/**    * All initialization needed before we go register with Master.    * Do bare minimum. Do bulk of initializations AFTER we've connected to the Master.    * In here we just put up the RpcServer, setup Connection, and ZooKeeper.    *    * @throws IOException    * @throws InterruptedException    */
specifier|private
name|void
...
comment|// In here we create the Replication instances. Later they are initialized and started up.
...
comment|// So we here we need to construct WALFactory first, and then pass it to the initialized method
comment|// of ReplicationSourceHandler.
comment|// TODO: I can't follow replication; it has initialize and then later on we start it!
name|WALFactory
name|factory
init|=
operator|new
...
name|info
argument_list|(
literal|"Failed to report transition "
operator|+
name|TextFormat
operator|.
name|shortDebugString
