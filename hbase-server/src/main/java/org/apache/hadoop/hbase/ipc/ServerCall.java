...
decl_stmt|;
comment|// This is a dirty hack to address HBASE-22539. The lowest bit is for normal rpc cleanup, and the
comment|// second bit is for WAL reference. We can only call release if both of them are zero. The reason
comment|// why we can not use a general reference counting is that, we may call cleanup multiple times in
comment|// the current implementation. We should fix this in the future.
specifier|private
specifier|final
name|AtomicInteger
name|reference
