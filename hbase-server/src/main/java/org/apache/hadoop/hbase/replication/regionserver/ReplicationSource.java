...
return|;
block|}
comment|/*    * Checks whether the current log file is empty, and it is not a recovered queue. This is to    * handle scenario when in an idle cluster, there is no entry in the current log and we keep on    * trying to read the log file and get EOFEception. In case of a recovered queue the last log file    * may be empty, and we don't want to retry that.    */
specifier|private
name|boolean
name|isCurrentLogEmpty
parameter_list|()
...
argument_list|()
decl_stmt|;
comment|// clear kvs(prevent replicating) if logKey's table isn't in this peer's
