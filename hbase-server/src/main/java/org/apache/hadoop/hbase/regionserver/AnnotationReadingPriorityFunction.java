...
return|;
block|}
comment|// all requests executed by super users have high QoS
try|try
block|{
if|if
condition|(
...
return|;
block|}
comment|/**    * See if the method has an annotation.    * @param header    * @return Return the priority from the annotation. If there isn't    * an annotation, this returns something below zero.    */
specifier|protected
name|int
name|getAnnotatedPriority
parameter_list|(
...
return|;
block|}
comment|/**    * Get the priority for a given request from the header and the param    * This doesn't consider which user is sending the request at all.    * This doesn't consider annotations    */
specifier|protected
name|int
name|getBasePriority
parameter_list|(
...
return|;
block|}
comment|// Trust the client-set priorities if set
if|if
condition|(
name|header
operator|.
