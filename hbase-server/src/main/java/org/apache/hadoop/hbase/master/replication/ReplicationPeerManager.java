...
argument_list|)
decl_stmt|;
comment|// Only allow to add one sync replication peer concurrently
specifier|private
specifier|final
name|Semaphore
name|syncReplicationPeerLock
init|=
operator|new
name|Semaphore
argument_list|(
...
condition|)
block|{
name|checkSyncReplicationPeerConfigConflict
argument_list|(
name|peerConfig
argument_list|)
expr_stmt|;
...
specifier|private
name|void
name|checkPeerInDAStateIfSyncReplication
parameter_list|(
name|String
name|peerId
parameter_list|)
...
name|DoNotRetryIOException
argument_list|(
literal|"Couldn't remove synchronous replication peer with state="
operator|+
name|desc
operator|.
name|getSyncReplicationState
argument_list|()
operator|+
literal|", Transit the synchronous replication state to be DOWNGRADE_ACTIVE firstly."
argument_list|)
throw|;
block|}
block|}
...
return|;
block|}
comment|/**    * @return the old desciption of the peer    */
name|ReplicationPeerDescription
...
condition|)
block|{
comment|// try creating a instance
name|ReplicationEndpoint
name|endpoint
decl_stmt|;
try|try
...
name|DoNotRetryIOException
argument_list|(
literal|"Can not instantiate configured replication endpoint class="
operator|+
name|replicationEndpointImpl
argument_list|,
name|e
...
throw|;
block|}
comment|// do not check cluster key if we are not HBaseInterClusterReplicationEndpoint
if|if
condition|(
operator|!
operator|(
...
argument_list|()
operator|+
literal|" is not qualified, you must provide scheme and authority"
argument_list|)
throw|;
block|}
block|}
...
name|tableName
operator|+
literal|" has been replicated by peer "
operator|+
name|entry
operator|.
name|getKey
...
specifier|public
name|void
name|acquireSyncReplicationPeerLock
parameter_list|()
throws|throws
...
specifier|public
name|void
name|releaseSyncReplicationPeerLock
parameter_list|()
block|{
name|syncReplicationPeerLock
operator|.
