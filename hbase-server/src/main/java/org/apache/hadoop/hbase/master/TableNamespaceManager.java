...
specifier|private
name|long
name|exclusiveLockTimeoutMs
decl_stmt|;
specifier|private
name|long
name|sharedLockTimeoutMs
decl_stmt|;
name|TableNamespaceManager
parameter_list|(
name|MasterServices
...
name|IOException
argument_list|(
literal|"Fail to acquire lock to scan namespace list.  Some namespace DDL is in progress."
argument_list|)
throw|;
block|}
for|for
...
return|;
block|}
comment|/**    * Create Namespace in a blocking manner; don't return till success.    * Note, by-passes notifying coprocessors and name checks. Use for system namespaces only.    * @throws IOException     * @throws InterruptedException     */
specifier|private
name|void
...
expr_stmt|;
block|}
comment|/**    * This method checks if the namespace table is assigned and then    * tries to create its Table reference. If it was already created before, it also makes    * sure that the connection isn't closed.    * @return true if the namespace table manager is ready to serve, false    * otherwise    * @throws IOException    */
annotation|@
name|SuppressWarnings
argument_list|(
literal|"deprecation"
