...
condition|)
block|{
comment|// Policies are the same, so we have nothing to do except log this. Don't need to re-update the quota table
if|if
condition|(
operator|!
name|currentStatus
...
block|{
comment|// Namespace quota is still in violation, need to enact if the table quota is not taking priority.
for|for
control|(
name|TableName
name|tableInNS
