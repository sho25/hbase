...
argument_list|)
expr_stmt|;
comment|// a larger minBatchSize may slow down recovery because replay writer has to wait for
comment|// enough edits before replaying them
name|this
operator|.
name|minBatchSize
operator|=
...
argument_list|()
decl_stmt|;
comment|// clear scopes which isn't needed for recovery
name|entry
operator|.
name|getKey
argument_list|()
...
index|]
expr_stmt|;
comment|// startKey of the region
name|family
operator|=
name|compaction
operator|.
...
name|warn
argument_list|(
literal|"Unexpected exception received, ignoring "
operator|+
name|ex
argument_list|)
expr_stmt|;
...
argument_list|)
expr_stmt|;
comment|// skip replaying the compaction if the region is gone
if|if
condition|(
name|isCompactionEntry
operator|&&
...
name|info
argument_list|(
literal|"Not replaying a compaction marker for an older region: "
operator|+
name|encodeRegionNameStr
argument_list|)
expr_stmt|;
...
condition|)
block|{
comment|// Gets closed over in closeRegionServerWriters
name|hconn
operator|=
operator|(
name|HConnection
...
decl_stmt|;
block|}
comment|/**   * Tag original sequence number for each edit to be replayed   * @param seqId   * @param cell   */
specifier|private
specifier|static
name|Cell
name|tagReplayLogSequenceNumber
parameter_list|(
name|long
name|seqId
parameter_list|,
...
parameter_list|)
block|{
comment|// Tag puts with original sequence number if there is no LOG_REPLAY_TAG yet
name|boolean
name|needAddRecoveryTag
init|=
literal|true
decl_stmt|;
if|if
...
condition|)
block|{
comment|// found an existing log replay tag so reuse it
name|needAddRecoveryTag
operator|=
literal|false
expr_stmt|;
...
return|;
block|}
comment|/**    * This function is used to construct mutations from a WALEntry. It also reconstructs HLogKey&    * WALEdit from the passed in WALEntry    * @param entry    * @param cells    * @param logEntry pair of HLogKey and WALEdit instance stores HLogKey and WALEdit instances    *          extracted from the passed in WALEntry.    * @param addLogReplayTag    * @return list of Pair<MutationType, Mutation> to be replayed    * @throws IOException    */
specifier|public
specifier|static
name|List
argument_list|<
