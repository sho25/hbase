...
name|minBatchSize
decl_stmt|;
comment|/**    * Create a new HLogSplitter using the given {@link Configuration} and the    *<code>hbase.hlog.splitter.impl</code> property to derived the instance class to use.    * distributedLogReplay won't be enabled by this constructor.    *<p>    * @param conf    * @param rootDir hbase directory    * @param srcDir logs directory    * @param oldLogDir directory where processed logs are archived to    * @param fs FileSystem    * @return New HLogSplitter instance    */
specifier|public
specifier|static
name|HLogSplitter
...
return|;
block|}
comment|/**    * Splits or Replays the HLog edits in the given list of logfiles (that are a mix of edits on    * multiple regions) by region and then splits(or replay when distributedLogReplay is true) them    * per region directories, in batches.    *<p>    * This process is split into multiple threads. In the main thread, we loop through the logs to be    * split. For each log, we:    *<ul>    *<li>Recover it (take and drop HDFS lease) to ensure no other process can write</li>    *<li>Read each edit (see {@link #parseHLog}</li>    *<li>Mark as "processed" or "corrupt" depending on outcome</li>    *</ul>    *<p>    * Each edit is passed into the EntryBuffers instance, which takes care of memory accounting and    * splitting the edits by region.    *<p>    * The OutputSink object then manages N other WriterThreads which pull chunks of edits from    * EntryBuffers and write them to either recovered.edits files or replay them to newly assigned    * region servers directly    *<p>    * After the process is complete, the log files are archived to a separate directory.    */
specifier|private
name|List
argument_list|<
name|Path
...
comment|// quite a while inside HConnection layer. The worker won't available for other
...
name|warn
argument_list|(
literal|"Can't find store max sequence ids map for region:"
operator|+
name|loc
operator|.
name|getRegionInfo
