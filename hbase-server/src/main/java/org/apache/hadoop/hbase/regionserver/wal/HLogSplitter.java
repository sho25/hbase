...
return|;
block|}
comment|/**    * Split up a bunch of regionserver commit log files that are no longer being    * written to, into new files, one per region for region to replay on startup.    * Delete the old log files when finished.    *    * @param latch    * @throws IOException will throw if corrupted hlogs aren't tolerated    * @return the list of splits    */
specifier|public
name|List
argument_list|<
name|Path
...
name|warn
argument_list|(
literal|"wait for latch interrupted"
argument_list|)
expr_stmt|;
name|Thread
operator|.
