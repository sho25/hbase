...
argument_list|()
decl_stmt|;
comment|// do not prune the entries if table is in violation and
comment|// violation policy is disable to avoid cycle of enable/disable.
comment|// Please refer HBASE-22012 for more details.
comment|// prune entries older than time.
if|if
condition|(
name|currentEntryTime
operator|<
...
operator|&&
operator|!
name|isInViolationAndPolicyDisable
argument_list|(
name|regionInfo
operator|.
name|getTable
...
return|;
block|}
comment|/**    * Method to check if a table is in violation and policy set on table is DISABLE.    *    * @param tableName          tableName to check.    * @param quotaObserverChore QuotaObserverChore instance    * @return returns true if table is in violation and policy is disable else false.    */
specifier|private
name|boolean
name|isInViolationAndPolicyDisable
parameter_list|(
...
block|{
name|boolean
name|isInViolationAtTable
init|=
literal|false
decl_stmt|;
name|boolean
name|isInViolationAtNamespace
init|=
literal|false
decl_stmt|;
name|SpaceViolationPolicy
...
literal|null
decl_stmt|;
comment|// Get Current Snapshot for the given table
name|SpaceQuotaSnapshot
...
condition|)
block|{
comment|// check if table in violation
name|isInViolationAtTable
operator|=
name|tableQuotaSnapshot
operator|.
...
condition|)
block|{
comment|// check namespace in violation
name|isInViolationAtNamespace
operator|=
name|namespaceQuotaSnapshot
operator|.
