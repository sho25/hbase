...
begin_comment
comment|/**  * The procedure for replaying all the remote wals for transitting a sync replication peer from  * STANDBY to DOWNGRADE_ACTIVE.  */
end_comment
...
parameter_list|)
block|{
comment|// For these two states, we need to register the peer to the replay manager, as the state are
comment|// only kept in memory and will be lost after restarting. And in
comment|// SyncReplicationReplayWALProcedure.afterReplay we will reconstruct the used workers.
switch|switch
condition|(
