...
literal|false
decl_stmt|;
comment|/**    * This is for for using HRegion as a local storage, where we may put the recovered edits in a    * special place. Once this is set, we will only replay the recovered edits under this directory    * and ignore the original replay directory configs.    */
specifier|public
specifier|static
specifier|final
name|String
...
init|=
literal|"hbase.hregion.special.recovered.edits.dir"
decl_stmt|;
specifier|final
name|AtomicBoolean
name|closed
...
argument_list|()
expr_stmt|;
name|decorateRegionConfiguration
argument_list|(
name|conf
argument_list|)
expr_stmt|;
...
name|warn
argument_list|(
literal|"Failed initialize of region= {}, starting to roll back memstore"
argument_list|,
name|getRegionInfo
argument_list|()
operator|.
...
argument_list|)
expr_stmt|;
comment|// global memstore size will be decreased when dropping memstore
try|try
block|{
comment|//drop the memory used by memstore if open region fails
name|dropMemStoreContents
argument_list|()
expr_stmt|;
block|}
catch|catch
...
name|warn
argument_list|(
literal|"Failed drop memstore of region= {}, "
operator|+
literal|"some chunks may not released forever since MSLAB is enabled"
argument_list|,
name|getRegionInfo
argument_list|()
operator|.
...
argument_list|)
expr_stmt|;
comment|// Recover any hfiles if available
name|maxSeqId
operator|=
name|Math
operator|.
...
condition|)
block|{
comment|// always get openSeqNum from the default replica, even if we are secondary replicas
name|long
...
comment|// determine whether a region has been successfully reopened, so here we always need to update
...
argument_list|)
expr_stmt|;
comment|// This means we have replayed all the recovered edits and also written out the max sequence
comment|// id file, let's delete the wrong directories introduced in HBASE-20734, see HBASE-22617
...
name|debug
argument_list|(
literal|"Failed to clean up wrong region WAL directory {}"
argument_list|,
name|wrongRegionWALDir
argument_list|)
expr_stmt|;
...
argument_list|)
expr_stmt|;
comment|// Store SeqId in WAL FileSystem when a region closes
...
return|;
block|}
comment|/**    * Only used for unit test which doesn't start region server.    */
annotation|@
name|VisibleForTesting
specifier|public
name|void
...
return|;
block|}
comment|/** @return the WAL {@link FileSystem} being used by this region */
name|FileSystem
name|getWalFileSystem
parameter_list|()
throws|throws
...
return|;
block|}
comment|/**    * @return the Region directory under WALRootDirectory    * @throws IOException if there is an error getting WALRootDir    */
annotation|@
name|VisibleForTesting
specifier|public
name|Path
...
specifier|private
name|boolean
name|shouldForbidMajorCompaction
parameter_list|()
block|{
if|if
condition|(
...
name|warn
argument_list|(
literal|"Skipping major compaction on "
operator|+
name|this
operator|+
literal|" because this cluster is transiting sync replication state"
operator|+
literal|" from STANDBY to DOWNGRADE_ACTIVE"
argument_list|)
expr_stmt|;
name|store
operator|.
...
name|InvalidMutationDurabilityException
argument_list|(
literal|"Mutation's durability is SKIP_WAL but table's column family "
operator|+
name|Bytes
operator|.
name|toString
...
decl_stmt|;
name|String
name|specialRecoveredEditsDirStr
init|=
name|conf
operator|.
name|get
...
argument_list|)
decl_stmt|;
comment|// We made a mistake in HBASE-20734 so we need to do this dirty hack...
name|NavigableSet
argument_list|<
name|Path
argument_list|>
name|filesUnderWrongRegionWALDir
init|=
name|WALSplitUtil
operator|.
...
name|seqId
argument_list|,
name|replayRecoveredEditsForPaths
argument_list|(
name|minSeqIdForTheRegion
argument_list|,
name|walFS
...
argument_list|)
expr_stmt|;
comment|// This is to ensure backwards compatability with HBASE-20723 where recovered edits can appear
comment|// under the root dir even if walDir is set.
name|NavigableSet
argument_list|<
name|Path
argument_list|>
...
comment|// column family. Have to fake out file type too by casting our recovered.edits as
comment|// storefiles
name|String
name|fakeFamilyName
init|=
...
name|debug
argument_list|(
literal|"Found {} recovered edits file(s) under {}"
argument_list|,
name|files
operator|==
literal|null
...
name|WALSplitUtil
operator|.
name|getSeqIdForRecoveredHFile
argument_list|(
name|file
operator|.
name|getPath
...
name|store
argument_list|,
literal|"load recovered hfiles request compaction"
argument_list|,
name|Store
operator|.
name|PRIORITY_USER
...
return|;
block|}
comment|/**    * Be careful, this method will drop all data in the memstore of this region.    * Currently, this method is used to drop memstore to prevent memory leak    * when replaying recovered.edits while opening region.    */
specifier|public
name|MemStoreSize
name|dropMemStoreContents
parameter_list|()
...
name|info
argument_list|(
literal|"Drop memstore for Store "
operator|+
name|s
operator|.
name|getColumnFamilyName
...
block|}
block|}
comment|/**    * Convenience method creating new HRegions. Used by createTable.    *    * @param info Info for region to create.    * @param rootDir Root directory for HBase instance    * @param wal shared WAL    * @param initialize - true to initialize the region    * @return new HRegion    */
specifier|public
specifier|static
name|HRegion
name|createHRegion
...
return|;
block|}
comment|/**    * Create a region under the given table directory.    */
specifier|public
specifier|static
name|HRegion
name|createHRegion
...
name|info
argument_list|(
literal|"Creating {}, tableDescriptor={}, under table dir {}"
argument_list|,
name|regionInfo
argument_list|,
name|tableDesc
...
return|;
block|}
comment|/**    * Create the region directory in the filesystem.    */
specifier|public
specifier|static
name|HRegionFileSystem
name|createRegionDir
...
argument_list|)
decl_stmt|;
comment|// If directory already exists, will log warning and keep going. Will try to create
comment|// .regioninfo. If one exists, will overwrite.
return|return
name|HRegionFileSystem
operator|.
name|createRegionOnFileSystem
...
block|}
block|}
comment|// Give coprocessors a chance to update the new cells before apply to WAL or memstore
if|if
condition|(
name|coprocessorHost
operator|!=
...
condition|)
block|{
comment|// Here the operation must be increment or append.
name|cellPairs
operator|=
name|op
operator|==
...
expr_stmt|;
block|}
comment|/**    * This method modifies the region's configuration in order to inject replication-related    * features    * @param conf region configurations    */
specifier|static
name|void
name|decorateRegionConfiguration
parameter_list|(
