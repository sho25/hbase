...
argument_list|)
expr_stmt|;
comment|// give compactions 30s to finish before we start to interrupt
...
condition|)
block|{
comment|// if we waited once for compactions to finish, interrupt them, and try again
if|if
condition|(
name|LOG
operator|.
...
name|millis
operator|+
literal|" ms for compactions to finish on close. Interrupting "
operator|+
name|currentCompactions
operator|.
name|size
...
control|)
block|{
comment|// interrupt any current IO in the currently running compactions.
name|t
operator|.
name|interrupt
argument_list|()
