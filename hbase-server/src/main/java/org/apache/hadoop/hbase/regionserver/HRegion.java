...
block|}
block|}
comment|/**    * @param delete The Delete command    * @param clusterId UUID of the originating cluster (for replication).    * @param writeToWAL    * @throws IOException    */
specifier|private
name|void
name|internalDelete
parameter_list|(
...
expr_stmt|;
block|}
comment|/**    * Add updates first to the hlog (if writeToWal) and then add values to memstore.    * Warning: Assumption is caller has lock on passed in row.    * @param put The Put command    * @param clusterId UUID of the originating cluster (for replication).    * @param writeToWAL if true, then we should write to the log    * @throws IOException    */
specifier|private
name|void
name|internalPut
parameter_list|(
...
name|lockedNanoTime
decl_stmt|,
name|processDoneNanoTime
decl_stmt|,
name|unlockedNanoTime
init|=
literal|0
decl_stmt|;
name|MultiVersionConsistencyControl
...
name|IOException
block|{
comment|// to be used for metrics
name|long
name|before
init|=
name|EnvironmentEdgeManager
...
name|debug
argument_list|(
literal|"Received dynamic protocol exec call with protocolName "
operator|+
name|protocolName
argument_list|)
expr_stmt|;
block|}
comment|// detect the actual protocol class
name|protocol
operator|=
name|protocolHandlerNames
operator|.
