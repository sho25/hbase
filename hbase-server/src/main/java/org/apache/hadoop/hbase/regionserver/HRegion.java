...
comment|// etc.). NOTE: The writeEntry write number is NOT in the WAL.. there is no WAL writing
comment|// here.
if|if
condition|(
name|wal
...
argument_list|)
decl_stmt|;
comment|// TODO: Lets see if we hang here, if there is a scenario where an outstanding reader
comment|// with a read point is in advance of this write point.
name|mvcc
operator|.
...
argument_list|)
expr_stmt|;
comment|// set writeEntry to null to prevent mvcc.complete from being called again inside finally
comment|// block
name|writeEntry
operator|=
literal|null
...
block|}
comment|// -------------------------
comment|// STEP 4. Append the final edit to WAL. Do not sync wal.
comment|// -------------------------
name|Mutation
name|mutation
init|=
...
condition|)
block|{
comment|// If this is a skip wal operation just get the read point from mvcc
name|walKey
operator|=
name|this
operator|.
...
comment|// first without syncing the WAL because we do not roll
...
argument_list|)
expr_stmt|;
comment|// The regionserver holding the first region of the table is responsible for taking the
comment|// manifest of the mob dir.
if|if
condition|(
operator|!
name|Bytes
...
condition|)
return|return;
comment|// if any cf's have is mob enabled, add the "mob region" to the manifest.
name|List
argument_list|<
name|Store
argument_list|>
...
condition|)
block|{
comment|// use the .mob as the start key and 0 as the regionid
name|HRegionInfo
name|mobRegionInfo
init|=
name|MobUtils
...
condition|)
block|{
comment|// Try adding a RowLockContext to the lockedRows.
comment|// If we can add it then there's no other transactions currently running.
name|rowLockContext
operator|=
operator|new
name|RowLockContext
...
argument_list|)
decl_stmt|;
comment|// if there was a running transaction then there's already a context.
if|if
condition|(
name|existingContext
operator|!=
...
expr_stmt|;
block|}
comment|// If copyCellsFromSharedMem = true, then we need to copy the cells. Otherwise
comment|// it is a call coming from the RsRpcServices.scan().
if|if
condition|(
name|copyCellsFromSharedMem
operator|&&
...
condition|)
block|{
comment|// Do the copy of the results here.
name|ListIterator
argument_list|<
name|Cell
argument_list|>
...
condition|)
block|{
comment|// In case of copyCellsFromSharedMem==true (where the CPs wrap a scanner) we return
comment|// the blocks then and there (for wrapped CPs)
name|this
operator|.
name|shipped
argument_list|()
...
name|IOException
block|{
comment|// tries to refresh the store files, otherwise shutdown the RS.
comment|// TODO: add support for abort() of a single region and trigger reassignment.
try|try
block|{
name|region
operator|.
...
name|IOException
argument_list|(
literal|"unable to read store file"
argument_list|)
return|;
block|}
catch|catch
...
name|error
argument_list|(
literal|"unable to refresh store files"
argument_list|,
name|e
argument_list|)
expr_stmt|;
...
argument_list|(
literal|"not able to abort RS after: "
operator|+
name|msg
argument_list|)
throw|;
...
control|)
block|{
comment|// This should give an estimate of the cell in the result. Why do we need
comment|// to know the serialization of how the codec works with it??
name|totalSize
operator|+=
name|CellUtil
operator|.
...
condition|)
block|{
comment|// 5. Call the preBatchMutate hook
name|processor
operator|.
name|preBatchMutate
argument_list|(
...
expr_stmt|;
block|}
comment|// 7. Start mvcc transaction
name|writeEntry
operator|=
name|walKey
operator|.
...
comment|// (so that we are guaranteed to see the latest state when we do our Get)
name|mvcc
operator|.
name|await
argument_list|()
...
comment|// Is this right? It immediately becomes visible? St.Ack 20150907
name|size
operator|+=
name|store
operator|.
...
block|}
block|}
comment|// We add to all KVs here whereas when doing increment, we do it
comment|// earlier... why?
name|allKVs
operator|.
name|addAll
...
comment|// We should refactor append and increment as local get-mutate-put
comment|// transactions, so all stores only go through one code path for puts.
comment|// They are subtley different in quiet a few ways. This came out only
comment|// after study. I am not sure that many of the differences are intentional.
comment|// TODO: St.Ack 20150907
annotation|@
name|Override
specifier|public
name|Result
...
literal|0
decl_stmt|;
comment|// HERE WE DIVERGE FROM APPEND
name|List
argument_list|<
name|Cell
argument_list|>
...
assert|;
block|}
comment|/**    * Do not change this sequence id.    * @return sequenceId    */
annotation|@
name|VisibleForTesting
specifier|public
name|long
...
return|;
block|}
comment|/**    * Append a faked WALEdit in order to get a long sequence number and wal syncer will just ignore    * the WALEdit append later.    * @param wal    * @return Return the key used appending with no sync and no append.    * @throws IOException    */
specifier|private
name|WALKey
name|appendEmptyEdit
parameter_list|(
