...
block|}
block|}
comment|/**    * Should the store be flushed because it is old enough.    *<p>    * Every FlushPolicy should call this to determine whether a store is old enough to flush(except    * that you always flush all stores). Otherwise the {@link #shouldFlush()} method will always    * returns true which will make a lot of flush requests.    */
name|boolean
...
argument_list|)
decl_stmt|;
comment|// no sync. Sync is below where we do not hold the updates lock
name|trxId
operator|=
name|WALUtil
operator|.
...
condition|)
block|{
comment|// check whether we have already written START_FLUSH to WAL
try|try
block|{
name|FlushDescriptor
name|desc
...
name|ex
throw|;
comment|// let upper layers deal with it.
block|}
block|}
finally|finally
block|{
...
argument_list|)
expr_stmt|;
comment|// sync unflushed WAL changes
...
argument_list|)
expr_stmt|;
comment|// ensure that the sequence id of the region is at least as big as orig log seq id
while|while
condition|(
literal|true
condition|)
...
argument_list|)
decl_stmt|;
comment|// Carry forward existing tags
while|while
condition|(
name|tagIterator
operator|.
...
condition|)
block|{
comment|// Add any filters or tag specific rewrites here
name|newTags
operator|.
name|add
argument_list|(
...
expr_stmt|;
block|}
comment|// Cell TTL handling
comment|// Check again if we need to add a cell TTL because early out logic
comment|// above may change when there are more tag based features in core.
if|if
condition|(
name|m
operator|.
...
comment|// there may be some in-flight transactions, but they won't be made visible since they are
comment|// either greater than flush seq number or they were already dropped via flush.
comment|// TODO: If we are using FlushAllStoresPolicy, then this can make edits visible from other
comment|// stores while they are still in flight because the flush commit marker will not contain
comment|// flushes from ALL stores.
name|getMVCC
argument_list|()
operator|.
...
block|}
block|}
comment|/**    * Drops the memstore contents after replaying a flush descriptor or region open event replay    * if the memstore edits have seqNums smaller than the given seq id    * @param flush the flush descriptor    * @throws IOException    */
specifier|private
name|void
name|dropMemstoreContentsForSeqId
parameter_list|(
...
control|)
block|{
name|dropStoreMemstoreContentsForSeqId
argument_list|(
name|s
argument_list|,
name|currentSeqId
...
block|}
block|}
comment|/**    * A version of getRowLock(byte[], boolean) to use when a region operation has already been    * started (the calling thread has already acquired the region-close-guard lock).    */
specifier|protected
name|RowLock
name|getRowLockInternal
parameter_list|(
...
name|startSpan
argument_list|(
literal|"HRegion.getRowLockInternal"
argument_list|)
expr_stmt|;
block|}
...
condition|)
block|{
comment|// write a bulk load event when not all hfiles are loaded
try|try
block|{
name|WALProtos
operator|.
...
return|;
block|}
comment|/**      * @return the state the joinedHeap returned on the call to      *         {@link KeyValueHeap#next(List, int, long)}      */
specifier|private
name|NextState
...
block|}
block|}
comment|/**      * Based on the nextKv in the heap, and the current row, decide whether or not there are more      * cells to be read in the heap. If the row of the nextKv in the heap matches the current row      * then there are more cells to be read in the row.      * @param nextKv      * @param currentRow      * @param offset      * @param length      * @return true When there are more cells in the row to be read      */
specifier|private
name|boolean
name|moreCellsInRow
parameter_list|(
...
literal|0
return|;
comment|// In general, the state should contain the estimate because the result size used to
comment|// determine when the scan has exceeded its size limit. If the estimate is contained in the
comment|// state then we can avoid an unnecesasry calculation.
if|if
condition|(
name|state
operator|!=
...
throw|;
block|}
comment|// Estimate of the size (heap size) of the results returned from this method
name|long
name|resultSize
init|=
literal|0
...
comment|// remainingResultSize to -1 so that the entire row's worth of cells are fetched.
if|if
condition|(
name|hasFilterRow
operator|&&
...
operator|+
literal|" formed. The remainingResultSize of: "
operator|+
name|remainingResultSize
operator|+
literal|" will not "
operator|+
literal|" be considered when fetching the cells for this row."
argument_list|)
expr_stmt|;
block|}
block|}
name|NextState
name|joinedHeapState
decl_stmt|;
...
comment|// since we have no way of telling how we should proceed
if|if
condition|(
operator|!
name|NextState
...
name|IOException
argument_list|(
literal|"NextState returned from call storeHeap was invalid"
argument_list|)
throw|;
block|}
...
throw|;
block|}
comment|// We hit the batch limit.
return|return
name|NextState
operator|.
name|makeState
...
condition|)
block|{
comment|// We try to guard against this case above when remainingResultSize is set to -1 if
comment|// hasFilterRow is true. In the even that the guard doesn't work, an exception must be
comment|// thrown
throw|throw
operator|new
name|IncompatibleFilterException
argument_list|(
literal|"Filter whose hasFilterRows() returns true is incompatible with scans that"
operator|+
literal|" return partial results"
argument_list|)
throw|;
block|}
comment|// We hit the size limit.
return|return
name|NextState
operator|.
name|makeState
...
comment|// Avoid as much copying as possible. We may need to rewrite and
comment|// consolidate tags. Bytes are only copied once.
...
argument_list|()
decl_stmt|;
comment|// Make a union of the set of tags in the old and new KVs
if|if
condition|(
name|oldCell
operator|.
...
argument_list|)
decl_stmt|;
comment|// allocate an empty cell once
name|newCell
operator|=
operator|new
name|KeyValue
...
argument_list|()
decl_stmt|;
comment|// Carry forward any tags that might have been added by a coprocessor
if|if
condition|(
name|cell
operator|.
...
throw|;
block|}
comment|// Carry tags forward from previous version
if|if
condition|(
name|c
operator|.
...
argument_list|)
decl_stmt|;
comment|// Add the TTL tag if the mutation carried one
if|if
condition|(
name|increment
operator|.
