...
comment|// individual puts then metrics can be reported as a multiput across
...
expr_stmt|;
block|}
comment|// Not sure what is going on here when replay is going on... does the below append get
comment|// called for replayed edits? Am afraid to change it without test.
if|if
condition|(
operator|!
name|walEdit
...
condition|)
block|{
comment|// If no walKey, then skipping WAL or some such. Being an mvcc transaction so sequenceid.
name|writeEntry
operator|=
name|mvcc
operator|.
...
operator|!=
literal|null
comment|/*Can be null if in replay mode*/
condition|)
block|{
name|mvcc
operator|.
...
return|;
block|}
comment|/**    * @param delta If we are doing delta changes -- e.g. increment/append -- then this flag will be    *  set; when set we will run operations that make sense in the increment/append scenario but    *  that do not make sense otherwise.    * @return Memstore change in size on insert of these Cells.    * @see #applyToMemstore(Store, Cell, long)    */
specifier|private
name|long
name|applyToMemstore
parameter_list|(
...
argument_list|)
decl_stmt|;
comment|// TODO: This looks wrong.. checking for sequenceid of zero is expensive!!!!! St.Ack
comment|// When is it zero anyways? When replay? Then just rely on that flag.
if|if
condition|(
name|cell
operator|.
...
return|;
block|}
comment|/**    * @return Memstore change in size on insert of these Cells.    * @see #applyToMemstore(Store, List, boolean, boolean, long)    */
specifier|private
name|long
name|applyToMemstore
parameter_list|(
...
return|;
block|}
comment|/**    * Do a specific Get on passed<code>columnFamily</code> and column qualifiers.    * @param mutation Mutation we are doing this Get for.    * @param columnFamily Which column family on row (TODO: Go all Gets in one go)    * @param coordinates Cells from<code>mutation</code> used as coordinates applied to Get.    * @return Return list of Cells found.    */
specifier|private
name|List
argument_list|<
name|Cell
