...
literal|1L
decl_stmt|;
comment|// collects Map(s) of Store to sequence Id when handleFileNotFound() is involved
specifier|protected
name|List
argument_list|<
name|Map
...
init|)
block|{
comment|// don't try to acquire write lock of updatesLock now
name|storeSeqIds
operator|.
name|add
argument_list|(
...
return|;
block|}
comment|// dropMemstoreContentsForSeqId() would acquire write lock of updatesLock
comment|// We perform this operation outside of the read lock of updatesLock to avoid dead lock
comment|// See HBASE-16304
annotation|@
name|SuppressWarnings
argument_list|(
literal|"unchecked"
...
argument_list|()
expr_stmt|;
comment|// For increment/append, a region scanner for doing a get operation could throw
comment|// FileNotFoundException. So we call dropMemstoreContents() in finally block
comment|// after releasing read lock
name|dropMemstoreContents
argument_list|()
expr_stmt|;
block|}
