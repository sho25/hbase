...
block|}
block|}
comment|/**    * Struct-like class that tracks the progress of a batch operation, accumulating status codes    * and tracking the index at which processing is proceeding. These batch operations may get    * split into mini-batches for processing.    */
specifier|private
specifier|abstract
specifier|static
class|class
...
return|;
block|}
comment|/**      * Validates each mutation and prepares a batch for write.      * NOTE: As CP prePut()/ preDelete() hooks may modify mutations, this method should be called      * after prePut()/ preDelete() CP hooks are run for all mutations in a batch.      */
specifier|public
name|void
...
condition|)
block|{
name|callPreMutateCPHooks
argument_list|(
name|batchOp
argument_list|)
expr_stmt|;
block|}
comment|// validate and prepare batch for write, after CP pre-hooks
name|batchOp
operator|.
name|checkAndPrepare
argument_list|(
...
return|;
block|}
comment|/**    * Runs prePut/ preDelete coprocessor hooks for each mutation in a batch.    * @param batchOp    */
specifier|private
name|void
name|callPreMutateCPHooks
parameter_list|(
...
condition|)
block|{
comment|// If no writeEntry, then not in replay and skipping WAL or some such. Begin an MVCC
comment|// transaction to get sequence id.
name|writeEntry
operator|=
name|mvcc
operator|.
...
name|SNAPSHOT
case|:
comment|// when a region is in recovering state, no read, split, merge or snapshot is allowed
if|if
condition|(
name|isRecovering
argument_list|()
