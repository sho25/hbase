...
name|ZooKeeperConnectionException
block|{
comment|// We take a shared master, but we will never release it,
comment|//  so we will have the same behavior as before.
return|return
name|this
operator|.
name|connection
...
expr_stmt|;
block|}
comment|/**    * @param tableNameOrRegionName Name of a table or name of a region.    * @param ct A {@link CatalogTracker} instance (caller of this method usually has one).    * @return True if<code>tableNameOrRegionName</code> is a verified region    * name (we call {@link  MetaReader#getRegion( CatalogTracker, byte[])}    *  else false.    * Throw an exception if<code>tableNameOrRegionName</code> is null.    * @throws IOException    */
specifier|private
name|boolean
name|isRegionName
parameter_list|(
...
block|}
block|}
comment|// Check Master, same logic.
name|MasterKeepAliveConnection
name|master
init|=
literal|null
...
decl_stmt|;
block|}
comment|/**    * This method allows to execute a function requiring a connection to    * master without having to manage the connection creation/close.    * Create a {@link MasterCallable} to use it.    */
specifier|private
parameter_list|<
name|V
parameter_list|>
