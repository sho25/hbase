...
return|;
block|}
comment|/**    * Archive the specified regions in parallel.    * @param conf the configuration to use    * @param fs {@link FileSystem} from which to remove the region    * @param rootDir {@link Path} to the root directory where hbase files are stored (for building    *                            the archive path)    * @param tableDir {@link Path} to where the table is being stored (for building the archive    *                             path)    * @param regionDirList {@link Path} to where regions are being stored (for building the archive    *                                  path)    * @throws IOException if the request cannot be completed    */
specifier|public
specifier|static
name|void
name|archiveRegions
...
name|getInt
argument_list|(
literal|"hbase.hfilearchiver.thread.pool.max"
argument_list|,
literal|8
argument_list|)
decl_stmt|;
...
argument_list|)
expr_stmt|;
comment|// Shutdown this ThreadPool in a shutdown hook
name|Runtime
operator|.
name|getRuntime
argument_list|()
...
return|;
block|}
comment|// We need this method instead of Threads.getNamedThreadFactory() to pass some tests.
comment|// The difference from Threads.getNamedThreadFactory() is that it doesn't fix ThreadGroup for
comment|// new threads. If we use Threads.getNamedThreadFactory(), we will face ThreadGroup related
comment|// issues in some tests.
specifier|private
specifier|static
name|ThreadFactory
name|getThreadFactory
...
argument_list|)
decl_stmt|;
comment|//extra sanity checks for the right FS
name|Path
name|path
init|=
operator|new
...
condition|)
block|{
comment|//no schema specified on wal dir value, so it's on same FS as StoreFiles
name|path
operator|=
operator|new
name|Path
...
name|IOException
argument_list|(
literal|"Wrong file system! Should be "
operator|+
name|path
operator|.
name|toUri
