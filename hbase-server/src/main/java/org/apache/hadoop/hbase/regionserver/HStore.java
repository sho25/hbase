...
name|cacheConf
decl_stmt|;
comment|// ttl in milliseconds. TODO: can this be removed? Already stored in scanInfo.
specifier|private
name|long
name|ttl
decl_stmt|;
...
argument_list|)
expr_stmt|;
comment|// Copy some things to local fields for convenience.
name|this
operator|.
name|storeFileManager
operator|=
...
return|;
block|}
comment|/**    * @param parentRegionDirectory directory for the parent region    * @param family family name of this store    * @return Path to the family/Store home directory    */
specifier|public
specifier|static
name|Path
name|getStoreHomedir
...
try|try
block|{
comment|// Path name is null if there is no entry to flush
if|if
condition|(
name|pathName
operator|!=
...
comment|// The hfile is current up to and including logCacheFlushId.
name|status
operator|.
name|setStatus
argument_list|(
...
return|;
block|}
comment|// Update filesCompacting (check that we do not try to compact the same StoreFile twice).
if|if
condition|(
operator|!
name|Collections
...
parameter_list|)
block|{
comment|// If this is a user-requested compaction, leave this at the user priority
if|if
condition|(
name|priority
operator|!=
