...
name|warn
argument_list|(
literal|"Refreshing region HDFS Block dist failed with exception, ignoring"
argument_list|,
name|e
argument_list|)
expr_stmt|;
...
block|{
comment|// for the case where we have region replica it is better we get the entire cluster's snapshot
name|clusterState
operator|=
name|getRegionAssignmentsByServer
argument_list|(
...
argument_list|()
decl_stmt|;
comment|// In the current set of regions even if one has region replica let us go with
comment|// getting the entire snapshot
if|if
condition|(
name|this
operator|.
...
condition|)
block|{
comment|// We have reached the max. Means the servers that we collected is still lowering the
...
condition|)
block|{
comment|// check if any other unused server is there for us to use.
comment|// If so use it. Else we have not other go but to go with one of them
if|if
condition|(
operator|!
name|cluster
