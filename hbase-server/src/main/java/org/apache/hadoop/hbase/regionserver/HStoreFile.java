...
expr_stmt|;
block|}
comment|/**    * Constructor, loads a reader and it's indices, etc. May allocate a substantial amount of ram    * depending on the underlying files (10-20MB?).    * @param fileInfo The store file information.    * @param cfBloomType The bloom type to use for this store file as specified by column    *          family configuration. This may or may not be the same as the Bloom filter type    *          actually present in the HFile, because column family configuration might change. If    *          this is {@link BloomType#NONE}, the existing Bloom filter is ignored.    * @param cacheConf The cache configuration and block cache reference.    */
specifier|public
name|HStoreFile
parameter_list|(
name|StoreFileInfo
...
literal|null
argument_list|,
literal|"Major compation has not been set yet"
argument_list|)
expr_stmt|;
return|return
name|this
...
argument_list|)
expr_stmt|;
comment|// steam reader need copy stuffs from pread reader
name|reader
operator|.
name|copyFields
argument_list|(
