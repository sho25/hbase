...
return|;
block|}
comment|/**    * Get the next row of values from this Store.    * @param outResult    * @param limit    * @param remainingResultSize    * @return true if there are more rows, false if scanner is done    */
annotation|@
name|Override
specifier|public
name|NextState
...
argument_list|()
decl_stmt|;
comment|// If limit< 0 and remainingResultSize< 0 we can skip the row comparison because we know
comment|// the row has changed. Else it is possible we are still traversing the same row so we
comment|// must perform the row comparison.
if|if
condition|(
operator|(
name|limit
