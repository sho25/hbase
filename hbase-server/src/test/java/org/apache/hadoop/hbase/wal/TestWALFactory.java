...
argument_list|)
expr_stmt|;
comment|// if providers are not set but enable SyncReplicationWALProvider by default for master node
comment|// with not only system tables
name|WALFactory
name|walFactory
init|=
operator|new
...
argument_list|)
expr_stmt|;
comment|// if providers are not set and do not enable SyncReplicationWALProvider
name|walFactory
operator|=
operator|new
name|WALFactory
...
argument_list|)
expr_stmt|;
comment|// class of WALProvider and metaWALProvider are the same when metaWALProvider is not set
name|assertEquals
argument_list|(
name|RegionGroupingProvider
operator|.
...
specifier|public
name|void
name|testOnlySetMetaWALProvider
parameter_list|()
throws|throws
name|IOException
block|{
...
argument_list|()
decl_stmt|;
comment|// AsyncFSWal is the default, we should be able to request any WAL.
specifier|final
name|WALFactory
name|normalWalFactory
init|=
operator|new
name|WALFactory
argument_list|(
...
argument_list|)
expr_stmt|;
comment|// Imagine a world where MultiWAL is the default
specifier|final
name|WALFactory
name|customizedWalFactory
init|=
operator|new
name|WALFactory
argument_list|(
...
block|}
decl_stmt|;
comment|// If we don't specify a WALProvider, we should get the default implementation.
name|Class
argument_list|<
name|?
extends|extends
