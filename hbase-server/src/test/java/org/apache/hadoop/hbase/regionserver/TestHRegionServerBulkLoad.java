...
specifier|static
class|class
name|OnlineSchemaChangeMaxVersionsThread
extends|extends
name|RepeatingTestThread
block|{
specifier|private
...
name|targetFamilies
expr_stmt|;
comment|// this should be validated, but
comment|// it's only a test-facing API, so I
comment|// can live with this
if|if
condition|(
name|totalNumIterations
argument_list|<
...
name|fail
argument_list|(
literal|"Unreasonable input provided to schema change thread. Please select a value between 1 and 20"
argument_list|)
expr_stmt|;
block|}
name|this
...
name|fail
argument_list|(
literal|"Not able to get a handle on the hbase admin."
argument_list|)
expr_stmt|;
block|}
block|}
...
name|fail
argument_list|(
literal|"Issue pulling table descriptor"
argument_list|)
expr_stmt|;
block|}
name|HColumnDescriptor
...
name|fail
argument_list|(
literal|"Timed out reaching before required modify count. Only had "
operator|+
name|numIterations
operator|+
literal|" updates"
...
name|println
argument_list|(
literal|"DEBUG: Current number of versions for family "
operator|+
name|Bytes
operator|.
name|toString
...
name|fail
argument_list|(
literal|"IO Issue while modifying column"
argument_list|)
expr_stmt|;
block|}
name|assertFalse
...
name|fail
argument_list|(
literal|"Table not found. Failing."
argument_list|)
expr_stmt|;
block|}
catch|catch
...
name|println
argument_list|(
literal|"WARN: Sleep was interrupted. This is unusual, but not grounds for TF"
argument_list|)
expr_stmt|;
name|e
operator|.
...
specifier|public
name|void
name|testAtomicBulkLoadWithSchemaChange
parameter_list|()
throws|throws
name|Exception
block|{
...
expr_stmt|;
block|}
comment|// Verification of data insertion
name|Assert
operator|.
name|assertEquals
argument_list|(
literal|"Incorrect number of rows found."
argument_list|,
name|NUM_ROWS
argument_list|,
name|UTIL
...
condition|)
block|{
comment|// Verification of data updated
name|htd
operator|=
name|UTIL
operator|.
...
control|)
block|{
comment|// verify that at least one pass occurred through the loop
name|HColumnDescriptor
name|hcd
init|=
name|htd
...
name|assertTrue
argument_list|(
literal|"The full number of iterations for family "
operator|+
name|Bytes
operator|.
name|toString
...
argument_list|)
operator|+
literal|" was not done. Expecting at least "
operator|+
operator|(
name|NUM_CF_ITERATIONS
operator|+
