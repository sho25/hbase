...
expr_stmt|;
block|}
comment|// Wait for regions to come back online again.  waitForAllRegionsOnline can come back before
comment|// we've assigned out regions on the cluster so retry if we are shy the wanted number
name|Collection
argument_list|<
name|HRegion
argument_list|>
...
name|info
argument_list|(
literal|"Regions of drained server: "
operator|+
name|regionsAfter
operator|+
literal|", all regions: "
...
expr_stmt|;
block|}
comment|// Sleep some else we spam the log w/ notice that servers are not yet alive.
name|Threads
operator|.
name|sleep
argument_list|(
