...
argument_list|)
expr_stmt|;
comment|// On construction bucketcache WriterThread is blocked on the writer queue so it will not
comment|// notice the disabling of the writer until after it has processed an entry.  Lets pass one
comment|// through after setting disable flag on the writer. We want to disable the WriterThread so
comment|// we can run the doDrain manually so we can watch it working and assert it doing right thing.
name|wt
operator|.
name|disableWriter
argument_list|()
...
argument_list|)
expr_stmt|;
comment|// Now writer thread should be disabled.
block|}
annotation|@
name|After
specifier|public
...
expr_stmt|;
block|}
comment|/**    * Do IOE. Take the RAMQueueEntry that was on the queue, doctor it to throw exception, then    * put it back and process it.    * @throws IOException     * @throws BucketAllocatorException     * @throws CacheFullException     * @throws InterruptedException     */
annotation|@
...
expr_stmt|;
block|}
comment|/**    * Do Cache full exception    * @throws IOException     * @throws BucketAllocatorException     * @throws CacheFullException     * @throws InterruptedException     */
annotation|@
name|Test
argument_list|(
name|timeout
