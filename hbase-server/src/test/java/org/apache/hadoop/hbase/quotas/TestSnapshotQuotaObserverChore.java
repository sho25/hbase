...
argument_list|)
expr_stmt|;
comment|// Snapshot size has to be 0 as the snapshot shares the data with the table
specifier|final
name|Table
name|quotaTable
init|=
...
argument_list|)
expr_stmt|;
comment|// Total usage has to remain same as what we saw before taking a snapshot
name|TEST_UTIL
operator|.
name|waitFor
argument_list|(
...
argument_list|)
expr_stmt|;
comment|// Now the snapshot size has to prev total size
name|TEST_UTIL
operator|.
name|waitFor
argument_list|(
...
argument_list|)
expr_stmt|;
comment|// The total size now has to be equal/more than double of prev total size
comment|// as double the number of store files exist now.
specifier|final
name|AtomicReference
argument_list|<
name|Long
argument_list|>
name|sizeAfterCompaction
init|=
operator|new
name|AtomicReference
argument_list|<>
...
argument_list|)
expr_stmt|;
comment|// Total size has to come down to prev totalsize - snapshot size(which was removed)
name|TEST_UTIL
operator|.
name|waitFor
argument_list|(
