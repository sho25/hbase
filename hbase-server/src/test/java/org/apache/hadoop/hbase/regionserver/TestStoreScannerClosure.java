...
specifier|public
name|void
name|testScannerCloseAndUpdateReadersWithMemstoreScanner
parameter_list|()
throws|throws
name|Exception
block|{
...
argument_list|)
expr_stmt|;
comment|// create the store scanner here.
comment|// for easiness, use Long.MAX_VALUE as read pt
try|try
init|(
...
name|lock
decl_stmt|;
comment|// use the lock to manually get a new memstore scanner. this is what
comment|// HStore#notifyChangedReadersObservers does under the lock.(lock is not needed here
comment|//since it is just a testcase).
name|lock
operator|.
name|readLock
argument_list|()
...
parameter_list|()
block|{
comment|// close should be completed
name|scan
operator|.
name|close
argument_list|(
...
try|try
block|{
comment|// use the updated memstoreScanners and pass it to updateReaders
name|scan
operator|.
name|updateReaders
argument_list|(
...
argument_list|()
expr_stmt|;
comment|// wait for close and updateThread to complete
name|closeThread
operator|.
name|join
argument_list|()
...
condition|)
block|{
comment|// There should be no unpooled chunks
name|int
...
name|assertTrue
argument_list|(
literal|"The memstore should not have unpooled chunks"
argument_list|,
name|openScannerCount
operator|==
literal|0
