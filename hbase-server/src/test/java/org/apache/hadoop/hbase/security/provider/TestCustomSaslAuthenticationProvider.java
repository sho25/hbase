...
condition|)
block|{
comment|// Set the connection registry to ZKConnectionRegistry since hedging is not supported on
comment|// blocking rpc clients.
name|CONF
operator|.
name|set
argument_list|(
...
name|Exception
block|{
comment|// Depending on the registry in use, the following code can throw exceptions at different
comment|// places. Master registry fails at the createConnection() step because the RPC to the
comment|// master fails with sasl auth. With ZK registry, connection creation succeeds (since there
comment|// is no RPC to HBase services involved) but the subsequent get() fails. The root cause
comment|// should still be a SaslException in both the cases.
try|try
init|(
name|Connection
name|conn
...
parameter_list|)
block|{
comment|// Any other exception is unexpected.
name|fail
argument_list|(
literal|"Unexpected exception caught, was expecting a authentication error: "
operator|+
name|Throwables
operator|.
