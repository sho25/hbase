...
expr_stmt|;
block|}
comment|/**    * Home our data in a dir under {@link #DEFAULT_BASE_TEST_DIRECTORY}.    * Give it a random name so can have many concurrent tests running if    * we need to.  It needs to amend the {@link #TEST_DIRECTORY_KEY}    * System property, as it's what minidfscluster bases    * it data dir on.  Moding a System property is not the way to do concurrent    * instances -- another instance could grab the temporary    * value unintentionally -- but not anything can do about it at moment;    * single instance only is how the minidfscluster works.    *    * We also create the underlying directory names for    *  hadoop.log.dir, mapreduce.cluster.local.dir and hadoop.tmp.dir, and set the values    *  in the conf, and as a system property for hadoop.tmp.dir (We do not create them!).    *    * @return The calculated data test build directory, if newly-created.    */
annotation|@
name|Override
specifier|protected
name|Path
...
expr_stmt|;
block|}
comment|/**    *  Check whether the tests should assume NEW_VERSION_BEHAVIOR when creating    *  new column families. Default to false.    */
specifier|public
name|boolean
name|isNewVersionBehaviorEnabled
parameter_list|()
block|{
specifier|final
name|String
name|propName
init|=
literal|"hbase.tests.new.version.behavior"
decl_stmt|;
name|String
name|v
init|=
...
expr_stmt|;
block|}
comment|/**    * Shutdown HBase mini cluster.Does not shutdown zk or dfs if running.    * @throws java.io.IOException in case command is unsuccessful    */
specifier|public
name|void
name|shutdownMiniHBaseCluster
parameter_list|()
...
block|}
block|}
comment|/**    * Abruptly Shutdown HBase mini cluster. Does not shutdown zk or dfs if running.    * @throws java.io.IOException throws in case command is unsuccessful    */
specifier|public
name|void
...
block|}
block|}
comment|// close hbase admin, close current connection and reset MIN MAX configs for RS.
specifier|private
name|void
name|cleanup
parameter_list|()
...
return|;
block|}
comment|/**    * Create a table.    * @param tableName the table name    * @param families the families    * @param splitKeys the splitkeys    * @param replicaCount the region replica count    * @return A Table instance for the created table.    * @throws IOException throws IOException    */
specifier|public
name|Table
name|createTable
parameter_list|(
...
return|;
block|}
comment|/**    * Create a table.    * @param tableName the table name    * @param families the families    * @param splitKeys the split keys    * @param replicaCount the replica count    * @param c Configuration to use    * @return A Table instance for the created table.    * @throws IOException    */
specifier|public
name|Table
name|createTable
parameter_list|(
...
argument_list|()
expr_stmt|;
comment|// Update the master addresses if they changed.
specifier|final
name|String
name|masterConfigBefore
init|=
name|conf
operator|.
name|get
...
name|info
argument_list|(
literal|"Invalidated connection. Updating master addresses before: {} after: {}"
argument_list|,
name|masterConfigBefore
argument_list|,
name|masterConfAfter
...
begin_comment
comment|/**    * Get a Connection to the cluster. Not thread-safe (This class needs a lot of work to make it    * thread-safe).    * @return A Connection that can be shared. Don't close. Will be closed on shutdown of cluster.    */
end_comment
...
begin_comment
comment|/**    * Wait until all regions in a table have been assigned    * @param table Table to wait on.    * @param timeoutMillis Timeout.    */
end_comment
...
begin_comment
comment|/**    * Waits for a table to be 'enabled'.  Enabled means that table is set as 'enabled' and the    * regions have been all assigned.  Will timeout after default period (30 seconds)    * Tolerates nonexistent table.    * @param table the table to wait on.    * @throws InterruptedException if interrupted while waiting    * @throws IOException if an IO problem is encountered    */
end_comment
