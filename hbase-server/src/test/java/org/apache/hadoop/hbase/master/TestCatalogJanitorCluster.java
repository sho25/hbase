...
argument_list|)
expr_stmt|;
comment|// Test what happens if we blow away an info:server row, if it is null. Should not kill CJ
comment|// and we should log the row that had the problem. HBASE-23192. Just make sure we don't
comment|// break if this happens.
name|LOG
operator|.
name|info
argument_list|(
literal|"Make null info:server"
argument_list|)
expr_stmt|;
name|Put
name|emptyInfoServerPut
init|=
operator|new
name|Put
argument_list|(
...
argument_list|)
expr_stmt|;
comment|// Mke an empty regioninfo in t1.
name|RegionInfo
name|t1Ri2
init|=
name|t1Ris
