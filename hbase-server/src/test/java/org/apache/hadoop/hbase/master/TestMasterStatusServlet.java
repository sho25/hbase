...
name|AssignmentManagerStatusTmpl
argument_list|()
comment|// NOT IMPLEMENTED!!!! .setLimit(50)
operator|.
name|render
argument_list|(
name|sw
...
argument_list|)
expr_stmt|;
comment|/* BROKEN BY  HBASE-13839 Fix AssgnmentManagerTmpl.jamon issues (coloring, content etc.) FIX!!     // Make sure we only see 50 of them     Matcher matcher = Pattern.compile("CLOSING").matcher(result);     int count = 0;     while (matcher.find()) {       count++;     }     assertEquals(50, count);     */
block|}
block|}
end_class
