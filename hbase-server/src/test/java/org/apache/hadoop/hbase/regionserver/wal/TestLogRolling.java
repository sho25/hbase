...
argument_list|()
expr_stmt|;
comment|// For slow sync threshold test: roll after 5 slow syncs in 10 seconds
name|TEST_UTIL
operator|.
name|getConfiguration
argument_list|()
...
name|FSHLog
operator|.
name|SLOW_SYNC_ROLL_INTERVAL_MS
argument_list|,
literal|10
operator|*
literal|1000
argument_list|)
expr_stmt|;
comment|// For slow sync threshold test: roll once after a sync above this threshold
name|TEST_UTIL
operator|.
name|getConfiguration
argument_list|()
...
try|try
block|{
comment|// Get a reference to the FSHLog
name|server
operator|=
name|TEST_UTIL
operator|.
...
argument_list|)
decl_stmt|;
comment|// Register a WALActionsListener to observe if a SLOW_SYNC roll is requested
specifier|final
name|AtomicBoolean
name|slowSyncHookCalled
init|=
operator|new
name|AtomicBoolean
argument_list|()
...
name|assertFalse
argument_list|(
literal|"Should not have triggered log roll due to SLOW_SYNC"
argument_list|,
name|slowSyncHookCalled
operator|.
name|get
...
argument_list|)
expr_stmt|;
comment|// Wrap the current writer with the anonymous class below that adds 200 ms of
comment|// latency to any sync on the hlog. This should be more than sufficient to trigger
comment|// slow sync warnings.
specifier|final
name|Writer
name|oldWriter1
init|=
...
expr_stmt|;
comment|// Write some data.
comment|// We need to write at least 5 times, but double it. We should only request
comment|// a SLOW_SYNC roll once in the current interval.
for|for
control|(
name|int
name|i
...
expr_stmt|;
block|}
comment|// Wait for our wait injecting writer to get rolled out, as needed.
name|TEST_UTIL
operator|.
name|waitFor
argument_list|(
...
block|{
return|return
literal|"Waited too long for our test writer to get rolled out"
return|;
block|}
block|}
argument_list|)
...
name|assertTrue
argument_list|(
literal|"Should have triggered log roll due to SLOW_SYNC"
argument_list|,
name|slowSyncHookCalled
operator|.
name|get
...
argument_list|)
expr_stmt|;
comment|// Wrap the current writer with the anonymous class below that adds 5000 ms of
comment|// latency to any sync on the hlog.
comment|// This will trip the other threshold.
specifier|final
name|Writer
name|oldWriter2
init|=
...
argument_list|)
expr_stmt|;
comment|// Write some data. Should only take one sync.
name|writeData
argument_list|(
name|table
argument_list|,
