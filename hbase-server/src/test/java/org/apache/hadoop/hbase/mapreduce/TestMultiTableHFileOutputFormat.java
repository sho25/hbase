...
begin_comment
comment|/**  * Test for{@link MultiTableHFileOutputFormat}. Sets up and runs a mapreduce job that output directories and  * writes hfiles.  */
end_comment
...
specifier|public
class|class
name|TestMultiTableHFileOutputFormat
block|{
specifier|private
specifier|static
specifier|final
...
argument_list|)
decl_stmt|;
comment|/**    * Run small MR job. this MR job will write HFile into    * testWritingDataIntoHFiles/tableNames/columnFamilies/    */
annotation|@
name|Test
specifier|public
name|void
...
block|}
block|}
comment|/**    * check whether create directory and hfiles as format designed in MultiHFilePartitioner    * and also check whether the output file has same related configuration as created table    */
annotation|@
name|Test
specifier|public
name|void
name|testMultiHFilePartitioner
parameter_list|()
throws|throws
name|Exception
block|{
...
name|getDataTestDirOnTestFS
argument_list|(
literal|"testMultiHFilePartitioner"
argument_list|)
decl_stmt|;
name|FileSystem
name|fs
...
name|info
argument_list|(
literal|"testMultiHFilePartitioner dir writing to : "
operator|+
name|testDir
argument_list|)
expr_stmt|;
...
argument_list|)
expr_stmt|;
comment|// Create several tables for testing
name|List
argument_list|<
name|TableName
argument_list|>
...
argument_list|()
decl_stmt|;
comment|// to store splitKeys for TABLE[0] for testing;
name|byte
index|[]
index|[]
name|testKeys
...
argument_list|)
decl_stmt|;
comment|//only set Tables[0] configuration, and specify compression type and DataBlockEncode
if|if
condition|(
name|i
operator|==
...
expr_stmt|;
block|}
comment|// set up for MapReduce job
try|try
block|{
name|Job
name|job
...
name|info
argument_list|(
literal|"Starting test testWritingDataIntoHFiles"
argument_list|)
expr_stmt|;
name|assertTrue
argument_list|(
...
name|info
argument_list|(
literal|"Waiting on checking MapReduce output"
argument_list|)
expr_stmt|;
name|assertTrue
argument_list|(
...
name|assertTrue
argument_list|(
name|checkFileConfAndSplitKeys
argument_list|(
name|conf
argument_list|,
name|fs
...
block|}
block|}
comment|/**    * check the output hfile has same configuration as created test table    * and also check whether hfiles get split correctly    * only check TABLES[0]    */
specifier|private
name|boolean
name|checkFileConfAndSplitKeys
parameter_list|(
...
return|;
block|}
comment|/**    * Check whether the Hfile has been split by region boundaries    * @param splitKeys split keys for that table    * @param firstKeys first rowKey for hfiles    * @param lastKeys last rowKey for hfiles    */
specifier|private
name|boolean
name|checkFileSplit
parameter_list|(
