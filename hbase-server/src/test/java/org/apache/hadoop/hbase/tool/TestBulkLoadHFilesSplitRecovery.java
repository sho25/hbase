...
expr_stmt|;
block|}
comment|/**    * Test that shows that exception thrown from the RS side will result in the expected number of    * retries set by ${@link HConstants#HBASE_CLIENT_RETRIES_NUMBER} when    * ${@link BulkLoadHFiles#RETRY_ON_IO_EXCEPTION} is set    */
annotation|@
name|Test
specifier|public
name|void
...
expr_stmt|;
block|}
comment|/**    * We are testing a split after initial validation but before the atomic bulk load call.    * We cannot use presplitting to test this path, so we actually inject a    * split just before the atomic region load. However, we will pass null item2RegionMap    * and that should not affect the bulk load behavior.    */
annotation|@
name|Test
specifier|public
name|void
name|testSplitWhileBulkLoadPhaseWithoutItemMap
parameter_list|()
throws|throws
name|Exception
block|{
...
expr_stmt|;
block|}
comment|// Passing item2RegionMap null
comment|// In the absence of LoadQueueItem, bulk load should work as expected
name|super
operator|.
name|bulkLoadPhase
argument_list|(
