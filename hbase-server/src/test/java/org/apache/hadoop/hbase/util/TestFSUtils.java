...
argument_list|)
expr_stmt|;
comment|// No meta dir so no complaint from checkVersion.
comment|// Presumes it a new install. Will create version file.
name|FSUtils
operator|.
name|checkVersion
argument_list|(
...
argument_list|)
expr_stmt|;
comment|// Now remove the version file and create a metadir so checkVersion fails.
name|Path
name|versionFile
init|=
operator|new
...
name|assertTrue
argument_list|(
literal|"Expected FileSystemVersionException"
argument_list|,
name|thrown
argument_list|)
expr_stmt|;
comment|// Write out a good version file.  See if we can read it in and convert.
name|String
name|version
init|=
name|HConstants
...
argument_list|)
expr_stmt|;
comment|// Write an old version file.
name|String
name|oldVersion
init|=
literal|"1"
...
name|Exception
block|{
name|verifyNoHDFSApiInvocationForDefaultPolicy
argument_list|()
expr_stmt|;
...
expr_stmt|;
block|}
comment|/**    * Note: currently the default policy is set to defer to HDFS and this case is to verify the    * logic, will need to remove the check if the default policy is changed    */
specifier|private
name|void
name|verifyNoHDFSApiInvocationForDefaultPolicy
parameter_list|()
...
init|=
operator|new
name|AlwaysFailSetStoragePolicyFileSystem
argument_list|()
decl_stmt|;
comment|// There should be no exception thrown when setting to default storage policy, which indicates
comment|// the HDFS API hasn't been called
try|try
block|{
name|FSUtils
operator|.
...
name|fail
argument_list|(
literal|"Should have bypassed the FS API when setting default storage policy"
argument_list|)
expr_stmt|;
block|}
comment|// There should be exception thrown when given non-default storage policy, which indicates the
comment|// HDFS API has been called
try|try
block|{
name|FSUtils
operator|.
...
name|fail
argument_list|(
literal|"Should have invoked the FS API but haven't"
argument_list|)
expr_stmt|;
block|}
catch|catch
...
parameter_list|)
block|{
comment|// expected given an invalid path
block|}
block|}
class|class
name|AlwaysFailSetStoragePolicyFileSystem
...
name|IOException
argument_list|(
literal|"The setStoragePolicy method is invoked"
argument_list|)
throw|;
block|}
block|}
...
name|debug
argument_list|(
literal|"The storage policy of path "
operator|+
name|p
operator|+
literal|" is "
...
name|debug
argument_list|(
literal|"The default hdfs storage policy (indicated by home path: "
operator|+
name|hfs
operator|.
name|getHomeDirectory
