...
argument_list|)
expr_stmt|;
comment|// install region co-processor to fail splits
name|region
operator|.
...
name|load
argument_list|(
name|FailingSplitRegionObserver
operator|.
name|class
argument_list|,
name|Coprocessor
...
block|}
block|}
comment|/**    * A test that intentionally has master fail the processing of the split message.    * Tests that after we process server shutdown, the daughters are up on line.    * @throws IOException    * @throws InterruptedException    * @throws ServiceException    */
annotation|@
name|Test
argument_list|(
name|timeout
...
specifier|public
name|void
name|testRSSplitDaughtersAreOnlinedAfterShutdownHandling
parameter_list|()
throws|throws
name|IOException
throws|,
...
name|valueOf
argument_list|(
literal|"testRSSplitDaughtersAreOnlinedAfterShutdownHandling"
argument_list|)
decl_stmt|;
comment|// Create table then get the single region for our new table.
name|Table
...
parameter_list|)
block|{
comment|// Expected. The regionserver should crash
block|}
...
comment|// returns true in checking rebuilt region states map.
name|hri
operator|.
name|setOffline
argument_list|(
...
name|assertTrue
argument_list|(
literal|"Split parent should be in SPLIT state"
argument_list|,
name|regionStates
operator|.
...
specifier|public
name|void
name|testSplitAndRestartingMaster
parameter_list|()
throws|throws
name|Exception
block|{
...
name|info
argument_list|(
literal|"Starting testSplitAndRestartingMaster"
argument_list|)
expr_stmt|;
specifier|final
name|TableName
...
name|valueOf
argument_list|(
literal|"testSplitAndRestartingMaster"
argument_list|)
decl_stmt|;
comment|// Create table then get the single region for our new table.
name|createTableAndWait
...
specifier|public
name|void
name|testSplitHooksBeforeAndAfterPONR
parameter_list|()
throws|throws
name|Exception
block|{
...
name|valueOf
argument_list|(
literal|"testSplitHooksBeforeAndAfterPONR_1"
argument_list|)
decl_stmt|;
name|TableName
name|secondTable
...
name|valueOf
argument_list|(
literal|"testSplitHooksBeforeAndAfterPONR_2"
argument_list|)
decl_stmt|;
name|HColumnDescriptor
name|hcd
...
argument_list|)
decl_stmt|;
comment|// Check that both tables actually have regions.
if|if
condition|(
name|firstTableRegions
operator|.
...
name|fail
argument_list|(
literal|"Each table should have at least one region."
argument_list|)
expr_stmt|;
block|}
name|ServerName
...
name|assertEquals
argument_list|(
literal|"Number of regions after split should be 2."
argument_list|,
literal|2
argument_list|,
name|firstTableRegions
...
specifier|public
name|void
name|testSSHCleanupDaugtherRegionsOfAbortedSplit
parameter_list|()
throws|throws
name|Exception
block|{
...
name|valueOf
argument_list|(
literal|"testSSHCleanupDaugtherRegionsOfAbortedSplit"
argument_list|)
decl_stmt|;
try|try
block|{
...
argument_list|()
expr_stmt|;
comment|// Before we check deadServerInProgress, we should ensure server is dead at master side.
while|while
condition|(
operator|!
name|cluster
...
specifier|static
class|class
name|MockedCoordinatedStateManager
extends|extends
name|ZkCoordinatedStateManager
block|{
specifier|public
