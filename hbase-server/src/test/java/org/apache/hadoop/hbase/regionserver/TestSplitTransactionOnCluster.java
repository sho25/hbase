...
block|}
block|}
comment|/**    * Test that if daughter split on us, we won't do the shutdown handler fixup just because we can't    * find the immediate daughter of an offlined parent.    */
annotation|@
name|Test
specifier|public
name|void
...
condition|)
block|{
comment|// Wait on any current compaction to complete first.
name|CompactionProgress
name|progress
init|=
name|store
...
block|{
comment|// Run new compaction. Shoudn't be any others running.
name|region
operator|.
name|compact
argument_list|(
...
block|}
block|}
comment|/**    * Verifies HBASE-5806. Here the case is that splitting is completed but before the CJ could    * remove the parent region the master is killed and restarted.    */
annotation|@
name|Test
specifier|public
name|void
...
return|;
block|}
comment|/**    * Ensure single table region is not on same server as the single hbase:meta table    * region.    * @return Index of the server hosting the single table region    * @throws UnknownRegionException    * @throws MasterNotRunningException    * @throws org.apache.hadoop.hbase.ZooKeeperConnectionException    * @throws InterruptedException    */
specifier|private
name|int
name|ensureTableRegionNotOnSameServerAsMeta
parameter_list|(
