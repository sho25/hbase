...
name|Exception
block|{   }
comment|/**    * Simulates splitting a WAL out from under a regionserver that is still trying to write it.  Ensures we do not    * lose edits.    * @throws IOException    */
annotation|@
name|Test
specifier|public
name|void
...
literal|9
decl_stmt|;
comment|// The below method writes NUM_WRITERS files each with ENTRIES entries it for a total of NUM_WRITERS * ENTRIES added
comment|// per column family in the region.
name|generateHLogs
argument_list|(
name|walToKeepOpen
argument_list|)
...
argument_list|)
decl_stmt|;
comment|// This WAL should be open still
name|HLog
operator|.
name|Writer
name|stillOpenWAL
...
index|]
decl_stmt|;
comment|// Thread that will keep writing the WAL across the WAL splitting below.  Before we start writing, make sure the
comment|// counter has the edits that were written above in generateHLogs.
name|counter
operator|.
name|addAndGet
argument_list|(
...
argument_list|()
expr_stmt|;
comment|// Get the recovered edits file made by the WAL log splitting process above.
name|Path
name|logfile
init|=
...
argument_list|)
decl_stmt|;
comment|// It's possible that the writer got an error while appending and didn't count an edit
comment|// however the entry will in fact be written to file and split with the rest
name|long
name|numberOfEditsInRegion
init|=
name|countHLog
argument_list|(
name|logfile
...
name|assertFalse
argument_list|(
literal|"Previous test should clean up table dir"
argument_list|,
name|fs
operator|.
name|exists
