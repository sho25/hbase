...
annotation|@
name|Ignore
comment|//TODO: This still does not finish, making the LoadBalancer unusable at this scale. We should solve this.
comment|//There are two reasons so far;
comment|// - It takes too long for iterating for all servers
comment|// - Moving one region out of the loaded server only costs a slight decrease in the cost of regionCountSkewCost
comment|// but also a slight increase on the moveCost. loadMultiplier / moveCostMultiplier is not high enough to bring down
comment|// the total cost, so that the eager selection cannot continue. This can be solved by smt like
comment|// http://en.wikipedia.org/wiki/Simulated_annealing instead of random walk with eager selection
specifier|public
name|void
