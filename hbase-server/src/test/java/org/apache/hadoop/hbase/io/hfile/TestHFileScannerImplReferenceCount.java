...
return|;
block|}
comment|// It's a deep copy of configuration of UTIL, DON'T use shallow copy.
specifier|private
name|Configuration
name|conf
decl_stmt|;
...
argument_list|)
expr_stmt|;
comment|// Create a bucket cache with 32MB.
name|conf
operator|.
name|set
argument_list|(
...
argument_list|)
expr_stmt|;
comment|// All allocated ByteBuff are pooled ByteBuff.
name|conf
operator|.
name|setInt
argument_list|(
...
name|info
argument_list|(
literal|"Start to write {} cells into hfile: {}, case:{}"
argument_list|,
name|CELL_COUNT
argument_list|,
name|hfilePath
...
name|TestBucketCache
operator|.
name|waitUntilAllFlushedToBucket
argument_list|(
operator|(
name|BucketCache
operator|)
...
block|}
block|}
comment|/**    * A careful UT for validating the reference count mechanism, if want to change this UT please    * read the design doc in HBASE-21879 firstly and make sure that understand the refCnt design.    */
specifier|private
name|void
...
argument_list|)
expr_stmt|;
comment|// Only one refCnt for RPC path.
name|Assert
operator|.
name|assertEquals
argument_list|(
...
argument_list|)
expr_stmt|;
comment|// After shipped, the prevBlock will be release, but curBlock is still referenced by the
comment|// curBlock.
name|scanner
operator|.
name|shipped
...
argument_list|)
expr_stmt|;
comment|// The curBlock will also be released.
name|scanner
operator|.
name|close
argument_list|()
...
argument_list|)
expr_stmt|;
comment|// Wait until flushed to IOEngine;
name|waitBucketCacheFlushed
argument_list|(
name|defaultBC
argument_list|)
expr_stmt|;
comment|// One RPC reference path.
name|Assert
operator|.
name|assertEquals
argument_list|(
...
name|curBlock
expr_stmt|;
comment|// Release the block1, no other reference.
name|Assert
operator|.
name|assertTrue
argument_list|(
...
argument_list|)
expr_stmt|;
comment|// Release the block2, no other reference.
name|Assert
operator|.
name|assertTrue
argument_list|(
...
name|curBlock
expr_stmt|;
comment|// the curBlock is read from IOEngine, so a different block.
name|Assert
operator|.
name|assertFalse
argument_list|(
...
argument_list|)
expr_stmt|;
comment|// Two reference for curBlock: 1. scanner; 2. blockCache.
name|this
operator|.
name|assertRefCnt
argument_list|(
...
argument_list|)
expr_stmt|;
comment|// Reference count of prevBlock must be unchanged because we haven't shipped.
name|this
operator|.
name|assertRefCnt
argument_list|(
...
argument_list|)
expr_stmt|;
comment|// Only pooled HFileBlock will be kept in prevBlocks and ExclusiveMemHFileBlock will never keep
comment|// in prevBlocks.
name|Assert
operator|.
name|assertTrue
argument_list|(
