...
specifier|public
name|void
name|testForceSplitRegionWithReference
parameter_list|()
throws|throws
name|IOException
block|{
...
specifier|public
name|void
name|testBusyRegionSplitPolicy
parameter_list|()
throws|throws
name|Exception
block|{
...
argument_list|)
expr_stmt|;
comment|// Not enough time since region came online
name|assertFalse
argument_list|(
name|policy
operator|.
...
argument_list|)
expr_stmt|;
comment|// Reset min age for split to zero
name|conf
operator|.
name|setLong
argument_list|(
...
argument_list|)
expr_stmt|;
comment|// Aggregate over 500 ms periods
name|conf
operator|.
name|setLong
argument_list|(
...
argument_list|)
expr_stmt|;
comment|// Enough blocked requests since last time, but aggregate blocked request
comment|// rate over last 500 ms is still low, because major portion of the window is constituted
comment|// by the previous zero blocked request period which lasted at least 300 ms off last 500 ms.
if|if
condition|(
name|EnvironmentEdgeManager
operator|.
...
specifier|public
name|void
name|testConstantSizePolicyWithJitter
parameter_list|()
throws|throws
name|IOException
block|{
...
argument_list|)
expr_stmt|;
comment|// Jitter shouldn't cause overflow when HTableDescriptor.MAX_FILESIZE set to Long.MAX_VALUE
name|assertFalse
argument_list|(
name|policy
operator|.
