...
name|info
argument_list|(
literal|"meta is on the same server: "
operator|+
name|regionSvr
argument_list|)
expr_stmt|;
comment|// when region is on same server as hbase:meta, reassigning meta would abort the server
comment|// since WAL is broken.
comment|// so the region is moved to a different server
name|HRegionServer
name|otherRs
init|=
name|UTIL
