...
return|;
block|}
comment|/**    * Get the queue id's and meta data (Owner and History) for the queues belonging to the named    * server    *    * @param server name of the server    * @return a ResultScanner over the QueueIds belonging to the server    * @throws IOException if getting the table or the scanner fails    */
specifier|protected
name|ResultScanner
name|getQueuesBelongingToServer
parameter_list|(
...
block|}
block|}
comment|/**    * Attempts to acquire the Replication Table. This operation will block until it is assigned by    * the CreateReplicationWorker thread. It is up to the caller of this method to close the    * returned Table    * @return the Replication Table when it is created    * @throws IOException if getting the table or the scanner fails    */
specifier|protected
name|Table
name|getOrBlockOnReplicationTable
parameter_list|()
...
return|;
block|}
comment|/**    * Creates a new copy of the Replication Table and sets up the proper Table time outs for it    *    * @return the Replication Table    * @throws IOException if getting the table fails    */
specifier|private
name|Table
name|getAndSetUpReplicationTable
parameter_list|()
...
block|}
block|}
comment|/**      * Create the replication table with the provided HColumnDescriptor REPLICATION_COL_DESCRIPTOR      * in TableBasedReplicationQueuesImpl      *      * @throws IOException if creating the table fails      */
specifier|private
name|void
name|createReplicationTable
parameter_list|()
...
block|}
block|}
comment|/**      * Checks whether the Replication Table exists yet      *      * @return whether the Replication Table exists      */
specifier|private
name|boolean
name|replicationTableExists
parameter_list|()
