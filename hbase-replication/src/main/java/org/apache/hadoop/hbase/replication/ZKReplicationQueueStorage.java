...
continue|continue;
block|}
comment|// Perform CAS in a specific version v0 (HBASE-20138)
name|int
name|v0
init|=
name|p
...
block|}
block|}
comment|/**    * Return the {lastPushedSequenceId, ZNodeDataVersion} pair. if ZNodeDataVersion is -1, it means    * that the ZNode does not exist.    */
annotation|@
name|VisibleForTesting
specifier|protected
name|Pair
...
condition|)
block|{
comment|// ZNode does not exist, so just return version -1 to indicate that no node exist.
return|return
name|Pair
operator|.
name|newPair
...
name|ReplicationException
argument_list|(
literal|"Failed to get last pushed sequence id (encodedRegionName="
operator|+
name|encodedRegionName
operator|+
literal|", peerId="
...
try|try
block|{
comment|// No need CAS and retry here, because it'll call setLastSequenceIds() for disabled peers
comment|// only, so no conflict happen.
name|List
argument_list|<
name|ZKUtilOp
argument_list|>
...
argument_list|)
decl_stmt|;
comment|// it is possible that levelOneDirs is null if we haven't write any last pushed sequence ids
comment|// yet, so we need an extra check here.
if|if
condition|(
name|CollectionUtils
operator|.
...
name|ReplicationException
argument_list|(
literal|"Failed to remove all last sequence ids, peerId="
operator|+
name|peerId
argument_list|,
name|e
...
name|ReplicationException
argument_list|(
literal|"Failed to remove last sequence ids, peerId="
operator|+
name|peerId
operator|+
literal|", encodedRegionNames.size="
operator|+
name|encodedRegionNames
operator|.
name|size
