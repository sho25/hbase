...
return|;
block|}
comment|/**    * Put all regions under /hbase/replication/regions znode will lead to too many children because    * of the huge number of regions in real production environment. So here we use hash of encoded    * region name to distribute the znode into multiple znodes.<br>    * So the final znode path will be format like this:    *    *<pre>    * /hbase/replication/regions/e1/ff/dd04e76a6966d4ffa908ed0586764767-100    *</pre>    *    * The e1 indicate the first level hash of encoded region name, and the ff indicate the second    * level hash of encoded region name, the 100 indicate the peer id.<br>    * Note that here we use two-level hash because if only one-level hash (such as mod 65535), it    * will still lead to too many children under the /hbase/replication/regions znode.    * @param encodedRegionName the encoded region name.    * @param peerId peer id for replication.    * @return ZNode path to persist the max sequence id that we've pushed for the given region and    *         peer.    */
annotation|@
name|VisibleForTesting
specifier|public
name|String
