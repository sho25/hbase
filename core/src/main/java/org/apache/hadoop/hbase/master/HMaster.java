...
expr_stmt|;
block|}
comment|/*    * Try to get an operation off of the todo queue and perform it.    * We actually have two tiers of todo; those that we couldn't do immediately    * which we put aside and then current current todos.  We look at put-asides    * first.    * @return True if we have nothing to do or we're to close.    */
specifier|private
name|boolean
name|processToDoQueue
parameter_list|()
...
name|warn
argument_list|(
literal|"Adding to delayed queue: "
operator|+
name|op
operator|.
name|toString
...
return|;
block|}
comment|/**    * @param op operation to requeue; added to the delayedToDoQueue.    */
name|void
name|requeue
parameter_list|(
specifier|final
...
expr_stmt|;
block|}
comment|/**    * @param op Operation to queue.  Added to the TODO queue.    */
name|void
name|queue
parameter_list|(
specifier|final
...
argument_list|()
expr_stmt|;
comment|// Wake main thread; TODO: Is this necessary?
name|this
operator|.
name|toDoQueue
operator|.
...
condition|)
block|{
comment|// TODO make zookeepercluster a field and do an orderly shutdown
name|MiniZooKeeperCluster
