...
return|;
block|}
comment|/**    * Return a map of the regions in transition on a server.    * Returned map entries are region name -> RegionState    */
name|Map
argument_list|<
name|String
argument_list|,
...
condition|)
block|{
comment|// TODO this code appears to be entirely broken, since
comment|// server.toString() has no start code, but s.getServerName()
comment|// does!
name|LOG
operator|.
name|fatal
argument_list|(
literal|"I DONT BELIEVE YOU WILL EVER SEE THIS!"
argument_list|)
expr_stmt|;
...
condition|)
block|{
comment|// Has an outstanding root region to be assigned.
return|return
literal|true
return|;
block|}
...
return|;
block|}
comment|/**    * Is this server assigned to transition a META table. HBASE-1928    *    * @param server Server    * @return if this server was transitioning a META table then a not null HRegionInfo pointing to it    */
specifier|public
name|HRegionInfo
...
name|UNASSIGNED
block|,
comment|// awaiting a server to be assigned
name|PENDING_OPEN
block|,
comment|// told a server to open, hasn't opened yet
name|OPEN
block|,
comment|// has been opened on RS, but not yet marked in META/ROOT
name|CLOSING
block|,
comment|// a msg has been enqueued to close ths region, but not delivered to RS yet
name|PENDING_CLOSE
block|,
comment|// msg has been delivered to RS to close this region
name|CLOSED
comment|// region has been closed but not yet marked in meta
block|}
specifier|private
name|State
name|state
