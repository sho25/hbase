...
argument_list|)
decl_stmt|;
comment|// Number of logs to read into memory before writing to their appropriate
comment|// regions when log splitting.  More means faster but bigger mem consumption
name|int
...
argument_list|)
decl_stmt|;
comment|// append support = we can avoid data loss (yay)
comment|// we open for append, then close to recover the correct file length
specifier|final
name|boolean
name|appendSupport
init|=
...
argument_list|)
decl_stmt|;
comment|// store corrupt logs for post-mortem analysis (empty string = discard)
specifier|final
name|String
name|corruptDir
init|=
...
control|)
block|{
comment|// Step 1: read N log files into memory
specifier|final
name|Map
argument_list|<
name|byte
...
argument_list|()
decl_stmt|;
comment|// make sure we get the right file length before opening for read
name|recoverLog
argument_list|(
name|fs
argument_list|,
...
name|msg
operator|+=
literal|"  Storing in hlog corruption directory."
expr_stmt|;
block|}
name|LOG
operator|.
...
name|warn
argument_list|(
literal|"File.close() threw exception -- continuing, "
operator|+
literal|"but marking file as corrupt."
argument_list|,
name|e
argument_list|)
expr_stmt|;
...
expr_stmt|;
block|}
comment|/* TODO FOR J-D REVIEW             // Archive the input file now so we do not replay edits. We could             // have gotten here because of an exception. If so, probably             // nothing we can do about it. Replaying it, it could work but we             // could be stuck replaying for ever. Just continue though we             // could have lost some edits.             fs.rename(logfiles[i].getPath(),                 getHLogArchivePath(oldLogDir, logfiles[i].getPath()));             */
block|}
block|}
comment|// Step 2: Some regionserver log files have been read into memory.
comment|//         Assign them to the appropriate region directory.
class|class
name|ThreadWithException
extends|extends
name|Thread
block|{
name|ThreadWithException
...
name|warn
argument_list|(
literal|"Got a region with no entries to process."
argument_list|)
expr_stmt|;
return|return;
block|}
...
literal|0
decl_stmt|;
comment|// get the logfile associated with this region.  2 logs often
comment|// write to the same region, so persist this info across logs
name|WriterAndPath
name|wap
init|=
name|logWriters
...
condition|)
block|{
comment|// first write to this region, make new logfile
assert|assert
name|entries
operator|.
name|size
...
argument_list|)
decl_stmt|;
comment|// If splitLog() was running when the user restarted his
comment|// cluster, then we could already have a 'logfile'.
comment|// Since we don't delete logs until everything is written to
comment|// their respective regions, we can safely remove this tmp.
if|if
condition|(
name|fs
operator|.
...
name|warn
argument_list|(
literal|"Deleting old hlog file: "
operator|+
name|logfile
argument_list|)
expr_stmt|;
comment|// TODO: Archive?
name|fs
operator|.
name|delete
argument_list|(
...
expr_stmt|;
block|}
comment|// associate an OutputStream with this logfile
name|Writer
name|w
init|=
name|createWriter
...
name|warn
argument_list|(
literal|"Hlog writers were interrupted during splitLog().  "
operator|+
literal|"Retaining log files to avoid data loss."
argument_list|)
expr_stmt|;
throw|throw
operator|new
...
throw|;
block|}
comment|// throw an exception if one of the threads reported one
for|for
control|(
name|ThreadWithException
name|t
...
block|}
block|}
comment|// End of for loop. Rinse and repeat
block|}
block|}
finally|finally
block|{
...
block|}
block|}
comment|// Step 3: All writes succeeded!  Get rid of the now-unnecessary logs
for|for
control|(
name|Path
name|p
...
name|debug
argument_list|(
literal|"Successfully split Hlog file.  Deleting "
operator|+
name|p
argument_list|)
expr_stmt|;
...
parameter_list|)
block|{
comment|// don't throw error here. worst case = double-read
name|LOG
operator|.
name|warn
argument_list|(
literal|"Error deleting successfully split Hlog ("
operator|+
name|p
operator|+
literal|") -- "
...
condition|)
block|{
comment|// store any corrupt logs for later analysis
name|Path
name|cp
init|=
operator|new
...
block|{
comment|// data loss is less important than disk space, delete
try|try
block|{
if|if
condition|(
...
name|warn
argument_list|(
literal|"Error deleting corrupt Hlog ("
operator|+
name|p
operator|+
literal|") -- "
...
begin_comment
comment|/*    * Recover log.    * Try and open log in append mode.    * Doing this, we get a hold of the file that crashed writer    * was writing to.  Once we have it, close it.  This will    * allow subsequent reader to see up to last sync.    * @param fs    * @param p    * @param append    */
end_comment
...
name|debug
argument_list|(
literal|"Recovering DFS lease for path "
operator|+
name|p
argument_list|)
expr_stmt|;
...
begin_comment
comment|// We create a new file name with a ts in front of it to make sure we almost
end_comment

begin_comment
comment|// certainly don't have a file name conflict.
end_comment
