...
block|}
block|}
comment|/**    * Delete information for all connections.    * @param stopProxy stop the proxy as well    */
specifier|public
specifier|static
name|void
name|deleteAllConnections
...
block|}
block|}
comment|/**    * Get a watcher of a zookeeper connection for a given quorum address.    * If the connection isn't established, a new one is created.    * This acts like a multiton.    * @param conf configuration    * @return ZKW watcher    * @throws IOException if a remote or network exception occurs    */
specifier|public
specifier|static
...
name|conf
decl_stmt|;
comment|/**      * Takes a configuration to pass it to ZKW but won't instanciate it      * @param conf configuration      */
specifier|public
name|ClientZKWatcher
parameter_list|(
name|Configuration
...
block|}
block|}
comment|/**      * Get this watcher's ZKW, instanciate it if necessary.      * @return ZKW      * @throws java.io.IOException if a remote or network exception occurs      */
specifier|public
specifier|synchronized
name|ZooKeeperWrapper
...
name|NoServerForRegionException
argument_list|(
literal|"Timed out trying to locate "
operator|+
literal|"root region because: "
operator|+
name|t
operator|.
name|getMessage
...
expr_stmt|;
block|}
comment|/**        * This is the method subclasses must implement.        * @param currentList current list of rows        * @param tableName table we are processing        * @param row row        * @return Count of items processed or -1 if all.        * @throws IOException if a remote or network exception occurs        * @throws RuntimeException other undefined exception        */
specifier|abstract
name|int
name|doCall
parameter_list|(
...
name|RuntimeException
function_decl|;
comment|/**        * Process the passed<code>list</code>.        * @param list list of rows to process        * @param tableName table we are processing        * @return Count of how many added or -1 if all added.        * @throws IOException if a remote or network exception occurs        */
name|int
name|process
parameter_list|(
specifier|final
...
argument_list|)
expr_stmt|;
comment|// Just give up, leaving the batch put list in an untouched/semi-committed state
if|if
condition|(
name|e
operator|.
