...
comment|// {RegionSplitPolicy,replica count} does not match requested, or when the
comment|// number of column families does not match requested.
if|if
condition|(
operator|(
name|exists
...
condition|)
block|{
comment|// set to thread number if connCount is not set
name|opts
operator|.
name|connCount
operator|=
...
literal|1
decl_stmt|;
comment|//wil decide the actual num later
name|boolean
name|useTags
init|=
literal|false
...
name|setStatus
argument_list|(
literal|"No valueSize statistics available"
argument_list|)
expr_stmt|;
block|}
if|if
...
condition|)
block|{
comment|// If multiget or multiput is enabled, say set to 10, testRow() returns immediately
comment|// first 9 times and sends the actual get request in the 10th iteration.
comment|// We should only set latency when actual request is sent because otherwise
comment|// it turns out to be 0.
if|if
condition|(
name|requestSent
condition|)
...
return|;
block|}
comment|/**      * Test for individual row.      * @param i Row index.      * @return true if the row was sent to server and need to record metrics.      *         False if not, multiGet and multiPut e.g., the rows are sent      *         to server only if enough gets/puts are gathered.      */
specifier|abstract
name|boolean
name|testRow
parameter_list|(
...
name|info
argument_list|(
literal|"MultiPut enabled. Sending PUTs in batches of "
operator|+
name|opts
operator|.
name|multiPut
...
argument_list|)
decl_stmt|;
comment|// unlike checkAndXXX tests, which make most sense to do on a single value,
comment|// if multiple families are specified for an increment test we assume it is
comment|// meant to raise the work factor
for|for
control|(
name|int
name|family
...
decl_stmt|;
comment|// unlike checkAndXXX tests, which make most sense to do on a single value,
comment|// if multiple families are specified for an append test we assume it is
comment|// meant to raise the work factor
for|for
control|(
name|int
...
argument_list|)
decl_stmt|;
comment|// checkAndXXX tests operate on only a single value
...
name|println
argument_list|(
literal|" connCount          connections all threads share. "
operator|+
literal|"For example, if set to 2, then all thread share 2 connection. "
operator|+
literal|"Default: depend on oneCon parameter. if oneCon set to true, then connCount=1, "
operator|+
literal|"if not, connCount=thread number"
argument_list|)
expr_stmt|;
name|System
operator|.
...
name|println
argument_list|(
literal|" multiPut        Batch puts together into groups of N. Only supported "
operator|+
literal|"by write. If multiPut is bigger than 0, autoFlush need to set to true. Default: 0"
argument_list|)
expr_stmt|;
name|System
operator|.
...
name|println
argument_list|(
literal|" families        Specify number of column families for the table. Default: 1"
argument_list|)
expr_stmt|;
name|System
operator|.
...
name|println
argument_list|(
literal|" bufferSize      Set the value of client side buffering. Default: 2MB"
argument_list|)
expr_stmt|;
name|System
operator|.
...
name|IllegalArgumentException
argument_list|(
literal|"autoFlush must be true when multiPut is more than 0"
argument_list|)
throw|;
block|}
continue|continue;
...
name|IllegalArgumentException
argument_list|(
literal|"oneCon is set to true, "
operator|+
literal|"connCount should not bigger than 1"
argument_list|)
throw|;
block|}
continue|continue;
