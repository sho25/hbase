...
block|}
block|}
name|submitBatchedMultiOrSequential
argument_list|(
name|zkw
argument_list|,
name|runSequentialOnMultiFailure
...
expr_stmt|;
block|}
comment|/**    * Chunks the provided {@code ops} when their approximate size exceeds the the configured limit.    * Take caution that this can ONLY be used for operations where atomicity is not important,    * e.g. deletions. It must not be used when atomicity of the operations is critical.    *    * @param zkw reference to the {@link ZKWatcher} which contains configuration and constants    * @param runSequentialOnMultiFailure if true when we get a ZooKeeper exception that could    *        retry the operations one-by-one (sequentially)    * @param ops list of ZKUtilOp {@link ZKUtilOp} to partition while submitting batched multi    *        or sequential    * @throws KeeperException unexpected ZooKeeper Exception / Zookeeper unreachable    */
specifier|private
specifier|static
name|void
name|submitBatchedMultiOrSequential
...
name|KeeperException
block|{
comment|// at least one element should exist
if|if
condition|(
name|ops
operator|.
...
argument_list|()
decl_stmt|;
comment|// Batch up the items to over smashing through jute.maxbuffer with too many Ops.
specifier|final
name|List
argument_list|<
name|List
...
argument_list|)
decl_stmt|;
comment|// Would use forEach() but have to handle KeeperException
for|for
control|(
name|List
argument_list|<
...
block|}
block|}
comment|/**    * Partition the list of {@code ops} by size (using {@link #estimateSize(ZKUtilOp)}).    */
specifier|static
name|List
argument_list|<
name|List
...
argument_list|)
decl_stmt|;
comment|// Roll a new partition if necessary
comment|// If the current partition is empty, put the element in there anyways.
comment|// We can roll a new partition if we get another element
if|if
condition|(
operator|!
name|currentPartition
...
expr_stmt|;
block|}
comment|// Add the current op to the partition
name|currentPartition
operator|.
name|add
argument_list|(
...
name|String
argument_list|>
name|backupMasterChildrenNoWatchList
init|=
name|listChildrenNoWatch
argument_list|(
name|zkw
...
name|String
argument_list|>
name|hFileRefChildrenNoWatchList
init|=
name|ZKUtil
operator|.
name|listChildrenNoWatch
