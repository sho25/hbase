...
name|justification
operator|=
literal|"This class is never serialized nor restored."
argument_list|)
specifier|private
specifier|transient
name|Lock
...
name|info
argument_list|(
literal|"Servlet process PID: "
operator|+
name|pid
operator|+
literal|" asyncProfilerHome: "
operator|+
name|asyncProfilerHome
argument_list|)
expr_stmt|;
...
argument_list|)
expr_stmt|;
comment|// to avoid auto-refresh by ProfileOutputServlet, refreshDelay can be specified
comment|// via url param
name|int
name|refreshDelay
init|=
name|getInteger
...
argument_list|)
decl_stmt|;
comment|// instead of sending redirect, set auto-refresh so that browsers will refresh
comment|// with redirected url
name|resp
operator|.
name|setHeader
argument_list|(
...
name|warn
argument_list|(
literal|"Unable to acquire lock in "
operator|+
name|lockTimeoutSecs
operator|+
literal|" seconds. Another instance of profiler might be running."
argument_list|)
expr_stmt|;
block|}
block|}
