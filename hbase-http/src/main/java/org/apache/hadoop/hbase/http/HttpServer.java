...
specifier|final
name|String
name|HTTP_PRIVILEGED_CONF_KEY
init|=
literal|"hbase.security.authentication.ui.config.protected"
decl_stmt|;
specifier|public
specifier|static
specifier|final
name|boolean
name|HTTP_PRIVILEGED_CONF_DEFAULT
init|=
literal|false
decl_stmt|;
...
argument_list|)
expr_stmt|;
comment|// Default apps need to be set first, so that all filters are applied to them.
comment|// Because they're added to defaultContexts, we need them there before we start
comment|// adding filters
name|addDefaultApps
argument_list|(
name|contexts
argument_list|,
...
argument_list|)
expr_stmt|;
comment|// But security needs to be enabled prior to adding the other servlets
if|if
condition|(
name|authenticationEnabled
condition|)
...
argument_list|)
expr_stmt|;
comment|// While we don't expect users to have sensitive information in their configuration, they
comment|// might. Give them an option to not expose the service configuration to all users.
if|if
condition|(
name|conf
operator|.
...
expr_stmt|;
block|}
comment|/**    * Adds a servlet in the server that only administrators can access. This method differs from    * {@link #addUnprivilegedServlet(String, String, Class)} in that only those authenticated user    * who are identified as administrators can interact with the servlet added by this method.    */
specifier|public
name|void
name|addPrivilegedServlet
parameter_list|(
...
expr_stmt|;
block|}
comment|/**    * Internal method to add a servlet to the HTTP server. Developers should not call this method    * directly, but invoke it via {@link #addUnprivilegedServlet(String, String, Class)} or    * {@link #addPrivilegedServlet(String, String, Class)}.    */
name|void
name|addServletWithAuth
parameter_list|(
name|String
