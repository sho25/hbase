begin_unit|revision:0.9.5;language:Java;cregit-version:0.0.1
begin_comment
comment|/*  *  * Licensed to the Apache Software Foundation (ASF) under one  * or more contributor license agreements.  See the NOTICE file  * distributed with this work for additional information  * regarding copyright ownership.  The ASF licenses this file  * to you under the Apache License, Version 2.0 (the  * "License"); you may not use this file except in compliance  * with the License.  You may obtain a copy of the License at  *  *   http://www.apache.org/licenses/LICENSE-2.0  *  * Unless required by applicable law or agreed to in writing, software  * distributed under the License is distributed on an "AS IS" BASIS,  * WITHOUT WARRANTIES OR CONDITIONS OF ANY KIND, either express or implied.  * See the License for the specific language governing permissions and  * limitations under the License.  */
end_comment

begin_package
package|package
name|org
operator|.
name|apache
operator|.
name|hadoop
operator|.
name|hbase
operator|.
name|rest
package|;
end_package

begin_import
import|import
name|org
operator|.
name|apache
operator|.
name|yetus
operator|.
name|audience
operator|.
name|InterfaceAudience
import|;
end_import

begin_import
import|import
name|org
operator|.
name|apache
operator|.
name|hadoop
operator|.
name|hbase
operator|.
name|CompatibilitySingletonFactory
import|;
end_import

begin_import
import|import
name|org
operator|.
name|apache
operator|.
name|hadoop
operator|.
name|hbase
operator|.
name|rest
operator|.
name|MetricsRESTSource
import|;
end_import

begin_class
annotation|@
name|InterfaceAudience
operator|.
name|Private
specifier|public
class|class
name|MetricsREST
block|{
specifier|public
name|MetricsRESTSource
name|getSource
parameter_list|()
block|{
return|return
name|source
return|;
block|}
specifier|private
name|MetricsRESTSource
name|source
decl_stmt|;
specifier|public
name|MetricsREST
parameter_list|()
block|{
name|source
operator|=
name|CompatibilitySingletonFactory
operator|.
name|getInstance
argument_list|(
name|MetricsRESTSource
operator|.
name|class
argument_list|)
expr_stmt|;
block|}
comment|/**    * @param inc How much to add to requests.    */
specifier|public
name|void
name|incrementRequests
parameter_list|(
specifier|final
name|int
name|inc
parameter_list|)
block|{
name|source
operator|.
name|incrementRequests
argument_list|(
name|inc
argument_list|)
expr_stmt|;
block|}
comment|/**    * @param inc How much to add to sucessfulGetCount.    */
specifier|public
name|void
name|incrementSucessfulGetRequests
parameter_list|(
specifier|final
name|int
name|inc
parameter_list|)
block|{
name|source
operator|.
name|incrementSucessfulGetRequests
argument_list|(
name|inc
argument_list|)
expr_stmt|;
block|}
comment|/**    * @param inc How much to add to sucessfulPutCount.    */
specifier|public
name|void
name|incrementSucessfulPutRequests
parameter_list|(
specifier|final
name|int
name|inc
parameter_list|)
block|{
name|source
operator|.
name|incrementSucessfulPutRequests
argument_list|(
name|inc
argument_list|)
expr_stmt|;
block|}
comment|/**    * @param inc How much to add to failedPutCount.    */
specifier|public
name|void
name|incrementFailedPutRequests
parameter_list|(
specifier|final
name|int
name|inc
parameter_list|)
block|{
name|source
operator|.
name|incrementFailedPutRequests
argument_list|(
name|inc
argument_list|)
expr_stmt|;
block|}
comment|/**    * @param inc How much to add to failedGetCount.    */
specifier|public
name|void
name|incrementFailedGetRequests
parameter_list|(
specifier|final
name|int
name|inc
parameter_list|)
block|{
name|source
operator|.
name|incrementFailedGetRequests
argument_list|(
name|inc
argument_list|)
expr_stmt|;
block|}
comment|/**    * @param inc How much to add to sucessfulDeleteCount.    */
specifier|public
name|void
name|incrementSucessfulDeleteRequests
parameter_list|(
specifier|final
name|int
name|inc
parameter_list|)
block|{
name|source
operator|.
name|incrementSucessfulDeleteRequests
argument_list|(
name|inc
argument_list|)
expr_stmt|;
block|}
comment|/**    * @param inc How much to add to failedDeleteCount.    */
specifier|public
name|void
name|incrementFailedDeleteRequests
parameter_list|(
specifier|final
name|int
name|inc
parameter_list|)
block|{
name|source
operator|.
name|incrementFailedDeleteRequests
argument_list|(
name|inc
argument_list|)
expr_stmt|;
block|}
comment|/**    * @param inc How much to add to sucessfulScanCount.    */
specifier|public
specifier|synchronized
name|void
name|incrementSucessfulScanRequests
parameter_list|(
specifier|final
name|int
name|inc
parameter_list|)
block|{
name|source
operator|.
name|incrementSucessfulScanRequests
argument_list|(
name|inc
argument_list|)
expr_stmt|;
block|}
comment|/**    * @param inc How much to add to failedScanCount.    */
specifier|public
name|void
name|incrementFailedScanRequests
parameter_list|(
specifier|final
name|int
name|inc
parameter_list|)
block|{
name|source
operator|.
name|incrementFailedScanRequests
argument_list|(
name|inc
argument_list|)
expr_stmt|;
block|}
comment|/**    * @param inc How much to add to sucessfulAppendCount.    */
specifier|public
specifier|synchronized
name|void
name|incrementSucessfulAppendRequests
parameter_list|(
specifier|final
name|int
name|inc
parameter_list|)
block|{
name|source
operator|.
name|incrementSucessfulAppendRequests
argument_list|(
name|inc
argument_list|)
expr_stmt|;
block|}
comment|/**    * @param inc How much to add to failedAppendCount.    */
specifier|public
name|void
name|incrementFailedAppendRequests
parameter_list|(
specifier|final
name|int
name|inc
parameter_list|)
block|{
name|source
operator|.
name|incrementFailedAppendRequests
argument_list|(
name|inc
argument_list|)
expr_stmt|;
block|}
comment|/**    * @param inc How much to add to sucessfulIncrementCount.    */
specifier|public
specifier|synchronized
name|void
name|incrementSucessfulIncrementRequests
parameter_list|(
specifier|final
name|int
name|inc
parameter_list|)
block|{
name|source
operator|.
name|incrementSucessfulIncrementRequests
argument_list|(
name|inc
argument_list|)
expr_stmt|;
block|}
comment|/**    * @param inc How much to add to failedIncrementCount.    */
specifier|public
name|void
name|incrementFailedIncrementRequests
parameter_list|(
specifier|final
name|int
name|inc
parameter_list|)
block|{
name|source
operator|.
name|incrementFailedIncrementRequests
argument_list|(
name|inc
argument_list|)
expr_stmt|;
block|}
block|}
end_class

end_unit

