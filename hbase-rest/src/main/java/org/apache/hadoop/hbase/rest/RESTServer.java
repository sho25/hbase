...
specifier|private
name|void
name|addClickjackingPreventionFilter
parameter_list|(
name|ServletContextHandler
name|ctxHandler
parameter_list|,
...
argument_list|)
expr_stmt|;
comment|// Hook for unit tests, this will log out any other user and mess up tests.
if|if
condition|(
operator|!
name|conf
...
name|debug
argument_list|(
literal|"Skipping Kerberos login for REST server"
argument_list|)
expr_stmt|;
block|}
block|}
...
argument_list|)
decl_stmt|;
comment|// Using our custom ServletContainer is tremendously important. This is what makes sure the
comment|// UGI.doAs() is done for the remoteUser, and calls are not made as the REST server itself.
name|ServletContainer
name|servletContainer
init|=
name|ReflectionUtils
...
name|IllegalStateException
argument_list|(
literal|"InfoServer is not running"
argument_list|)
throw|;
block|}
return|return
