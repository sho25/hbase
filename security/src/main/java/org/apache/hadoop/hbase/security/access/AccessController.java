...
return|;
block|}
comment|// 2. The table owner has full privileges
name|String
name|owner
init|=
name|htd
...
condition|)
block|{
comment|// owner of the table has full access
return|return
name|AuthResult
operator|.
name|allow
argument_list|(
literal|"User is table owner"
argument_list|,
name|user
argument_list|,
name|permRequest
...
return|;
block|}
comment|// 3. check for the table-level, if successful we can short-circuit
if|if
condition|(
name|authManager
operator|.
...
return|;
block|}
comment|// 4. check permissions against the requested families
if|if
condition|(
name|families
operator|!=
...
return|;
block|}
comment|// 5. no families to check and table level access failed
return|return
name|AuthResult
operator|.
name|deny
...
argument_list|)
expr_stmt|;
comment|// default the table owner if not specified
name|User
name|owner
init|=
name|getActiveUser
...
name|IOException
block|{
comment|/* TODO: Allow for users with global CREATE permission and the table owner */
name|requirePermission
argument_list|(
name|Permission
operator|.
...
name|debug
argument_list|(
literal|"Received request to grant access permission to '"
operator|+
name|Bytes
operator|.
name|toString
...
name|debug
argument_list|(
literal|"Received request to revoke access permission for '"
operator|+
name|Bytes
operator|.
name|toString
