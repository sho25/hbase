...
argument_list|)
decl_stmt|;
comment|// Connection timeout on disconnect event
specifier|private
name|long
name|connWaitTimeOut
decl_stmt|;
specifier|private
name|AtomicBoolean
name|connected
...
specifier|private
name|boolean
name|forceAbortOnZKDisconnect
decl_stmt|;
comment|// Execute service for zookeeper disconnect event watcher
specifier|private
name|ExecutorService
name|zkEventWatcherExecService
init|=
literal|null
decl_stmt|;
specifier|private
...
expr_stmt|;
block|}
comment|/**    * Instantiate a ZooKeeper connection and watcher.    * @param conf Configuration    * @param identifier string that is passed to RecoverableZookeeper to be used as identifier for    *          this instance. Use null for default.    * @param abortable Can be null if there is on error there is no host to abort: e.g. client    *          context.    * @param canCreateBaseZNode whether create base node.    * @param forceAbortOnZKDisconnect abort the watcher if true.    * @throws IOException when any IO exception    * @throws ZooKeeperConnectionException when any zookeeper connection exception    */
specifier|public
name|ZooKeeperWatcher
parameter_list|(
name|Configuration
...
argument_list|)
expr_stmt|;
comment|// On Disconnected event a thread will wait for sometime (2/3 of zookeeper.session.timeout),
comment|// it will abort the process if no SyncConnected event reported by the time.
name|connWaitTimeOut
operator|=
name|this
operator|.
...
name|prefix
argument_list|(
literal|"Received Disconnected from ZooKeeper."
argument_list|)
argument_list|)
expr_stmt|;
if|if
...
block|}
block|}
comment|/*    * Task to watch zookeper disconnect event.    */
class|class
name|ZKDisconnectEventWatcher
implements|implements
name|Runnable
...
name|debug
argument_list|(
literal|"Client got reconnected to zookeeper."
argument_list|)
expr_stmt|;
return|return;
block|}
...
name|prefix
argument_list|(
literal|"Couldn't connect to ZooKeeper after waiting "
operator|+
name|connWaitTimeOut
operator|+
literal|" ms, aborting"
