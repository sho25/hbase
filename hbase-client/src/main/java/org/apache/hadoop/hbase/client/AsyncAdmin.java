...
parameter_list|)
function_decl|;
comment|/**    * List specific tables including system tables.    * @param tableNames the table list to match against    * @return - returns a list of TableDescriptors wrapped by a {@link CompletableFuture}.    */
name|CompletableFuture
argument_list|<
name|List
argument_list|<
...
return|;
block|}
comment|/**    * Take a snapshot and wait for the server to complete that snapshot asynchronously. Snapshots    * are taken sequentially even when requested concurrently, across all tables. Snapshots are    * considered unique based on<b>the name of the snapshot</b>.    * Attempts to take a snapshot with the same name (even a different type or with different    * parameters) will fail with a {@link org.apache.hadoop.hbase.snapshot.SnapshotCreationException}    * indicating the duplicate naming. Snapshot names follow the same naming constraints as tables in    * HBase. See {@link org.apache.hadoop.hbase.TableName#isLegalFullyQualifiedTableName(byte[])}.    * You should probably use {@link #snapshot(String, org.apache.hadoop.hbase.TableName)} unless you    * are sure about the type of snapshot that you want to take.    * @param snapshot snapshot to take    */
name|CompletableFuture
argument_list|<
name|Void
argument_list|>
...
parameter_list|)
function_decl|;
comment|/**    * Get the global/namespace/table permissions for user    * @param getUserPermissionsRequest A request contains which user, global, namespace or table    *          permissions needed    * @return The user and permission list    */
name|CompletableFuture
argument_list|<
name|List
argument_list|<
...
parameter_list|)
function_decl|;
comment|/**    * Check if the user has specific permissions    * @param userName the user name    * @param permissions the specific permission list    * @return True if user has the specific permissions    */
name|CompletableFuture
argument_list|<
name|List
argument_list|<
...
parameter_list|)
function_decl|;
comment|/**    * Check if call user has specific permissions    * @param permissions the specific permission list    * @return True if user has the specific permissions    */
specifier|default
name|CompletableFuture
argument_list|<
name|List
...
parameter_list|)
function_decl|;
comment|/**    * Move given set of servers to the specified target RegionServer group    * @param servers set of servers to move    * @param groupName the group to move servers to    * @throws IOException if a remote or network exception occurs    */
name|CompletableFuture
argument_list|<
name|Void
argument_list|>
