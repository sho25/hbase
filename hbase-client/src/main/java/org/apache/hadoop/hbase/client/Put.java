...
operator|+
literal|"And its length cannot be 0 or greater than "
operator|+
name|HConstants
operator|.
name|MAX_FAMILY_LENGTH
...
throw|;
block|}
comment|// As array.length returns int , qualifier.length can not be greater than Integer.MAX_VALUE,
comment|// so it is safe that Cell.getQualifierLength() returns int. Do not need to check here.
comment|// Check value
if|if
condition|(
name|value
...
throw|;
block|}
comment|// As array.length returns int, value.length can not be greater than Integer.MAX_VALUE,
comment|// so it is safe that Cell.getValueLength() returns int. Do not need to check here.
...
parameter_list|()
block|{
comment|// row.length is checked by Mutation.checkRow() in the constructor of Put,
...
parameter_list|()
block|{
comment|// family.length is checked at the beginning of this function,
comment|// so it is safe to make the type conversion.
return|return
call|(
name|byte
