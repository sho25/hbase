...
expr_stmt|;
block|}
comment|/**    * Construct the Put with user defined data. NOTED:    * 1) all cells in the familyMap must have the DataType.Put    * 2) the row of each cell must be same with passed row.    * @param row row. CAN'T be null    * @param ts timestamp    * @param familyMap the map to collect all cells internally. CAN'T be null    */
specifier|public
name|Put
parameter_list|(
name|byte
...
parameter_list|)
block|{
comment|// Family can not be null, otherwise NullPointerException is thrown when putting the cell into familyMap
if|if
condition|(
name|family
operator|==
...
name|IOException
block|{
comment|// Family can not be null, otherwise NullPointerException is thrown when putting
comment|// the cell into familyMap
if|if
condition|(
name|kv
operator|.
