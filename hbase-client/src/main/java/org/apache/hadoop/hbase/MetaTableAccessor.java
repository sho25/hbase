...
begin_comment
comment|/**    * @return Array of Cells made from all columns on the<code>regionName</code> row    *   that match the regex 'info:merge.*'.    */
end_comment
...
init|)
block|{
comment|// Should be only one result in this scanner if any.
name|Result
name|result
init|=
name|scanner
...
return|;
block|}
comment|// Should be safe to just return all Cells found since we had filter in place.
comment|// All values should be RegionInfos or something wrong.
return|return
name|result
operator|.
name|rawCells
