...
specifier|final
name|String
name|REPLICATION_PARENT_SEPARATOR
init|=
literal|"|"
decl_stmt|;
specifier|private
...
specifier|final
name|String
name|REPLICATION_PARENT_SEPARATOR_REGEX
init|=
literal|"\\|"
decl_stmt|;
...
begin_comment
comment|/**    * Merge the two regions into one in an atomic operation. Deletes the two merging regions in    * hbase:meta and adds the merged region with the information of two merging regions.    * @param connection connection we're using    * @param mergedRegion the merged region    * @param regionA merge parent region A    * @param regionAOpenSeqNum the next open sequence id for region A, used by serial replication. -1    *          if not necessary.    * @param regionB merge parent region B    * @param regionBOpenSeqNum the next open sequence id for region B, used by serial replication. -1    *          if not necessary.    * @param sn the location of the region    */
end_comment
