...
condition|)
block|{
comment|// if, for whatever reason, we looped, we want to be sure that something has changed.
...
condition|)
block|{
comment|// loc is null if there is an error such as meta not available.
name|it
operator|.
name|remove
argument_list|()
...
expr_stmt|;
block|}
comment|/**    * Group the actions per region server.    *    * @param loc - the destination. Must not be null.    * @param action - the action to add to the multiaction    * @param actionsByServer the multiaction per server    * @param ng Nonce generator, or null if no nonces are needed.    */
specifier|private
name|void
name|addAction
parameter_list|(
...
condition|)
block|{
comment|// TODO: this code executes for every (re)try, and calls getNonceGroup again
comment|//       for the same action. It must return the same value across calls.
name|multiAction
operator|.
name|setNonceGroup
argument_list|(
...
block|}
comment|/**    * Group a list of actions per region servers, and send them. The created MultiActions are    * added to the inProgress list. Does not take into account the region/server load.    *    * @param initialActions - the full list of the actions in progress    * @param currentActions - the list of row to submit    * @param numAttempt - the current numAttempt (first attempt is 1)    */
specifier|private
name|void
name|submit
parameter_list|(
...
return|return;
block|}
comment|// Nominal case: we received an answer from the server, and it's not an exception.
...
return|;
block|}
comment|/**    * For tests.    * @param callable: used in tests.    * @return Returns a caller.    */
specifier|protected
...
expr_stmt|;
block|}
comment|/**    * Log as many info as possible, and, if there is something to replay, submit it again after    *  a back off sleep.    */
specifier|private
name|void
name|logAndResubmit
parameter_list|(
