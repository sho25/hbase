...
init|=
literal|"hbase.client.primaryCallTimeout"
decl_stmt|;
...
name|callCount
decl_stmt|;
comment|/** Call that succeeds sets the count to 0 and sets this to result. Call that fails but      * is not last, adds error to list. If all calls fail the last one sets this to list. */
name|Object
name|result
init|=
literal|null
...
condition|)
return|return;
comment|// Nothing to do - done or no replicas found.
name|sendMultiAction
argument_list|(
name|actionsByServer
argument_list|,
...
try|try
block|{
comment|// For perf, we assume that this location coming from cache, since we just got location
comment|// from meta for the primary call. If it turns out to not be the case, we'd need local
comment|// cache since we want to keep as little time as possible before replica call.
name|loc
operator|=
name|findDestLocation
argument_list|(
...
name|error
argument_list|(
literal|"Cannot get location - no replica calls for some actions"
argument_list|,
name|ex
argument_list|)
expr_stmt|;
...
condition|)
block|{
comment|// On retry, we couldn't find location for some replica we saw before.
name|String
name|str
init|=
literal|"Cannot find location for replica "
operator|+
name|action
operator|.
name|getReplicaId
...
block|{
comment|// See if we are dealing with a replica action that was completed from other server.
comment|// Doesn't have to be synchronized, worst case we'd retry and be unable to set result.
name|canRetry
operator|=
operator|!
...
condition|)
block|{
comment|// Convert replica sync structures to results.
name|int
name|staleCount
init|=
literal|0
...
name|AssertionError
argument_list|(
literal|"Cannot set actions in progress to 0"
argument_list|)
throw|;
block|}
if|if
...
index|]
decl_stmt|;
comment|// We know that noone will touch state with 0 callCount, no need to lock
if|if
condition|(
name|state
operator|.
...
name|AssertionError
argument_list|(
literal|"Actions are done but callcount is "
operator|+
name|state
operator|.
name|callCount
...
throw|;
block|}
comment|// TODO: we expect the Result coming from server to already have "isStale" specified.
name|Object
name|res
init|=
name|results
