...
return|;
block|}
comment|/**    * Detects whether replication has been already enabled on any of the column families of this    * table descriptor.    * @return true if any of the column families has replication enabled.    */
specifier|public
name|boolean
name|isReplicationEnabled
parameter_list|()
block|{
comment|// Go through each Column-Family descriptor and check if the
comment|// Replication has been enabled already.
comment|// Return 'true' if replication has been enabled on any CF,
...
return|;
block|}
comment|/**    * Compare the contents of the descriptor with another one passed as a parameter for replication    * purpose. The REPLICATION_SCOPE field is ignored during comparison.    * @param obj descriptor on source cluster which needs to be replicated.    * @return true if the contents of the two descriptors match (ignoring just REPLICATION_SCOPE).    * @see java.lang.Object#equals(java.lang.Object)    */
specifier|public
name|boolean
...
argument_list|)
decl_stmt|;
comment|// Copy the replication scope of local Htd to remote Htd.
name|HTableDescriptor
name|localHtd
init|=
operator|(
