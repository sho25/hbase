...
name|IOException
argument_list|(
literal|"idle connection closed or stopped"
argument_list|)
argument_list|)
expr_stmt|;
return|return
...
name|justification
operator|=
literal|"on close the reader thread must stop"
argument_list|)
specifier|protected
name|void
name|writeRequest
...
argument_list|)
expr_stmt|;
comment|// On error, the call will be removed by the timeout.
try|try
block|{
synchronized|synchronized
init|(
...
init|)
block|{
comment|// We added a call, and may start the connection clode. In both cases, we
...
argument_list|()
operator|+
literal|": marking at should closed, reason ="
operator|+
name|e
operator|.
name|getMessage
...
name|justification
operator|=
literal|"Notify because timeout"
argument_list|)
specifier|protected
name|void
name|cleanupCalls
