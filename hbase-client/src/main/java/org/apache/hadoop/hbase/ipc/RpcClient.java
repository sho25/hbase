...
name|effectiveUser
operator|+
literal|" is different from the ticket user "
operator|+
name|ticket
operator|.
name|getShortUserName
argument_list|()
operator|+
literal|". But the ticket is already a proxy user with real user "
operator|+
name|ticket
operator|.
name|getRealUser
...
block|{
comment|// If the ticket is not a proxy user, and the current user
comment|// is not the same as the ticket user, pass the current user
comment|// over to the server as the actual effective user.
name|builder
operator|.
name|setEffectiveUser
argument_list|(
...
block|}
block|}
comment|/**    * Construct an IPC cluster client whose values are of the {@link Message} class.    * @param conf configuration    * @param factory socket factory    */
name|RpcClient
parameter_list|(
name|Configuration
name|conf
