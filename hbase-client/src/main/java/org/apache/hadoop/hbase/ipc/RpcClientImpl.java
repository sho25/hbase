...
name|error
argument_list|(
literal|"Error while closing socket"
argument_list|,
name|e
argument_list|)
expr_stmt|;
...
block|}
block|}
comment|/**    * Used in test only. Construct an IPC cluster client whose values are of the    * {@link Message} class.    * @param conf configuration    * @param clusterId the cluster id    * @param factory socket factory    */
annotation|@
name|VisibleForTesting
name|RpcClientImpl
parameter_list|(
...
expr_stmt|;
block|}
comment|/**    * Construct an IPC cluster client whose values are of the {@link Message} class.    * @param conf configuration    * @param clusterId the cluster id    * @param factory socket factory    * @param localAddr client socket bind address    * @param metrics the connection metrics    */
name|RpcClientImpl
parameter_list|(
name|Configuration
name|conf
...
return|;
block|}
comment|/**    * Make a call, passing<code>param</code>, to the IPC server running at    *<code>address</code> which is servicing the<code>protocol</code> protocol,    * with the<code>ticket</code> credentials, returning the value.    * Throws exceptions if there are network problems or if the remote code    * threw an exception.    * @param ticket Be careful which ticket you pass. A new user will mean a new Connection.    *          {@link UserProvider#getCurrent()} makes a new instance of User each time so will be a    *          new Connection each time.    * @return A pair with the Message response and the Cell data (if any).    * @throws InterruptedException if the call is interrupted    * @throws IOException if something fails on the connection    */
annotation|@
name|Override
specifier|protected
name|Pair
...
return|;
block|}
comment|/**    * Make a call, passing<code>param</code>, to the IPC server running at    *<code>address</code> which is servicing the<code>protocol</code> protocol,    * with the<code>ticket</code> credentials, returning the value.    * Throws exceptions if there are network problems or if the remote code    * threw an exception.    * @param ticket Be careful which ticket you pass. A new user will mean a new Connection.    *          {@link UserProvider#getCurrent()} makes a new instance of User each time so will be a    *          new Connection each time.    * @return A Call    * @throws InterruptedException if the call is interrupted    * @throws IOException if something fails on the connection    */
specifier|private
parameter_list|<
name|R
extends|extends
...
block|}
block|}
comment|/**    * Wraps the call in an async channel.    */
specifier|private
class|class
name|AsyncRpcChannel
implements|implements
...
name|ticket
decl_stmt|;
comment|/**      * Constructor      * @param sn servername to connect to      * @param user to connect with      */
specifier|public
name|AsyncRpcChannel
parameter_list|(
name|ServerName
