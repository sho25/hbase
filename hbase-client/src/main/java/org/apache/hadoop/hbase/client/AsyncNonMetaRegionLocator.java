...
return|;
block|}
comment|// if we successfully add the locations to cache, return the locations, otherwise return the one
comment|// which prevents us being added. The upper layer can use this value to complete pending requests.
specifier|private
name|RegionLocations
name|addToCache
parameter_list|(
...
return|;
block|}
comment|// check whether the regions are the same, this usually happens when table is split/merged, or
comment|// deleted and recreated again.
name|RegionInfo
name|region
init|=
name|locs
...
block|{
comment|// the region is different, here we trust the one we fetched. This maybe wrong but finally
comment|// the upper layer can detect this and trigger removal of the wrong locations
if|if
condition|(
name|LOG
operator|.
...
name|debug
argument_list|(
literal|"The newnly fetch region {} is different from the old one {} for row '{}',"
operator|+
literal|" try replaing the old one..."
argument_list|,
name|region
argument_list|,
name|oldRegion
