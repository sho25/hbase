...
argument_list|()
decl_stmt|;
comment|// We need to compare nextRowKey with currentCell
name|int
name|compareResult
init|=
name|Bytes
...
condition|)
block|{
comment|// This can happen when we have multilpe filters and some other filter
comment|// returns next row with hint which is larger (smaller for reverse)
comment|// than the current (really?)
name|result
operator|=
name|tracker
operator|.
...
expr_stmt|;
block|}
comment|// Sort all next row candidates
name|Collections
operator|.
name|sort
argument_list|(
