...
condition|)
block|{
name|changeStartAndStopRowIfMeta
argument_list|(
name|scan
argument_list|)
expr_stmt|;
...
block|{
comment|// mutate list so that it is empty for complete success, or contains
comment|// only failed records. Results are returned in the same order as the
comment|// requests in list. Walk the list backwards, so we can remove from list
comment|// without impacting the indexes of earlier members
name|currentWriteBufferSize
operator|=
literal|0
expr_stmt|;
