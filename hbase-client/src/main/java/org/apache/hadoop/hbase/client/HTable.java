...
block|}
block|}
comment|/**    * Send the operations in the buffer to the servers. Does not wait for the server's answer.    * If the is an error (max retried reach from a previous flush or bad operation), it tries to    * send all operations in the buffer and sends an exception.    */
specifier|private
name|void
name|backgroundFlushCommits
parameter_list|(
...
try|try
block|{
comment|// If there is an error on the operations in progress, we don't add new operations.
if|if
condition|(
name|writeAsyncBuffer
operator|.
...
return|;
block|}
comment|/**    * Goal of this inner class is to keep track of the initial position of a get in a list before    * sorting it. This is used to send back results in the same orders we got the Gets before we sort    * them.    */
specifier|private
specifier|static
class|class
name|SortedGet
...
literal|1
decl_stmt|;
comment|// Used to store the get initial index in a list.
specifier|protected
name|Get
name|get
decl_stmt|;
comment|// Encapsulated Get instance.
specifier|public
name|SortedGet
parameter_list|(
name|Get
...
name|IOException
block|{
comment|// Prepare the sorted list of gets. Take the list of gets received, and encapsulate them into
comment|// a list of SortedGet instances. Simple list parsing, so complexity here is O(n)
comment|// The list is later used to recreate the response order based on the order the Gets
comment|// got received.
name|ArrayList
argument_list|<
name|SortedGet
argument_list|>
...
expr_stmt|;
block|}
comment|// Sorting the list to get the Gets ordered based on the key.
name|Collections
operator|.
name|sort
argument_list|(
...
expr_stmt|;
comment|// O(n log n)
comment|// step 1: sort the requests by regions to send them bundled.
comment|// Map key is startKey index. Map value is the list of Gets related to the region starting
comment|// with the startKey.
name|Map
argument_list|<
name|Integer
argument_list|,
...
argument_list|()
decl_stmt|;
comment|// Reference map to quickly find back in which region a get belongs.
name|Map
argument_list|<
name|Get
argument_list|,
...
control|)
block|{
comment|// Progress on the regions until we find the one the current get resides in.
while|while
condition|(
operator|(
name|regionIndex
...
expr_stmt|;
block|}
comment|// step 3: collect the failures and successes
name|Map
argument_list|<
name|Integer
argument_list|,
...
name|warn
argument_list|(
literal|"Failed for gets on region: "
operator|+
name|sortedGetEntry
operator|.
name|getKey
...
index|]
decl_stmt|;
comment|// step 4: build the response.
name|Map
argument_list|<
name|Integer
argument_list|,
...
name|RetriesExhaustedWithDetailsException
block|{
comment|// We're looping, as if one region is overloaded we keep its operations in the buffer.
