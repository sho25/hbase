...
literal|true
expr_stmt|;
comment|// Implied that no results were returned back, either.
return|return
literal|null
return|;
block|}
comment|// moreResultsInRegion explicitly defines when a RS may choose to terminate a batch due
comment|// to size or quantity of results in the response.
if|if
condition|(
name|response
operator|.
...
block|{
comment|// Server didn't respond whether it has more results or not.
name|setHasMoreResultsContext
argument_list|(
literal|false
argument_list|)
...
return|;
block|}
comment|/**    * Should the client attempt to fetch more results from this region    * @return True if the client should attempt to fetch more results, false otherwise.    */
specifier|protected
name|boolean
...
expr_stmt|;
block|}
comment|/**    * Did the server respond with information about whether more results might exist.    * Not guaranteed to respond with older server versions    * @return True if the server responded with information about more results.    */
specifier|protected
name|boolean
