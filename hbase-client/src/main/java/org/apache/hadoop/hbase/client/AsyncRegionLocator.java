...
return|;
block|}
comment|/**    * Locate the previous region using the current regions start key. Used for reverse scan as the    * end key is not included in a region so we need to treat it differently.    */
name|CompletableFuture
argument_list|<
name|HRegionLocation
argument_list|>
...
parameter_list|)
block|{
comment|// meta region can not be split right now so we call the same method as getRegionLocation.
