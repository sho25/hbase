...
expr_stmt|;
block|}
comment|// Index to track RegionAction within the MultiRequest
name|int
name|regionActionIndex
init|=
operator|-
literal|1
decl_stmt|;
comment|// Map from a created RegionAction to the original index for a RowMutations within
comment|// its original list of actions
name|Map
argument_list|<
name|Integer
argument_list|,
...
condition|)
block|{
comment|// Build a multi request absent its Cell payload. Send data in cellblocks.
name|regionActionBuilder
operator|=
name|RequestConverter
operator|.
...
condition|)
block|{
comment|// Send data in cellblocks. The call to buildNoDataRegionAction will skip RowMutations.
