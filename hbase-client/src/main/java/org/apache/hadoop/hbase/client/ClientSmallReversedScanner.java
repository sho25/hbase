...
name|smallReversedScanCallable
init|=
literal|null
decl_stmt|;
specifier|private
...
expr_stmt|;
block|}
comment|/**    * Create a new ReversibleClientScanner for the specified table. Take note that the passed    * {@link Scan}'s start row may be changed.    *    * @param conf    *          The {@link Configuration} to use.    * @param scan    *          {@link Scan} to use in this scanner    * @param tableName    *          The table that we wish to rangeGet    * @param connection    *          Connection identifying the cluster    * @param rpcFactory    *          Factory used to create the {@link RpcRetryingCaller}    * @param controllerFactory    *          Factory used to access RPC payloads    * @param pool    *          Threadpool for RPC threads    * @param primaryOperationTimeout    *          Call timeout    * @param callableFactory    *          Factory used to create the {@link SmallScannerCallable}    * @throws IOException    *           If the remote call fails    */
annotation|@
name|VisibleForTesting
name|ClientSmallReversedScanner
parameter_list|(
...
condition|)
block|{
comment|// when non-firstRegion& localStartKey is empty bytes, no more rowKey should scan.
comment|// otherwise, maybe infinity results with RowKey=0x00 will return.
return|return
literal|false
return|;
block|}
...
expr_stmt|;
block|}
comment|/**    * A reversed ScannerCallable which supports backward small scanning.    */
specifier|static
class|class
name|SmallReversedScannerCallable
extends|extends
...
condition|)
block|{
comment|// HBASE-16886: if not setting startRow, then we will use a range [MAX_BYTE_ARRAY, +oo) to
