...
name|info
argument_list|(
literal|"Can not take the lock now"
argument_list|)
expr_stmt|;
return|return
literal|null
...
name|warn
argument_list|(
literal|"Remove copied queue failed"
argument_list|,
name|e
argument_list|)
expr_stmt|;
...
return|;
block|}
comment|/**    * This methods moves all the wals queues from another region server and returns them all sorted    * per peer cluster (appended with the dead server's znode)    * @param znode server names to copy    * @return all wals for the peer of that cluster, null if an error occurred    */
specifier|private
name|Pair
argument_list|<
name|String
...
name|warn
argument_list|(
literal|"Got exception in copyQueueFromLockedRS: "
argument_list|,
name|e
argument_list|)
expr_stmt|;
