...
parameter_list|()
block|{
comment|/* All Operation subclasses should have a builder style setup where setXXX/addXXX methods      * can be chainable together:      * . For example:      * Scan scan = new Scan()      *     .setFoo(foo)      *     .setBar(bar)      *     .setBuz(buz)      *      * This test ensures that all methods starting with "set" returns an Operation object      */
...
argument_list|()
operator|+
literal|" object in builder style. Offending method:"
operator|+
name|method
operator|.
name|getName
