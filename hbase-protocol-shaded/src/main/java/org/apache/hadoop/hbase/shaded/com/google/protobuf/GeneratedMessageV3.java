...
block|}
comment|/**    * TODO(xiaofeng): remove this after b/29368482 is fixed. We need to move this    * interface to AbstractMessage in order to versioning GeneratedMessageV3 but    * this move breaks binary compatibility for AppEngine. After AppEngine is    * fixed we can exlude this from google3.    */
specifier|protected
interface|interface
name|BuilderParent
extends|extends
...
name|BuilderParent
block|{}
comment|/**    * TODO(xiaofeng): remove this together with GeneratedMessageV3.BuilderParent.    */
specifier|protected
specifier|abstract
name|Message
operator|.
...
comment|// to dispatch dirty invalidations. See GeneratedMessageV3.BuilderListener.
specifier|private
name|boolean
name|isClean
decl_stmt|;
...
return|return;
block|}
comment|// Sorting the unboxed keys and then look up the values during serialziation is 2x faster
comment|// than sorting map entries with a custom comparator directly.
name|int
index|[]
name|keys
init|=
...
return|return;
block|}
comment|// Sorting the String keys and then look up the values during serialziation is 25% faster than
comment|// sorting map entries with a custom comparator directly.
name|String
index|[]
name|keys
init|=
...
return|return;
block|}
name|maybeSerializeBooleanEntryTo
argument_list|(
name|out
argument_list|,
name|m
...
block|}
block|}
comment|/** Serialize the map using the iteration order. */
specifier|private
specifier|static
parameter_list|<
name|K
