...
block|}
block|}
comment|/**      * Cache this block if there is a block cache available.<p>      *       * Makes a copy of the ByteBuffer, not the one we are sending back, so the       * position does not get messed up.      * @param blockName      * @param buf      */
name|void
name|cacheBlock
parameter_list|(
name|String
...
block|}
block|}
comment|/**    * HFile Reader that does not cache blocks that were not already cached.<p>    *     * Used for compactions.    */
specifier|public
specifier|static
class|class
...
expr_stmt|;
block|}
comment|/**      * Do not cache this block when doing a compaction.      */
annotation|@
name|Override
name|void
name|cacheBlock
...
name|println
argument_list|(
literal|"usage:<filename> -- dumps hfile stats"
argument_list|)
expr_stmt|;
return|return;
block|}
...
argument_list|()
decl_stmt|;
comment|// scan thru and count the # of unique rows.
comment|//    HashSet<Integer> rows = new HashSet<Integer>(reader.getEntries()/4);
comment|//    long start = System.currentTimeMillis();
comment|//    HFileScanner scanner = reader.getScanner();
comment|//    HStoreKey hsk;
comment|//    scanner.seekTo();
comment|//    do {
comment|//      hsk = new HStoreKey(scanner.getKey());
comment|//      rows.add(Bytes.hashCode(hsk.getRow()));
comment|//    } while (scanner.next());
