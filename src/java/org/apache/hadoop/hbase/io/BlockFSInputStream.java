...
begin_comment
comment|/**  * An implementation of {@link FSInputStream} that reads the stream in blocks  * of a fixed, configurable size. The blocks are stored in a memory-sensitive  * cache.  Implements Runnable.  Run it on a period so we clean up soft  * references from the reference queue.  */
end_comment
...
argument_list|)
decl_stmt|;
comment|/*    * Set up scheduled execution of cleanup of soft references.  Run with one    * thread for now.  May need more when many files.  Should be an option but    * also want BlockFSInputStream to be self-contained.    */
specifier|private
specifier|static
specifier|final
name|ScheduledExecutorService
...
throw|;
block|}
comment|/**    * Call frequently to clear Soft Reference Reference Queue.    * @return Count of references cleared.    */
specifier|public
specifier|synchronized
name|int
name|checkReferences
