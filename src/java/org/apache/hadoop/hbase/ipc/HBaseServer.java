...
argument_list|()
decl_stmt|;
comment|// accept up to 10 connections
for|for
control|(
name|int
name|i
...
block|}
block|}
comment|/// Reads the header following version
specifier|private
name|void
name|processHeader
parameter_list|()
...
name|IOException
block|{
comment|/* In the current version, it is just a ticket.        * Later we could introduce a "ConnectionHeader" class.        */
name|DataInputStream
name|in
init|=
operator|new
...
condition|)
block|{
comment|// Allocate a new BAOS as reset only moves size back to zero but
comment|// keeps the buffer of whatever the largest write was -- see
comment|// hbase-900.
name|buf
operator|=
operator|new
