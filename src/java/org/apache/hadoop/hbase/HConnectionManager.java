...
condition|)
block|{
comment|// Table already closed. Ignore it.
return|return;
block|}
name|SortedMap
argument_list|<
...
condition|)
block|{
comment|// Table not open. Ignore it.
return|return;
block|}
name|closedTables
operator|.
...
condition|)
block|{
comment|// (re)find the root region
name|rootServers
operator|=
name|findServersForTable
argument_list|(
...
condition|)
block|{
comment|// Already found root once. Give up.
throw|throw
name|e
throw|;
block|}
comment|// The root region must have moved - refind it
name|rootServers
operator|.
name|clear
argument_list|()
...
condition|)
block|{
comment|// (re)find the meta table
name|metaServers
operator|=
name|findServersForTable
argument_list|(
...
condition|)
block|{
comment|// Already refound meta once. Give up.
throw|throw
name|e
throw|;
block|}
comment|// The meta table must have moved - refind it
name|metaServers
operator|.
name|clear
argument_list|()
...
return|;
block|}
comment|/*      * @param metaServers the meta servers that would know where the table is      * @param tableName name of the table      * @return map of region start key -> server location      * @throws IOException      */
specifier|private
name|SortedMap
argument_list|<
name|Text
...
name|IOException
block|{
comment|// If there is more than one meta server, find the first one that should
comment|// know about the table we are looking for, and reduce the number of
comment|// servers we need to query.
name|SortedMap
argument_list|<
name|Text
argument_list|,
