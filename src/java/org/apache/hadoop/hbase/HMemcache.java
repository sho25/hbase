...
block|{     }
block|}
comment|/**    * We want to return a snapshot of the current HMemcache with a known HLog     * sequence number at the same time.    *     * Return both the frozen HMemcache TreeMap, as well as the HLog seq number.    *    * We need to prevent any writing to the cache during this time, so we obtain     * a write lock for the duration of the operation.    */
specifier|public
name|Snapshot
