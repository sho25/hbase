...
name|writer
decl_stmt|;
comment|// This is the above writer's output stream. Its private but we use reflection
comment|// to expose it so we can call sync on it.
name|FSDataOutputStream
name|writer_out
decl_stmt|;
...
argument_list|()
expr_stmt|;
comment|// Above is sequencefile.writer sync.  It doesn't actually synce the
comment|// backing stream.  Need to do the below to do that.
if|if
condition|(
name|this
operator|.
