...
name|compression
decl_stmt|;
comment|// Comparing HStoreKeys in byte arrays.
specifier|final
name|HStoreKey
operator|.
...
name|rawcomparator
decl_stmt|;
comment|// Comparing HStoreKey objects.
specifier|final
name|Comparator
argument_list|<
name|HStoreKey
...
argument_list|)
decl_stmt|;
comment|// TODO: Suboptimal. And below where we are going from ByteBuffer to
comment|// byte array. FIX!! Can we get rid of HSK instantiations?
name|HStoreKey
name|hsk
init|=
name|HStoreKey
...
argument_list|)
decl_stmt|;
comment|// This is map of columns to timestamp
name|Map
argument_list|<
name|byte
index|[]
...
comment|// changes in the flow below should also probably be done in get.
comment|// TODO: Refactor so same code used.
name|Set
argument_list|<
name|HStoreKey
...
argument_list|)
decl_stmt|;
comment|// if the row and column matches, we might want this one.
if|if
condition|(
name|rowMatches
argument_list|(
...
block|}
block|}
comment|/*    * Check an individual MapFile for the row at or before a given key     * and timestamp    * @param f    * @param row    * @param candidateKeys    * @throws IOException    */
specifier|private
name|void
name|rowAtOrBeforeFromStoreFile
parameter_list|(
...
argument_list|()
decl_stmt|;
comment|// TODO: FIX THIS PROFLIGACY!!!
if|if
condition|(
operator|!
name|scanner
...
return|;
block|}
comment|/* Find a candidate for row that is at or before passed key, sk, in mapfile.    * @param f    * @param sk Key to go search the mapfile with.    * @param candidateKeys    * @param now    * @throws IOException    * @see {@link #rowAtOrBeforeCandidate(HStoreKey, org.apache.hadoop.io.MapFile.Reader, byte[], SortedMap, long)}    */
specifier|private
name|void
name|rowAtOrBeforeCandidate
parameter_list|(
...
comment|// TODO: FIX THIS PROFLIGATE OBJECT MAKING!!!
name|HStoreKey
...
return|;
block|}
comment|/**    * Convenience method that implements the old MapFile.getClosest on top of    * HFile Scanners.  getClosest used seek to the asked-for key or just after    * (HFile seeks to the key or just before).    * @param s    * @param b    * @return True if we were able to seek the scanner to<code>b</code> or to    * the key just after.    * @throws IOException     */
specifier|static
name|boolean
name|getClosest
parameter_list|(
