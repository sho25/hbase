...
block|}
block|}
comment|/**    * Write an update    * @param key    * @param value    * @return memcache Approximate size of the passed key and value.  Includes    * cost of hosting HSK and byte arrays as well as the Map.Entry this addition    * costs when we insert into the backing TreeMap.    */
name|long
name|add
parameter_list|(
specifier|final
...
return|;
block|}
comment|/*    * Calcuate how the memcache size has changed, approximately.    * Add in tax of TreeMap.Entry.    * @param key    * @param value    * @param oldValue    * @return    */
name|long
name|heapSize
parameter_list|(
specifier|final
...
parameter_list|)
block|{
comment|// First add value length.
name|long
name|keySize
init|=
name|key
...
block|{
comment|// If old value, don't add overhead again nor key size. Just add
comment|// difference in  value sizes.
name|size
operator|-=
name|oldValue
operator|.
...
return|;
block|}
comment|/**    * Look back through all the backlog TreeMaps to find the target.    * @param key    * @param numVersions    * @param deletes    * @param now    * @return An array of byte arrays ordered by timestamp.    */
name|List
argument_list|<
name|Cell
argument_list|>
...
name|results
decl_stmt|;
comment|// The synchronizations here are because the below get iterates
...
return|;
block|}
comment|/**    * Return all the available columns for the given key.  The key indicates a     * row and timestamp, but not a column name.    * @param key    * @param columns Pass null for all columns else the wanted subset.    * @param numVersions number of versions to retrieve    * @param deletes Map to accumulate deletes found.    * @param results Where to stick row results found.    */
name|void
name|getFull
parameter_list|(
name|HStoreKey
...
return|;
block|}
comment|/*    * Examine a single map for the desired key.    *    * TODO - This is kinda slow.  We need a data structure that allows for     * proximity-searches, not just precise-matches.    *     * @param map    * @param key    * @param numVersions    * @param deletes    * @return Ordered list of items found in passed<code>map</code>.  If no    * matching values, returns an empty list (does not return null).    */
specifier|private
name|ArrayList
argument_list|<
name|Cell
...
return|;
block|}
comment|/*    * Add<code>key</code> to the list of 'victims'.    * @param victims    * @param key    */
specifier|private
name|void
name|addVictim
parameter_list|(
...
name|key
operator|+
literal|": expired or in deletes, skipped"
argument_list|)
expr_stmt|;
block|}
block|}
...
return|;
block|}
comment|/*    * @param b Cell value.    * @return True if this is a delete value.    */
specifier|private
name|boolean
name|isDeleted
parameter_list|(
...
block|}
block|}
comment|/**    * Code to help figure if our approximation of object heap sizes is close    * enough.  See hbase-900.  Fills memcaches then waits so user can heap    * dump and bring up resultant hprof in something like jprofiler which    * allows you get 'deep size' on objects.    * @param args    * @throws InterruptedException    */
specifier|public
specifier|static
name|void
name|main
