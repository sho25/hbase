...
argument_list|()
expr_stmt|;
comment|//    validateValuesLength(put);
...
block|{
comment|// Next row:
comment|// what happens if there are _no_ results:
if|if
condition|(
name|results
operator|.
...
name|row
expr_stmt|;
comment|// But did we pass the stop row?
if|if
condition|(
name|stopRow
operator|!=
...
argument_list|)
decl_stmt|;
comment|// Determine what to do and perform increment on returned KV, no insertion
name|Store
operator|.
name|ICVResult
name|vas
...
argument_list|)
decl_stmt|;
comment|// Write incremented value to WAL before inserting
if|if
condition|(
name|writeToWAL
condition|)
