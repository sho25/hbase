...
name|debug
argument_list|(
literal|"batchUpdate on row for .META.: "
operator|+
name|Bytes
operator|.
name|toString
...
expr_stmt|;
block|}
comment|/**     * Add updates first to the hlog and then add values to memcache.    * Warning: Assumption is caller has lock on passed in row.    * @param updatesByColumn Cell updates by column    * @throws IOException    */
specifier|private
name|void
name|update
parameter_list|(
...
name|DoNotRetryIOException
argument_list|(
literal|"more than 1 value returned in incrementColumnValue from memcache"
argument_list|)
throw|;
block|}
if|if
...
name|DoNotRetryIOException
argument_list|(
literal|"more than 1 value returned in incrementColumnValue from Store"
argument_list|)
throw|;
block|}
block|}
