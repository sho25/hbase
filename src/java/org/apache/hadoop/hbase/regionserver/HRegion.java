...
comment|//////////////////////////////////////////////////////////////////////////////
comment|/**    * Fetch multiple versions of a single data item, with timestamp.    *    * @param row    * @param column    * @param ts    * @param nv    * @return Results or null if none.    * @throws IOException    */
specifier|public
name|List
argument_list|<
name|KeyValue
...
argument_list|)
decl_stmt|;
comment|// Guarantee that we return null instead of a zero-length array,
comment|// if there are no results to return.
return|return
operator|(
name|result
operator|==
...
block|}
block|}
comment|/**    * Data structure with a counter that is accessible rather than create a    * new Integer every time we want to up the counter.  Initializes at count 1.    */
specifier|static
class|class
name|Counter
block|{
...
decl_stmt|;
block|}
comment|/*    * Check to see if we've not gone over threshold for this particular    * column.    * @param kv    * @param versions    * @param versionsCount    * @return True if its ok to add current value.    */
specifier|static
name|boolean
name|okToAddResult
parameter_list|(
...
return|;
block|}
comment|/*    * Used adding item found to list of results getting.    * @param kv    * @param versionsCount    * @param results    */
specifier|static
name|void
name|addResult
parameter_list|(
...
parameter_list|)
block|{
comment|// Don't add if already present; i.e. ignore second entry.
if|if
condition|(
name|results
operator|.
...
block|}
block|}
comment|/*    * @param versions Number of versions to get.    * @param versionsCount May be null.    * @param columns Columns we want to fetch.    * @return True if has enough versions.    */
specifier|static
name|boolean
...
condition|)
block|{
comment|// Wants all columns so just keep going
return|return
literal|false
return|;
block|}
...
return|;
block|}
comment|// Need to look at each to make sure at least versions.
for|for
control|(
name|Map
operator|.
...
control|)
block|{
comment|// TODO: Fix so we use comparator in KeyValue that looks at
comment|// column family portion only.
if|if
condition|(
name|KeyValue
operator|.
...
argument_list|)
expr_stmt|;
comment|// Convert to RowResult.  TODO: Remove need to do this.
return|return
name|RowResult
operator|.
...
argument_list|)
decl_stmt|;
comment|// Below we make up set of stores we want scanners on and we fill out the
...
condition|)
block|{
comment|// Save off these deletes of the most recent thing added on the
comment|// family.
if|if
condition|(
name|latestTimestampDeletes
...
continue|continue;
block|}
comment|// Its an explicit timestamp delete
name|kv
operator|=
operator|new
name|KeyValue
...
condition|)
block|{
comment|// We have some LATEST_TIMESTAMP deletes to run.  Can't do them inline
comment|// as edits.  Need to do individually after figuring which is latest
comment|// timestamp to delete.
for|for
control|(
name|byte
index|[]
...
name|IOException
block|{
comment|// This is basically a copy of batchUpdate with the atomic check and save
comment|// added in. So you should read this method with batchUpdate. I will
comment|// comment the areas that I have changed where I have not changed, you
comment|// should read the comments from the batchUpdate method
name|boolean
name|success
init|=
literal|true
...
condition|)
block|{
comment|// Save off these deletes of the most recent thing added on
comment|// the family.
if|if
condition|(
name|latestTimestampDeletes
...
return|;
block|}
comment|/*    * Utility method to verify values length    * @param batchUpdate The update to verify    * @throws IOException Thrown if a value is too long    */
specifier|private
name|void
name|validateValuesLength
parameter_list|(
...
block|}
block|}
comment|/**    * Delete all cells of the same age as the passed timestamp or older.    * @param row    * @param column    * @param ts Delete all entries that have this timestamp or older    * @param lockid Row lock    * @throws IOException    */
specifier|public
name|void
name|deleteAll
parameter_list|(
...
try|try
block|{
comment|// Delete ALL versions rather than column family VERSIONS.  If we just did
comment|// VERSIONS, then if 2* VERSION cells, subsequent gets would get old stuff.
name|deleteMultiple
argument_list|(
name|row
argument_list|,
...
control|)
block|{
comment|// This is UGLY. COPY OF KEY PART OF KeyValue.
name|edits
operator|.
name|add
argument_list|(
...
block|}
block|}
comment|/**    * Delete all cells for a row with matching columns with timestamps    * less than or equal to<i>timestamp</i>.     *     * @param row The row to operate on    * @param columnRegex The column regex     * @param timestamp Timestamp to match    * @param lockid Row lock    * @throws IOException    */
specifier|public
name|void
...
block|}
block|}
comment|/**    * Delete all cells for a row with all the matching column families by    * familyRegex with timestamps less than or equal to<i>timestamp</i>.    *     * @param row The row to operate on    * @param familyRegex The column family regex for matching. This regex    * expression just match the family name, it didn't include<code>:<code>    * @param timestamp Timestamp to match    * @param lockid Row lock    * @throws IOException    */
specifier|public
name|void
...
argument_list|()
expr_stmt|;
comment|// construct the family regex pattern
name|Pattern
name|familyPattern
init|=
name|Pattern
...
argument_list|)
decl_stmt|;
comment|// check the family name match the family pattern.
if|if
condition|(
operator|!
operator|(
...
argument_list|()
expr_stmt|;
comment|// We used to have a getKeys method that purportedly only got the keys and
comment|// not the keys and values.  We now just do getFull.  For memcache values,
comment|// shouldn't matter if we get key and value since it'll be the entry that
comment|// is in memcache.  For the keyvalues from storefile, could be saving if
comment|// we only returned key component. TODO.
name|List
argument_list|<
name|KeyValue
argument_list|>
...
condition|)
block|{
comment|// I think the below edits don't have to be storted.  Its deletes.
comment|// hey don't have to go in in exact sorted order (we don't have to worry
...
block|}
block|}
comment|/**    * Tests for the existence of any cells for a given coordinate.    *     * @param row the row    * @param column the column, or null    * @param timestamp the timestamp, or HConstants.LATEST_VERSION for any    * @param lockid the existing lock, or null     * @return true if cells exist for the row, false otherwise    * @throws IOException    */
specifier|public
name|boolean
name|exists
parameter_list|(
...
control|)
block|{
comment|// TODO: Fix -- do I have to do a getColumn here?
name|size
operator|=
name|this
operator|.
...
do|do
block|{
comment|// Find the lowest key across all stores.
name|KeyValue
name|chosen
init|=
literal|null
...
block|}
block|}
comment|// Store results from each sub-scanner.
if|if
condition|(
name|chosenTimestamp
operator|>=
...
name|c
decl_stmt|;
comment|// Try the memcache first.
name|store
operator|.
name|lock
operator|.
...
expr_stmt|;
block|}
comment|// Pick the latest value out of List<Cell> c:
if|if
condition|(
name|c
operator|.
...
condition|)
block|{
comment|// Use the memcache timestamp value.
name|LOG
operator|.
name|debug
argument_list|(
literal|"Overwriting the memcache value for "
operator|+
name|Bytes
operator|.
name|toString
...
condition|)
block|{
comment|// Check the store (including disk) for the previous value.
name|c
operator|=
name|store
operator|.
...
name|debug
argument_list|(
literal|"Using HFile previous value for "
operator|+
name|Bytes
operator|.
name|toString
...
name|DoNotRetryIOException
argument_list|(
literal|"more than 1 value returned in "
operator|+
literal|"incrementColumnValue from Store"
argument_list|)
throw|;
block|}
block|}
...
name|debug
argument_list|(
literal|"Creating new counter value for "
operator|+
name|Bytes
operator|.
name|toString
