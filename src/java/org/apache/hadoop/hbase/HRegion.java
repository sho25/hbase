...
argument_list|)
expr_stmt|;
comment|// By default, we compact the region if an HStore has more than
...
argument_list|()
operator|+
literal|" needs compaction because "
operator|+
name|store
operator|.
name|getNMaps
argument_list|()
operator|+
literal|" store files present and threshold is "
operator|+
name|this
operator|.
name|compactionThreshold
...
block|}
block|}
comment|/**    * Flushing the cache is a little tricky. We have a lot of updates in the    * HMemcache, all of which have also been written to the log. We need to    * write those updates in the HMemcache out to disk, while being able to    * process reads/writes as much as possible during the flush operation. Also,    * the log has to state clearly the point in time at which the HMemcache was    * flushed. (That way, during recovery, we know when we can rely on the    * on-disk flushed structures and when we have to recover the HMemcache from    * the log.)    *     *<p>So, we have a three-step process:    *     *<ul><li>A. Flush the memcache to the on-disk stores, noting the current    * sequence ID for the log.<li>    *     *<li>B. Write a FLUSHCACHE-COMPLETE message to the log, using the sequence    * ID that was current at the time of memcache-flush.</li>    *     *<li>C. Get rid of the memcache structures that are now redundant, as    * they've been flushed to the on-disk HStores.</li>    *</ul>    *<p>This method is protected, but can be accessed via several public    * routes.    *     *<p> This method may block for some time.    *     * @return List of store files including just-made new flushes per-store. If    * not flush, returns list of all store files.    */
name|Vector
argument_list|<
name|HStoreFile
argument_list|>
...
block|}
block|}
comment|/**    * Delete<code>split</code> column from META<code>table</code>.    * @param t    * @param split    * @param regionName Region to remove.    * @throws IOException    */
specifier|static
name|void
...
block|}
block|}
comment|/**    *<code>region</code> has split.  Update META<code>table</code>.    * @param client Client to use running update.    * @param table META table we are to delete region from.    * @param regionName Region to remove.    * @throws IOException    */
specifier|static
name|void
...
block|}
block|}
comment|/**    * @param whichSplit COL_SPLITA or COL_SPLITB?    * @param data Map of META row labelled column data.    * @return HRegionInfo or null if not found.    * @throws IOException     */
specifier|static
name|HRegionInfo
name|getSplit
parameter_list|(
...
return|;
block|}
comment|/**    * @param data Map of META row labelled column data.    * @return An HRegionInfo instance.    * @throws IOException     */
specifier|static
name|HRegionInfo
name|getRegionInfo
parameter_list|(
...
return|;
block|}
comment|/**    * @param bytes Bytes of a HRegionInfo.    * @return An HRegionInfo instance.    * @throws IOException    */
specifier|static
name|HRegionInfo
name|getRegionInfo
parameter_list|(
...
return|;
block|}
comment|/**    * Look for HStoreFile references in passed region.    * @param fs    * @param baseDirectory    * @param hri    * @return True if we found references.    * @throws IOException     */
specifier|static
name|boolean
name|hasReferences
parameter_list|(
