...
expr_stmt|;
block|}
comment|// By default, we flush the cache when 32M.
name|this
operator|.
name|memcacheFlushSize
operator|=
...
throw|;
block|}
comment|// We just copied most of the data. Now get whatever updates are up in
comment|// the memcache (after shutting down new updates).
comment|// Notify the caller that we are about to close the region. This moves
comment|// us ot the 'retiring' queue. Means no more updates coming in -- just
...
name|debug
argument_list|(
literal|"Starting wait on row locks."
argument_list|)
expr_stmt|;
name|waitOnRowLocks
argument_list|()
...
name|debug
argument_list|(
literal|"Calling flushcache inside closeAndSplit"
argument_list|)
expr_stmt|;
name|Vector
argument_list|<
...
condition|)
block|{
comment|// It should always return a list of hstore files even if memcache is
comment|// empty.  It will return null if concurrent compaction or splits which
comment|// should not happen.
throw|throw
operator|new
name|NullPointerException
argument_list|(
literal|"Flushcache did not return any files"
argument_list|)
throw|;
block|}
name|TreeSet
...
name|debug
argument_list|(
literal|"Started splitting HStore "
operator|+
name|hsf
operator|.
name|getRegionName
...
name|debug
argument_list|(
literal|"Finished splitting HStore "
operator|+
name|hsf
operator|.
name|getRegionName
...
argument_list|()
operator|+
literal|" because largest file is "
operator|+
name|StringUtils
operator|.
