...
name|debug
argument_list|(
literal|"flushing and getting file names for region "
operator|+
name|srcA
operator|.
name|getRegionName
...
name|IOException
argument_list|(
literal|"Region splitkey must lie within region boundaries."
argument_list|)
throw|;
block|}
name|LOG
...
name|info
argument_list|(
literal|"region split complete. new regions are: "
operator|+
name|regions
index|[
literal|0
...
expr_stmt|;
block|}
comment|/**    * Private implementation.    *     * localput() is used for both puts and deletes. We just place the values into    * a per-row pending area, until a commit() or abort() call is received.    * (Or until the user's write-lock expires.)    */
name|void
name|localput
parameter_list|(
name|long
...
block|}
block|}
comment|/**    * Commit a pending set of writes to the memcache. This also results in writing    * to the change log.    *    * Once updates hit the change log, they are safe.  They will either be moved     * into an HStore in the future, or they will be recovered from the log.    */
specifier|public
name|void
name|commit
parameter_list|(
...
init|)
block|{
comment|// We can now commit the changes.
...
name|IOException
argument_list|(
literal|"Requested row out of range for HRegion "
operator|+
name|regionInfo
operator|.
name|regionName
...
block|}
block|}
comment|/**    * Obtain a lock on the given row.  Blocks until success.    *    * I know it's strange to have two mappings:    *   ROWS  ==> LOCKS    * as well as    *   LOCKS ==> ROWS    *    * But it acts as a guard on the client; a miswritten client just can't submit    * the name of a row and start writing to it; it must know the correct lockid,    * which matches the lock list in memory.    *     * It would be more memory-efficient to assume a correctly-written client,     * which maybe we'll do in the future.    */
name|long
name|obtainLock
parameter_list|(
name|Text
