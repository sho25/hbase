...
block|}
block|}
comment|/**    * Get a watcher of a zookeeper connection for a given quorum address.    * If the connection isn't established, a new one is created.    * This acts like a multiton.    * @param conf    * @return ZKW watcher    * @throws IOException    */
specifier|public
specifier|static
...
return|;
block|}
comment|/**    * This class is responsible to handle connection and reconnection    * to a zookeeper quorum.    *    */
specifier|public
specifier|static
class|class
name|ClientZKWatcher
...
return|;
block|}
comment|/**      * Clear this connection to zookeeper.      */
specifier|private
specifier|synchronized
name|void
name|resetZooKeeper
...
argument_list|()
expr_stmt|;
comment|// Doing below will call 'next' again and this will close the scanner
comment|// Without it we leave scanners open.
name|getRegionServerWithRetries
argument_list|(
name|s
argument_list|)
...
return|;
block|}
comment|/*      * Helper class for batch updates.      * Holds code shared doing batch puts and batch deletes.      */
specifier|private
specifier|abstract
class|class
name|Batch
...
expr_stmt|;
block|}
comment|/**        * This is the method subclasses must implement.        * @param currentList        * @param tableName        * @param row        * @return Count of items processed or -1 if all.        * @throws IOException        * @throws RuntimeException        */
specifier|abstract
name|int
name|doCall
parameter_list|(
...
name|RuntimeException
function_decl|;
comment|/**        * Process the passed<code>list</code>.        * @param list        * @param tableName        * @return Count of how many added or -1 if all added.        * @throws IOException        */
name|int
name|process
parameter_list|(
specifier|final
...
argument_list|)
expr_stmt|;
comment|// If the next record goes to a new region, then we are to clear
comment|// currentList now during this cycle.
name|isLastRow
operator|=
operator|(
name|i
...
argument_list|)
decl_stmt|;
comment|// index is == -1 if all processed successfully, else its index
comment|// of last record successfully processed.
if|if
condition|(
name|index
operator|!=
...
return|;
block|}
comment|/*        * @param t        * @param r        * @param re        * @return Region name that holds passed row<code>r</code>        * @throws IOException        */
specifier|private
name|byte
index|[]
name|getRegionName
...
return|;
block|}
comment|/*        * Do pause processing before retrying...        * @param currentRegion        * @param tries        * @return New value for tries.        */
specifier|private
name|int
name|doBatchPause
parameter_list|(
...
argument_list|)
operator|+
literal|" location because regionserver didn't accept updates; tries="
operator|+
name|tries
operator|+
literal|" of max="
...
decl_stmt|;
comment|// step 1:
comment|//  break up into regionserver-sized chunks and build the data structs
for|for
control|(
name|Put
name|put
...
block|}
comment|// step 2:
comment|//  make the requests
comment|// Discard the map, just use a list now, makes error recovery easier.
name|List
argument_list|<
name|MultiPut
argument_list|>
...
decl_stmt|;
comment|// step 3:
comment|//  collect the failures and tries from step 1.
for|for
control|(
name|int
name|i
...
parameter_list|)
block|{
comment|// go into the failed list.
name|LOG
operator|.
name|debug
argument_list|(
...
argument_list|)
expr_stmt|;
comment|// all go into the failed list.
name|LOG
operator|.
name|debug
argument_list|(
...
name|debug
argument_list|(
literal|"processBatchOfPuts had some failures, sleeping for "
operator|+
name|sleepTime
operator|+
literal|" ms!"
...
condition|)
block|{
comment|// ran out of retries and didnt succeed everything!
throw|throw
operator|new
name|RetriesExhaustedException
argument_list|(
...
argument_list|()
operator|+
literal|" puts left after retrying "
operator|+
name|numRetries
operator|+
literal|" times. Should have detail on which Regions failed the most"
argument_list|)
throw|;
block|}
block|}
