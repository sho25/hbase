...
init|)
block|{
name|checkForSplitsOrCompactions
argument_list|()
expr_stmt|;
block|}
block|}
...
argument_list|)
expr_stmt|;
comment|// Check regions to see if they need to be split or compacted chore thread
name|this
operator|.
name|splitOrCompactCheckerThread
operator|=
...
argument_list|)
expr_stmt|;
comment|// Worker is not the same as the above threads in that it does not
comment|// inherit from Chore.  Set an UncaughtExceptionHandler on it in case its
comment|// the one to see an OOME, etc., first.  The handler will set the stop
comment|// flag.
name|Threads
operator|.
name|setDaemonThreadRunning
...
block|}
block|}
comment|/*    * Let the master know we're here    * @throws IOException    */
specifier|private
name|void
name|reportForDuty
parameter_list|()
