...
expr_stmt|;
block|}
comment|/**    * Rewrite the meta tables so that HRI is versioned and so we move to new    * HCD and HCD.    * @throws IOException     */
specifier|private
name|void
name|rewriteMetaHRegionInfo
parameter_list|()
...
return|return;
block|}
comment|// Read using old classes.
specifier|final
name|org
operator|.
name|apache
...
argument_list|)
decl_stmt|;
comment|// If here, we were able to read with old classes.  If readOnly, then
comment|// needs migration.
if|if
condition|(
name|readOnly
operator|&&
...
block|}
block|}
comment|/*    * Move from old pre-v5 hregioninfo to current HRegionInfo    * Persist back into<code>r</code>    * @param mr    * @param oldHri    */
name|void
name|updateHRegionInfo
parameter_list|(
name|org
...
argument_list|)
expr_stmt|;
comment|// Update the HRegionInfo in meta setting the bloomfilter
comment|// to be disabled.
name|column
operator|.
name|setBloomfilter
argument_list|(
