...
argument_list|)
decl_stmt|;
comment|/**    * Extracts table name prefix from a metaregion row name.    * @param regionName A metaregion row name.    * @return The table prefix of a region name.    */
specifier|public
specifier|static
name|byte
index|[]
...
return|;
block|}
comment|/**    * Parses passed metaregion row into its constituent parts.    * Presumes region names are ASCII characters only.    * @param regionName A metaregion row name.    * @return A list where first element is the tablename, second the row    * portion, and the third the id.    */
specifier|public
specifier|static
name|List
argument_list|<
...
argument_list|)
expr_stmt|;
comment|// Now move in from the tail till we hit DELIMITER to find the id
name|offset
operator|=
operator|-
literal|1
...
argument_list|)
operator|+
literal|" does not have parseable tail"
argument_list|)
throw|;
block|}
name|byte
...
argument_list|)
expr_stmt|;
comment|// Now make up an array to hold the three parse pieces.
name|List
argument_list|<
name|byte
index|[]
