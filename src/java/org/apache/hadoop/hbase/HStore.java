...
expr_stmt|;
block|}
comment|/*     * @param deleteSequenceInfo True if we are to set the sequence number to -1    * on compacted file.    * @param maxSeenSeqID We may have already calculated the maxSeenSeqID.  If    * so, pass it here.  Otherwise, pass -1 and it will be calculated inside in    * this method.    * @param deleteSequenceInfo    * @param maxSeenSeqID    * @throws IOException    */
name|void
name|compactHelper
parameter_list|(
specifier|final
...
block|}
block|}
comment|/*    * @return list of files to compact sorted so most recent comes first.    */
specifier|private
name|List
argument_list|<
name|HStoreFile
...
comment|// We need to return out of here a List that has the newest file as
comment|// first.
name|filesToCompact
operator|=
operator|new
...
return|;
block|}
comment|/**    * Get the value for the indicated HStoreKey.  Grab the target value and the     * previous 'numVersions-1' values, as well.    *    * If 'numVersions' is negative, the method returns all available versions.    * @param key    * @param numVersions Number of versions to fetch.  Must be> 0.    * @param memcache Checked for deletions    * @return values for the specified versions    * @throws IOException    */
name|byte
index|[]
index|[]
name|get
