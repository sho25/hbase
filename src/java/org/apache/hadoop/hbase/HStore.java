...
expr_stmt|;
block|}
comment|// 3. Unload all the replaced MapFiles.  Do it by getting keys of all
comment|// to remove.  Then cycling on keys, removing, closing and deleting.
...
argument_list|)
decl_stmt|;
comment|// 4. Add to the toDelete files all old files, no longer needed
name|toDelete
operator|.
name|add
argument_list|(
...
expr_stmt|;
block|}
comment|// What if we fail now?  The above deletes will fail silently. We'd
comment|// better make sure not to write out any new files with the same names as
...
return|return;
block|}
comment|// Safe to delete now compaction has been moved into place.
for|for
control|(
name|HStoreFile
name|hsf
...
condition|)
block|{
comment|// Be careful we do not delte the just compacted file.
name|LOG
operator|.
name|warn
argument_list|(
literal|"Weird. File to delete has same name as one we are "
operator|+
literal|"about to delete (skipping): "
operator|+
name|hsf
operator|.
name|getFileId
