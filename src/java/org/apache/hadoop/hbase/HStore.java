...
comment|// point.
comment|//
comment|// TODO: Only do the compaction if we are over a threshold, not
comment|// every time. Not necessary if only two or three store files.  Fix after
comment|// revamp of compaction.
if|if
condition|(
name|storefiles
operator|.
...
condition|)
block|{
comment|// TODO: Only rewrite if NOT a HSF reference file.
if|if
condition|(
name|LOG
operator|.
...
argument_list|)
decl_stmt|;
comment|// 4. Delete all old files, no longer needed
name|hsf
operator|.
name|delete
argument_list|()
