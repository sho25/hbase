...
begin_comment
comment|/**  * Formatter that outputs data inside an HTML table.  * If only a single cell result, then no formatting is done.  Presumption is  * that client manages serial access outputting tables.  Does not close passed  * {@link Writer}.  *<p>TODO: Uses xmlenc. Hopefully it flushes every so often (Claims its a   * stream-based outputter).  Verify.  *<p>For now, invoke it this way (until shell starts to take cmdline params);  *<code>$ HBASE_OPTS='-Dhbaseshell.formatter=org.apache.hadoop.hbase.shell.TableFormatterFactory$HtmlTableFormatter' ./bin/hbase shell</code>  */
end_comment
