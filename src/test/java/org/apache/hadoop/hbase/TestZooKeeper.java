...
argument_list|)
expr_stmt|;
comment|// Depending on how long you wait here, the state after dump will
comment|// be 'closed' or 'Connecting'.
comment|// There should be no reason to wait, the connection is closed on the server
comment|// Thread.sleep(sessionTimeout * 3L);
name|LOG
operator|.
name|info
argument_list|(
literal|"Before dump state="
operator|+
name|connectionZK
operator|.
...
argument_list|()
decl_stmt|;
comment|//Here, if you wait, you will have a CONNECTED state. If you don't,
comment|//  you may have the CONNECTING one.
comment|//Thread.sleep(sessionTimeout * 3L);
name|States
name|state2
init|=
...
argument_list|)
expr_stmt|;
comment|// Helps the test to succeed!!!
name|testSanity
argument_list|()
expr_stmt|;
block|}
