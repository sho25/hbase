...
name|info
argument_list|(
literal|"Running testRegionCloseWhenNoMetaHBase2428"
argument_list|)
expr_stmt|;
name|MiniHBaseCluster
name|cluster
...
name|info
argument_list|(
literal|"Running testAddingServerBeforeOldIsDead2413"
argument_list|)
expr_stmt|;
name|MiniHBaseCluster
name|cluster
...
argument_list|)
expr_stmt|;
comment|// Try and start new regionserver.  It might clash with the old
comment|// regionserver port so keep trying to get past the BindException.
name|HRegionServer
name|hrs
init|=
literal|null
...
name|info
argument_list|(
literal|"BindException; retrying: "
operator|+
name|e
operator|.
name|toString
...
argument_list|)
expr_stmt|;
comment|// Wait until he's been given at least 3 regions before we go on to try
comment|// and count rows in table.
while|while
condition|(
name|hrs
operator|.
...
name|info
argument_list|(
literal|"Running testKillRSWithOpeningRegion2482"
argument_list|)
expr_stmt|;
name|MiniHBaseCluster
name|cluster
...
name|closeAllNonCatalogRegions
argument_list|(
name|cluster
argument_list|,
name|hrs
