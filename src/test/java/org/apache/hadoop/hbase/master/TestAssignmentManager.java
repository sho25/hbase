...
argument_list|)
decl_stmt|;
comment|// Boolean variable used for waiting until randomAssignment is called and new
...
argument_list|)
expr_stmt|;
comment|// a dummy plan inserted into the regionPlans. This plan is cleared and new one is formed
name|am
operator|.
name|regionPlans
operator|.
...
argument_list|)
expr_stmt|;
comment|// here we are waiting until the random assignment in the load balancer is called.
while|while
condition|(
operator|!
name|gate
...
expr_stmt|;
block|}
comment|// new region plan may take some time to get updated after random assignment is called and
...
expr_stmt|;
block|}
comment|// the new region plan created may contain the same RS as destination but it should
comment|// be new plan.
name|assertNotSame
argument_list|(
