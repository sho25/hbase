...
begin_comment
comment|/**  * Facility for testing HBase. Added as tool to abet junit4 testing.  Replaces  * old HBaseTestCase and HBaseCluserTestCase functionality.  * Create an instance and keep it around doing HBase testing.  This class is  * meant to be your one-stop shop for anything you might need testing.  Manages  * one cluster at a time only.  Depends on log4j being on classpath and  * hbase-site.xml for logging and test-run configuration.  It does not set  * logging levels nor make changes to configuration parameters.  */
end_comment
...
return|;
block|}
comment|/**    * @return Where to write test data on local filesystem; usually    * {@link #DEFAULT_TEST_DIRECTORY}    * @see #setupClusterTestBuildDir()    */
specifier|public
specifier|static
name|Path
name|getTestDir
...
return|;
block|}
comment|/**    * @param subdirName    * @return Path to a subdirectory named<code>subdirName</code> under    * {@link #getTestDir()}.    * @see #setupClusterTestBuildDir()    */
specifier|public
specifier|static
name|Path
name|getTestDir
...
return|;
block|}
comment|/**    * Create a copy of the passed configuration laden with a new user.  Use it    * to do things like get a new FileSystem instance.    * @param c    * @param index Some unique number used to make a unique user.    * @return Copy of<code>c</code> with new user stuff loaded into it.    * @throws IOException    */
specifier|public
specifier|static
name|Configuration
