...
expr_stmt|;
block|}
comment|/* Weird -- below passes in IDE but not command-line     rowkey = Bytes.toBytes("z");     for (int i = 0; i< MAX_PAGES - 1; i++) {       assertFalse("i=" + i, filterMPONE.filterRowKey(rowkey, 0, rowkey.length));       KeyValue kv = new KeyValue(rowkey, rowkey, Bytes.toBytes(i),         Bytes.toBytes(i));       assertTrue(Filter.ReturnCode.INCLUDE == filterMPONE.filterKeyValue(kv));     }     assertFalse(filterMPONE.filterRowKey(rowkey, 0, rowkey.length));     // Should fail here     KeyValue kv = new KeyValue(rowkey, rowkey, rowkey, rowkey);     assertTrue(Filter.ReturnCode.SKIP == filterMPONE.filterKeyValue(kv));      // Both filters in Set should be satisfied by now     assertTrue(filterMPONE.filterRow());      */
block|}
