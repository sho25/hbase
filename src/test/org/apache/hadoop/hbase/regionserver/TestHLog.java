...
expr_stmt|;
block|}
comment|// Assert I cannot read back my edits because a flush has not been called
comment|// -- it happens automatically at 100 edits ... see top of this method for
comment|// where we set it.
name|Path
name|walPath
init|=
name|wal
...
argument_list|)
decl_stmt|;
comment|/**SequenceFile.Reader reader =       new SequenceFile.Reader(this.fs, walPath, this.conf);     int count = 0;     HLogKey key = new HLogKey();     while(reader.next(key)) count++;     assertFalse(count< total);     reader.close();     */
comment|// Now call sync and retry read.
name|wal
operator|.
name|sync
argument_list|()
