begin_unit|revision:0.9.5;language:Java;cregit-version:0.0.1
begin_comment
comment|/**  * Copyright 2007 The Apache Software Foundation  *  * Licensed to the Apache Software Foundation (ASF) under one  * or more contributor license agreements.  See the NOTICE file  * distributed with this work for additional information  * regarding copyright ownership.  The ASF licenses this file  * to you under the Apache License, Version 2.0 (the  * "License"); you may not use this file except in compliance  * with the License.  You may obtain a copy of the License at  *  *     http://www.apache.org/licenses/LICENSE-2.0  *  * Unless required by applicable law or agreed to in writing, software  * distributed under the License is distributed on an "AS IS" BASIS,  * WITHOUT WARRANTIES OR CONDITIONS OF ANY KIND, either express or implied.  * See the License for the specific language governing permissions and  * limitations under the License.  */
end_comment

begin_package
package|package
name|org
operator|.
name|apache
operator|.
name|hadoop
operator|.
name|hbase
operator|.
name|regionserver
package|;
end_package

begin_import
import|import
name|org
operator|.
name|apache
operator|.
name|commons
operator|.
name|logging
operator|.
name|Log
import|;
end_import

begin_import
import|import
name|org
operator|.
name|apache
operator|.
name|commons
operator|.
name|logging
operator|.
name|LogFactory
import|;
end_import

begin_import
import|import
name|org
operator|.
name|apache
operator|.
name|hadoop
operator|.
name|hdfs
operator|.
name|MiniDFSCluster
import|;
end_import

begin_import
import|import
name|org
operator|.
name|apache
operator|.
name|hadoop
operator|.
name|hbase
operator|.
name|HBaseTestCase
import|;
end_import

begin_import
import|import
name|org
operator|.
name|apache
operator|.
name|hadoop
operator|.
name|hbase
operator|.
name|HConstants
import|;
end_import

begin_import
import|import
name|org
operator|.
name|apache
operator|.
name|hadoop
operator|.
name|hbase
operator|.
name|HTableDescriptor
import|;
end_import

begin_import
import|import
name|org
operator|.
name|apache
operator|.
name|hadoop
operator|.
name|hbase
operator|.
name|io
operator|.
name|BatchUpdate
import|;
end_import

begin_import
import|import
name|org
operator|.
name|apache
operator|.
name|hadoop
operator|.
name|hbase
operator|.
name|util
operator|.
name|Bytes
import|;
end_import

begin_comment
comment|/**  * Test the functionality of deleteAll.  */
end_comment

begin_class
specifier|public
class|class
name|TestDeleteAll
extends|extends
name|HBaseTestCase
block|{
specifier|static
specifier|final
name|Log
name|LOG
init|=
name|LogFactory
operator|.
name|getLog
argument_list|(
name|TestDeleteAll
operator|.
name|class
argument_list|)
decl_stmt|;
specifier|private
specifier|final
name|String
name|COLUMN_REGEX
init|=
literal|"[a-zA-Z0-9]*:[b|c]?"
decl_stmt|;
specifier|private
name|MiniDFSCluster
name|miniHdfs
decl_stmt|;
annotation|@
name|Override
specifier|protected
name|void
name|setUp
parameter_list|()
throws|throws
name|Exception
block|{
name|super
operator|.
name|setUp
argument_list|()
expr_stmt|;
try|try
block|{
name|this
operator|.
name|miniHdfs
operator|=
operator|new
name|MiniDFSCluster
argument_list|(
name|this
operator|.
name|conf
argument_list|,
literal|1
argument_list|,
literal|true
argument_list|,
literal|null
argument_list|)
expr_stmt|;
comment|// Set the hbase.rootdir to be the home directory in mini dfs.
name|this
operator|.
name|conf
operator|.
name|set
argument_list|(
name|HConstants
operator|.
name|HBASE_DIR
argument_list|,
name|this
operator|.
name|miniHdfs
operator|.
name|getFileSystem
argument_list|()
operator|.
name|getHomeDirectory
argument_list|()
operator|.
name|toString
argument_list|()
argument_list|)
expr_stmt|;
block|}
catch|catch
parameter_list|(
name|Exception
name|e
parameter_list|)
block|{
name|LOG
operator|.
name|fatal
argument_list|(
literal|"error starting MiniDFSCluster"
argument_list|,
name|e
argument_list|)
expr_stmt|;
throw|throw
name|e
throw|;
block|}
block|}
comment|/**    * Tests for HADOOP-1550.    * @throws Exception    */
specifier|public
name|void
name|testDeleteAll
parameter_list|()
throws|throws
name|Exception
block|{
name|HRegion
name|region
init|=
literal|null
decl_stmt|;
name|HRegionIncommon
name|region_incommon
init|=
literal|null
decl_stmt|;
try|try
block|{
name|HTableDescriptor
name|htd
init|=
name|createTableDescriptor
argument_list|(
name|getName
argument_list|()
argument_list|)
decl_stmt|;
name|region
operator|=
name|createNewHRegion
argument_list|(
name|htd
argument_list|,
literal|null
argument_list|,
literal|null
argument_list|)
expr_stmt|;
name|region_incommon
operator|=
operator|new
name|HRegionIncommon
argument_list|(
name|region
argument_list|)
expr_stmt|;
comment|// test memcache
name|makeSureItWorks
argument_list|(
name|region
argument_list|,
name|region_incommon
argument_list|,
literal|false
argument_list|)
expr_stmt|;
comment|// test hstore
name|makeSureItWorks
argument_list|(
name|region
argument_list|,
name|region_incommon
argument_list|,
literal|true
argument_list|)
expr_stmt|;
comment|// regex test memcache
name|makeSureRegexWorks
argument_list|(
name|region
argument_list|,
name|region_incommon
argument_list|,
literal|false
argument_list|)
expr_stmt|;
comment|// regex test hstore
name|makeSureRegexWorks
argument_list|(
name|region
argument_list|,
name|region_incommon
argument_list|,
literal|true
argument_list|)
expr_stmt|;
block|}
finally|finally
block|{
if|if
condition|(
name|region
operator|!=
literal|null
condition|)
block|{
try|try
block|{
name|region
operator|.
name|close
argument_list|()
expr_stmt|;
block|}
catch|catch
parameter_list|(
name|Exception
name|e
parameter_list|)
block|{
name|e
operator|.
name|printStackTrace
argument_list|()
expr_stmt|;
block|}
name|region
operator|.
name|getLog
argument_list|()
operator|.
name|closeAndDelete
argument_list|()
expr_stmt|;
block|}
block|}
block|}
specifier|private
name|void
name|makeSureItWorks
parameter_list|(
name|HRegion
name|region
parameter_list|,
name|HRegionIncommon
name|region_incommon
parameter_list|,
name|boolean
name|flush
parameter_list|)
throws|throws
name|Exception
block|{
comment|// insert a few versions worth of data for a row
name|byte
index|[]
name|row
init|=
name|Bytes
operator|.
name|toBytes
argument_list|(
literal|"test_row"
argument_list|)
decl_stmt|;
name|long
name|t0
init|=
name|System
operator|.
name|currentTimeMillis
argument_list|()
decl_stmt|;
name|long
name|t1
init|=
name|t0
operator|-
literal|15000
decl_stmt|;
name|long
name|t2
init|=
name|t1
operator|-
literal|15000
decl_stmt|;
name|byte
index|[]
name|colA
init|=
name|Bytes
operator|.
name|toBytes
argument_list|(
name|Bytes
operator|.
name|toString
argument_list|(
name|COLUMNS
index|[
literal|0
index|]
argument_list|)
operator|+
literal|"a"
argument_list|)
decl_stmt|;
name|byte
index|[]
name|colB
init|=
name|Bytes
operator|.
name|toBytes
argument_list|(
name|Bytes
operator|.
name|toString
argument_list|(
name|COLUMNS
index|[
literal|0
index|]
argument_list|)
operator|+
literal|"b"
argument_list|)
decl_stmt|;
name|byte
index|[]
name|colC
init|=
name|Bytes
operator|.
name|toBytes
argument_list|(
name|Bytes
operator|.
name|toString
argument_list|(
name|COLUMNS
index|[
literal|0
index|]
argument_list|)
operator|+
literal|"c"
argument_list|)
decl_stmt|;
name|byte
index|[]
name|colD
init|=
name|Bytes
operator|.
name|toBytes
argument_list|(
name|Bytes
operator|.
name|toString
argument_list|(
name|COLUMNS
index|[
literal|0
index|]
argument_list|)
argument_list|)
decl_stmt|;
name|BatchUpdate
name|batchUpdate
init|=
operator|new
name|BatchUpdate
argument_list|(
name|row
argument_list|,
name|t0
argument_list|)
decl_stmt|;
name|batchUpdate
operator|.
name|put
argument_list|(
name|colA
argument_list|,
name|cellData
argument_list|(
literal|0
argument_list|,
name|flush
argument_list|)
operator|.
name|getBytes
argument_list|()
argument_list|)
expr_stmt|;
name|batchUpdate
operator|.
name|put
argument_list|(
name|colB
argument_list|,
name|cellData
argument_list|(
literal|0
argument_list|,
name|flush
argument_list|)
operator|.
name|getBytes
argument_list|()
argument_list|)
expr_stmt|;
name|batchUpdate
operator|.
name|put
argument_list|(
name|colC
argument_list|,
name|cellData
argument_list|(
literal|0
argument_list|,
name|flush
argument_list|)
operator|.
name|getBytes
argument_list|()
argument_list|)
expr_stmt|;
name|batchUpdate
operator|.
name|put
argument_list|(
name|colD
argument_list|,
name|cellData
argument_list|(
literal|0
argument_list|,
name|flush
argument_list|)
operator|.
name|getBytes
argument_list|()
argument_list|)
expr_stmt|;
name|region_incommon
operator|.
name|commit
argument_list|(
name|batchUpdate
argument_list|)
expr_stmt|;
name|batchUpdate
operator|=
operator|new
name|BatchUpdate
argument_list|(
name|row
argument_list|,
name|t1
argument_list|)
expr_stmt|;
name|batchUpdate
operator|.
name|put
argument_list|(
name|colA
argument_list|,
name|cellData
argument_list|(
literal|1
argument_list|,
name|flush
argument_list|)
operator|.
name|getBytes
argument_list|()
argument_list|)
expr_stmt|;
name|batchUpdate
operator|.
name|put
argument_list|(
name|colB
argument_list|,
name|cellData
argument_list|(
literal|1
argument_list|,
name|flush
argument_list|)
operator|.
name|getBytes
argument_list|()
argument_list|)
expr_stmt|;
name|batchUpdate
operator|.
name|put
argument_list|(
name|colC
argument_list|,
name|cellData
argument_list|(
literal|1
argument_list|,
name|flush
argument_list|)
operator|.
name|getBytes
argument_list|()
argument_list|)
expr_stmt|;
name|batchUpdate
operator|.
name|put
argument_list|(
name|colD
argument_list|,
name|cellData
argument_list|(
literal|1
argument_list|,
name|flush
argument_list|)
operator|.
name|getBytes
argument_list|()
argument_list|)
expr_stmt|;
name|region_incommon
operator|.
name|commit
argument_list|(
name|batchUpdate
argument_list|)
expr_stmt|;
name|batchUpdate
operator|=
operator|new
name|BatchUpdate
argument_list|(
name|row
argument_list|,
name|t2
argument_list|)
expr_stmt|;
name|batchUpdate
operator|.
name|put
argument_list|(
name|colA
argument_list|,
name|cellData
argument_list|(
literal|2
argument_list|,
name|flush
argument_list|)
operator|.
name|getBytes
argument_list|()
argument_list|)
expr_stmt|;
name|batchUpdate
operator|.
name|put
argument_list|(
name|colB
argument_list|,
name|cellData
argument_list|(
literal|2
argument_list|,
name|flush
argument_list|)
operator|.
name|getBytes
argument_list|()
argument_list|)
expr_stmt|;
name|batchUpdate
operator|.
name|put
argument_list|(
name|colC
argument_list|,
name|cellData
argument_list|(
literal|2
argument_list|,
name|flush
argument_list|)
operator|.
name|getBytes
argument_list|()
argument_list|)
expr_stmt|;
name|batchUpdate
operator|.
name|put
argument_list|(
name|colD
argument_list|,
name|cellData
argument_list|(
literal|2
argument_list|,
name|flush
argument_list|)
operator|.
name|getBytes
argument_list|()
argument_list|)
expr_stmt|;
name|region_incommon
operator|.
name|commit
argument_list|(
name|batchUpdate
argument_list|)
expr_stmt|;
if|if
condition|(
name|flush
condition|)
block|{
name|region_incommon
operator|.
name|flushcache
argument_list|()
expr_stmt|;
block|}
comment|// call delete all at a timestamp, make sure only the most recent stuff is left behind
name|region
operator|.
name|deleteAll
argument_list|(
name|row
argument_list|,
name|t1
argument_list|,
literal|null
argument_list|)
expr_stmt|;
if|if
condition|(
name|flush
condition|)
block|{
name|region_incommon
operator|.
name|flushcache
argument_list|()
expr_stmt|;
block|}
name|assertCellEquals
argument_list|(
name|region
argument_list|,
name|row
argument_list|,
name|colA
argument_list|,
name|t0
argument_list|,
name|cellData
argument_list|(
literal|0
argument_list|,
name|flush
argument_list|)
argument_list|)
expr_stmt|;
name|assertCellEquals
argument_list|(
name|region
argument_list|,
name|row
argument_list|,
name|colA
argument_list|,
name|t1
argument_list|,
literal|null
argument_list|)
expr_stmt|;
name|assertCellEquals
argument_list|(
name|region
argument_list|,
name|row
argument_list|,
name|colA
argument_list|,
name|t2
argument_list|,
literal|null
argument_list|)
expr_stmt|;
name|assertCellEquals
argument_list|(
name|region
argument_list|,
name|row
argument_list|,
name|colD
argument_list|,
name|t0
argument_list|,
name|cellData
argument_list|(
literal|0
argument_list|,
name|flush
argument_list|)
argument_list|)
expr_stmt|;
name|assertCellEquals
argument_list|(
name|region
argument_list|,
name|row
argument_list|,
name|colD
argument_list|,
name|t1
argument_list|,
literal|null
argument_list|)
expr_stmt|;
name|assertCellEquals
argument_list|(
name|region
argument_list|,
name|row
argument_list|,
name|colD
argument_list|,
name|t2
argument_list|,
literal|null
argument_list|)
expr_stmt|;
comment|// call delete all w/o a timestamp, make sure nothing is left.
name|region
operator|.
name|deleteAll
argument_list|(
name|row
argument_list|,
name|HConstants
operator|.
name|LATEST_TIMESTAMP
argument_list|,
literal|null
argument_list|)
expr_stmt|;
if|if
condition|(
name|flush
condition|)
block|{
name|region_incommon
operator|.
name|flushcache
argument_list|()
expr_stmt|;
block|}
name|assertCellEquals
argument_list|(
name|region
argument_list|,
name|row
argument_list|,
name|colA
argument_list|,
name|t0
argument_list|,
literal|null
argument_list|)
expr_stmt|;
name|assertCellEquals
argument_list|(
name|region
argument_list|,
name|row
argument_list|,
name|colA
argument_list|,
name|t1
argument_list|,
literal|null
argument_list|)
expr_stmt|;
name|assertCellEquals
argument_list|(
name|region
argument_list|,
name|row
argument_list|,
name|colA
argument_list|,
name|t2
argument_list|,
literal|null
argument_list|)
expr_stmt|;
name|assertCellEquals
argument_list|(
name|region
argument_list|,
name|row
argument_list|,
name|colD
argument_list|,
name|t0
argument_list|,
literal|null
argument_list|)
expr_stmt|;
name|assertCellEquals
argument_list|(
name|region
argument_list|,
name|row
argument_list|,
name|colD
argument_list|,
name|t1
argument_list|,
literal|null
argument_list|)
expr_stmt|;
name|assertCellEquals
argument_list|(
name|region
argument_list|,
name|row
argument_list|,
name|colD
argument_list|,
name|t2
argument_list|,
literal|null
argument_list|)
expr_stmt|;
block|}
specifier|private
name|void
name|makeSureRegexWorks
parameter_list|(
name|HRegion
name|region
parameter_list|,
name|HRegionIncommon
name|region_incommon
parameter_list|,
name|boolean
name|flush
parameter_list|)
throws|throws
name|Exception
block|{
comment|// insert a few versions worth of data for a row
name|byte
index|[]
name|row
init|=
name|Bytes
operator|.
name|toBytes
argument_list|(
literal|"test_row"
argument_list|)
decl_stmt|;
name|long
name|t0
init|=
name|System
operator|.
name|currentTimeMillis
argument_list|()
decl_stmt|;
name|long
name|t1
init|=
name|t0
operator|-
literal|15000
decl_stmt|;
name|long
name|t2
init|=
name|t1
operator|-
literal|15000
decl_stmt|;
name|byte
index|[]
name|colA
init|=
name|Bytes
operator|.
name|toBytes
argument_list|(
name|Bytes
operator|.
name|toString
argument_list|(
name|COLUMNS
index|[
literal|0
index|]
argument_list|)
operator|+
literal|"a"
argument_list|)
decl_stmt|;
name|byte
index|[]
name|colB
init|=
name|Bytes
operator|.
name|toBytes
argument_list|(
name|Bytes
operator|.
name|toString
argument_list|(
name|COLUMNS
index|[
literal|0
index|]
argument_list|)
operator|+
literal|"b"
argument_list|)
decl_stmt|;
name|byte
index|[]
name|colC
init|=
name|Bytes
operator|.
name|toBytes
argument_list|(
name|Bytes
operator|.
name|toString
argument_list|(
name|COLUMNS
index|[
literal|0
index|]
argument_list|)
operator|+
literal|"c"
argument_list|)
decl_stmt|;
name|byte
index|[]
name|colD
init|=
name|Bytes
operator|.
name|toBytes
argument_list|(
name|Bytes
operator|.
name|toString
argument_list|(
name|COLUMNS
index|[
literal|0
index|]
argument_list|)
argument_list|)
decl_stmt|;
name|BatchUpdate
name|batchUpdate
init|=
operator|new
name|BatchUpdate
argument_list|(
name|row
argument_list|,
name|t0
argument_list|)
decl_stmt|;
name|batchUpdate
operator|.
name|put
argument_list|(
name|colA
argument_list|,
name|cellData
argument_list|(
literal|0
argument_list|,
name|flush
argument_list|)
operator|.
name|getBytes
argument_list|()
argument_list|)
expr_stmt|;
name|batchUpdate
operator|.
name|put
argument_list|(
name|colB
argument_list|,
name|cellData
argument_list|(
literal|0
argument_list|,
name|flush
argument_list|)
operator|.
name|getBytes
argument_list|()
argument_list|)
expr_stmt|;
name|batchUpdate
operator|.
name|put
argument_list|(
name|colC
argument_list|,
name|cellData
argument_list|(
literal|0
argument_list|,
name|flush
argument_list|)
operator|.
name|getBytes
argument_list|()
argument_list|)
expr_stmt|;
name|batchUpdate
operator|.
name|put
argument_list|(
name|colD
argument_list|,
name|cellData
argument_list|(
literal|0
argument_list|,
name|flush
argument_list|)
operator|.
name|getBytes
argument_list|()
argument_list|)
expr_stmt|;
name|region_incommon
operator|.
name|commit
argument_list|(
name|batchUpdate
argument_list|)
expr_stmt|;
name|batchUpdate
operator|=
operator|new
name|BatchUpdate
argument_list|(
name|row
argument_list|,
name|t1
argument_list|)
expr_stmt|;
name|batchUpdate
operator|.
name|put
argument_list|(
name|colA
argument_list|,
name|cellData
argument_list|(
literal|1
argument_list|,
name|flush
argument_list|)
operator|.
name|getBytes
argument_list|()
argument_list|)
expr_stmt|;
name|batchUpdate
operator|.
name|put
argument_list|(
name|colB
argument_list|,
name|cellData
argument_list|(
literal|1
argument_list|,
name|flush
argument_list|)
operator|.
name|getBytes
argument_list|()
argument_list|)
expr_stmt|;
name|batchUpdate
operator|.
name|put
argument_list|(
name|colC
argument_list|,
name|cellData
argument_list|(
literal|1
argument_list|,
name|flush
argument_list|)
operator|.
name|getBytes
argument_list|()
argument_list|)
expr_stmt|;
name|batchUpdate
operator|.
name|put
argument_list|(
name|colD
argument_list|,
name|cellData
argument_list|(
literal|1
argument_list|,
name|flush
argument_list|)
operator|.
name|getBytes
argument_list|()
argument_list|)
expr_stmt|;
name|region_incommon
operator|.
name|commit
argument_list|(
name|batchUpdate
argument_list|)
expr_stmt|;
name|batchUpdate
operator|=
operator|new
name|BatchUpdate
argument_list|(
name|row
argument_list|,
name|t2
argument_list|)
expr_stmt|;
name|batchUpdate
operator|.
name|put
argument_list|(
name|colA
argument_list|,
name|cellData
argument_list|(
literal|2
argument_list|,
name|flush
argument_list|)
operator|.
name|getBytes
argument_list|()
argument_list|)
expr_stmt|;
name|batchUpdate
operator|.
name|put
argument_list|(
name|colB
argument_list|,
name|cellData
argument_list|(
literal|2
argument_list|,
name|flush
argument_list|)
operator|.
name|getBytes
argument_list|()
argument_list|)
expr_stmt|;
name|batchUpdate
operator|.
name|put
argument_list|(
name|colC
argument_list|,
name|cellData
argument_list|(
literal|2
argument_list|,
name|flush
argument_list|)
operator|.
name|getBytes
argument_list|()
argument_list|)
expr_stmt|;
name|batchUpdate
operator|.
name|put
argument_list|(
name|colD
argument_list|,
name|cellData
argument_list|(
literal|2
argument_list|,
name|flush
argument_list|)
operator|.
name|getBytes
argument_list|()
argument_list|)
expr_stmt|;
name|region_incommon
operator|.
name|commit
argument_list|(
name|batchUpdate
argument_list|)
expr_stmt|;
if|if
condition|(
name|flush
condition|)
block|{
name|region_incommon
operator|.
name|flushcache
argument_list|()
expr_stmt|;
block|}
comment|// call delete the matching columns at a timestamp,
comment|// make sure only the most recent stuff is left behind
name|region
operator|.
name|deleteAllByRegex
argument_list|(
name|row
argument_list|,
name|COLUMN_REGEX
argument_list|,
name|t1
argument_list|,
literal|null
argument_list|)
expr_stmt|;
if|if
condition|(
name|flush
condition|)
block|{
name|region_incommon
operator|.
name|flushcache
argument_list|()
expr_stmt|;
block|}
name|assertCellEquals
argument_list|(
name|region
argument_list|,
name|row
argument_list|,
name|colA
argument_list|,
name|t0
argument_list|,
name|cellData
argument_list|(
literal|0
argument_list|,
name|flush
argument_list|)
argument_list|)
expr_stmt|;
name|assertCellEquals
argument_list|(
name|region
argument_list|,
name|row
argument_list|,
name|colA
argument_list|,
name|t1
argument_list|,
name|cellData
argument_list|(
literal|1
argument_list|,
name|flush
argument_list|)
argument_list|)
expr_stmt|;
name|assertCellEquals
argument_list|(
name|region
argument_list|,
name|row
argument_list|,
name|colA
argument_list|,
name|t2
argument_list|,
name|cellData
argument_list|(
literal|2
argument_list|,
name|flush
argument_list|)
argument_list|)
expr_stmt|;
name|assertCellEquals
argument_list|(
name|region
argument_list|,
name|row
argument_list|,
name|colB
argument_list|,
name|t0
argument_list|,
name|cellData
argument_list|(
literal|0
argument_list|,
name|flush
argument_list|)
argument_list|)
expr_stmt|;
name|assertCellEquals
argument_list|(
name|region
argument_list|,
name|row
argument_list|,
name|colB
argument_list|,
name|t1
argument_list|,
literal|null
argument_list|)
expr_stmt|;
name|assertCellEquals
argument_list|(
name|region
argument_list|,
name|row
argument_list|,
name|colB
argument_list|,
name|t2
argument_list|,
literal|null
argument_list|)
expr_stmt|;
name|assertCellEquals
argument_list|(
name|region
argument_list|,
name|row
argument_list|,
name|colC
argument_list|,
name|t0
argument_list|,
name|cellData
argument_list|(
literal|0
argument_list|,
name|flush
argument_list|)
argument_list|)
expr_stmt|;
name|assertCellEquals
argument_list|(
name|region
argument_list|,
name|row
argument_list|,
name|colC
argument_list|,
name|t1
argument_list|,
literal|null
argument_list|)
expr_stmt|;
name|assertCellEquals
argument_list|(
name|region
argument_list|,
name|row
argument_list|,
name|colC
argument_list|,
name|t2
argument_list|,
literal|null
argument_list|)
expr_stmt|;
name|assertCellEquals
argument_list|(
name|region
argument_list|,
name|row
argument_list|,
name|colD
argument_list|,
name|t0
argument_list|,
name|cellData
argument_list|(
literal|0
argument_list|,
name|flush
argument_list|)
argument_list|)
expr_stmt|;
name|assertCellEquals
argument_list|(
name|region
argument_list|,
name|row
argument_list|,
name|colD
argument_list|,
name|t1
argument_list|,
literal|null
argument_list|)
expr_stmt|;
name|assertCellEquals
argument_list|(
name|region
argument_list|,
name|row
argument_list|,
name|colD
argument_list|,
name|t2
argument_list|,
literal|null
argument_list|)
expr_stmt|;
comment|// call delete all w/o a timestamp, make sure nothing is left.
name|region
operator|.
name|deleteAllByRegex
argument_list|(
name|row
argument_list|,
name|COLUMN_REGEX
argument_list|,
name|HConstants
operator|.
name|LATEST_TIMESTAMP
argument_list|,
literal|null
argument_list|)
expr_stmt|;
if|if
condition|(
name|flush
condition|)
block|{
name|region_incommon
operator|.
name|flushcache
argument_list|()
expr_stmt|;
block|}
name|assertCellEquals
argument_list|(
name|region
argument_list|,
name|row
argument_list|,
name|colA
argument_list|,
name|t0
argument_list|,
name|cellData
argument_list|(
literal|0
argument_list|,
name|flush
argument_list|)
argument_list|)
expr_stmt|;
name|assertCellEquals
argument_list|(
name|region
argument_list|,
name|row
argument_list|,
name|colA
argument_list|,
name|t1
argument_list|,
name|cellData
argument_list|(
literal|1
argument_list|,
name|flush
argument_list|)
argument_list|)
expr_stmt|;
name|assertCellEquals
argument_list|(
name|region
argument_list|,
name|row
argument_list|,
name|colA
argument_list|,
name|t2
argument_list|,
name|cellData
argument_list|(
literal|2
argument_list|,
name|flush
argument_list|)
argument_list|)
expr_stmt|;
name|assertCellEquals
argument_list|(
name|region
argument_list|,
name|row
argument_list|,
name|colB
argument_list|,
name|t0
argument_list|,
literal|null
argument_list|)
expr_stmt|;
name|assertCellEquals
argument_list|(
name|region
argument_list|,
name|row
argument_list|,
name|colB
argument_list|,
name|t1
argument_list|,
literal|null
argument_list|)
expr_stmt|;
name|assertCellEquals
argument_list|(
name|region
argument_list|,
name|row
argument_list|,
name|colB
argument_list|,
name|t2
argument_list|,
literal|null
argument_list|)
expr_stmt|;
name|assertCellEquals
argument_list|(
name|region
argument_list|,
name|row
argument_list|,
name|colC
argument_list|,
name|t0
argument_list|,
literal|null
argument_list|)
expr_stmt|;
name|assertCellEquals
argument_list|(
name|region
argument_list|,
name|row
argument_list|,
name|colC
argument_list|,
name|t1
argument_list|,
literal|null
argument_list|)
expr_stmt|;
name|assertCellEquals
argument_list|(
name|region
argument_list|,
name|row
argument_list|,
name|colC
argument_list|,
name|t2
argument_list|,
literal|null
argument_list|)
expr_stmt|;
name|assertCellEquals
argument_list|(
name|region
argument_list|,
name|row
argument_list|,
name|colD
argument_list|,
name|t0
argument_list|,
literal|null
argument_list|)
expr_stmt|;
name|assertCellEquals
argument_list|(
name|region
argument_list|,
name|row
argument_list|,
name|colD
argument_list|,
name|t1
argument_list|,
literal|null
argument_list|)
expr_stmt|;
name|assertCellEquals
argument_list|(
name|region
argument_list|,
name|row
argument_list|,
name|colD
argument_list|,
name|t2
argument_list|,
literal|null
argument_list|)
expr_stmt|;
block|}
specifier|private
name|String
name|cellData
parameter_list|(
name|int
name|tsNum
parameter_list|,
name|boolean
name|flush
parameter_list|)
block|{
return|return
literal|"t"
operator|+
name|tsNum
operator|+
literal|" data"
operator|+
operator|(
name|flush
condition|?
literal|" - with flush"
else|:
literal|""
operator|)
return|;
block|}
annotation|@
name|Override
specifier|protected
name|void
name|tearDown
parameter_list|()
throws|throws
name|Exception
block|{
if|if
condition|(
name|this
operator|.
name|miniHdfs
operator|!=
literal|null
condition|)
block|{
name|shutdownDfs
argument_list|(
name|this
operator|.
name|miniHdfs
argument_list|)
expr_stmt|;
block|}
name|super
operator|.
name|tearDown
argument_list|()
expr_stmt|;
block|}
block|}
end_class

end_unit

