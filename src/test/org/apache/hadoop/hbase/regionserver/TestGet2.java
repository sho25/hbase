begin_unit|revision:0.9.5;language:Java;cregit-version:0.0.1
begin_comment
comment|/**  * Copyright 2007 The Apache Software Foundation  *  * Licensed to the Apache Software Foundation (ASF) under one  * or more contributor license agreements.  See the NOTICE file  * distributed with this work for additional information  * regarding copyright ownership.  The ASF licenses this file  * to you under the Apache License, Version 2.0 (the  * "License"); you may not use this file except in compliance  * with the License.  You may obtain a copy of the License at  *  *     http://www.apache.org/licenses/LICENSE-2.0  *  * Unless required by applicable law or agreed to in writing, software  * distributed under the License is distributed on an "AS IS" BASIS,  * WITHOUT WARRANTIES OR CONDITIONS OF ANY KIND, either express or implied.  * See the License for the specific language governing permissions and  * limitations under the License.  */
end_comment

begin_package
package|package
name|org
operator|.
name|apache
operator|.
name|hadoop
operator|.
name|hbase
operator|.
name|regionserver
package|;
end_package

begin_import
import|import
name|java
operator|.
name|io
operator|.
name|IOException
import|;
end_import

begin_import
import|import
name|java
operator|.
name|util
operator|.
name|Map
import|;
end_import

begin_import
import|import
name|java
operator|.
name|util
operator|.
name|Set
import|;
end_import

begin_import
import|import
name|java
operator|.
name|util
operator|.
name|TreeMap
import|;
end_import

begin_import
import|import
name|java
operator|.
name|util
operator|.
name|TreeSet
import|;
end_import

begin_import
import|import
name|org
operator|.
name|apache
operator|.
name|hadoop
operator|.
name|hdfs
operator|.
name|MiniDFSCluster
import|;
end_import

begin_import
import|import
name|org
operator|.
name|apache
operator|.
name|hadoop
operator|.
name|hbase
operator|.
name|HBaseTestCase
import|;
end_import

begin_import
import|import
name|org
operator|.
name|apache
operator|.
name|hadoop
operator|.
name|hbase
operator|.
name|HColumnDescriptor
import|;
end_import

begin_import
import|import
name|org
operator|.
name|apache
operator|.
name|hadoop
operator|.
name|hbase
operator|.
name|HConstants
import|;
end_import

begin_import
import|import
name|org
operator|.
name|apache
operator|.
name|hadoop
operator|.
name|hbase
operator|.
name|HStoreKey
import|;
end_import

begin_import
import|import
name|org
operator|.
name|apache
operator|.
name|hadoop
operator|.
name|hbase
operator|.
name|HTableDescriptor
import|;
end_import

begin_import
import|import
name|org
operator|.
name|apache
operator|.
name|hadoop
operator|.
name|hbase
operator|.
name|filter
operator|.
name|StopRowFilter
import|;
end_import

begin_import
import|import
name|org
operator|.
name|apache
operator|.
name|hadoop
operator|.
name|hbase
operator|.
name|filter
operator|.
name|WhileMatchRowFilter
import|;
end_import

begin_import
import|import
name|org
operator|.
name|apache
operator|.
name|hadoop
operator|.
name|hbase
operator|.
name|io
operator|.
name|BatchUpdate
import|;
end_import

begin_import
import|import
name|org
operator|.
name|apache
operator|.
name|hadoop
operator|.
name|hbase
operator|.
name|io
operator|.
name|Cell
import|;
end_import

begin_import
import|import
name|org
operator|.
name|apache
operator|.
name|hadoop
operator|.
name|hbase
operator|.
name|util
operator|.
name|Bytes
import|;
end_import

begin_comment
comment|/**  * {@link TestGet} is a medley of tests of get all done up as a single test.  * This class   */
end_comment

begin_class
specifier|public
class|class
name|TestGet2
extends|extends
name|HBaseTestCase
implements|implements
name|HConstants
block|{
specifier|private
name|MiniDFSCluster
name|miniHdfs
decl_stmt|;
specifier|private
specifier|static
specifier|final
name|String
name|T00
init|=
literal|"000"
decl_stmt|;
specifier|private
specifier|static
specifier|final
name|String
name|T10
init|=
literal|"010"
decl_stmt|;
specifier|private
specifier|static
specifier|final
name|String
name|T11
init|=
literal|"011"
decl_stmt|;
specifier|private
specifier|static
specifier|final
name|String
name|T12
init|=
literal|"012"
decl_stmt|;
specifier|private
specifier|static
specifier|final
name|String
name|T20
init|=
literal|"020"
decl_stmt|;
specifier|private
specifier|static
specifier|final
name|String
name|T30
init|=
literal|"030"
decl_stmt|;
specifier|private
specifier|static
specifier|final
name|String
name|T31
init|=
literal|"031"
decl_stmt|;
annotation|@
name|Override
specifier|protected
name|void
name|setUp
parameter_list|()
throws|throws
name|Exception
block|{
name|super
operator|.
name|setUp
argument_list|()
expr_stmt|;
name|this
operator|.
name|miniHdfs
operator|=
operator|new
name|MiniDFSCluster
argument_list|(
name|this
operator|.
name|conf
argument_list|,
literal|1
argument_list|,
literal|true
argument_list|,
literal|null
argument_list|)
expr_stmt|;
comment|// Set the hbase.rootdir to be the home directory in mini dfs.
name|this
operator|.
name|conf
operator|.
name|set
argument_list|(
name|HConstants
operator|.
name|HBASE_DIR
argument_list|,
name|this
operator|.
name|miniHdfs
operator|.
name|getFileSystem
argument_list|()
operator|.
name|getHomeDirectory
argument_list|()
operator|.
name|toString
argument_list|()
argument_list|)
expr_stmt|;
block|}
comment|/**    * Test for HBASE-808 and HBASE-809.    * @throws Exception    */
specifier|public
name|void
name|testMaxVersionsAndDeleting
parameter_list|()
throws|throws
name|Exception
block|{
name|HRegion
name|region
init|=
literal|null
decl_stmt|;
try|try
block|{
name|HTableDescriptor
name|htd
init|=
name|createTableDescriptor
argument_list|(
name|getName
argument_list|()
argument_list|)
decl_stmt|;
name|region
operator|=
name|createNewHRegion
argument_list|(
name|htd
argument_list|,
literal|null
argument_list|,
literal|null
argument_list|)
expr_stmt|;
name|byte
index|[]
name|column
init|=
name|COLUMNS
index|[
literal|0
index|]
decl_stmt|;
for|for
control|(
name|int
name|i
init|=
literal|0
init|;
name|i
operator|<
literal|100
condition|;
name|i
operator|++
control|)
block|{
name|addToRow
argument_list|(
name|region
argument_list|,
name|T00
argument_list|,
name|column
argument_list|,
name|i
argument_list|,
name|T00
operator|.
name|getBytes
argument_list|()
argument_list|)
expr_stmt|;
block|}
name|checkVersions
argument_list|(
name|region
argument_list|,
name|T00
argument_list|,
name|column
argument_list|)
expr_stmt|;
comment|// Flush and retry.
name|region
operator|.
name|flushcache
argument_list|()
expr_stmt|;
name|checkVersions
argument_list|(
name|region
argument_list|,
name|T00
argument_list|,
name|column
argument_list|)
expr_stmt|;
comment|// Now delete all then retry
name|region
operator|.
name|deleteAll
argument_list|(
name|Bytes
operator|.
name|toBytes
argument_list|(
name|T00
argument_list|)
argument_list|,
name|System
operator|.
name|currentTimeMillis
argument_list|()
argument_list|,
literal|null
argument_list|)
expr_stmt|;
name|Cell
index|[]
name|cells
init|=
name|region
operator|.
name|get
argument_list|(
name|Bytes
operator|.
name|toBytes
argument_list|(
name|T00
argument_list|)
argument_list|,
name|column
argument_list|,
operator|-
literal|1
argument_list|,
name|HColumnDescriptor
operator|.
name|DEFAULT_VERSIONS
argument_list|)
decl_stmt|;
name|assertTrue
argument_list|(
name|cells
operator|==
literal|null
argument_list|)
expr_stmt|;
name|region
operator|.
name|flushcache
argument_list|()
expr_stmt|;
name|cells
operator|=
name|region
operator|.
name|get
argument_list|(
name|Bytes
operator|.
name|toBytes
argument_list|(
name|T00
argument_list|)
argument_list|,
name|column
argument_list|,
operator|-
literal|1
argument_list|,
name|HColumnDescriptor
operator|.
name|DEFAULT_VERSIONS
argument_list|)
expr_stmt|;
name|assertTrue
argument_list|(
name|cells
operator|==
literal|null
argument_list|)
expr_stmt|;
comment|// Now add back the rows
for|for
control|(
name|int
name|i
init|=
literal|0
init|;
name|i
operator|<
literal|100
condition|;
name|i
operator|++
control|)
block|{
name|addToRow
argument_list|(
name|region
argument_list|,
name|T00
argument_list|,
name|column
argument_list|,
name|i
argument_list|,
name|T00
operator|.
name|getBytes
argument_list|()
argument_list|)
expr_stmt|;
block|}
comment|// Run same verifications.
name|checkVersions
argument_list|(
name|region
argument_list|,
name|T00
argument_list|,
name|column
argument_list|)
expr_stmt|;
comment|// Flush and retry.
name|region
operator|.
name|flushcache
argument_list|()
expr_stmt|;
name|checkVersions
argument_list|(
name|region
argument_list|,
name|T00
argument_list|,
name|column
argument_list|)
expr_stmt|;
block|}
finally|finally
block|{
if|if
condition|(
name|region
operator|!=
literal|null
condition|)
block|{
try|try
block|{
name|region
operator|.
name|close
argument_list|()
expr_stmt|;
block|}
catch|catch
parameter_list|(
name|Exception
name|e
parameter_list|)
block|{
name|e
operator|.
name|printStackTrace
argument_list|()
expr_stmt|;
block|}
name|region
operator|.
name|getLog
argument_list|()
operator|.
name|closeAndDelete
argument_list|()
expr_stmt|;
block|}
block|}
block|}
specifier|private
name|void
name|addToRow
parameter_list|(
specifier|final
name|HRegion
name|r
parameter_list|,
specifier|final
name|String
name|row
parameter_list|,
specifier|final
name|byte
index|[]
name|column
parameter_list|,
specifier|final
name|long
name|ts
parameter_list|,
specifier|final
name|byte
index|[]
name|bytes
parameter_list|)
throws|throws
name|IOException
block|{
name|BatchUpdate
name|batchUpdate
init|=
operator|new
name|BatchUpdate
argument_list|(
name|row
argument_list|,
name|ts
argument_list|)
decl_stmt|;
name|batchUpdate
operator|.
name|put
argument_list|(
name|column
argument_list|,
name|bytes
argument_list|)
expr_stmt|;
name|r
operator|.
name|batchUpdate
argument_list|(
name|batchUpdate
argument_list|,
literal|null
argument_list|)
expr_stmt|;
block|}
specifier|private
name|void
name|checkVersions
parameter_list|(
specifier|final
name|HRegion
name|region
parameter_list|,
specifier|final
name|String
name|row
parameter_list|,
specifier|final
name|byte
index|[]
name|column
parameter_list|)
throws|throws
name|IOException
block|{
name|byte
index|[]
name|r
init|=
name|Bytes
operator|.
name|toBytes
argument_list|(
name|row
argument_list|)
decl_stmt|;
name|Cell
index|[]
name|cells
init|=
name|region
operator|.
name|get
argument_list|(
name|r
argument_list|,
name|column
argument_list|,
operator|-
literal|1
argument_list|,
literal|100
argument_list|)
decl_stmt|;
name|assertTrue
argument_list|(
name|cells
operator|.
name|length
operator|==
name|HColumnDescriptor
operator|.
name|DEFAULT_VERSIONS
argument_list|)
expr_stmt|;
name|cells
operator|=
name|region
operator|.
name|get
argument_list|(
name|r
argument_list|,
name|column
argument_list|,
operator|-
literal|1
argument_list|,
literal|1
argument_list|)
expr_stmt|;
name|assertTrue
argument_list|(
name|cells
operator|.
name|length
operator|==
literal|1
argument_list|)
expr_stmt|;
name|cells
operator|=
name|region
operator|.
name|get
argument_list|(
name|r
argument_list|,
name|column
argument_list|,
operator|-
literal|1
argument_list|,
name|HConstants
operator|.
name|ALL_VERSIONS
argument_list|)
expr_stmt|;
name|assertTrue
argument_list|(
name|cells
operator|.
name|length
operator|==
name|HColumnDescriptor
operator|.
name|DEFAULT_VERSIONS
argument_list|)
expr_stmt|;
block|}
comment|/**    * Test file of multiple deletes and with deletes as final key.    * @see<a href="https://issues.apache.org/jira/browse/HBASE-751">HBASE-751</a>    */
specifier|public
name|void
name|testGetClosestRowBefore3
parameter_list|()
throws|throws
name|IOException
block|{
name|HRegion
name|region
init|=
literal|null
decl_stmt|;
name|BatchUpdate
name|batchUpdate
init|=
literal|null
decl_stmt|;
try|try
block|{
name|HTableDescriptor
name|htd
init|=
name|createTableDescriptor
argument_list|(
name|getName
argument_list|()
argument_list|)
decl_stmt|;
name|region
operator|=
name|createNewHRegion
argument_list|(
name|htd
argument_list|,
literal|null
argument_list|,
literal|null
argument_list|)
expr_stmt|;
name|batchUpdate
operator|=
operator|new
name|BatchUpdate
argument_list|(
name|T00
argument_list|)
expr_stmt|;
name|batchUpdate
operator|.
name|put
argument_list|(
name|COLUMNS
index|[
literal|0
index|]
argument_list|,
name|T00
operator|.
name|getBytes
argument_list|()
argument_list|)
expr_stmt|;
name|region
operator|.
name|batchUpdate
argument_list|(
name|batchUpdate
argument_list|,
literal|null
argument_list|)
expr_stmt|;
name|batchUpdate
operator|=
operator|new
name|BatchUpdate
argument_list|(
name|T10
argument_list|)
expr_stmt|;
name|batchUpdate
operator|.
name|put
argument_list|(
name|COLUMNS
index|[
literal|0
index|]
argument_list|,
name|T10
operator|.
name|getBytes
argument_list|()
argument_list|)
expr_stmt|;
name|region
operator|.
name|batchUpdate
argument_list|(
name|batchUpdate
argument_list|,
literal|null
argument_list|)
expr_stmt|;
name|batchUpdate
operator|=
operator|new
name|BatchUpdate
argument_list|(
name|T20
argument_list|)
expr_stmt|;
name|batchUpdate
operator|.
name|put
argument_list|(
name|COLUMNS
index|[
literal|0
index|]
argument_list|,
name|T20
operator|.
name|getBytes
argument_list|()
argument_list|)
expr_stmt|;
name|region
operator|.
name|batchUpdate
argument_list|(
name|batchUpdate
argument_list|,
literal|null
argument_list|)
expr_stmt|;
name|Map
argument_list|<
name|byte
index|[]
argument_list|,
name|Cell
argument_list|>
name|results
init|=
name|region
operator|.
name|getClosestRowBefore
argument_list|(
name|Bytes
operator|.
name|toBytes
argument_list|(
name|T20
argument_list|)
argument_list|)
decl_stmt|;
name|assertEquals
argument_list|(
name|T20
argument_list|,
operator|new
name|String
argument_list|(
name|results
operator|.
name|get
argument_list|(
name|COLUMNS
index|[
literal|0
index|]
argument_list|)
operator|.
name|getValue
argument_list|()
argument_list|)
argument_list|)
expr_stmt|;
name|batchUpdate
operator|=
operator|new
name|BatchUpdate
argument_list|(
name|T20
argument_list|)
expr_stmt|;
name|batchUpdate
operator|.
name|delete
argument_list|(
name|COLUMNS
index|[
literal|0
index|]
argument_list|)
expr_stmt|;
name|region
operator|.
name|batchUpdate
argument_list|(
name|batchUpdate
argument_list|,
literal|null
argument_list|)
expr_stmt|;
name|results
operator|=
name|region
operator|.
name|getClosestRowBefore
argument_list|(
name|Bytes
operator|.
name|toBytes
argument_list|(
name|T20
argument_list|)
argument_list|)
expr_stmt|;
name|assertEquals
argument_list|(
name|T10
argument_list|,
operator|new
name|String
argument_list|(
name|results
operator|.
name|get
argument_list|(
name|COLUMNS
index|[
literal|0
index|]
argument_list|)
operator|.
name|getValue
argument_list|()
argument_list|)
argument_list|)
expr_stmt|;
name|batchUpdate
operator|=
operator|new
name|BatchUpdate
argument_list|(
name|T30
argument_list|)
expr_stmt|;
name|batchUpdate
operator|.
name|put
argument_list|(
name|COLUMNS
index|[
literal|0
index|]
argument_list|,
name|T30
operator|.
name|getBytes
argument_list|()
argument_list|)
expr_stmt|;
name|region
operator|.
name|batchUpdate
argument_list|(
name|batchUpdate
argument_list|,
literal|null
argument_list|)
expr_stmt|;
name|results
operator|=
name|region
operator|.
name|getClosestRowBefore
argument_list|(
name|Bytes
operator|.
name|toBytes
argument_list|(
name|T30
argument_list|)
argument_list|)
expr_stmt|;
name|assertEquals
argument_list|(
name|T30
argument_list|,
operator|new
name|String
argument_list|(
name|results
operator|.
name|get
argument_list|(
name|COLUMNS
index|[
literal|0
index|]
argument_list|)
operator|.
name|getValue
argument_list|()
argument_list|)
argument_list|)
expr_stmt|;
name|batchUpdate
operator|=
operator|new
name|BatchUpdate
argument_list|(
name|T30
argument_list|)
expr_stmt|;
name|batchUpdate
operator|.
name|delete
argument_list|(
name|COLUMNS
index|[
literal|0
index|]
argument_list|)
expr_stmt|;
name|region
operator|.
name|batchUpdate
argument_list|(
name|batchUpdate
argument_list|,
literal|null
argument_list|)
expr_stmt|;
name|results
operator|=
name|region
operator|.
name|getClosestRowBefore
argument_list|(
name|Bytes
operator|.
name|toBytes
argument_list|(
name|T30
argument_list|)
argument_list|)
expr_stmt|;
name|assertEquals
argument_list|(
name|T10
argument_list|,
operator|new
name|String
argument_list|(
name|results
operator|.
name|get
argument_list|(
name|COLUMNS
index|[
literal|0
index|]
argument_list|)
operator|.
name|getValue
argument_list|()
argument_list|)
argument_list|)
expr_stmt|;
name|results
operator|=
name|region
operator|.
name|getClosestRowBefore
argument_list|(
name|Bytes
operator|.
name|toBytes
argument_list|(
name|T31
argument_list|)
argument_list|)
expr_stmt|;
name|assertEquals
argument_list|(
name|T10
argument_list|,
operator|new
name|String
argument_list|(
name|results
operator|.
name|get
argument_list|(
name|COLUMNS
index|[
literal|0
index|]
argument_list|)
operator|.
name|getValue
argument_list|()
argument_list|)
argument_list|)
expr_stmt|;
name|region
operator|.
name|flushcache
argument_list|()
expr_stmt|;
comment|// try finding "010" after flush
name|results
operator|=
name|region
operator|.
name|getClosestRowBefore
argument_list|(
name|Bytes
operator|.
name|toBytes
argument_list|(
name|T30
argument_list|)
argument_list|)
expr_stmt|;
name|assertEquals
argument_list|(
name|T10
argument_list|,
operator|new
name|String
argument_list|(
name|results
operator|.
name|get
argument_list|(
name|COLUMNS
index|[
literal|0
index|]
argument_list|)
operator|.
name|getValue
argument_list|()
argument_list|)
argument_list|)
expr_stmt|;
name|results
operator|=
name|region
operator|.
name|getClosestRowBefore
argument_list|(
name|Bytes
operator|.
name|toBytes
argument_list|(
name|T31
argument_list|)
argument_list|)
expr_stmt|;
name|assertEquals
argument_list|(
name|T10
argument_list|,
operator|new
name|String
argument_list|(
name|results
operator|.
name|get
argument_list|(
name|COLUMNS
index|[
literal|0
index|]
argument_list|)
operator|.
name|getValue
argument_list|()
argument_list|)
argument_list|)
expr_stmt|;
comment|// Put into a different column family.  Should make it so I get
comment|// and answer of t20.
name|batchUpdate
operator|=
operator|new
name|BatchUpdate
argument_list|(
name|T20
argument_list|)
expr_stmt|;
name|batchUpdate
operator|.
name|put
argument_list|(
name|COLUMNS
index|[
literal|1
index|]
argument_list|,
name|T20
operator|.
name|getBytes
argument_list|()
argument_list|)
expr_stmt|;
name|region
operator|.
name|batchUpdate
argument_list|(
name|batchUpdate
argument_list|,
literal|null
argument_list|)
expr_stmt|;
name|results
operator|=
name|region
operator|.
name|getClosestRowBefore
argument_list|(
name|Bytes
operator|.
name|toBytes
argument_list|(
name|T30
argument_list|)
argument_list|)
expr_stmt|;
name|assertEquals
argument_list|(
name|T20
argument_list|,
operator|new
name|String
argument_list|(
name|results
operator|.
name|get
argument_list|(
name|COLUMNS
index|[
literal|1
index|]
argument_list|)
operator|.
name|getValue
argument_list|()
argument_list|)
argument_list|)
expr_stmt|;
name|results
operator|=
name|region
operator|.
name|getClosestRowBefore
argument_list|(
name|Bytes
operator|.
name|toBytes
argument_list|(
name|T31
argument_list|)
argument_list|)
expr_stmt|;
name|assertEquals
argument_list|(
name|T20
argument_list|,
operator|new
name|String
argument_list|(
name|results
operator|.
name|get
argument_list|(
name|COLUMNS
index|[
literal|1
index|]
argument_list|)
operator|.
name|getValue
argument_list|()
argument_list|)
argument_list|)
expr_stmt|;
name|region
operator|.
name|flushcache
argument_list|()
expr_stmt|;
name|results
operator|=
name|region
operator|.
name|getClosestRowBefore
argument_list|(
name|Bytes
operator|.
name|toBytes
argument_list|(
name|T30
argument_list|)
argument_list|)
expr_stmt|;
name|assertEquals
argument_list|(
name|T20
argument_list|,
operator|new
name|String
argument_list|(
name|results
operator|.
name|get
argument_list|(
name|COLUMNS
index|[
literal|1
index|]
argument_list|)
operator|.
name|getValue
argument_list|()
argument_list|)
argument_list|)
expr_stmt|;
name|results
operator|=
name|region
operator|.
name|getClosestRowBefore
argument_list|(
name|Bytes
operator|.
name|toBytes
argument_list|(
name|T31
argument_list|)
argument_list|)
expr_stmt|;
name|assertEquals
argument_list|(
name|T20
argument_list|,
operator|new
name|String
argument_list|(
name|results
operator|.
name|get
argument_list|(
name|COLUMNS
index|[
literal|1
index|]
argument_list|)
operator|.
name|getValue
argument_list|()
argument_list|)
argument_list|)
expr_stmt|;
comment|// Now try combo of memcache and mapfiles.  Delete the t20 COLUMS[1]
comment|// in memory; make sure we get back t10 again.
name|batchUpdate
operator|=
operator|new
name|BatchUpdate
argument_list|(
name|T20
argument_list|)
expr_stmt|;
name|batchUpdate
operator|.
name|delete
argument_list|(
name|COLUMNS
index|[
literal|1
index|]
argument_list|)
expr_stmt|;
name|region
operator|.
name|batchUpdate
argument_list|(
name|batchUpdate
argument_list|,
literal|null
argument_list|)
expr_stmt|;
name|results
operator|=
name|region
operator|.
name|getClosestRowBefore
argument_list|(
name|Bytes
operator|.
name|toBytes
argument_list|(
name|T30
argument_list|)
argument_list|)
expr_stmt|;
name|assertEquals
argument_list|(
name|T10
argument_list|,
operator|new
name|String
argument_list|(
name|results
operator|.
name|get
argument_list|(
name|COLUMNS
index|[
literal|0
index|]
argument_list|)
operator|.
name|getValue
argument_list|()
argument_list|)
argument_list|)
expr_stmt|;
comment|// Ask for a value off the end of the file.  Should return t10.
name|results
operator|=
name|region
operator|.
name|getClosestRowBefore
argument_list|(
name|Bytes
operator|.
name|toBytes
argument_list|(
name|T31
argument_list|)
argument_list|)
expr_stmt|;
name|assertEquals
argument_list|(
name|T10
argument_list|,
operator|new
name|String
argument_list|(
name|results
operator|.
name|get
argument_list|(
name|COLUMNS
index|[
literal|0
index|]
argument_list|)
operator|.
name|getValue
argument_list|()
argument_list|)
argument_list|)
expr_stmt|;
name|region
operator|.
name|flushcache
argument_list|()
expr_stmt|;
name|results
operator|=
name|region
operator|.
name|getClosestRowBefore
argument_list|(
name|Bytes
operator|.
name|toBytes
argument_list|(
name|T31
argument_list|)
argument_list|)
expr_stmt|;
name|assertEquals
argument_list|(
name|T10
argument_list|,
operator|new
name|String
argument_list|(
name|results
operator|.
name|get
argument_list|(
name|COLUMNS
index|[
literal|0
index|]
argument_list|)
operator|.
name|getValue
argument_list|()
argument_list|)
argument_list|)
expr_stmt|;
comment|// Ok.  Let the candidate come out of mapfiles but have delete of
comment|// the candidate be in memory.
name|batchUpdate
operator|=
operator|new
name|BatchUpdate
argument_list|(
name|T11
argument_list|)
expr_stmt|;
name|batchUpdate
operator|.
name|put
argument_list|(
name|COLUMNS
index|[
literal|0
index|]
argument_list|,
name|T11
operator|.
name|getBytes
argument_list|()
argument_list|)
expr_stmt|;
name|region
operator|.
name|batchUpdate
argument_list|(
name|batchUpdate
argument_list|,
literal|null
argument_list|)
expr_stmt|;
name|batchUpdate
operator|=
operator|new
name|BatchUpdate
argument_list|(
name|T10
argument_list|)
expr_stmt|;
name|batchUpdate
operator|.
name|delete
argument_list|(
name|COLUMNS
index|[
literal|0
index|]
argument_list|)
expr_stmt|;
name|region
operator|.
name|batchUpdate
argument_list|(
name|batchUpdate
argument_list|,
literal|null
argument_list|)
expr_stmt|;
name|results
operator|=
name|region
operator|.
name|getClosestRowBefore
argument_list|(
name|Bytes
operator|.
name|toBytes
argument_list|(
name|T12
argument_list|)
argument_list|)
expr_stmt|;
name|assertEquals
argument_list|(
name|T11
argument_list|,
operator|new
name|String
argument_list|(
name|results
operator|.
name|get
argument_list|(
name|COLUMNS
index|[
literal|0
index|]
argument_list|)
operator|.
name|getValue
argument_list|()
argument_list|)
argument_list|)
expr_stmt|;
block|}
finally|finally
block|{
if|if
condition|(
name|region
operator|!=
literal|null
condition|)
block|{
try|try
block|{
name|region
operator|.
name|close
argument_list|()
expr_stmt|;
block|}
catch|catch
parameter_list|(
name|Exception
name|e
parameter_list|)
block|{
name|e
operator|.
name|printStackTrace
argument_list|()
expr_stmt|;
block|}
name|region
operator|.
name|getLog
argument_list|()
operator|.
name|closeAndDelete
argument_list|()
expr_stmt|;
block|}
block|}
block|}
comment|/**    * Tests for HADOOP-2161.    * @throws Exception    */
specifier|public
name|void
name|testGetFull
parameter_list|()
throws|throws
name|Exception
block|{
name|HRegion
name|region
init|=
literal|null
decl_stmt|;
name|InternalScanner
name|scanner
init|=
literal|null
decl_stmt|;
try|try
block|{
name|HTableDescriptor
name|htd
init|=
name|createTableDescriptor
argument_list|(
name|getName
argument_list|()
argument_list|)
decl_stmt|;
name|region
operator|=
name|createNewHRegion
argument_list|(
name|htd
argument_list|,
literal|null
argument_list|,
literal|null
argument_list|)
expr_stmt|;
for|for
control|(
name|int
name|i
init|=
literal|0
init|;
name|i
operator|<
name|COLUMNS
operator|.
name|length
condition|;
name|i
operator|++
control|)
block|{
name|addContent
argument_list|(
name|region
argument_list|,
name|COLUMNS
index|[
name|i
index|]
argument_list|)
expr_stmt|;
block|}
comment|// Find two rows to use doing getFull.
specifier|final
name|byte
index|[]
name|arbitraryStartRow
init|=
name|Bytes
operator|.
name|toBytes
argument_list|(
literal|"b"
argument_list|)
decl_stmt|;
name|byte
index|[]
name|actualStartRow
init|=
literal|null
decl_stmt|;
specifier|final
name|byte
index|[]
name|arbitraryStopRow
init|=
name|Bytes
operator|.
name|toBytes
argument_list|(
literal|"c"
argument_list|)
decl_stmt|;
name|byte
index|[]
name|actualStopRow
init|=
literal|null
decl_stmt|;
name|byte
index|[]
index|[]
name|columns
init|=
block|{
name|COLFAMILY_NAME1
block|}
decl_stmt|;
name|scanner
operator|=
name|region
operator|.
name|getScanner
argument_list|(
name|columns
argument_list|,
name|arbitraryStartRow
argument_list|,
name|HConstants
operator|.
name|LATEST_TIMESTAMP
argument_list|,
operator|new
name|WhileMatchRowFilter
argument_list|(
operator|new
name|StopRowFilter
argument_list|(
name|arbitraryStopRow
argument_list|)
argument_list|)
argument_list|)
expr_stmt|;
name|HStoreKey
name|key
init|=
operator|new
name|HStoreKey
argument_list|()
decl_stmt|;
name|TreeMap
argument_list|<
name|byte
index|[]
argument_list|,
name|Cell
argument_list|>
name|value
init|=
operator|new
name|TreeMap
argument_list|<
name|byte
index|[]
argument_list|,
name|Cell
argument_list|>
argument_list|(
name|Bytes
operator|.
name|BYTES_COMPARATOR
argument_list|)
decl_stmt|;
while|while
condition|(
name|scanner
operator|.
name|next
argument_list|(
name|key
argument_list|,
name|value
argument_list|)
condition|)
block|{
if|if
condition|(
name|actualStartRow
operator|==
literal|null
condition|)
block|{
name|actualStartRow
operator|=
name|key
operator|.
name|getRow
argument_list|()
expr_stmt|;
block|}
else|else
block|{
name|actualStopRow
operator|=
name|key
operator|.
name|getRow
argument_list|()
expr_stmt|;
block|}
block|}
comment|// Assert I got all out.
name|assertColumnsPresent
argument_list|(
name|region
argument_list|,
name|actualStartRow
argument_list|)
expr_stmt|;
name|assertColumnsPresent
argument_list|(
name|region
argument_list|,
name|actualStopRow
argument_list|)
expr_stmt|;
comment|// Force a flush so store files come into play.
name|region
operator|.
name|flushcache
argument_list|()
expr_stmt|;
comment|// Assert I got all out.
name|assertColumnsPresent
argument_list|(
name|region
argument_list|,
name|actualStartRow
argument_list|)
expr_stmt|;
name|assertColumnsPresent
argument_list|(
name|region
argument_list|,
name|actualStopRow
argument_list|)
expr_stmt|;
block|}
finally|finally
block|{
if|if
condition|(
name|scanner
operator|!=
literal|null
condition|)
block|{
name|scanner
operator|.
name|close
argument_list|()
expr_stmt|;
block|}
if|if
condition|(
name|region
operator|!=
literal|null
condition|)
block|{
try|try
block|{
name|region
operator|.
name|close
argument_list|()
expr_stmt|;
block|}
catch|catch
parameter_list|(
name|Exception
name|e
parameter_list|)
block|{
name|e
operator|.
name|printStackTrace
argument_list|()
expr_stmt|;
block|}
name|region
operator|.
name|getLog
argument_list|()
operator|.
name|closeAndDelete
argument_list|()
expr_stmt|;
block|}
block|}
block|}
comment|/**    * @throws IOException    */
specifier|public
name|void
name|testGetAtTimestamp
parameter_list|()
throws|throws
name|IOException
block|{
name|HRegion
name|region
init|=
literal|null
decl_stmt|;
name|HRegionIncommon
name|region_incommon
init|=
literal|null
decl_stmt|;
try|try
block|{
name|HTableDescriptor
name|htd
init|=
name|createTableDescriptor
argument_list|(
name|getName
argument_list|()
argument_list|)
decl_stmt|;
name|region
operator|=
name|createNewHRegion
argument_list|(
name|htd
argument_list|,
literal|null
argument_list|,
literal|null
argument_list|)
expr_stmt|;
name|region_incommon
operator|=
operator|new
name|HRegionIncommon
argument_list|(
name|region
argument_list|)
expr_stmt|;
name|long
name|right_now
init|=
name|System
operator|.
name|currentTimeMillis
argument_list|()
decl_stmt|;
name|long
name|one_second_ago
init|=
name|right_now
operator|-
literal|1000
decl_stmt|;
name|String
name|t
init|=
literal|"test_row"
decl_stmt|;
name|BatchUpdate
name|batchUpdate
init|=
operator|new
name|BatchUpdate
argument_list|(
name|t
argument_list|,
name|one_second_ago
argument_list|)
decl_stmt|;
name|batchUpdate
operator|.
name|put
argument_list|(
name|COLUMNS
index|[
literal|0
index|]
argument_list|,
literal|"old text"
operator|.
name|getBytes
argument_list|()
argument_list|)
expr_stmt|;
name|region_incommon
operator|.
name|commit
argument_list|(
name|batchUpdate
argument_list|)
expr_stmt|;
name|batchUpdate
operator|=
operator|new
name|BatchUpdate
argument_list|(
name|t
argument_list|,
name|right_now
argument_list|)
expr_stmt|;
name|batchUpdate
operator|.
name|put
argument_list|(
name|COLUMNS
index|[
literal|0
index|]
argument_list|,
literal|"new text"
operator|.
name|getBytes
argument_list|()
argument_list|)
expr_stmt|;
name|region_incommon
operator|.
name|commit
argument_list|(
name|batchUpdate
argument_list|)
expr_stmt|;
name|assertCellEquals
argument_list|(
name|region
argument_list|,
name|Bytes
operator|.
name|toBytes
argument_list|(
name|t
argument_list|)
argument_list|,
name|COLUMNS
index|[
literal|0
index|]
argument_list|,
name|right_now
argument_list|,
literal|"new text"
argument_list|)
expr_stmt|;
name|assertCellEquals
argument_list|(
name|region
argument_list|,
name|Bytes
operator|.
name|toBytes
argument_list|(
name|t
argument_list|)
argument_list|,
name|COLUMNS
index|[
literal|0
index|]
argument_list|,
name|one_second_ago
argument_list|,
literal|"old text"
argument_list|)
expr_stmt|;
comment|// Force a flush so store files come into play.
name|region_incommon
operator|.
name|flushcache
argument_list|()
expr_stmt|;
name|assertCellEquals
argument_list|(
name|region
argument_list|,
name|Bytes
operator|.
name|toBytes
argument_list|(
name|t
argument_list|)
argument_list|,
name|COLUMNS
index|[
literal|0
index|]
argument_list|,
name|right_now
argument_list|,
literal|"new text"
argument_list|)
expr_stmt|;
name|assertCellEquals
argument_list|(
name|region
argument_list|,
name|Bytes
operator|.
name|toBytes
argument_list|(
name|t
argument_list|)
argument_list|,
name|COLUMNS
index|[
literal|0
index|]
argument_list|,
name|one_second_ago
argument_list|,
literal|"old text"
argument_list|)
expr_stmt|;
block|}
finally|finally
block|{
if|if
condition|(
name|region
operator|!=
literal|null
condition|)
block|{
try|try
block|{
name|region
operator|.
name|close
argument_list|()
expr_stmt|;
block|}
catch|catch
parameter_list|(
name|Exception
name|e
parameter_list|)
block|{
name|e
operator|.
name|printStackTrace
argument_list|()
expr_stmt|;
block|}
name|region
operator|.
name|getLog
argument_list|()
operator|.
name|closeAndDelete
argument_list|()
expr_stmt|;
block|}
block|}
block|}
comment|/**    * For HADOOP-2443    * @throws IOException    */
specifier|public
name|void
name|testGetClosestRowBefore
parameter_list|()
throws|throws
name|IOException
block|{
name|HRegion
name|region
init|=
literal|null
decl_stmt|;
name|HRegionIncommon
name|region_incommon
init|=
literal|null
decl_stmt|;
name|BatchUpdate
name|batchUpdate
init|=
literal|null
decl_stmt|;
try|try
block|{
name|HTableDescriptor
name|htd
init|=
name|createTableDescriptor
argument_list|(
name|getName
argument_list|()
argument_list|)
decl_stmt|;
name|region
operator|=
name|createNewHRegion
argument_list|(
name|htd
argument_list|,
literal|null
argument_list|,
literal|null
argument_list|)
expr_stmt|;
name|region_incommon
operator|=
operator|new
name|HRegionIncommon
argument_list|(
name|region
argument_list|)
expr_stmt|;
comment|// set up some test data
name|String
name|t10
init|=
literal|"010"
decl_stmt|;
name|String
name|t20
init|=
literal|"020"
decl_stmt|;
name|String
name|t30
init|=
literal|"030"
decl_stmt|;
name|String
name|t35
init|=
literal|"035"
decl_stmt|;
name|String
name|t40
init|=
literal|"040"
decl_stmt|;
name|batchUpdate
operator|=
operator|new
name|BatchUpdate
argument_list|(
name|t10
argument_list|)
expr_stmt|;
name|batchUpdate
operator|.
name|put
argument_list|(
name|COLUMNS
index|[
literal|0
index|]
argument_list|,
literal|"t10 bytes"
operator|.
name|getBytes
argument_list|()
argument_list|)
expr_stmt|;
name|region
operator|.
name|batchUpdate
argument_list|(
name|batchUpdate
argument_list|,
literal|null
argument_list|)
expr_stmt|;
name|batchUpdate
operator|=
operator|new
name|BatchUpdate
argument_list|(
name|t20
argument_list|)
expr_stmt|;
name|batchUpdate
operator|.
name|put
argument_list|(
name|COLUMNS
index|[
literal|0
index|]
argument_list|,
literal|"t20 bytes"
operator|.
name|getBytes
argument_list|()
argument_list|)
expr_stmt|;
name|region
operator|.
name|batchUpdate
argument_list|(
name|batchUpdate
argument_list|,
literal|null
argument_list|)
expr_stmt|;
name|batchUpdate
operator|=
operator|new
name|BatchUpdate
argument_list|(
name|t30
argument_list|)
expr_stmt|;
name|batchUpdate
operator|.
name|put
argument_list|(
name|COLUMNS
index|[
literal|0
index|]
argument_list|,
literal|"t30 bytes"
operator|.
name|getBytes
argument_list|()
argument_list|)
expr_stmt|;
name|region
operator|.
name|batchUpdate
argument_list|(
name|batchUpdate
argument_list|,
literal|null
argument_list|)
expr_stmt|;
name|batchUpdate
operator|=
operator|new
name|BatchUpdate
argument_list|(
name|t35
argument_list|)
expr_stmt|;
name|batchUpdate
operator|.
name|put
argument_list|(
name|COLUMNS
index|[
literal|0
index|]
argument_list|,
literal|"t35 bytes"
operator|.
name|getBytes
argument_list|()
argument_list|)
expr_stmt|;
name|region
operator|.
name|batchUpdate
argument_list|(
name|batchUpdate
argument_list|,
literal|null
argument_list|)
expr_stmt|;
name|batchUpdate
operator|=
operator|new
name|BatchUpdate
argument_list|(
name|t35
argument_list|)
expr_stmt|;
name|batchUpdate
operator|.
name|delete
argument_list|(
name|COLUMNS
index|[
literal|0
index|]
argument_list|)
expr_stmt|;
name|region
operator|.
name|batchUpdate
argument_list|(
name|batchUpdate
argument_list|,
literal|null
argument_list|)
expr_stmt|;
name|batchUpdate
operator|=
operator|new
name|BatchUpdate
argument_list|(
name|t40
argument_list|)
expr_stmt|;
name|batchUpdate
operator|.
name|put
argument_list|(
name|COLUMNS
index|[
literal|0
index|]
argument_list|,
literal|"t40 bytes"
operator|.
name|getBytes
argument_list|()
argument_list|)
expr_stmt|;
name|region
operator|.
name|batchUpdate
argument_list|(
name|batchUpdate
argument_list|,
literal|null
argument_list|)
expr_stmt|;
comment|// try finding "015"
name|String
name|t15
init|=
literal|"015"
decl_stmt|;
name|Map
argument_list|<
name|byte
index|[]
argument_list|,
name|Cell
argument_list|>
name|results
init|=
name|region
operator|.
name|getClosestRowBefore
argument_list|(
name|Bytes
operator|.
name|toBytes
argument_list|(
name|t15
argument_list|)
argument_list|)
decl_stmt|;
name|assertEquals
argument_list|(
operator|new
name|String
argument_list|(
name|results
operator|.
name|get
argument_list|(
name|COLUMNS
index|[
literal|0
index|]
argument_list|)
operator|.
name|getValue
argument_list|()
argument_list|)
argument_list|,
literal|"t10 bytes"
argument_list|)
expr_stmt|;
comment|// try "020", we should get that row exactly
name|results
operator|=
name|region
operator|.
name|getClosestRowBefore
argument_list|(
name|Bytes
operator|.
name|toBytes
argument_list|(
name|t20
argument_list|)
argument_list|)
expr_stmt|;
name|assertEquals
argument_list|(
operator|new
name|String
argument_list|(
name|results
operator|.
name|get
argument_list|(
name|COLUMNS
index|[
literal|0
index|]
argument_list|)
operator|.
name|getValue
argument_list|()
argument_list|)
argument_list|,
literal|"t20 bytes"
argument_list|)
expr_stmt|;
comment|// try "038", should skip deleted "035" and get "030"
name|String
name|t38
init|=
literal|"038"
decl_stmt|;
name|results
operator|=
name|region
operator|.
name|getClosestRowBefore
argument_list|(
name|Bytes
operator|.
name|toBytes
argument_list|(
name|t38
argument_list|)
argument_list|)
expr_stmt|;
name|assertEquals
argument_list|(
operator|new
name|String
argument_list|(
name|results
operator|.
name|get
argument_list|(
name|COLUMNS
index|[
literal|0
index|]
argument_list|)
operator|.
name|getValue
argument_list|()
argument_list|)
argument_list|,
literal|"t30 bytes"
argument_list|)
expr_stmt|;
comment|// try "050", should get stuff from "040"
name|String
name|t50
init|=
literal|"050"
decl_stmt|;
name|results
operator|=
name|region
operator|.
name|getClosestRowBefore
argument_list|(
name|Bytes
operator|.
name|toBytes
argument_list|(
name|t50
argument_list|)
argument_list|)
expr_stmt|;
name|assertEquals
argument_list|(
operator|new
name|String
argument_list|(
name|results
operator|.
name|get
argument_list|(
name|COLUMNS
index|[
literal|0
index|]
argument_list|)
operator|.
name|getValue
argument_list|()
argument_list|)
argument_list|,
literal|"t40 bytes"
argument_list|)
expr_stmt|;
comment|// force a flush
name|region
operator|.
name|flushcache
argument_list|()
expr_stmt|;
comment|// try finding "015"
name|results
operator|=
name|region
operator|.
name|getClosestRowBefore
argument_list|(
name|Bytes
operator|.
name|toBytes
argument_list|(
name|t15
argument_list|)
argument_list|)
expr_stmt|;
name|assertEquals
argument_list|(
operator|new
name|String
argument_list|(
name|results
operator|.
name|get
argument_list|(
name|COLUMNS
index|[
literal|0
index|]
argument_list|)
operator|.
name|getValue
argument_list|()
argument_list|)
argument_list|,
literal|"t10 bytes"
argument_list|)
expr_stmt|;
comment|// try "020", we should get that row exactly
name|results
operator|=
name|region
operator|.
name|getClosestRowBefore
argument_list|(
name|Bytes
operator|.
name|toBytes
argument_list|(
name|t20
argument_list|)
argument_list|)
expr_stmt|;
name|assertEquals
argument_list|(
operator|new
name|String
argument_list|(
name|results
operator|.
name|get
argument_list|(
name|COLUMNS
index|[
literal|0
index|]
argument_list|)
operator|.
name|getValue
argument_list|()
argument_list|)
argument_list|,
literal|"t20 bytes"
argument_list|)
expr_stmt|;
comment|// try "038", should skip deleted "035" and get "030"
name|results
operator|=
name|region
operator|.
name|getClosestRowBefore
argument_list|(
name|Bytes
operator|.
name|toBytes
argument_list|(
name|t38
argument_list|)
argument_list|)
expr_stmt|;
name|assertEquals
argument_list|(
operator|new
name|String
argument_list|(
name|results
operator|.
name|get
argument_list|(
name|COLUMNS
index|[
literal|0
index|]
argument_list|)
operator|.
name|getValue
argument_list|()
argument_list|)
argument_list|,
literal|"t30 bytes"
argument_list|)
expr_stmt|;
comment|// try "050", should get stuff from "040"
name|results
operator|=
name|region
operator|.
name|getClosestRowBefore
argument_list|(
name|Bytes
operator|.
name|toBytes
argument_list|(
name|t50
argument_list|)
argument_list|)
expr_stmt|;
name|assertEquals
argument_list|(
operator|new
name|String
argument_list|(
name|results
operator|.
name|get
argument_list|(
name|COLUMNS
index|[
literal|0
index|]
argument_list|)
operator|.
name|getValue
argument_list|()
argument_list|)
argument_list|,
literal|"t40 bytes"
argument_list|)
expr_stmt|;
block|}
finally|finally
block|{
if|if
condition|(
name|region
operator|!=
literal|null
condition|)
block|{
try|try
block|{
name|region
operator|.
name|close
argument_list|()
expr_stmt|;
block|}
catch|catch
parameter_list|(
name|Exception
name|e
parameter_list|)
block|{
name|e
operator|.
name|printStackTrace
argument_list|()
expr_stmt|;
block|}
name|region
operator|.
name|getLog
argument_list|()
operator|.
name|closeAndDelete
argument_list|()
expr_stmt|;
block|}
block|}
block|}
comment|/** For HBASE-694 */
specifier|public
name|void
name|testGetClosestRowBefore2
parameter_list|()
throws|throws
name|IOException
block|{
name|HRegion
name|region
init|=
literal|null
decl_stmt|;
name|BatchUpdate
name|batchUpdate
init|=
literal|null
decl_stmt|;
try|try
block|{
name|HTableDescriptor
name|htd
init|=
name|createTableDescriptor
argument_list|(
name|getName
argument_list|()
argument_list|)
decl_stmt|;
name|region
operator|=
name|createNewHRegion
argument_list|(
name|htd
argument_list|,
literal|null
argument_list|,
literal|null
argument_list|)
expr_stmt|;
comment|// set up some test data
name|String
name|t10
init|=
literal|"010"
decl_stmt|;
name|String
name|t20
init|=
literal|"020"
decl_stmt|;
name|String
name|t30
init|=
literal|"030"
decl_stmt|;
name|String
name|t40
init|=
literal|"040"
decl_stmt|;
name|batchUpdate
operator|=
operator|new
name|BatchUpdate
argument_list|(
name|t10
argument_list|)
expr_stmt|;
name|batchUpdate
operator|.
name|put
argument_list|(
name|COLUMNS
index|[
literal|0
index|]
argument_list|,
literal|"t10 bytes"
operator|.
name|getBytes
argument_list|()
argument_list|)
expr_stmt|;
name|region
operator|.
name|batchUpdate
argument_list|(
name|batchUpdate
argument_list|,
literal|null
argument_list|)
expr_stmt|;
name|batchUpdate
operator|=
operator|new
name|BatchUpdate
argument_list|(
name|t30
argument_list|)
expr_stmt|;
name|batchUpdate
operator|.
name|put
argument_list|(
name|COLUMNS
index|[
literal|0
index|]
argument_list|,
literal|"t30 bytes"
operator|.
name|getBytes
argument_list|()
argument_list|)
expr_stmt|;
name|region
operator|.
name|batchUpdate
argument_list|(
name|batchUpdate
argument_list|,
literal|null
argument_list|)
expr_stmt|;
name|batchUpdate
operator|=
operator|new
name|BatchUpdate
argument_list|(
name|t40
argument_list|)
expr_stmt|;
name|batchUpdate
operator|.
name|put
argument_list|(
name|COLUMNS
index|[
literal|0
index|]
argument_list|,
literal|"t40 bytes"
operator|.
name|getBytes
argument_list|()
argument_list|)
expr_stmt|;
name|region
operator|.
name|batchUpdate
argument_list|(
name|batchUpdate
argument_list|,
literal|null
argument_list|)
expr_stmt|;
comment|// try finding "035"
name|String
name|t35
init|=
literal|"035"
decl_stmt|;
name|Map
argument_list|<
name|byte
index|[]
argument_list|,
name|Cell
argument_list|>
name|results
init|=
name|region
operator|.
name|getClosestRowBefore
argument_list|(
name|Bytes
operator|.
name|toBytes
argument_list|(
name|t35
argument_list|)
argument_list|)
decl_stmt|;
name|assertEquals
argument_list|(
operator|new
name|String
argument_list|(
name|results
operator|.
name|get
argument_list|(
name|COLUMNS
index|[
literal|0
index|]
argument_list|)
operator|.
name|getValue
argument_list|()
argument_list|)
argument_list|,
literal|"t30 bytes"
argument_list|)
expr_stmt|;
name|region
operator|.
name|flushcache
argument_list|()
expr_stmt|;
comment|// try finding "035"
name|results
operator|=
name|region
operator|.
name|getClosestRowBefore
argument_list|(
name|Bytes
operator|.
name|toBytes
argument_list|(
name|t35
argument_list|)
argument_list|)
expr_stmt|;
name|assertEquals
argument_list|(
operator|new
name|String
argument_list|(
name|results
operator|.
name|get
argument_list|(
name|COLUMNS
index|[
literal|0
index|]
argument_list|)
operator|.
name|getValue
argument_list|()
argument_list|)
argument_list|,
literal|"t30 bytes"
argument_list|)
expr_stmt|;
name|batchUpdate
operator|=
operator|new
name|BatchUpdate
argument_list|(
name|t20
argument_list|)
expr_stmt|;
name|batchUpdate
operator|.
name|put
argument_list|(
name|COLUMNS
index|[
literal|0
index|]
argument_list|,
literal|"t20 bytes"
operator|.
name|getBytes
argument_list|()
argument_list|)
expr_stmt|;
name|region
operator|.
name|batchUpdate
argument_list|(
name|batchUpdate
argument_list|,
literal|null
argument_list|)
expr_stmt|;
comment|// try finding "035"
name|results
operator|=
name|region
operator|.
name|getClosestRowBefore
argument_list|(
name|Bytes
operator|.
name|toBytes
argument_list|(
name|t35
argument_list|)
argument_list|)
expr_stmt|;
name|assertEquals
argument_list|(
operator|new
name|String
argument_list|(
name|results
operator|.
name|get
argument_list|(
name|COLUMNS
index|[
literal|0
index|]
argument_list|)
operator|.
name|getValue
argument_list|()
argument_list|)
argument_list|,
literal|"t30 bytes"
argument_list|)
expr_stmt|;
name|region
operator|.
name|flushcache
argument_list|()
expr_stmt|;
comment|// try finding "035"
name|results
operator|=
name|region
operator|.
name|getClosestRowBefore
argument_list|(
name|Bytes
operator|.
name|toBytes
argument_list|(
name|t35
argument_list|)
argument_list|)
expr_stmt|;
name|assertEquals
argument_list|(
operator|new
name|String
argument_list|(
name|results
operator|.
name|get
argument_list|(
name|COLUMNS
index|[
literal|0
index|]
argument_list|)
operator|.
name|getValue
argument_list|()
argument_list|)
argument_list|,
literal|"t30 bytes"
argument_list|)
expr_stmt|;
block|}
finally|finally
block|{
if|if
condition|(
name|region
operator|!=
literal|null
condition|)
block|{
try|try
block|{
name|region
operator|.
name|close
argument_list|()
expr_stmt|;
block|}
catch|catch
parameter_list|(
name|Exception
name|e
parameter_list|)
block|{
name|e
operator|.
name|printStackTrace
argument_list|()
expr_stmt|;
block|}
name|region
operator|.
name|getLog
argument_list|()
operator|.
name|closeAndDelete
argument_list|()
expr_stmt|;
block|}
block|}
block|}
comment|/**    * For HBASE-40    */
specifier|public
name|void
name|testGetFullWithSpecifiedColumns
parameter_list|()
throws|throws
name|IOException
block|{
name|HRegion
name|region
init|=
literal|null
decl_stmt|;
name|HRegionIncommon
name|region_incommon
init|=
literal|null
decl_stmt|;
try|try
block|{
name|HTableDescriptor
name|htd
init|=
name|createTableDescriptor
argument_list|(
name|getName
argument_list|()
argument_list|)
decl_stmt|;
name|region
operator|=
name|createNewHRegion
argument_list|(
name|htd
argument_list|,
literal|null
argument_list|,
literal|null
argument_list|)
expr_stmt|;
name|region_incommon
operator|=
operator|new
name|HRegionIncommon
argument_list|(
name|region
argument_list|)
expr_stmt|;
comment|// write a row with a bunch of columns
name|byte
index|[]
name|row
init|=
name|Bytes
operator|.
name|toBytes
argument_list|(
literal|"some_row"
argument_list|)
decl_stmt|;
name|BatchUpdate
name|bu
init|=
operator|new
name|BatchUpdate
argument_list|(
name|row
argument_list|)
decl_stmt|;
name|bu
operator|.
name|put
argument_list|(
name|COLUMNS
index|[
literal|0
index|]
argument_list|,
literal|"column 0"
operator|.
name|getBytes
argument_list|()
argument_list|)
expr_stmt|;
name|bu
operator|.
name|put
argument_list|(
name|COLUMNS
index|[
literal|1
index|]
argument_list|,
literal|"column 1"
operator|.
name|getBytes
argument_list|()
argument_list|)
expr_stmt|;
name|bu
operator|.
name|put
argument_list|(
name|COLUMNS
index|[
literal|2
index|]
argument_list|,
literal|"column 2"
operator|.
name|getBytes
argument_list|()
argument_list|)
expr_stmt|;
name|region
operator|.
name|batchUpdate
argument_list|(
name|bu
argument_list|,
literal|null
argument_list|)
expr_stmt|;
name|assertSpecifiedColumns
argument_list|(
name|region
argument_list|,
name|row
argument_list|)
expr_stmt|;
comment|// try it again with a cache flush to involve the store, not just the
comment|// memcache.
name|region_incommon
operator|.
name|flushcache
argument_list|()
expr_stmt|;
name|assertSpecifiedColumns
argument_list|(
name|region
argument_list|,
name|row
argument_list|)
expr_stmt|;
block|}
finally|finally
block|{
if|if
condition|(
name|region
operator|!=
literal|null
condition|)
block|{
try|try
block|{
name|region
operator|.
name|close
argument_list|()
expr_stmt|;
block|}
catch|catch
parameter_list|(
name|Exception
name|e
parameter_list|)
block|{
name|e
operator|.
name|printStackTrace
argument_list|()
expr_stmt|;
block|}
name|region
operator|.
name|getLog
argument_list|()
operator|.
name|closeAndDelete
argument_list|()
expr_stmt|;
block|}
block|}
block|}
specifier|private
name|void
name|assertSpecifiedColumns
parameter_list|(
specifier|final
name|HRegion
name|region
parameter_list|,
specifier|final
name|byte
index|[]
name|row
parameter_list|)
throws|throws
name|IOException
block|{
name|TreeSet
argument_list|<
name|byte
index|[]
argument_list|>
name|all
init|=
operator|new
name|TreeSet
argument_list|<
name|byte
index|[]
argument_list|>
argument_list|(
name|Bytes
operator|.
name|BYTES_COMPARATOR
argument_list|)
decl_stmt|;
name|TreeSet
argument_list|<
name|byte
index|[]
argument_list|>
name|one
init|=
operator|new
name|TreeSet
argument_list|<
name|byte
index|[]
argument_list|>
argument_list|(
name|Bytes
operator|.
name|BYTES_COMPARATOR
argument_list|)
decl_stmt|;
name|TreeSet
argument_list|<
name|byte
index|[]
argument_list|>
name|none
init|=
operator|new
name|TreeSet
argument_list|<
name|byte
index|[]
argument_list|>
argument_list|(
name|Bytes
operator|.
name|BYTES_COMPARATOR
argument_list|)
decl_stmt|;
name|all
operator|.
name|add
argument_list|(
name|COLUMNS
index|[
literal|0
index|]
argument_list|)
expr_stmt|;
name|all
operator|.
name|add
argument_list|(
name|COLUMNS
index|[
literal|1
index|]
argument_list|)
expr_stmt|;
name|all
operator|.
name|add
argument_list|(
name|COLUMNS
index|[
literal|2
index|]
argument_list|)
expr_stmt|;
name|one
operator|.
name|add
argument_list|(
name|COLUMNS
index|[
literal|0
index|]
argument_list|)
expr_stmt|;
comment|// make sure we get all of them with standard getFull
name|Map
argument_list|<
name|byte
index|[]
argument_list|,
name|Cell
argument_list|>
name|result
init|=
name|region
operator|.
name|getFull
argument_list|(
name|row
argument_list|,
literal|null
argument_list|,
name|HConstants
operator|.
name|LATEST_TIMESTAMP
argument_list|,
literal|null
argument_list|)
decl_stmt|;
name|assertEquals
argument_list|(
operator|new
name|String
argument_list|(
name|result
operator|.
name|get
argument_list|(
name|COLUMNS
index|[
literal|0
index|]
argument_list|)
operator|.
name|getValue
argument_list|()
argument_list|)
argument_list|,
literal|"column 0"
argument_list|)
expr_stmt|;
name|assertEquals
argument_list|(
operator|new
name|String
argument_list|(
name|result
operator|.
name|get
argument_list|(
name|COLUMNS
index|[
literal|1
index|]
argument_list|)
operator|.
name|getValue
argument_list|()
argument_list|)
argument_list|,
literal|"column 1"
argument_list|)
expr_stmt|;
name|assertEquals
argument_list|(
operator|new
name|String
argument_list|(
name|result
operator|.
name|get
argument_list|(
name|COLUMNS
index|[
literal|2
index|]
argument_list|)
operator|.
name|getValue
argument_list|()
argument_list|)
argument_list|,
literal|"column 2"
argument_list|)
expr_stmt|;
comment|// try to get just one
name|result
operator|=
name|region
operator|.
name|getFull
argument_list|(
name|row
argument_list|,
name|one
argument_list|,
name|HConstants
operator|.
name|LATEST_TIMESTAMP
argument_list|,
literal|null
argument_list|)
expr_stmt|;
name|assertEquals
argument_list|(
operator|new
name|String
argument_list|(
name|result
operator|.
name|get
argument_list|(
name|COLUMNS
index|[
literal|0
index|]
argument_list|)
operator|.
name|getValue
argument_list|()
argument_list|)
argument_list|,
literal|"column 0"
argument_list|)
expr_stmt|;
name|assertNull
argument_list|(
name|result
operator|.
name|get
argument_list|(
name|COLUMNS
index|[
literal|1
index|]
argument_list|)
argument_list|)
expr_stmt|;
name|assertNull
argument_list|(
name|result
operator|.
name|get
argument_list|(
name|COLUMNS
index|[
literal|2
index|]
argument_list|)
argument_list|)
expr_stmt|;
comment|// try to get all of them (specified)
name|result
operator|=
name|region
operator|.
name|getFull
argument_list|(
name|row
argument_list|,
name|all
argument_list|,
name|HConstants
operator|.
name|LATEST_TIMESTAMP
argument_list|,
literal|null
argument_list|)
expr_stmt|;
name|assertEquals
argument_list|(
operator|new
name|String
argument_list|(
name|result
operator|.
name|get
argument_list|(
name|COLUMNS
index|[
literal|0
index|]
argument_list|)
operator|.
name|getValue
argument_list|()
argument_list|)
argument_list|,
literal|"column 0"
argument_list|)
expr_stmt|;
name|assertEquals
argument_list|(
operator|new
name|String
argument_list|(
name|result
operator|.
name|get
argument_list|(
name|COLUMNS
index|[
literal|1
index|]
argument_list|)
operator|.
name|getValue
argument_list|()
argument_list|)
argument_list|,
literal|"column 1"
argument_list|)
expr_stmt|;
name|assertEquals
argument_list|(
operator|new
name|String
argument_list|(
name|result
operator|.
name|get
argument_list|(
name|COLUMNS
index|[
literal|2
index|]
argument_list|)
operator|.
name|getValue
argument_list|()
argument_list|)
argument_list|,
literal|"column 2"
argument_list|)
expr_stmt|;
comment|// try to get none with empty column set
name|result
operator|=
name|region
operator|.
name|getFull
argument_list|(
name|row
argument_list|,
name|none
argument_list|,
name|HConstants
operator|.
name|LATEST_TIMESTAMP
argument_list|,
literal|null
argument_list|)
expr_stmt|;
name|assertNull
argument_list|(
name|result
operator|.
name|get
argument_list|(
name|COLUMNS
index|[
literal|0
index|]
argument_list|)
argument_list|)
expr_stmt|;
name|assertNull
argument_list|(
name|result
operator|.
name|get
argument_list|(
name|COLUMNS
index|[
literal|1
index|]
argument_list|)
argument_list|)
expr_stmt|;
name|assertNull
argument_list|(
name|result
operator|.
name|get
argument_list|(
name|COLUMNS
index|[
literal|2
index|]
argument_list|)
argument_list|)
expr_stmt|;
block|}
specifier|public
name|void
name|testGetFullMultiMapfile
parameter_list|()
throws|throws
name|IOException
block|{
name|HRegion
name|region
init|=
literal|null
decl_stmt|;
name|HRegionIncommon
name|region_incommon
init|=
literal|null
decl_stmt|;
name|BatchUpdate
name|batchUpdate
init|=
literal|null
decl_stmt|;
name|Map
argument_list|<
name|byte
index|[]
argument_list|,
name|Cell
argument_list|>
name|results
init|=
literal|null
decl_stmt|;
try|try
block|{
name|HTableDescriptor
name|htd
init|=
name|createTableDescriptor
argument_list|(
name|getName
argument_list|()
argument_list|)
decl_stmt|;
name|region
operator|=
name|createNewHRegion
argument_list|(
name|htd
argument_list|,
literal|null
argument_list|,
literal|null
argument_list|)
expr_stmt|;
name|region_incommon
operator|=
operator|new
name|HRegionIncommon
argument_list|(
name|region
argument_list|)
expr_stmt|;
comment|//
comment|// Test ordering issue
comment|//
name|byte
index|[]
name|row
init|=
name|Bytes
operator|.
name|toBytes
argument_list|(
literal|"row1"
argument_list|)
decl_stmt|;
comment|// write some data
name|batchUpdate
operator|=
operator|new
name|BatchUpdate
argument_list|(
name|row
argument_list|)
expr_stmt|;
name|batchUpdate
operator|.
name|put
argument_list|(
name|COLUMNS
index|[
literal|0
index|]
argument_list|,
literal|"olderValue"
operator|.
name|getBytes
argument_list|()
argument_list|)
expr_stmt|;
name|region
operator|.
name|batchUpdate
argument_list|(
name|batchUpdate
argument_list|,
literal|null
argument_list|)
expr_stmt|;
comment|// flush
name|region
operator|.
name|flushcache
argument_list|()
expr_stmt|;
comment|// assert that getFull gives us the older value
name|results
operator|=
name|region
operator|.
name|getFull
argument_list|(
name|row
argument_list|,
operator|(
name|Set
argument_list|<
name|byte
index|[]
argument_list|>
operator|)
literal|null
argument_list|,
name|LATEST_TIMESTAMP
argument_list|,
literal|null
argument_list|)
expr_stmt|;
name|assertEquals
argument_list|(
literal|"olderValue"
argument_list|,
operator|new
name|String
argument_list|(
name|results
operator|.
name|get
argument_list|(
name|COLUMNS
index|[
literal|0
index|]
argument_list|)
operator|.
name|getValue
argument_list|()
argument_list|)
argument_list|)
expr_stmt|;
comment|// write a new value for the cell
name|batchUpdate
operator|=
operator|new
name|BatchUpdate
argument_list|(
name|row
argument_list|)
expr_stmt|;
name|batchUpdate
operator|.
name|put
argument_list|(
name|COLUMNS
index|[
literal|0
index|]
argument_list|,
literal|"newerValue"
operator|.
name|getBytes
argument_list|()
argument_list|)
expr_stmt|;
name|region
operator|.
name|batchUpdate
argument_list|(
name|batchUpdate
argument_list|,
literal|null
argument_list|)
expr_stmt|;
comment|// flush
name|region
operator|.
name|flushcache
argument_list|()
expr_stmt|;
comment|// assert that getFull gives us the later value
name|results
operator|=
name|region
operator|.
name|getFull
argument_list|(
name|row
argument_list|,
operator|(
name|Set
argument_list|<
name|byte
index|[]
argument_list|>
operator|)
literal|null
argument_list|,
name|LATEST_TIMESTAMP
argument_list|,
literal|null
argument_list|)
expr_stmt|;
name|assertEquals
argument_list|(
literal|"newerValue"
argument_list|,
operator|new
name|String
argument_list|(
name|results
operator|.
name|get
argument_list|(
name|COLUMNS
index|[
literal|0
index|]
argument_list|)
operator|.
name|getValue
argument_list|()
argument_list|)
argument_list|)
expr_stmt|;
comment|//
comment|// Test the delete masking issue
comment|//
name|byte
index|[]
name|row2
init|=
name|Bytes
operator|.
name|toBytes
argument_list|(
literal|"row2"
argument_list|)
decl_stmt|;
name|byte
index|[]
name|cell1
init|=
name|Bytes
operator|.
name|toBytes
argument_list|(
name|Bytes
operator|.
name|toString
argument_list|(
name|COLUMNS
index|[
literal|0
index|]
argument_list|)
operator|+
literal|"a"
argument_list|)
decl_stmt|;
name|byte
index|[]
name|cell2
init|=
name|Bytes
operator|.
name|toBytes
argument_list|(
name|Bytes
operator|.
name|toString
argument_list|(
name|COLUMNS
index|[
literal|0
index|]
argument_list|)
operator|+
literal|"b"
argument_list|)
decl_stmt|;
name|byte
index|[]
name|cell3
init|=
name|Bytes
operator|.
name|toBytes
argument_list|(
name|Bytes
operator|.
name|toString
argument_list|(
name|COLUMNS
index|[
literal|0
index|]
argument_list|)
operator|+
literal|"c"
argument_list|)
decl_stmt|;
comment|// write some data at two columns
name|batchUpdate
operator|=
operator|new
name|BatchUpdate
argument_list|(
name|row2
argument_list|)
expr_stmt|;
name|batchUpdate
operator|.
name|put
argument_list|(
name|cell1
argument_list|,
literal|"column0 value"
operator|.
name|getBytes
argument_list|()
argument_list|)
expr_stmt|;
name|batchUpdate
operator|.
name|put
argument_list|(
name|cell2
argument_list|,
literal|"column1 value"
operator|.
name|getBytes
argument_list|()
argument_list|)
expr_stmt|;
name|region
operator|.
name|batchUpdate
argument_list|(
name|batchUpdate
argument_list|,
literal|null
argument_list|)
expr_stmt|;
comment|// flush
name|region
operator|.
name|flushcache
argument_list|()
expr_stmt|;
comment|// assert i get both columns
name|results
operator|=
name|region
operator|.
name|getFull
argument_list|(
name|row2
argument_list|,
operator|(
name|Set
argument_list|<
name|byte
index|[]
argument_list|>
operator|)
literal|null
argument_list|,
name|LATEST_TIMESTAMP
argument_list|,
literal|null
argument_list|)
expr_stmt|;
name|assertEquals
argument_list|(
literal|"Should have two columns in the results map"
argument_list|,
literal|2
argument_list|,
name|results
operator|.
name|size
argument_list|()
argument_list|)
expr_stmt|;
name|assertEquals
argument_list|(
literal|"column0 value"
argument_list|,
operator|new
name|String
argument_list|(
name|results
operator|.
name|get
argument_list|(
name|cell1
argument_list|)
operator|.
name|getValue
argument_list|()
argument_list|)
argument_list|)
expr_stmt|;
name|assertEquals
argument_list|(
literal|"column1 value"
argument_list|,
operator|new
name|String
argument_list|(
name|results
operator|.
name|get
argument_list|(
name|cell2
argument_list|)
operator|.
name|getValue
argument_list|()
argument_list|)
argument_list|)
expr_stmt|;
comment|// write a delete for the first column
name|batchUpdate
operator|=
operator|new
name|BatchUpdate
argument_list|(
name|row2
argument_list|)
expr_stmt|;
name|batchUpdate
operator|.
name|delete
argument_list|(
name|cell1
argument_list|)
expr_stmt|;
name|batchUpdate
operator|.
name|put
argument_list|(
name|cell2
argument_list|,
literal|"column1 new value"
operator|.
name|getBytes
argument_list|()
argument_list|)
expr_stmt|;
name|region
operator|.
name|batchUpdate
argument_list|(
name|batchUpdate
argument_list|,
literal|null
argument_list|)
expr_stmt|;
comment|// flush
name|region
operator|.
name|flushcache
argument_list|()
expr_stmt|;
comment|// assert i get the second column only
name|results
operator|=
name|region
operator|.
name|getFull
argument_list|(
name|row2
argument_list|,
operator|(
name|Set
argument_list|<
name|byte
index|[]
argument_list|>
operator|)
literal|null
argument_list|,
name|LATEST_TIMESTAMP
argument_list|,
literal|null
argument_list|)
expr_stmt|;
name|assertEquals
argument_list|(
literal|"Should have one column in the results map"
argument_list|,
literal|1
argument_list|,
name|results
operator|.
name|size
argument_list|()
argument_list|)
expr_stmt|;
name|assertNull
argument_list|(
literal|"column0 value"
argument_list|,
name|results
operator|.
name|get
argument_list|(
name|cell1
argument_list|)
argument_list|)
expr_stmt|;
name|assertEquals
argument_list|(
literal|"column1 new value"
argument_list|,
operator|new
name|String
argument_list|(
name|results
operator|.
name|get
argument_list|(
name|cell2
argument_list|)
operator|.
name|getValue
argument_list|()
argument_list|)
argument_list|)
expr_stmt|;
comment|//
comment|// Include a delete and value from the memcache in the mix
comment|//
name|batchUpdate
operator|=
operator|new
name|BatchUpdate
argument_list|(
name|row2
argument_list|)
expr_stmt|;
name|batchUpdate
operator|.
name|delete
argument_list|(
name|cell2
argument_list|)
expr_stmt|;
name|batchUpdate
operator|.
name|put
argument_list|(
name|cell3
argument_list|,
literal|"column2 value!"
operator|.
name|getBytes
argument_list|()
argument_list|)
expr_stmt|;
name|region
operator|.
name|batchUpdate
argument_list|(
name|batchUpdate
argument_list|,
literal|null
argument_list|)
expr_stmt|;
comment|// assert i get the third column only
name|results
operator|=
name|region
operator|.
name|getFull
argument_list|(
name|row2
argument_list|,
operator|(
name|Set
argument_list|<
name|byte
index|[]
argument_list|>
operator|)
literal|null
argument_list|,
name|LATEST_TIMESTAMP
argument_list|,
literal|null
argument_list|)
expr_stmt|;
name|assertEquals
argument_list|(
literal|"Should have one column in the results map"
argument_list|,
literal|1
argument_list|,
name|results
operator|.
name|size
argument_list|()
argument_list|)
expr_stmt|;
name|assertNull
argument_list|(
literal|"column0 value"
argument_list|,
name|results
operator|.
name|get
argument_list|(
name|cell1
argument_list|)
argument_list|)
expr_stmt|;
name|assertNull
argument_list|(
literal|"column1 value"
argument_list|,
name|results
operator|.
name|get
argument_list|(
name|cell2
argument_list|)
argument_list|)
expr_stmt|;
name|assertEquals
argument_list|(
literal|"column2 value!"
argument_list|,
operator|new
name|String
argument_list|(
name|results
operator|.
name|get
argument_list|(
name|cell3
argument_list|)
operator|.
name|getValue
argument_list|()
argument_list|)
argument_list|)
expr_stmt|;
block|}
finally|finally
block|{
if|if
condition|(
name|region
operator|!=
literal|null
condition|)
block|{
try|try
block|{
name|region
operator|.
name|close
argument_list|()
expr_stmt|;
block|}
catch|catch
parameter_list|(
name|Exception
name|e
parameter_list|)
block|{
name|e
operator|.
name|printStackTrace
argument_list|()
expr_stmt|;
block|}
name|region
operator|.
name|getLog
argument_list|()
operator|.
name|closeAndDelete
argument_list|()
expr_stmt|;
block|}
block|}
block|}
specifier|private
name|void
name|assertColumnsPresent
parameter_list|(
specifier|final
name|HRegion
name|r
parameter_list|,
specifier|final
name|byte
index|[]
name|row
parameter_list|)
throws|throws
name|IOException
block|{
name|Map
argument_list|<
name|byte
index|[]
argument_list|,
name|Cell
argument_list|>
name|result
init|=
name|r
operator|.
name|getFull
argument_list|(
name|row
argument_list|,
literal|null
argument_list|,
name|HConstants
operator|.
name|LATEST_TIMESTAMP
argument_list|,
literal|null
argument_list|)
decl_stmt|;
name|int
name|columnCount
init|=
literal|0
decl_stmt|;
for|for
control|(
name|Map
operator|.
name|Entry
argument_list|<
name|byte
index|[]
argument_list|,
name|Cell
argument_list|>
name|e
range|:
name|result
operator|.
name|entrySet
argument_list|()
control|)
block|{
name|columnCount
operator|++
expr_stmt|;
name|byte
index|[]
name|column
init|=
name|e
operator|.
name|getKey
argument_list|()
decl_stmt|;
name|boolean
name|legitColumn
init|=
literal|false
decl_stmt|;
for|for
control|(
name|int
name|i
init|=
literal|0
init|;
name|i
operator|<
name|COLUMNS
operator|.
name|length
condition|;
name|i
operator|++
control|)
block|{
comment|// Assert value is same as row.  This is 'nature' of the data added.
name|assertTrue
argument_list|(
name|Bytes
operator|.
name|equals
argument_list|(
name|row
argument_list|,
name|e
operator|.
name|getValue
argument_list|()
operator|.
name|getValue
argument_list|()
argument_list|)
argument_list|)
expr_stmt|;
if|if
condition|(
name|Bytes
operator|.
name|equals
argument_list|(
name|COLUMNS
index|[
name|i
index|]
argument_list|,
name|column
argument_list|)
condition|)
block|{
name|legitColumn
operator|=
literal|true
expr_stmt|;
break|break;
block|}
block|}
name|assertTrue
argument_list|(
literal|"is legit column name"
argument_list|,
name|legitColumn
argument_list|)
expr_stmt|;
block|}
name|assertEquals
argument_list|(
literal|"count of columns"
argument_list|,
name|columnCount
argument_list|,
name|COLUMNS
operator|.
name|length
argument_list|)
expr_stmt|;
block|}
annotation|@
name|Override
specifier|protected
name|void
name|tearDown
parameter_list|()
throws|throws
name|Exception
block|{
if|if
condition|(
name|this
operator|.
name|miniHdfs
operator|!=
literal|null
condition|)
block|{
name|this
operator|.
name|miniHdfs
operator|.
name|shutdown
argument_list|()
expr_stmt|;
block|}
name|super
operator|.
name|tearDown
argument_list|()
expr_stmt|;
block|}
block|}
end_class

end_unit

