...
argument_list|)
decl_stmt|;
comment|// we don't want to fill up the logs with this message, so only log it
comment|// once every 30 seconds at most
comment|// I also want to avoid locks on the 'critical path' (the common case will be
comment|// uncontended) - hence the CAS
specifier|private
specifier|static
name|void
name|logDroppedLatencyStat
parameter_list|()
block|{
specifier|final
name|long
...
name|warn
argument_list|(
literal|"Dropping fs latency stats since buffer is full"
argument_list|)
expr_stmt|;
break|break;
block|}
comment|// otherwise (if the compaseAndSet failed) the while loop retries
block|}
else|else
block|{
break|break;
