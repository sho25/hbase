...
name|blockCache
decl_stmt|;
comment|// Byte buffer output stream made per block written.
specifier|private
name|ByteBufferOutputStream
name|bbos
init|=
...
condition|)
return|return;
comment|// Flush Data Output Stream
name|bbosDos
operator|.
name|flush
argument_list|()
expr_stmt|;
comment|// Compress Data and write to output stream
name|DataOutputStream
name|compressStream
init|=
...
literal|null
expr_stmt|;
comment|// to avoid too many calls to realloc(),
comment|// pre-allocates the byte stream to the block size + 25%
comment|// only if blocksize is under 1Gb
name|int
name|bbosBlocksize
init|=
name|Math
...
literal|false
decl_stmt|;
comment|// Whether blocks of file should be evicted on close of file
specifier|private
specifier|final
name|boolean
name|evictOnClose
...
return|;
block|}
comment|/*      * Read in the index that is at<code>indexOffset</code>      * Must match what was written by writeIndex in the Writer.close.      * @param c Comparator to use.      * @param in      * @param indexSize      * @throws IOException      */
specifier|static
name|BlockIndex
name|readIndex
parameter_list|(
...
name|println
argument_list|(
literal|"Could not get bloom data from meta block"
argument_list|)
expr_stmt|;
block|}
block|}
