...
name|SIZEOF_LONG
decl_stmt|;
comment|/**    * We store a two-byte encoder ID at the beginning of every encoded data    * block payload (immediately after the block header).    */
specifier|public
specifier|static
specifier|final
name|int
...
name|includesMemstoreTS
decl_stmt|;
comment|/**      * Unencoded data block for caching on write. Populated before encoding.      */
specifier|private
name|HFileBlock
name|unencodedDataBlockForCaching
decl_stmt|;
...
condition|)
block|{
comment|// We now know the final on-disk size, save it for caching.
name|unencodedDataBlockForCaching
operator|.
name|onDiskSizeWithoutHeader
operator|=
...
block|}
block|}
comment|/**      * Do compression if it is enabled, or re-use the uncompressed buffer if      * it is not. Fills in the compressed block's header if doing compression.      */
specifier|private
name|void
name|doCompression
parameter_list|()
...
argument_list|)
expr_stmt|;
comment|// finish compression stream
name|compressingOutputStream
operator|.
name|flush
argument_list|()
...
block|{
return|return;
comment|// skip any non-data block
block|}
...
condition|)
block|{
comment|// Save the unencoded block in case we need to cache it on write.
comment|// We don't know the final on-disk size at this point, because
comment|// compression has not been done yet, to set it to uncompressed size
comment|// and override later.
name|int
name|uncompressedSizeWithoutHeader
init|=
name|getUncompressedSizeWithoutHeader
...
name|uncompressedSizeWithoutHeader
argument_list|,
comment|// will override this later
name|uncompressedSizeWithoutHeader
argument_list|,
name|prevOffset
argument_list|,
...
block|{
comment|// There is no encoding configured. Do some extra sanity-checking.
if|if
condition|(
name|encodedBlockType
operator|!=
...
name|IOException
argument_list|(
literal|"Unexpected block type coming out of data "
operator|+
literal|"block encoder: "
operator|+
name|encodedBlockType
argument_list|)
throw|;
