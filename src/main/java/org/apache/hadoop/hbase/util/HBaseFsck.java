...
expr_stmt|;
block|}
comment|//modTInfo.addEdge(hbi.metaEntry.getStartKey(), hbi.metaEntry.getEndKey());
name|modTInfo
operator|.
name|addRegionInfo
argument_list|(
...
condition|)
block|{
comment|// This is the first region, check that the start key is empty
if|if
condition|(
operator|!
name|Bytes
...
block|{
comment|// Check if endKey< startKey
comment|// Previous implementation of this code checked for a cycle in the
comment|// region chain.  A cycle would imply that the endKey comes before
comment|// the startKey (i.e. endKey< startKey).
if|if
condition|(
operator|!
name|Bytes
...
condition|)
block|{
comment|// continue with this check if this is not the last region
name|int
name|cmpRegionKeys
init|=
name|Bytes
...
name|DUPE_STARTKEYS
argument_list|,
literal|"Two regions have the same startkey: "
operator|+
name|Bytes
operator|.
name|toStringBinary
...
block|{
comment|// Check that the startkey is the same as the previous end key
name|int
name|cmp
init|=
name|Bytes
...
argument_list|,
literal|"There is a hole in the region chain."
argument_list|,
name|this
argument_list|,
name|r
