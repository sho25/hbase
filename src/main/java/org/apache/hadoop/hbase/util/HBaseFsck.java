...
argument_list|()
decl_stmt|;
comment|/**    * This map from Tablename -> TableInfo contains the structures necessary to    * detect table consistency problems (holes, dupes, overlaps).  It is sorted    * to prevent dupes.    */
specifier|private
name|SortedMap
argument_list|<
name|String
...
argument_list|()
operator|+
literal|" is not in the include table "
operator|+
literal|"list.  Just suggesting fixes."
argument_list|)
expr_stmt|;
name|handler
operator|=
...
name|descriptiveName
operator|+
literal|" is a split parent in META and in HDFS"
argument_list|)
expr_stmt|;
return|return;
block|}
...
name|println
argument_list|(
literal|"To be bulk loaded sidelined region dir: "
operator|+
name|k
operator|.
name|toString
...
name|reportError
argument_list|(
literal|"Fatal error: unable to get root region location. Exiting..."
argument_list|)
expr_stmt|;
return|return
literal|false
...
expr_stmt|;
return|return;
comment|// TODO convert this in to a future
block|}
name|hbck
operator|.
name|orphanHdfsDirs
...
return|;
block|}
comment|/**    * Only fix tables specified by the list    */
name|boolean
name|shouldFixTable
parameter_list|(
name|byte
...
return|;
block|}
comment|// doing this naively since byte[] equals may not do what we want.
for|for
control|(
name|byte
index|[]
...
name|println
argument_list|(
literal|"  Repair options: (expert features, use with caution!)"
argument_list|)
expr_stmt|;
name|System
operator|.
...
operator|+
literal|"-fixHdfsOrphans -fixHdfsOverlaps -fixVersionFile -sidelineBigOverlaps"
argument_list|)
expr_stmt|;
name|System
operator|.
