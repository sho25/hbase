...
parameter_list|)
function_decl|;
comment|/**      * Split an entire table.      *      * @param numRegions      *          number of regions to split the table into      *      * @return array of split keys for the initial regions of the table. The length of the      * returned array should be numRegions-1.      */
name|byte
index|[]
index|[]
name|split
...
return|;
block|}
comment|/**    * HexStringSplit is one possible {@link SplitAlgorithm} for choosing region    * boundaries. The format of a HexStringSplit region boundary is the    * ASCII representation of an MD5 checksum, or any other uniformly distributed    * bytes. Row are hex-encoded long values in the range<b>"00000000" =>    * "FFFFFFFF"</b> and are left-padded with zeros to keep the same order    * lexicographically as if they were binary.    *    * This split algorithm is only appropriate if you will use hex strings as    * keys.    */
specifier|public
specifier|static
class|class
name|HexStringSplit
...
block|}
block|}
comment|/**    * A SplitAlgorithm that divides the space of possible keys evenly. Useful    * when the keys are approximately uniform random bytes (e.g. hashes).    * You probably shouldn't use this if your keys are ASCII, or if your keys    * tend to have similar prefixes.    */
specifier|public
specifier|static
class|class
name|UniformSplit
