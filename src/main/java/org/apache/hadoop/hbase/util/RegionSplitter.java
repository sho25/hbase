...
argument_list|)
decl_stmt|;
comment|/**    * A generic interface for the RegionSplitter code to use for all it's    * functionality. Note that the original authors of this code use    * {@link MD5StringSplit} to partition their table and set it as default, but    * provided this for your custom algorithm. To use, create a new derived class    * from this interface and call the RegionSplitter class with the argument:<br>    *<b>-D split.algorithm=<your_class_path></b>    */
specifier|public
specifier|static
interface|interface
name|SplitAlgorithm
block|{
comment|/**      * Split a pre-existing region into 2 regions.      *      * @param start      *          row      * @param end      *          row      * @return the split row to use      */
name|byte
index|[]
name|split
parameter_list|(
...
parameter_list|)
function_decl|;
comment|/**      * Split an entire table.      *      * @param numberOfSplits      *          number of regions to split the table into      *      * @return array of split keys for the initial regions of the table      */
name|byte
index|[]
index|[]
name|split
...
argument_list|)
expr_stmt|;
comment|// round-robin through the RS list
for|for
control|(
name|HServerAddress
name|rsLoc
...
condition|)
block|{
comment|// wait for one of the daughter regions to come online
name|boolean
name|daughterOnline
init|=
literal|false
...
name|debug
argument_list|(
literal|"Waiting for daughter region to come online..."
argument_list|)
expr_stmt|;
name|table
operator|.
...
name|debug
argument_list|(
literal|"Daughter region is online."
argument_list|)
expr_stmt|;
block|}
comment|// mark the region as successfully split.
comment|// NOTE: split done, but daughter regions still need to major compact
name|splitOut
operator|.
name|writeChars
argument_list|(
...
condition|)
block|{
comment|// if we have too many outstanding splits, wait for oldest ones to
comment|// finish
name|outstanding
operator|.
name|addLast
...
argument_list|()
decl_stmt|;
comment|// figure out where this region should be in HDFS
name|check
operator|.
name|add
argument_list|(
...
name|debug
argument_list|(
literal|"Reference still exists for "
operator|+
name|startKey
operator|+
literal|" at "
...
name|debug
argument_list|(
literal|"- finished compaction of "
operator|+
name|startKey
argument_list|)
expr_stmt|;
block|}
block|}
comment|// sleep in between requests
if|if
condition|(
operator|!
name|check
...
return|;
block|}
comment|/**    * MD5StringSplit is the default {@link SplitAlgorithm} for creating pre-split    * tables. The format of MD5StringSplit is the ASCII representation of an MD5    * checksum. Row are long values in the range<b>"00000000" => "7FFFFFFF"</b>    * and are left-padded with zeros to keep the same order lexographically as if    * they were binary.    */
specifier|public
specifier|static
class|class
name|MD5StringSplit
...
comment|// This is not a big deal if we're assuming n<< MAXMD5
name|splits
index|[
name|i
operator|-
...
return|;
block|}
comment|/**      * Returns an array of bytes corresponding to an array of BigIntegers      *      * @param bigIntegers      * @return bytes corresponding to the bigIntegers      */
specifier|static
name|byte
index|[]
index|[]
...
return|;
block|}
comment|/**      * Returns the bytes corresponding to the BigInteger      *      * @param bigInteger      * @return byte corresponding to input BigInteger      */
specifier|static
name|byte
index|[]
name|convertToByte
...
return|;
block|}
comment|/**      * Returns the BigInteger represented by thebyte array      *      * @param row      * @return the corresponding BigInteger      */
specifier|static
name|BigInteger
name|convertToBigInteger
parameter_list|(
