...
argument_list|)
decl_stmt|;
comment|// Lock to manage when a HLog is changing path (archiving)
specifier|private
specifier|final
name|ReentrantLock
name|pathLock
...
comment|// the handling story of that queue in the form of id-startcode-*
specifier|private
name|void
name|checkIfQueueRecovered
parameter_list|(
...
parameter_list|)
block|{
comment|// in sync with queue polling
name|this
operator|.
name|pathLock
operator|.
...
name|debug
argument_list|(
literal|"Current log moved, changing currentPath to "
operator|+
name|newPath
argument_list|)
expr_stmt|;
...
name|present
condition|?
literal|"present, changing the queue"
else|:
literal|"already processed"
operator|)
argument_list|)
expr_stmt|;
if|if
...
condition|)
block|{
comment|// In sync with logArchived
name|this
operator|.
name|pathLock
operator|.
...
literal|false
decl_stmt|;
comment|// TODO this is a hack for HDFS-1057
name|currentNbEntries
operator|=
literal|0
expr_stmt|;
...
name|warn
argument_list|(
literal|"Unable to replicate because "
argument_list|,
name|ioe
argument_list|)
expr_stmt|;
...
name|debug
argument_list|(
literal|"The region server we tried to ping didn't answer, "
operator|+
literal|"sleeping "
operator|+
name|sleepForRetries
