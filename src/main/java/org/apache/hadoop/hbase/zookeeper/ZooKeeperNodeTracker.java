...
expr_stmt|;
block|}
comment|/**    * Starts the tracking of the node in ZooKeeper.    *    *<p>Use {@link #blockUntilAvailable()} to block until the node is available    * or {@link #getData(boolean)} to get the data of the node if it is available.    *     * @param allowAbort If allowAbort is false, the abortable should not abort when a    *          KeeperException occur.     * @return start result. true if start successfully.    */
specifier|public
