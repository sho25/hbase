...
block|}
block|}
comment|/**    * Returns the date of child znodes of the specified znode.  Also sets a watch on    * the specified znode which will capture a NodeDeleted event on the specified    * znode as well as NodeChildrenChanged if any children of the specified znode    * are created or deleted.    *    * Returns null if the specified node does not exist.  Otherwise returns a    * list of children of the specified node.  If the node exists but it has no    * children, an empty list will be returned.    *    * @param zkw zk reference    * @param baseNode path of node to list and watch children of    * @return list of data of children of the specified node, an empty list if the node    *          exists but has no children, and null if the node does not exist    * @throws KeeperException if unexpected zookeeper exception    */
specifier|public
specifier|static
name|List
argument_list|<
...
parameter_list|)
block|{
comment|// TODO: We need a better check for security enabled ZooKeeper. Currently
comment|// the secure ZooKeeper client is set up using a supplied JaaS
comment|// configuration file. But if the system property for the JaaS
comment|// configuration file is set, this may not be an exclusive indication
comment|// that HBase should set ACLs on znodes. As an alternative, we could do
comment|// this more like Hadoop and build a JaaS configuration programmatically
comment|// based on a site conf setting. The scope of such a change will be
comment|// addressed in HBASE-4791.
return|return
operator|(
name|System
operator|.
...
condition|)
block|{
comment|// Certain znodes must be readable by non-authenticated clients
if|if
condition|(
operator|(
name|node
...
name|debug
argument_list|(
literal|"Waiting for ZooKeeperWatcher to authenticate"
argument_list|)
expr_stmt|;
name|zkw
operator|.
...
return|;
block|}
comment|/**    * Get a ServerName from the passed in znode data bytes.    * @param data ZNode data with a server name in it; can handle the old style    * servername where servername was host and port.  Works too with data that    * begins w/ the pb 'PBUF' magic and that its then followed by a protobuf that    * has a serialized {@link ServerName} in it.    * @return Returns null if<code>data</code> is null else converts passed data    * to a ServerName instance.    */
specifier|public
specifier|static
name|ServerName
