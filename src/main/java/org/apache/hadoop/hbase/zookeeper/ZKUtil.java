...
return|;
block|}
comment|/**    * Get the unique node-name for the specified regionserver.    *    * Used when a server puts up an ephemeral node for itself and needs to use    * a unique name.    *    * @param serverInfo server information    * @return unique, zookeeper-safe znode path for the server instance    */
specifier|public
specifier|static
name|String
name|getNodeName
...
return|;
block|}
comment|/**    * Atomically add watches and read data from all unwatched unassigned nodes.    *    *<p>This works because master is the only person deleting nodes.    */
specifier|public
specifier|static
name|List
argument_list|<
