...
try|try
block|{
comment|// Initialize threads and wait for a master
name|initialize
argument_list|()
expr_stmt|;
block|}
...
begin_comment
comment|/**    * Get the current master from ZooKeeper and open the RPC connection to it.    *    * Method will block until a master is available. You can break from this    * block by requesting the server stop.    *    * @return master address, or null if server has been stopped    */
end_comment
...
argument_list|()
expr_stmt|;
comment|// TODO catch exceptions so we can report them on a per-item basis.
if|if
condition|(
name|action
operator|instanceof
...
expr_stmt|;
block|}
comment|// TODO turning the alternate exception into a different result
name|response
operator|.
name|add
argument_list|(
