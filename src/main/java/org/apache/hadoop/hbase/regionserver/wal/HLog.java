...
name|prefix
decl_stmt|;
comment|/** tracking information about what has been synced */
specifier|private
name|SyncInfo
name|syncInfo
init|=
...
begin_comment
comment|/**    * Method used internal to this class and for tests only.    * @return The wrapped stream our writer is using; its not the    * writer's 'out' FSDatoOutputStream but the stream that this 'out' wraps    * (In hdfs its an instance of DFSDataOutputStream).    */
end_comment
...
begin_comment
comment|/** Append an entry to the log.    *    * @param regionInfo    * @param logEdit    * @param logKey    * @param doSync shall we sync after writing the transaction    * @return The seqnum of this transaction    * @throws IOException    */
end_comment
...
begin_comment
comment|/**    * Append a set of edits to the log. Log edits are keyed by (encoded)    * regionName, rowname, and log-sequence-id. The HLog is flushed    * after this transaction is written to the log.    *    * @param info    * @param tableName    * @param edits    * @param clusterId The originating clusterId for this edit (for replication)    * @param now    * @return txid of this transaction    * @throws IOException    */
end_comment
...
condition|)
block|{
comment|// push back our batch into the pending list
synchronized|synchronized
init|(
name|this
init|)
...
argument_list|)
expr_stmt|;
comment|// We try to not acquire the updateLock just to update statistics.
comment|// Make these statistics as AtomicLong.
name|syncTime
operator|.
name|inc
argument_list|(
...
parameter_list|)
block|{
comment|// The data is not actually used here - we just need to write
comment|// something to the log to make sure we're still the owner of the
comment|// pipeline.
name|byte
index|[]
name|data
