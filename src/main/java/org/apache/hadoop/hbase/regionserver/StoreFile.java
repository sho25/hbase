...
decl_stmt|;
name|boolean
name|enableOffHeapCache
init|=
name|conf
operator|.
name|getBoolean
argument_list|(
literal|"hbase.offheapcache.enable"
argument_list|,
literal|false
argument_list|)
decl_stmt|;
...
name|info
argument_list|(
literal|"Allocating OffHeapCache with maximum size "
operator|+
name|StringUtils
operator|.
name|humanReadableInt
...
return|;
block|}
comment|/**    * Create a store file writer. Client is responsible for closing file when done.    * If metadata, add BEFORE closing using appendMetadata()    * @param fs    * @param dir Path to family directory.  Makes the directory if doesn't exist.    * Creates a file with a unique name in this directory.    * @param blocksize    * @param algorithm Pass null to get default.    * @param c Pass null to get default.    * @param conf HBase system configuration. used with bloom filters    * @param bloomType column family setting for bloom filters    * @param maxKeyCount estimated maximum number of keys we expect to add    * @return HFile.Writer    * @throws IOException    */
specifier|public
specifier|static
name|StoreFile
operator|.
