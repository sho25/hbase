...
expr_stmt|;
block|}
comment|/**    * @param delete The Delete command    * @param familyMap map of family to edits for the given family.    * @param writeToWAL    * @param writeEntry Optional mvcc write point to use    * @param walEdit Optional walEdit to use. A non-null walEdit indicates    * that the coprocessor hooks are run by the caller    * @throws IOException    */
specifier|private
name|void
name|internalDelete
parameter_list|(
...
expr_stmt|;
block|}
comment|/**    * Add updates first to the hlog (if writeToWal) and then add values to memstore.    * Warning: Assumption is caller has lock on passed in row.    * @param put The Put command    * @param writeToWAL if true, then we should write to the log    * @param writeEntry Optional mvcc write point to use    * @param walEdit Optional walEdit to use. A non-null walEdit indicates    * that the coprocessor hooks are run by the caller    * @throws IOException    */
specifier|private
name|void
name|internalPut
parameter_list|(
...
comment|// Note that this requires creating the WALEdits here and passing
comment|// them to the actual Put/Delete operations.
for|for
control|(
name|Mutation
name|m
...
block|}
block|}
comment|// 2. acquire the row lock
name|Integer
name|lid
init|=
name|getLock
...
literal|0
decl_stmt|;
comment|// 5. Perform the actual mutations
for|for
control|(
name|Mutation
name|m
...
try|try
block|{
comment|// 8. run all coprocessor post hooks
if|if
condition|(
name|coprocessorHost
operator|!=
...
condition|)
block|{
comment|// 9. release the row lock
name|releaseRowLock
argument_list|(
name|lid
argument_list|)
