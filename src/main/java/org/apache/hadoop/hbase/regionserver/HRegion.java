...
block|}
block|}
comment|// 6. append/sync all edits at once
comment|// TODO: Do batching as in doMiniBatchPut
name|this
operator|.
name|log
operator|.
...
expr_stmt|;
block|}
comment|// 10. run all coprocessor post hooks, after region lock is released
if|if
condition|(
name|coprocessorHost
operator|!=
...
condition|)
block|{
comment|// 11. release the row lock
for|for
control|(
name|Integer
name|lid
...
condition|)
block|{
comment|// 12. Flush cache if needed. Do it outside update lock.
name|requestFlush
argument_list|()
expr_stmt|;
block|}
