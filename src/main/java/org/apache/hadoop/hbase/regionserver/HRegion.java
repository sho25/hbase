...
block|}
block|}
comment|// append the edit to WAL. The append also does the sync.
if|if
condition|(
operator|!
name|walEdit
...
argument_list|()
decl_stmt|;
comment|// check if column families are valid;
comment|// check if any timestampupdates are needed;
comment|// and if writeToWAL is set, then also collapse edits into a single list.
for|for
control|(
name|Map
operator|.
...
argument_list|()
decl_stmt|;
comment|// is this a valid column family?
name|checkFamily
argument_list|(
name|family
argument_list|)
...
block|}
block|}
comment|// append to and sync WAL
if|if
condition|(
operator|!
name|walEdit
