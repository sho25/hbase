...
comment|// mark as success and skip below
name|batchOp
operator|.
name|retCodeDetails
index|[
...
operator|++
expr_stmt|;
comment|// If first time around, designate a prefix for metrics based on the CF
comment|// set. After that, watch for inconsistencies.
specifier|final
name|String
name|curMetricPrefix
init|=
name|SchemaMetrics
operator|.
name|generateSchemaMetricsPrefix
...
condition|)
block|{
comment|// The column family set for this batch put is undefined.
name|cfSetConsistent
operator|=
literal|false
expr_stmt|;
...
try|try
block|{
comment|// 1. run all pre-hooks before the atomic operation
comment|// if any pre hook indicates "bypass", bypass the entire operation
comment|// one WALEdit is used for all edits.
name|WALEdit
name|walEdit
init|=
operator|new
...
expr_stmt|;
block|}
comment|// 3. acquire the region lock
name|this
operator|.
name|updatesLock
operator|.
...
literal|true
expr_stmt|;
comment|// 4. Get a mvcc write number
name|MultiVersionConsistencyControl
operator|.
name|WriteEntry
name|w
...
try|try
block|{
comment|// 5. Check mutations and apply edits to a single WALEdit
for|for
control|(
name|Mutation
name|m
...
block|}
block|}
comment|// 6. append all edits at once (don't sync)
if|if
condition|(
name|walEdit
operator|.
...
argument_list|)
expr_stmt|;
comment|// 8. release region and row lock(s)
name|this
operator|.
name|updatesLock
operator|.
...
expr_stmt|;
block|}
comment|// 9. sync WAL if required
if|if
condition|(
name|walEdit
operator|.
...
literal|null
expr_stmt|;
comment|// 11. run coprocessor post host hooks
comment|// after the WAL is sync'ed and all locks are released
comment|// (similar to doMiniBatchPut)
if|if
condition|(
name|coprocessorHost
operator|!=
...
name|info
argument_list|(
literal|"mutateRowWithLocks: rolled back "
operator|+
name|kvsRolledback
operator|+
literal|" KeyValues"
...
condition|)
block|{
comment|// 13. Flush cache if needed. Do it outside update lock.
name|requestFlush
argument_list|()
expr_stmt|;
block|}
...
try|try
block|{
comment|// 3. Let the processor scan the row and generate mutations
name|WALEdit
name|walEdits
init|=
operator|new
...
name|IOException
argument_list|(
literal|"Processor is readOnly but generating mutations on row:"
operator|+
name|Bytes
operator|.
name|toStringBinary
...
argument_list|()
expr_stmt|;
comment|// 5. Apply to memstore and a WALEdit
for|for
control|(
name|KeyValue
name|kv
...
argument_list|()
operator|+
literal|" memstore keyvalues for row:"
operator|+
name|processor
operator|.
name|getRow
...
name|error
argument_list|(
literal|"RowProcessor timeout on row:"
operator|+
name|Bytes
operator|.
name|toStringBinary
