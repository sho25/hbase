...
block|}
block|}
comment|// TODO fix this not to use QueryMatcher!
comment|/**    * Gets from either the memstore or the snapshop, and returns a code    * to let you know which is which.    *    * @param matcher query matcher    * @param result puts results here    * @return 1 == memstore, 2 == snapshot, 0 == none    */
name|int
name|getWithCode
parameter_list|(
name|QueryMatcher
...
expr_stmt|;
block|}
comment|/**    *    * @param set memstore or snapshot    * @param matcher query matcher    * @param result list to add results to    * @return true if done with store (early-out), false if not    */
name|boolean
name|internalGet
parameter_list|(
specifier|final
