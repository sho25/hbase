...
block|}
block|}
comment|/**      * Get this watcher's ZKW, instantiate it if necessary.      * @return ZKW      * @throws java.io.IOException if a remote or network exception occurs      */
specifier|public
specifier|synchronized
name|ZooKeeperWrapper
...
block|}
block|}
comment|/**    * It is provided for unit test cases which verify the behavior of region    * location cache prefetch.    * @return Number of cached regions for the table.    */
specifier|static
name|int
...
return|;
block|}
comment|/**    * It's provided for unit test cases which verify the behavior of region    * location cache prefetch.    * @return true if the region where the table and row reside is cached.    */
specifier|static
name|boolean
name|isRegionCached
parameter_list|(
...
argument_list|()
decl_stmt|;
comment|// Don't print stack trace if NSRE; NSRE is 'normal' operation.
if|if
condition|(
name|cause
operator|instanceof
...
condition|)
block|{
comment|// msg is the exception as a String... we just want first line.
name|msg
operator|=
name|msg
operator|.
...
name|debug
argument_list|(
literal|"Failed execution of all on "
operator|+
name|request
operator|.
name|address
