...
control|)
block|{
comment|// An offlined region will not be present out on a regionserver.  A region
comment|// is offlined if table is offlined -- will still have an entry in .META.
comment|// of a region is offlined because its a parent region and its daughters
comment|// still have references.
if|if
condition|(
name|metaEntry
operator|.
