...
name|warn
argument_list|(
literal|"new hlogs were produced while logs in "
operator|+
name|logDir
operator|+
literal|" were being split"
argument_list|)
expr_stmt|;
throw|throw
operator|new
...
name|path
operator|+
literal|"is already being split. "
operator|+
literal|"Two threads cannot wait for the same task"
argument_list|)
expr_stmt|;
return|return
literal|false
...
condition|)
block|{
comment|// == is correct. equals not necessary.
name|tasks
operator|.
name|remove
argument_list|(
...
name|fatal
argument_list|(
literal|"logic error - batch being overwritten"
argument_list|)
expr_stmt|;
block|}
name|this
...
name|path
operator|+
literal|" does not exist, either was never created or was deleted"
operator|+
literal|" in earlier rounds, zkretries = "
operator|+
operator|(
name|Long
operator|)
...
block|}
block|}
comment|/**    * checks whether any new files have appeared in logDir which were    * not present in the original logfiles set    * @param logdir    * @param logfiles    * @return True if a new log file is found    * @throws IOException    */
specifier|public
name|boolean
name|anyNewLogFiles
parameter_list|(
...
name|warn
argument_list|(
literal|"Discovered orphan hlog "
operator|+
name|newfile
operator|+
literal|" after split."
operator|+
literal|" Maybe HRegionServer was not dead when we started"
argument_list|)
expr_stmt|;
return|return
literal|true
...
return|;
block|}
comment|/**    * {@link SplitLogManager} can use objects implementing this interface to    * finish off a partially done task by {@link SplitLogWorker}. This provides    * a serialization point at the end of the task processing.    */
specifier|static
specifier|public
interface|interface
name|TaskFinisher
