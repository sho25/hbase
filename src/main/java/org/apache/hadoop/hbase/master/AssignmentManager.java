...
argument_list|()
decl_stmt|;
comment|// Pickup any disabled tables from ZK
...
name|tableName
operator|+
literal|" disabled; skipping assign of "
operator|+
name|region
operator|.
name|getRegionNameAsString
...
argument_list|()
operator|+
literal|" which is already closing but "
operator|+
literal|"forcing an additional close"
argument_list|)
expr_stmt|;
name|state
operator|.
...
name|abort
argument_list|(
literal|"Uncaught exception bulk assigning in "
operator|+
name|t
operator|.
name|getName
...
try|try
block|{
comment|// How long to wait on empty regions-in-transition.  When we timeout,
comment|// we'll put back in place the monitor of R-I-T.  It should do fixup
comment|// if server crashed during bulk assign, etc.
name|long
name|timeout
init|=
name|this
...
name|info
argument_list|(
literal|"Region transitioned out of OPENING so "
operator|+
literal|"skipping timeout, region="
operator|+
name|regionInfo
operator|.
name|getRegionNameAsString
