...
name|warn
argument_list|(
literal|"Node deleted but still in RIT: "
operator|+
name|rs
argument_list|)
expr_stmt|;
...
expr_stmt|;
block|}
comment|/**    * @param region    * @param setOfflineInZK    * @param forceNewPlan    * @param reassign    *          - true if timeout monitor calls assign    */
specifier|public
name|void
name|assign
parameter_list|(
...
parameter_list|)
block|{
comment|//If reassign is true do not call disableRegionIfInRIT as
...
return|;
block|}
comment|/**    * @param region    * @param reassign    * @return The current RegionState    */
specifier|private
name|RegionState
name|addToRegionsInTransition
parameter_list|(
...
return|;
block|}
comment|/**    * Sets regions {@link RegionState} to {@link RegionState.State#OFFLINE}.    * Caller must hold lock on this.regionsInTransition.    * @param region    * @param reassign    * @return Amended RegionState.    */
specifier|private
name|RegionState
name|forceRegionStateToOffline
parameter_list|(
...
block|{
comment|// If invoked from timeout monitor do not force in-memory to OFFLINE.
comment|// Based on the
comment|// znode state we will decide if to change in-memory state to OFFLINE or
comment|// not. It
comment|// will
...
return|;
block|}
comment|/**    * Caller must hold lock on the passed<code>state</code> object.    * @param state    * @param setOfflineInZK    * @param forceNewPlan    * @param reassign    */
specifier|private
name|void
name|assign
parameter_list|(
...
return|;
block|}
comment|/**    * Set region as OFFLINED up in zookeeper    *     * @param state    * @param reassign    *          - true if comes from timeout monitor, false otherwise    * @return the version of the offline node if setting of the OFFLINE node was    *         successful, -1 otherwise.    */
name|int
name|setOfflineInZooKeeper
parameter_list|(
specifier|final
...
parameter_list|)
block|{
comment|// If invoked from timeoutmonitor the current state in memory need not be
comment|// OFFLINE.
if|if
condition|(
operator|!
...
literal|false
decl_stmt|;
comment|// If reassign is true and the current state is PENDING_OPEN
comment|// or OPENING then refresh the in-memory state to PENDING_OPEN. This is
comment|// important because
comment|// if timeoutmonitor deduces that a region was in RS_OPENING state for a long
comment|// time but when the master forces
comment|// the znode to OFFLINE state the RS could have opened
comment|// the corresponding region and the
comment|// state in znode will be RS_ZK_REGION_OPENED. The
comment|// OpenedRegionHandler
comment|// expects the in-memory state to be PENDING_OPEN or OPENING.
...
block|}
block|}
comment|/**    * The type of operation that has to performed on TimeOut.    * This is not an inmemory state.  Just an enum to determine whether    * the operation to be taken after timeout is to assign the region    * or unassign the region.     * ASSIGN - need to assign a region to an RS     * UNASSIGN - need to unassign a region    */
specifier|public
specifier|static
enum|enum
name|TimeOutOperationType
...
return|return;
block|}
comment|// do not make the znode to OFFLINE. The timeoutManager will do it
name|invokeTimeOutManager
argument_list|(
name|regionInfo
argument_list|,
