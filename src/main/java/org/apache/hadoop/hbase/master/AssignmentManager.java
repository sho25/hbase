...
block|}
block|}
comment|/**    * New unassigned node has been created.    *    *<p>This happens when an RS begins the OPENING, SPLITTING or CLOSING of a    * region by creating a znode.    *    *<p>When this happens we must:    *<ol>    *<li>Watch the node for further children changed events</li>    *<li>Watch all new children for changed events</li>    *<li>Read all children and handle them</li>    *</ol>    */
annotation|@
name|Override
specifier|public
name|void
...
try|try
block|{
comment|// Update the tableDesc map.
for|for
control|(
name|HRegionInfo
name|region
...
block|}
block|}
comment|// Finish the work for regions in PENDING_CLOSE state
for|for
control|(
name|HRegionInfo
name|hri
...
block|}
block|}
comment|/**    * Assigns list of user regions in round-robin fashion, if any.    * @param sync True if we are to wait on all assigns.    * @param startup True if this is server startup time.    * @throws InterruptedException    * @throws IOException    */
name|void
...
name|IOException
argument_list|(
literal|"InterruptedException bulk assigning"
argument_list|,
name|e
argument_list|)
throw|;
...
name|info
argument_list|(
literal|"IOException while initializing HTableDescriptor Map"
argument_list|)
expr_stmt|;
block|}
block|}
...
name|error
argument_list|(
literal|"Table Descriptor cache is null. "
operator|+
literal|"Skipping desc map update for table = "
operator|+
name|tableName
argument_list|)
expr_stmt|;
...
name|info
argument_list|(
literal|"Updating TableDesc Map for tablename = "
operator|+
name|tableName
operator|+
literal|"htd == "
...
name|info
argument_list|(
literal|"HTable Descriptor is NULL for table = "
operator|+
name|tableName
argument_list|)
expr_stmt|;
...
name|error
argument_list|(
literal|"Table descriptor missing in DescMap. for tablename = "
operator|+
name|tableName
argument_list|)
expr_stmt|;
...
name|info
argument_list|(
literal|"TableDesc updated successfully for table = "
operator|+
name|tableName
argument_list|)
expr_stmt|;
...
name|info
argument_list|(
literal|"TableDesc removed successfully for table = "
operator|+
name|tableName
argument_list|)
expr_stmt|;
