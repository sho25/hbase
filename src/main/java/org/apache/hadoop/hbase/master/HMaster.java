...
name|tableDescriptors
decl_stmt|;
comment|// Whether or not schema alter changes go through ZK or not.
specifier|private
name|boolean
...
argument_list|()
expr_stmt|;
comment|// initialize schema change tracker
name|this
operator|.
name|schemaChangeTracker
operator|=
...
argument_list|)
expr_stmt|;
comment|// Schema janitor chore.
name|this
operator|.
name|schemaJanitorChore
operator|=
name|getAndStartSchemaJanitorChore
argument_list|(
name|this
argument_list|)
expr_stmt|;
...
begin_comment
comment|/**    * Start the schema janitor. This Janitor will periodically sweep the failed/expired schema    * changes.    * @param master    * @return    */
end_comment
...
argument_list|()
operator|+
literal|"-SchemaJanitorChore"
decl_stmt|;
name|int
name|schemaJanitorPeriod
init|=
name|master
operator|.
name|getConfiguration
...
argument_list|)
decl_stmt|;
comment|// Start up the schema janitor chore
name|Chore
name|chore
init|=
operator|new
...
begin_comment
comment|/**    * Check whether the Load Balancer is currently running.    * @return true if the Load balancer is currently running.    */
end_comment
...
name|debug
argument_list|(
literal|"Load balancer is currently running. Skipping the current execution."
argument_list|)
expr_stmt|;
return|return
literal|false
...
name|debug
argument_list|(
literal|"Schema change operation is in progress. Waiting for "
operator|+
literal|"it to complete before running the load balancer."
argument_list|)
expr_stmt|;
return|return
literal|false
...
block|{
return|return
name|getAlterStatusFromSchemaChangeTracker
argument_list|(
name|tableName
argument_list|)
return|;
...
name|error
argument_list|(
literal|"KeeperException while getting schema alter status for table = "
operator|+
name|Bytes
operator|.
name|toString
...
name|debug
argument_list|(
literal|"Getting AlterStatus from SchemaChangeTracker for table = "
operator|+
name|Bytes
operator|.
name|toString
...
name|debug
argument_list|(
literal|"MasterAlterStatus is NULL for table = "
operator|+
name|Bytes
operator|.
name|toString
...
argument_list|)
expr_stmt|;
comment|// should we throw IOException here as it makes more sense?
return|return
operator|new
name|Pair
argument_list|<
