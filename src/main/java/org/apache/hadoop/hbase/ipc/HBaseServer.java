...
decl_stmt|;
comment|/** Default value for above param */
specifier|private
specifier|static
specifier|final
name|int
...
return|;
block|}
comment|/**    * If invoked from inside an RPC, startDelay tells the current call to not     * send out * responses back to the client until endDelay() is invoked.    */
specifier|public
specifier|static
name|void
name|startDelay
...
name|delayResponse
decl_stmt|;
comment|// shall the RPC layer delay the response?
specifier|private
name|boolean
name|doneResponse
decl_stmt|;
comment|// have we already responded to the client?
specifier|private
name|Responder
name|responder
decl_stmt|;
comment|// the thread that sends out responses
specifier|public
name|Call
parameter_list|(
name|int
...
expr_stmt|;
block|}
comment|/**      * Delay this call, do not send its response back to the client      */
specifier|public
specifier|synchronized
name|void
name|startDelay
...
expr_stmt|;
block|}
comment|/**      * It is ok to send responses back to the client now. This is      * typically called by a non-server-handler thread.      */
specifier|public
specifier|synchronized
name|void
name|endDelay
...
block|}
block|}
comment|/*      * If we have a reponse, and delay is not set, then respond      * immediately. Otherwise, do not respond to client. This is      * called the by the RPC code in the context of the Handler thread.      */
specifier|public
specifier|synchronized
name|void
...
literal|true
expr_stmt|;
comment|// going to respond as soon as this call returns.
name|responder
operator|.
name|doRespond
argument_list|(
...
argument_list|)
expr_stmt|;
comment|// tell background thread to send it out later
block|}
block|}
specifier|private
specifier|synchronized
...
argument_list|()
expr_stmt|;
comment|// maybe delay sending out response to the client
block|}
catch|catch
parameter_list|(
name|InterruptedException
...
block|}
block|}
comment|// Packages the rpc invocation response back into the call.
specifier|static
specifier|public
name|void
name|processRpcResponse
