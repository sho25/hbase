...
begin_comment
comment|/**  * A basic {@link ByteRange} implementation.  */
end_comment
...
parameter_list|)
block|{
comment|// This writing is same as BB's putShort. When byte[] is wrapped in a BB and call putShort(),
...
parameter_list|)
block|{
comment|// This writing is same as BB's putInt. When byte[] is wrapped in a BB and call getInt(), one
...
parameter_list|)
block|{
comment|// This writing is same as BB's putLong. When byte[] is wrapped in a BB and call putLong(), one
comment|// can get the same result.
for|for
control|(
name|int
