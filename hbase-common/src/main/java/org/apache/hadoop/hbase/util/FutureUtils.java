...
return|;
block|}
comment|// This method is used to record the stack trace that calling the FutureUtils.get method. As in
comment|// async client, the retry will be done in the retry timer thread, so the exception we get from
comment|// the CompletableFuture will have a stack trace starting from the root of the retry timer. If we
comment|// just throw this exception out when calling future.get(by unwrapping the ExecutionException),
comment|// the upper layer even can not know where is the exception thrown...
comment|// See HBASE-22316.
specifier|private
specifier|static
name|void
name|setStackTrace
