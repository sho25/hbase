...
return|;
block|}
comment|/**      * Override the row key comparision to parse and compare the meta row key parts.      */
annotation|@
name|Override
specifier|protected
name|int
...
condition|)
block|{
comment|// leftKey's row is prefix of rightKey's. we can optimize it in future
return|return
name|Arrays
operator|.
name|copyOf
...
return|;
block|}
comment|// the following is optimizable in future
return|return
name|Arrays
operator|.
name|copyOf
...
return|;
block|}
comment|/**      * Generate a shorter faked key into index block. For example, consider a block boundary      * between the keys "the quick brown fox" and "the who test text".  We can use "the r" as the      * key for the index block entry since it is> all entries in the previous block and<= all      * entries in subsequent blocks.      *      * @param lastKeyOfPreviousBlock      * @param firstKeyInBlock      * @return a shortened null key, or if there are unexpected results, the firstKeyIn (new) Block      */
specifier|public
name|byte
index|[]
name|calcIndexKey
