...
block|}
block|}
comment|/**    * Compares the family and qualifier part of the cell    * TODO : Handle BB cases here    * @param left the left cell    * @param right the right cell    * @return 0 if both cells are equal, 1 if left cell is bigger than right, -1 otherwise    */
specifier|public
specifier|final
specifier|static
name|int
...
block|}
block|}
comment|/**    * Compares the family and qualifier part of the cell    * We explicitly pass the offset and length details of the cells to avoid    * re-parsing of the offset and length from the cell. Used only internally.    * @param left    * @param lfamilyOffset    * @param lfamilylength    * @param lqualOffset    * @param lQualLength    * @param right    * @param rfamilyOffset    * @param rfamilylength    * @param rqualOffset    * @param rqualLength    * @return 0 if both cells are equal, 1 if left cell is bigger than right, -1 otherwise    */
specifier|private
specifier|final
specifier|static
name|int
...
block|}
block|}
comment|/**    * Compares the family and qualifier part of a cell with a serialized Key value byte[]    * We explicitly pass the offset and length details of the cells to avoid    * re-parsing of the offset and length from the cell. Used only internally.    * @param left the cell to be compared    * @param lfamilyOffset    * @param lfamilylength    * @param lqualOffset    * @param lQualLength    * @param right the serialized key value byte array to be compared    * @param rfamilyOffset    * @param rfamilylength    * @param rqualOffset    * @param rqualLength    * @return 0 if both cells are equal, 1 if left cell is bigger than right, -1 otherwise    */
specifier|private
specifier|final
specifier|static
name|int
...
block|}
block|}
comment|/**    * Compare the families of left and right cell    * TODO : Handle BB cases here    * @param left    * @param right    * @return 0 if both cells are equal, 1 if left cell is bigger than right, -1 otherwise    */
specifier|public
specifier|final
specifier|static
name|int
...
return|;
block|}
comment|/**    * We explicitly pass the offset and length details of the cells to avoid    * re-parsing of the offset and length from the cell. Used only internally.    * @param left    * @param lOffset    * @param lLength    * @param right    * @param rOffset    * @param rLength    * @return 0 if both cells are equal, 1 if left cell is bigger than right, -1 otherwise    */
specifier|private
specifier|final
specifier|static
name|int
...
return|;
block|}
comment|/**    * Compare the qualifiers part of the left and right cells.    * TODO : Handle BB cases here    * @param left    * @param right    * @return 0 if both cells are equal, 1 if left cell is bigger than right, -1 otherwise    */
specifier|public
specifier|final
specifier|static
name|int
...
parameter_list|)
block|{
comment|// TODO : for BB based cells all the hasArray based checks would happen
comment|// here. But we may have
comment|// to end up in multiple APIs accepting byte[] and BBs
return|return
name|compareRows
argument_list|(
name|left
...
comment|// TODO : CLEANUP : in order to do this we may have to modify some code
comment|// HRegion.next() and will involve a
comment|// Filter API change also. Better to do that later along with
comment|// HBASE-11425/HBASE-13387.
specifier|public
name|int
name|compareRows
parameter_list|(
...
return|;
block|}
comment|/**    * Compare the timestamp of the left and right cell    *    * @param left    * @param right    * @return 0 if equal, -1 if left's ts is less than right's ts, 1 if left's ts    *         is greater than right's ts    */
specifier|public
specifier|static
name|int
name|compareTimestamps
...
return|;
block|}
comment|/**    * The below older timestamps sorting ahead of newer timestamps looks    * wrong but it is intentional. This way, newer timestamps are first    * found when we iterate over a memstore and newer versions are the    * first we trip over when reading from a store file.    * @param ltimestamp    * @param rtimestamp    * @return 1 if left timestamp> right timestamp    *         -1 if left timestamp< right timestamp    *         0 if both timestamps are equal    */
specifier|public
specifier|static
name|int
name|compareTimestamps
