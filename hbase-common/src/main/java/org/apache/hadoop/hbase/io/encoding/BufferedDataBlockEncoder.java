...
name|IOException
block|{
comment|// We have writeInt in ByteBufferOutputStream so that it can directly write int to underlying
...
name|IOException
block|{
comment|// We have write which takes ByteBuffer in ByteBufferOutputStream so that it can directly write
comment|// bytes from the src ByteBuffer to the destination ByteBuffer. This avoid need for temp array
comment|// creation and copy
if|if
condition|(
name|out
operator|instanceof
